...
export default async function AdminExperiencesPage({
  ...
  let query = db
    .from("experiences")
    .select(
      "id,title,category,status,searchable,organization_id,location_id,city,state,duration_minutes,min_party_size,max_party_size,price_per_person,created_at",
      { count: "exact" },
    )
  ...
  const [{ data, error, count }, { count: published }, { count: bookings }] =
    await Promise.all([
      query,
      db
        .from("experiences")
      ...
      <div className="mx-auto max-w-[1600px]">
        ...
        <form className="mt-5 flex flex-wrap gap-2">
          <input
            ...
            className="min-w-64 flex-1 rounded-xl border border-white/10 bg-black/30 p-3"
          />
          <select
            name="status"
            defaultValue={status}
          ...
          <button className="rounded-xl bg-[#e1062a] px-5 py-3 font-black">
            Filter
          </button>
        </form>
          ...
          {(data || []).map((row) => (
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="text-xs font-black uppercase tracking-[.14em] text-[#ff5570]">
                    {row.category || "Experience"}
                  </p>
                  <h2 className="mt-1 font-black">{row.title}</h2>
                  <p className="mt-1 text-xs text-white/40">
                    {row.status} · {row.location_id ? "Location" : "Organizer"} ·{" "}
                    {row.city || "—"}
                    {row.state ? `, ${row.state}` : ""}
                  </p>
                </div>
                <Link
                  href={`${consumerOrigin}/experiences/${row.id}`}
