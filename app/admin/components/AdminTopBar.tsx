...
type Props = {
  ...
  adminEmail: string;
  adminRole: AdminRole;
  adminPermissions: readonly AdminPermissionKey[];
};
...
export default function AdminTopBar({ adminName, adminEmail, adminRole, adminPermissions }: Props) {
  ...
  return <>
    ...
    <header className="sticky top-0 z-[80] border-b border-white/10 bg-[#070707]/95 text-white backdrop-blur-xl">
      ...
      {mobileOpen ? <div id="mobile-admin-navigation" className="max-h-[calc(100vh-4rem)] overflow-y-auto border-t border-white/10 px-4 py-3 xl:hidden"><Navigation permissions={adminPermissions} pathname={pathname} onNavigate={() => setMobileOpen(false)} /></div> : null}
    </header>
  </>;
}
