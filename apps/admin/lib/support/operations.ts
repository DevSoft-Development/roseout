...
async function getSupportOperationsSettingsLocally() {
  const [groups, slas, businessHours, macros, triggers, automations] = await Promise.all([
    getAdminDatabaseClient().from("support_groups").select("*").order("sort_order"),
    getAdminDatabaseClient().from("support_sla_policies").select("*").order("first_response_minutes"),
    getAdminDatabaseClient().from("support_business_hours").select("*").order("day_of_week"),
    getAdminDatabaseClient().from("support_macros").select("*").order("sort_order"),
    getAdminDatabaseClient().from("support_triggers").select("*").order("sort_order"),
    getAdminDatabaseClient().from("support_automation_rules").select("*").order("name"),
  ]);
...
export async function setSupportGroup(ticketId: string, group: string | null) {
  ...
  if (value) {
    const { data, error } = await getAdminDatabaseClient().from("support_groups").select("key").eq("key", value).eq("active", true).maybeSingle();
    if (error) throw error;
    if (!data) throw new Error("Unsupported support group.");
  }
  const { error } = await getAdminDatabaseClient().from("support_tickets").update({ assigned_group: value, updated_at: new Date().toISOString() }).eq("id", ticketId);
  if (error) throw error;
}
...
export async function setSupportTags(ticketId: string, tags: string[]) {
  ...
  const { error } = await getAdminDatabaseClient().from("support_tickets").update({ tags: normalized, updated_at: new Date().toISOString() }).eq("id", ticketId);
  if (error) throw error;
  return normalized;
}

export async function addSupportTags(ticketId: string, tags: string[]) {
  const { data, error } = await getAdminDatabaseClient().from("support_tickets").select("tags").eq("id", ticketId).single();
  if (error) throw error;
...
export async function getSupportMacro(key: string) {
  const { data, error } = await getAdminDatabaseClient().from("support_macros").select("*").eq("key", key).eq("active", true).single();
  if (error) throw error;
