...
export default async function AdminLocationCrmDetail({ params }: { params: Promise<{ type: string; id: string }> }) {
  ...
  return <main className="min-h-screen bg-[#090706] px-4 pb-10 pt-5 text-white sm:px-6 lg:px-8"><div className="mx-auto max-w-[1500px] space-y-6">
    ...
    <section className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
      <AdminCard title="Data Quality"><p>Review missing fields and repair opportunities.</p><Link href={`${basePath}/data-quality`} className="mt-3 inline-flex rounded-full border border-rose-300/30 bg-gradient-to-r from-rose-900/50 to-amber-900/40 px-4 py-2 font-bold text-rose-100">Open Data Quality</Link></AdminCard>
      <AdminCard title="Communication + Follow-up"><p>Track outreach history and next actions.</p><Link href={`${basePath}/communication`} className="mt-3 inline-flex rounded-full border border-rose-300/30 bg-gradient-to-r from-rose-900/50 to-amber-900/40 px-4 py-2 font-bold text-rose-100">Open Communication</Link></AdminCard>
      <AdminCard title="CRM"><p>Pipeline status, notes, and ownership tasks.</p><Link href={`${basePath}/crm`} className="mt-3 inline-flex rounded-full border border-rose-300/30 bg-gradient-to-r from-rose-900/50 to-amber-900/40 px-4 py-2 font-bold text-rose-100">Open CRM</Link></AdminCard>
