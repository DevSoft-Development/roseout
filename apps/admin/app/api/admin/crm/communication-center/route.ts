...
const CRM_COMMUNICATION_ROLES = ["superadmin","admin","manager","editor","reviewer","ambassador","experience_team","viewer","marketing_intern","marketing_specialist","marketing_manager"] as const;
const RESERVATION_COMMUNICATION_ROLES = ["superadmin","admin","manager","editor","reviewer","ambassador","experience_team","viewer"] as const;
const SUPPORT_COMMUNICATION_ROLES = ["superadmin","admin","experience_team","viewer"] as const;

async function authorize(scope: CommunicationScope) {
  if (scope === "reservations") return requireAdminRole(RESERVATION_COMMUNICATION_ROLES);
  if (scope === "support") return requireAdminRole(SUPPORT_COMMUNICATION_ROLES);
  return requireAdminRole(CRM_COMMUNICATION_ROLES);
}
