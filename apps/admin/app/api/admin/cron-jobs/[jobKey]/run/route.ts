...
export async function POST(request: NextRequest, { params }: { params: Promise<{ jobKey: string }> }) {
  ...
  const { data: job, error } = await getAdminDatabaseClient().from("cron_jobs").select("is_active").eq("job_key", jobKey).maybeSingle();
  ...
  const consumerOrigin = String(process.env.NEXT_PUBLIC_SITE_URL || "https://theouthaven.com").replace(/\/$/, "");
  const target = new URL("/api/cron/managed", consumerOrigin);
