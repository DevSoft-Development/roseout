...
export async function GET(request: NextRequest) {
  ...
  const silent =
    cookieStore.get("toh_m365_mode")?.value === "silent";
  const requestedNext = decodeNext(
    cookieStore.get("toh_m365_next")?.value,
  );
  ...
  if (!state || !expectedState || expectedState !== state) {
    return clearFlowCookies(
      redirectWith(
        origin,
        "error",
        "Microsoft authorization state expired. Please reconnect.",
      ),
    );
  }
  ...
  if (error) {
    ...
    return clearFlowCookies(
      redirectWith(origin, "error", errorDescription || error),
    );
  }
  ...
  if (!code || !verifier) {
    return clearFlowCookies(
      redirectWith(
        origin,
        "error",
        "Missing Microsoft authorization response.",
      ),
    );
  }
  ...
  try {
    ...
    const token = await exchangeMicrosoft365Code(
      code,
      verifier,
      redirectUri,
    );
    ...
    const provisional = {
      ...
      granted_scopes: (token.scope || "")
        .split(" ")
        .filter(Boolean),
      access_token_encrypted: encryptMicrosoftToken(
        token.access_token,
      ),
      refresh_token_encrypted: encryptMicrosoftToken(
        token.refresh_token,
      ),
    ...
    const { error: provisionalError } = await adminDb
      .from("microsoft_365_connections")
      .upsert(provisional, { onConflict: "user_id" });

    if (provisionalError) throw provisionalError;

    const me = await microsoftGraphFetch<GraphMe>(
      admin.user_id,
    ...
    if (!me.id || !email) {
      throw new Error("Microsoft account identity is incomplete.");
    }
    ...
    if (adminEmail && email !== adminEmail) {
      ...
      throw new Error(
        "The Microsoft 365 account must match the signed-in TheOutHaven administrator.",
      );
    }

    const { error: connectionError } = await adminDb
      .from("microsoft_365_connections")
      .update({
    ...
    const { error: preferencesError } = await adminDb
    ...
    return clearFlowCookies(
      redirectToNext(origin, next, true),
    );
  } catch (caught) {
    await adminDb
      .from("microsoft_365_connections")
