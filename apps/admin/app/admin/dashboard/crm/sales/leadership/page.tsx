...
export default async function SalesLeadershipPage(){
  ...
  return <CrmWorkspaceShell><main className="space-y-5 text-[var(--admin-shell-text)]">
    <header className="flex flex-wrap items-end justify-between gap-4">
      <div>
        <p className="text-xs font-black uppercase tracking-[.2em] text-rose-300">TheOutHaven Sales Leadership</p>
      ...
      <Link href="/admin/dashboard/crm/sales" className="rounded-xl bg-rose-600 px-4 py-2 text-sm font-black text-white">Open sales workspace</Link>
    </header>
    ...
    <section className="overflow-hidden rounded-3xl border border-[var(--admin-shell-border)] bg-[var(--admin-shell-card)]">
      <div className="border-b border-[var(--admin-shell-border)] p-5">
        <p className="text-xs font-black uppercase tracking-[.18em] text-rose-300">Team coverage</p>
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[1050px] text-left text-sm">
          <thead className="bg-white/[.03] text-xs uppercase tracking-wide text-[var(--admin-shell-muted)]"><tr>
          ...
          <tbody>{summary.reps.map((rep:any)=>{
            ...
            return <tr key={rep.userId} className="border-t border-[var(--admin-shell-border)] hover:bg-[var(--admin-shell-soft)]">
              ...
              <td className="px-4 py-4 font-black text-emerald-200">{rep.wins||0}</td>
              <td className="px-4 py-4 font-black text-rose-200">{rep.losses||0}</td>
    ...
    <section className="grid gap-5 xl:grid-cols-2">
      <article className="rounded-3xl border border-[var(--admin-shell-border)] bg-[var(--admin-shell-card)] p-5">
        <p className="text-xs font-black uppercase tracking-[.18em] text-rose-300">Opportunity mix</p>
      ...
      <article className="rounded-3xl border border-[var(--admin-shell-border)] bg-[var(--admin-shell-card)] p-5">
        <p className="text-xs font-black uppercase tracking-[.18em] text-rose-300">Assignment coverage</p>
        ...
        <div className="mt-4 space-y-2">{summary.territories.map((territory:any)=><div key={territory.id} className="rounded-xl border border-[var(--admin-shell-border)] p-3">
          <div className="flex items-center justify-between gap-3"><b>{territory.name}</b><span className={`rounded-full px-2 py-1 text-[10px] font-black uppercase ${territory.owner_user_id?"bg-emerald-400/10 text-emerald-100":"bg-amber-400/10 text-amber-100"}`}>{territory.owner_user_id?"Assigned":"Unassigned"}</span></div>
    ...
    <section className="rounded-3xl border border-rose-300/20 bg-rose-500/[.06] p-5">
