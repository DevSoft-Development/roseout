...
describe("Website V3 homepage conversion layer",()=>{
  it("covers all twenty V3 concepts",()=>{
    const ids=["nocturne","atelier","vista","social_house","quiet_luxury","maison","pulse","botanica","grandstand","gallery_house","riviera","ember","velvet_room","market_hall","skyline","daylight","foundry","supper_club","sanctuary","electric_garden"];
    for(const id of ids){expect(catalog).toContain(`id:\"${id}\"`);expect(conversion).toContain(`${id}:`)}
  });

  it("enhances every selected Home page before artifact assembly",()=>{
    expect(render).toContain("enhanceV3HomeHtml(concept,raw,location,profile)");
    expect(render).toContain("homeHtml:home");
  });
