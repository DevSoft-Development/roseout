...
async function sendReportEmail(to: string[], reportName: string, config: MarketingReportConfig) {
  ...
  const response = await sendEmailViaIntegrationApi({
...
async function processDueSchedules() {
  const now = new Date();
  const { data: due, error } = await getAdminDatabaseClient()
  ...
  for (const schedule of due || []) {
    try {
      ...
      await getAdminDatabaseClient().from("marketing_report_schedules").update({
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (body.action === "save") {
      ...
      const { data, error } = await getAdminDatabaseClient().from("marketing_saved_reports").insert({
        name,
    ...
    if (body.action === "schedule") {
      ...
      const { data, error } = await getAdminDatabaseClient().from("marketing_report_schedules").insert({
    ...
    if (body.action === "toggle_schedule") {
      ...
      const { error } = await getAdminDatabaseClient().from("marketing_report_schedules").update({ is_active: Boolean(body.isActive), updated_at: new Date().toISOString() }).eq("id", id);
      if (error) throw error;
