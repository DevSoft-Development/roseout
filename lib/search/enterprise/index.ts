...
export async function runEnterpriseSearch(query: string, options?: { useLLM?: boolean; body?: any; supabase?: any; displayLimit?: number }): Promise<EnterpriseSearchResult> {
  ...
  const restaurants=rankRestaurantResults(uniqueById(restaurantRaw),intent).slice(0,displayLimit); const activities=rankActivityResults(uniqueById(activityRaw),intent).slice(0,displayLimit); const pairingDebug=createPairingDebug(); const pairs=intent.wantsPairing?createSearchPairs(restaurants,activities,intent,pairingDebug):[];
