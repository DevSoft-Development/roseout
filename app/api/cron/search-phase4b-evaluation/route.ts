...
function getLocationId(value: Record<string, any> | null | undefined): string | null {
  const candidate =
    value?.location_id ??
    value?.locationId ??
    value?.id ??
...
function getPairIds(value: Record<string, any>) {
...
function numericScore(value: Record<string, any>): number | null {
  const score = Number(
...
function collectResults(result: Record<string, any> | null | undefined): CollectedResult[] {
  const cards = Array.isArray(result?.cards) ? result.cards : [];
  if (cards.length > 0) {
    return cards.map((card: Record<string, any>): CollectedResult => ({
      item: card,
      type:
        card.result_type ??
        card.resultType ??
  ...
  return [
    ...(Array.isArray(result?.pairs)
      ? result.pairs.map(
          (item: Record<string, any>): CollectedResult => ({ item, type: "pair" }),
        )
      : []),
    ...(Array.isArray(result?.restaurants)
      ? result.restaurants.map(
          (item: Record<string, any>): CollectedResult => ({
            item,
            type: "restaurant",
          }),
        )
...
async function runEvaluation() {
  ...
  const { data: scenarios, error: scenarioError } = await supabaseAdmin
    ...
    .select(
      "id,scenario_key,prompt,expected_result_type,expected_market,expected_min_results,metadata",
    )
    .eq("active", true)
    .order("scenario_key");
  ...
  if (activeScenarios.length > 0) {
    const { error: resultInsertError } = await supabaseAdmin
  ...
  for (const scenario of activeScenarios) {
    ...
    try {
      ...
      const collected: CollectedResult[] = collectResults(
        searchResult as unknown as Record<string, any>,
      ).slice(0, 12);

      const impressionRows = collected.flatMap(
        ({ item, type }: CollectedResult, index: number) => {
      ...
      const message =
        error instanceof Error ? error.message : "Unknown search test failure";
