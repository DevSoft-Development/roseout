...
function canTextReady(r: any) {
  return (
    (r.status === "checked_in" || r.status === "waiting" || r.status === "arrived") &&
...
const accent: Record<string, string> = {
  pending: "bg-[var(--reserve-primary)]",
  confirmed: "bg-white/45",
  checked_in: "bg-[var(--reserve-primary)]",
  waiting: "bg-[var(--reserve-primary)]",
  arrived: "bg-[var(--reserve-primary)]",
...
export default function ReserveTimeline({
  reservations,
  selectedId,
  onSelect,
  onStatus,
  ...
  function broadcastReservationSelection(r: any, bookableItemName?: string) {
    window.dispatchEvent(
      new CustomEvent("reserve:reservation-selected", {
        detail: {
          reservationId: r?.id || "",
  ...
  async function selectReservation(r: any) {
    ...
    if (
      ...
      try {
        ...
        // Keep the reservation selectable if automatic seating preparation cannot complete.
      }
    }
  ...
  async function submitMessage(e: FormEvent<HTMLFormElement>, r: any) {
    ...
    try {
      ...
      setMessageNotice((prev) => ({
        ...prev,
        [r.id]: data.message || "Message sent.",
      }));
      setMessageId("");
      setThreadRefresh((prev) => ({
        ...prev,
        [r.id]: (prev[r.id] || 0) + 1,
      }));
    } catch (error) {
      setMessageNotice((prev) => ({
        ...prev,
        [r.id]: error instanceof Error ? error.message : "Message could not be sent.",
      }));
    } finally {
      setMessageBusyId("");
    }
    ...
    <div className="space-y-2">
      {reservations.map((r) => {
        ...
        const showPrimaryAction =
          ...
          !isTerminal &&
          !(action.targetStatus === "seated" && !hasResource);
        const notes =
          r.special_request ||
          r.notes ||
          r.special_requests ||
        ...
        return (
          <div
            key={r.id}
            className={`reserve-timeline-row relative overflow-hidden rounded-2xl border bg-[var(--reserve-card-strong)] transition hover:border-[var(--reserve-border-strong)] ${
              selected
                ? "border-[var(--reserve-primary)]/50 shadow-[0_0_0_1px_rgba(225,6,42,.16),0_10px_28px_rgba(0,0,0,.22)]"
                : "border-[var(--reserve-border)]"
            }`}
          >
            <span
              className={`absolute inset-y-3 left-0 w-1 rounded-r-full ${
                accent[r.status] || "bg-[var(--reserve-primary)]"
              }`}
            />
            <button
              type="button"
            ...
            >
              <div className="reserve-timeline-grid grid gap-3">
                <div className="shrink-0 pl-2">
                  ...
                  <p className="whitespace-nowrap text-[11px] reserve-muted">
                    {duration(r)} min
                  </p>
                </div>
                <div className="reserve-timeline-content min-w-0">
                  <div className="flex min-w-0 items-center gap-2">
                    <h3
                      className="min-w-0 truncate text-sm font-black leading-tight md:text-[15px]"
                      title={guestName}
                    >
                      {guestName}
                    </h3>
                    {unreadCount > 0 ? (
                      <span className="shrink-0 rounded-full border border-[var(--reserve-primary)]/25 bg-[var(--reserve-primary-soft)] px-2 py-0.5 text-[9px] font-black uppercase tracking-[0.06em] text-[#ff8aa0]">
                    ...
                    <ReserveStatusBadge
                      ...
                      label={
                        r.table_ready_sms_sent
                          ...
                          : getReservationStatusLabel(r.status, vocab)
                      }
                    />
                    <p className="max-w-full truncate text-xs reserve-muted">
                      {vocab.partyLabel} {r.party_size || "—"} · {assigned(r)}
                    </p>
                  </div>
                </div>
              </div>
            ...
            {expanded && (
              <div
                id={`reservation-details-${r.id}`}
                className="border-t border-white/10 px-4 pb-4 pt-3"
              >
                <div className="grid gap-2 text-xs sm:grid-cols-2">
                  <div className="reserve-soft rounded-xl p-3">
                    <p className="reserve-muted">Phone</p>
                    <p className="mt-1 break-words font-bold text-white">
                      {value(r.customer_phone, "No phone")}
                    </p>
                  </div>
                  <div className="reserve-soft rounded-xl p-3">
                    <p className="reserve-muted">Email</p>
                    <p className="mt-1 break-words font-bold text-white">
                      {value(r.customer_email, "No email")}
                    </p>
                  </div>
                  <div className="reserve-soft rounded-xl p-3">
                    <p className="reserve-muted">Party & seating</p>
                    <p className="mt-1 font-bold text-white">
                      {vocab.partyLabel} {r.party_size || "—"} · {assigned(r)}
                    </p>
                  </div>
                  <div className="reserve-soft rounded-xl p-3">
                    <p className="reserve-muted">Reservation time</p>
                    <p className="mt-1 font-bold text-white">
                      {formatReservationTime(r.reservation_time)} · {duration(r)} min
                    </p>
                  </div>
                </div>

                <div className="reserve-soft mt-2 rounded-xl p-3 text-xs">
                  <p className="reserve-muted">Guest notes</p>
                ...
                {messageNotice[r.id] ? (
                  <p className="mt-3 rounded-xl border border-white/10 bg-white/5 p-3 text-xs font-bold">
                    {messageNotice[r.id]}
                  </p>
                ) : null}
                ...
                >
                  {canTextReady(r) && onTableReady ? (
                    <ReserveQuickActionButton
                      ...
                      onClick={() => onTableReady(r)}
                    >
                      {updatingId === r.id ? "Sending…" : vocab.readyAction}
                    </ReserveQuickActionButton>
                  ) : null}

                  {showPrimaryAction ? (
                    <ReserveQuickActionButton
                      disabled={updatingId === r.id || !!action.disabledReason}
                      title={action.disabledReason}
                      onClick={() => action.targetStatus && onStatus(r, action.targetStatus)}
                    >
                      {updatingId === r.id ? "Updating…" : action.label}
                    </ReserveQuickActionButton>
                  ) : null}

                  {onAssign && canAssign ? (
                    <ReserveQuickActionButton onClick={() => onAssign(r)}>
                  ...
                  <ReserveQuickActionButton
                    ...
                    title={
                      !canMessage
                        ? "Add a phone number or email address before messaging this guest."
                        : undefined
                    }
                    onClick={() => setMessageId(messageId === r.id ? "" : r.id)}
                  >
                    Message guest
                  </ReserveQuickActionButton>
                </div>
                ...
                {messageId === r.id && canMessage ? (
                  <form
                    onSubmit={(e) => submitMessage(e, r)}
                    onClick={(e) => e.stopPropagation()}
                    className="reserve-soft mt-3 grid gap-3 rounded-2xl p-3"
                  >
                    <div className="grid gap-2 sm:grid-cols-[180px_1fr] sm:items-center">
                      ...
                      >
                        {hasPhone ? <option value="sms">Text message</option> : null}
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {messageTemplates.map((template) => (
                        <button
                          ...
                          type="button"
                          onClick={(e) => {
                            const textarea = e.currentTarget.form?.elements.namedItem(
                              "message",
                            ) as HTMLTextAreaElement | null;
                            if (textarea) textarea.value = template;
                          }}
                          className="rounded-full border border-white/10 px-2.5 py-1 text-[10px] font-bold"
                        >
                          {template}
                        </button>
                      ))}
