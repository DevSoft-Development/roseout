...
export default async function BusinessDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const conversion = `${((business?.conversion_rate_30d ?? 0) * 100).toFixed(1)}%`;
  ...
  return (
    <AdminPageShell>
      <AdminPageHeader
        ...
        actions={<><AdminActionButton href="/admin/dashboard/businesses">Businesses</AdminActionButton><AdminActionButton href={`/admin/dashboard/claim-qrs?locationId=${id}`} variant="primary">Claim QR</AdminActionButton></>}
      />
      ...
      <AdminKpiGrid>
        <AdminKpiCard label="Opportunity Score" value={business?.opportunity_score ?? 0} helper="Commercial opportunity" />
        <AdminKpiCard label="Plan Status" value={business?.crm_status ?? "Unclaimed"} helper="Current CRM state" />
        <AdminKpiCard label="Churn Risk" value={business?.churn_risk_score ?? 0} helper="Retention signal" />
        <AdminKpiCard label="Conversion" value={conversion} helper="30-day conversion" />
      </AdminKpiGrid>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        ...
        <article className="rounded-3xl border border-white/10 bg-white/[0.03] p-5">
          ...
          <p className="mt-3 text-sm text-white/70">Use Claim QR Codes to regenerate, verify, or print the canonical claim code for this location.</p>
        </article>
        <article className="rounded-3xl border border-white/10 bg-white/[0.03] p-5">
          <h2 className="text-lg font-black">Outreach / Notes / Promotions</h2>
          <p className="mt-3 text-sm text-white/70">Outreach, notes, reservation link updates, upgrade opportunities, and promotion states remain tied to this business record.</p>
        </article>
      </section>
    </AdminPageShell>
  );
