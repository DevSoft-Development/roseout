...
async function authFor(body:any){
  ...
  const { context: ctx, error } = await requireLocationPermission({ body, locationId: body.id??body.location_id??body.locationId, adminLocationId: body.adminLocationId, demoLocationId: body.demoLocationId, sourceId: body.sourceId, type: body.type??body.table, requiredPermission: "recommendations.view", allowDemoPreview: true });
  if(error)return {ok:false,status:error.status,error:"You do not have access to this location.",isAdmin:false};
