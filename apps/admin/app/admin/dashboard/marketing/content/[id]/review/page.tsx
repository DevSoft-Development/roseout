  ...
  return (
    <main className="space-y-6 p-4 sm:p-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--admin-shell-muted)]">Marketing · Approval</p>
          <h1 className="text-3xl font-semibold">{item.title}</h1>
          <p className="mt-1 text-sm text-[var(--admin-shell-muted)]">Reviewing version {approval?.version || item.current_version} · {platforms.join(", ") || "No platforms selected"}</p>
        </div>
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        ...
        <div className="rounded-2xl border border-[var(--admin-shell-border)] bg-[var(--admin-shell-card)] p-5">
      ...
      <section className="rounded-2xl border border-[var(--admin-shell-border)] bg-[var(--admin-shell-card)] p-5">
        ...
        {(item.media_urls || []).length ? <div className="mt-3 space-y-2">{(item.media_urls || []).map((url) => <div key={url} className="break-all rounded-lg bg-[var(--admin-shell-soft)] p-3 text-sm text-[var(--admin-shell-text)]">{url}</div>)}</div> : <p className="mt-2 text-sm text-[var(--admin-shell-muted)]">No media attached.</p>}
      </section>
