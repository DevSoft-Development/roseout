...
export async function recordFraudSignal(input: FraudSignalInput) {
  ...
  const { data, error } = await getAdminDatabaseClient().from("fraud_signals").insert(payload).select("id").single();
  if (error) {
...
export async function linkFraudIdentity(input: {
  ...
  const { error } = await getAdminDatabaseClient().from("fraud_identity_links").upsert({
  ...
  const { data: peers, error: peerError } = await getAdminDatabaseClient()
...
export async function reportFraud(input: {
  ...
  const { data, error } = await getAdminDatabaseClient().from("fraud_reports").insert({
...
export async function getFraudDecision(subjectType: FraudSubjectType, subjectId: string): Promise<FraudDecision> {
  const { data, error } = await getAdminDatabaseClient().rpc("fraud_decide_subject", {
...
export async function getFraudSubject(subjectType: FraudSubjectType, subjectId: string) {
  const [{ data: subject, error: subjectError }, { data: signals, error: signalError }, { data: cases, error: caseError }, { data: actions, error: actionError }] = await Promise.all([
    getAdminDatabaseClient().from("fraud_subjects").select("*").eq("subject_type", subjectType).eq("subject_id", subjectId).maybeSingle(),
    getAdminDatabaseClient().from("fraud_signals").select("*").eq("subject_type", subjectType).eq("subject_id", subjectId).order("observed_at", { ascending: false }).limit(100),
    getAdminDatabaseClient().from("fraud_cases").select("*").eq("primary_subject_type", subjectType).eq("primary_subject_id", subjectId).order("opened_at", { ascending: false }).limit(20),
    getAdminDatabaseClient().from("fraud_actions").select("*").eq("subject_type", subjectType).eq("subject_id", subjectId).order("created_at", { ascending: false }).limit(50),
  ]);
