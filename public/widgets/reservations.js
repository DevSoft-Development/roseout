(() => {
  ...
  function mount(root) {
    ...
    function renderTimes() {
      ...
      timesEl.querySelector("[data-party]").onchange = (event) => { state.partySize = Number(event.target.value); state.selectedSlot = ""; loadAvailability(); };
      timesEl.querySelectorAll("[data-slot]").forEach((button) => button.addEventListener("click", () => { state.selectedSlot = button.dataset.slot; state.waitlist = false; renderTimes(); renderSubmit(); }));
      renderSubmit();
    }
    ...
    function renderSubmit() {
      ...
      submit.disabled = state.submitting || (!waitlist && !state.selectedSlot);
