...
Deno.serve(async (req) => {
  ...
  return new Response(JSON.stringify({ ok: true, two_hour: twoHour.data?.length || 0, thirty_minute: thirty.data?.length || 0, next_morning_followup: follow.data?.length || 0 }), { headers: cors });
});
