...
async function setStage(applicationId: string, stage: string, userId: string, reason: string) {
  const { error } = await getAdminDatabaseClient().rpc("career_set_application_stage", {
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (action === "schedule_interview") {
      ...
      const { data: interview, error } = await getAdminDatabaseClient()
    ...
    if (action === "complete_interview") {
      ...
      const { data: latest } = await getAdminDatabaseClient()
        .from("career_interviews")
        .select("id")
    ...
    if (action === "prepare_offer") {
      ...
      const { data: offer, error } = await getAdminDatabaseClient()
        .from("career_offers")
        .insert({
          application_id: id,
    ...
    if (action === "send_offer") {
      ...
      const { error } = await getAdminDatabaseClient().from("career_offers").update({ status: "sent", sent_at: now, updated_at: now }).eq("id", latest.id);
      if (error) throw new Error(error.message);
    ...
    if (action === "hire") {
      ...
      if (latest?.id) {
        const now = new Date().toISOString();
        const { error } = await getAdminDatabaseClient().from("career_offers").update({ status: "accepted", accepted_at: now, updated_at: now }).eq("id", latest.id);
        if (error) throw new Error(error.message);
      }
