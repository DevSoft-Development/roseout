...
export async function GET() {
  try {
    const { admin, profile } = await ensureTeamProfileForCurrentAdmin();
    ...
    const [activeSession, allowedWorkTypes, recent] = await Promise.all([
      ...
      db.from("team_work_sessions").select("*").eq("user_id", admin.user_id).order("clock_in_at", { ascending: false }).limit(10),
    ]);
...
export async function POST(req: Request) {
  try {
    ...
    if (!active || active.id !== sessionId) {
      return Response.json({ error: "Active work session not found." }, { status: 404 });
    }
    ...
    const { data, error } = await db.from("team_work_sessions").update({
    ...
    }).eq("id", sessionId).eq("user_id", admin.user_id).select("*").single();
    if (error) throw error;
