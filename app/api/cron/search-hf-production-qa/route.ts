...
function behaviorChecks(query: string, response: any): BehaviorCheck[] {
  ...
  const hasActivityExclusion = (term: RegExp) => (plan?.activity?.exclusions ?? []).some((value: unknown) => term.test(String(value)));
  const returnedActivityMatches = (term: RegExp) => activities.some((item: any) => term.test(itemText(item)));
  const restaurantFoods = (plan?.restaurant?.foods ?? []).map((value: unknown) => String(value).toLowerCase());
  ...
  switch (query) {
    ...
    case "restaurant with hookah in Forest Hills":
      add("forest_hills_is_geography", String(plan?.geo?.neighborhood ?? "").toLowerCase() === "forest hills", JSON.stringify(plan?.geo ?? {}));
      break;
    case "dinner then hookah in Forest Hills":
      add("sequential_hookah_is_paired", plan?.mode === "paired_outing" && plan?.restaurant?.required === true && plan?.activity?.required === true, `mode=${plan?.mode}`);
    ...
    case "seafood rooftop restaurant in Queens":
      add("seafood_rooftop_restaurant_only", plan?.mode === "restaurant_only" && plan?.activity?.required === false, `mode=${plan?.mode}`);
    ...
    case "dinner then bowling in Forest Hills":
      add("sequential_bowling_is_paired", plan?.mode === "paired_outing" && plan?.restaurant?.required === true && plan?.activity?.required === true, `mode=${plan?.mode}`);
    ...
    case "family outing in Queens":
      add("broad_occasion_builds_complete_outing", Boolean(plan?.restaurant?.required && plan?.activity?.required), `mode=${plan?.mode};occasion=${plan?.occasion ?? "none"}`);
      add("broad_occasion_has_results", restaurants.length > 0 || activities.length > 0 || pairs.length > 0, `restaurants=${restaurants.length};activities=${activities.length};pairs=${pairs.length}`);
      break;
    case "cocktails and something relaxing":
      add("relaxing_request_has_activity_lane", plan?.activity?.required === true, `mode=${plan?.mode}`);
      add("relaxing_request_has_results", activities.length > 0 || pairs.length > 0, `activities=${activities.length};pairs=${pairs.length}`);
      break;
    case "restaurant with hookah under one roof in Forest Hills":
      add("under_one_roof_uses_same_venue_mode", plan?.mode === "same_venue" && plan?.pairing?.sameVenueRequired === true, `mode=${plan?.mode};sameVenueRequired=${plan?.pairing?.sameVenueRequired}`);
      add("under_one_roof_has_same_venue_result", sameVenueResults.length > 0, `sameVenueResults=${sameVenueResults.length}`);
      break;
    case "jerk chicken pasta in Queens": {
      const joinedFoods = restaurantFoods.join(" | ");
      const phrasePreserved = joinedFoods.includes("jerk chicken pasta") || (joinedFoods.includes("jerk chicken") && joinedFoods.includes("pasta"));
      add("multiword_dish_preserved", phrasePreserved, JSON.stringify(restaurantFoods));
      add("multiword_dish_stays_restaurant_only", plan?.restaurant?.required === true && plan?.activity?.required === false, `mode=${plan?.mode}`);
      break;
    }
    case "purple dragon noodles in Queens":
      add("unknown_dish_stays_restaurant_only", plan?.restaurant?.required === true && plan?.activity?.required === false, `mode=${plan?.mode}`);
      add("unknown_dish_falls_back_gracefully", restaurants.length > 0 || response?.requestFulfilled === true || response?.partialResults === true, `restaurants=${restaurants.length};fulfilled=${response?.requestFulfilled};partial=${response?.partialResults}`);
      break;
    case "date night in Brooklyn tomorrow at 8 pm":
      ...
      add("date_time_keeps_brooklyn_geo", String(plan?.geo?.borough ?? "").toLowerCase() === "brooklyn", JSON.stringify(plan?.geo ?? {}));
      break;
    case "dinner then bowling in Forest Hills, walking distance": {
      const walkingMetadataValid = pairs.length === 0
        ? response?.outcome === "expected_constraint_no_pair"
        : pairs.every((pair: any) => Number.isFinite(Number(pair?.walkingMinutes)) && Number(pair.walkingMinutes) >= 0);
      add("walking_constraint_is_explicit", plan?.travel?.mode === "walking" && plan?.travel?.explicit === true && plan?.pairing?.requireWalkable === true, JSON.stringify({ travel: plan?.travel, pairing: plan?.pairing }));
      add("walking_pairs_have_walking_metadata", walkingMetadataValid, `pairs=${pairs.length};outcome=${response?.outcome ?? "none"}`);
      break;
    }
    default:
      break;
...
export async function GET(request: Request) {
  ...
  for (const query of QUERIES) {
    ...
    try {
      ...
      results.push({
        query,
        ok: false,
        behavior: { ok: false, checks: [{ name: "search_completed", ok: false, detail: error instanceof Error ? error.message : "unknown_search_qa_failure" }] },
  ...
  const overallOk = failed.length === 0 && behavioralFailures.length === 0 && missingMlTrace.length === 0;
