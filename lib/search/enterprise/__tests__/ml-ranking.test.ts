...
function restaurant(overrides: Partial<EnterpriseLocation>): EnterpriseLocation {
  return {
    id: "id",
...
describe("enterprise ML ranking boost", () => {
  ...
  it("lets ML score improve ordering only as a small additive signal", () => {
    const intent = deterministicIntentFromQuery("restaurant dinner in Manhattan");
    const lowMl = restaurant({ id: "low", name: "Dinner Low", ml_score: 0 });
    const highMl = restaurant({ id: "high", name: "Dinner High", ml_score: 100 });
    const ranked = rankRestaurantResults([lowMl, highMl], intent);
    expect((ranked.find((item) => item.id === "high") as any).ml_boost).toBe(15);
  });

  it("does not allow bad domain/category candidates to beat valid restaurants only because of ML", () => {
    const intent = deterministicIntentFromQuery("sushi restaurant in Manhattan");
    const good = restaurant({ id: "good", name: "Good Sushi", cuisine: "sushi", search_document: "sushi restaurant Manhattan", ml_score: 0 });
    const bad = restaurant({ id: "bad", name: "Bad Arcade", location_type: "activity", primary_category: "arcade", search_document: "arcade games", google_types: ["amusement_center"], ml_score: 100 });
