...
export async function sendSuperadminCriticalErrorEmail(input: {
  ...
  if (!to) {
    return {
      sent: false,
      error: "admin_alert_recipient_not_configured",
    };
  }
  ...
  const cta = input.ctaUrl
    ? `<p style="margin-top:24px"><a href="${escapeHtml(
        input.ctaUrl,
      )}" style="display:inline-block;padding:12px 18px;border-radius:999px;background:#e1062a;color:#fff;text-decoration:none;font-weight:700">Open TheOutHaven Admin</a></p>`
    : "";
  ...
  try {
    await sendEmailViaIntegrationApi({
      ...
      html: `<div style="font-family:Arial,sans-serif;max-width:640px;margin:0 auto;padding:24px"><h1>${escapeHtml(
        input.heading,
      )}</h1><p style="line-height:1.6">${escapeHtml(
        input.message,
      )}</p>${cta}</div>`,
    });
