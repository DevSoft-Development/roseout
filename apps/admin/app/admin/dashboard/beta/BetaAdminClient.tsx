...
export default function BetaAdminClient({
  ...
  const weeklyTestModeEnabled = Boolean(weeklySettings.weekly_beta_e2e_test_mode_enabled);
  const weeklyRealEnabled = Boolean(weeklySettings.weekly_beta_enabled);
      ...
      {tab === "Weekly Program" && (
        <div className="space-y-4">
          ...
          <section className="grid gap-3 md:grid-cols-2">
            <ControlCard title="Run real weekly beta task" description="When this is on, active beta testers can receive and complete the real weekly 5-step beta task." enabled={weeklyRealEnabled} onToggle={() => patch("/api/admin/beta/weekly-settings", { weekly_beta_enabled: !weeklySettings.weekly_beta_enabled })} actions={<button onClick={() => post("/api/admin/beta/weekly-sessions")} className="rounded-full bg-rose-600 px-4 py-2 text-xs font-black">Create Real Weekly Sessions</button>} />
            <ControlCard title="End-to-end weekly beta test mode" description="Run the full weekly beta flow, including emails, reminders, search, feedback, and admin review, without counting anything toward real beta progress, giveaway eligibility, prize entries, or real analytics." enabled={weeklyTestModeEnabled} onToggle={() => patch("/api/admin/beta/weekly-settings", { weekly_beta_e2e_test_mode_enabled: !weeklySettings.weekly_beta_e2e_test_mode_enabled })} actions={<div className="space-y-2"><p className="text-xs font-black text-white/70">{weeklyTestModeEnabled ? "Weekly beta test mode: On" : "Weekly beta test mode: Off"}</p>{!weeklyTestModeEnabled ? <p className="rounded-2xl border border-amber-300/25 bg-amber-400/10 p-3 text-xs font-bold text-amber-50">Turn on weekly beta test mode before creating or opening test sessions.</p> : null}<div className="flex flex-wrap gap-2"><button disabled={!weeklyTestModeEnabled} onClick={() => post("/api/admin/beta/test-weekly-session", { action: "create" })} className="rounded-full bg-rose-600 px-4 py-2 text-xs font-black disabled:cursor-not-allowed disabled:opacity-50">Create Test Weekly Session</button><Link href="/user/dashboard/beta/weekly?test=1" className={`rounded-full px-4 py-2 text-xs font-black ${weeklyTestModeEnabled ? "border border-white/10 bg-white/[0.08] text-white" : "pointer-events-none border border-white/10 bg-white/[0.04] text-white/40"}`}>Open Test Weekly Task</Link><button disabled={!weeklyTestModeEnabled} onClick={() => post("/api/admin/beta/test-weekly-session", { action: "send_weekly_email" })} className="rounded-full border border-white/10 bg-white/[0.08] px-4 py-2 text-xs font-black disabled:cursor-not-allowed disabled:opacity-50">Send Test Weekly Email</button><button disabled={!weeklyTestModeEnabled} onClick={() => post("/api/admin/beta/test-weekly-session", { action: "send_reminder" })} className="rounded-full border border-white/10 bg-white/[0.08] px-4 py-2 text-xs font-black disabled:cursor-not-allowed disabled:opacity-50">Send Test Reminder</button><button disabled={!weeklyTestModeEnabled} onClick={() => post("/api/admin/beta/test-weekly-session", { action: "send_completion_email" })} className="rounded-full border border-white/10 bg-white/[0.08] px-4 py-2 text-xs font-black disabled:cursor-not-allowed disabled:opacity-50">Send Test Completion Email</button><button disabled={!weeklyTestModeEnabled} onClick={() => confirm("Reset this test weekly task? This only clears test-mode progress and does not affect real beta testers.") && post("/api/admin/beta/test-weekly-session", { action: "reset" })} className="rounded-full border border-white/10 bg-white/[0.08] px-4 py-2 text-xs font-black disabled:cursor-not-allowed disabled:opacity-50">Reset Test Session</button><button disabled={!weeklyTestModeEnabled} onClick={() => confirm("Delete this test session? This only removes test-mode records.") && post("/api/admin/beta/test-weekly-session", { action: "delete" })} className="rounded-full border border-red-300/30 bg-red-500/10 px-4 py-2 text-xs font-black text-red-100 disabled:cursor-not-allowed disabled:opacity-50">Delete Test Session</button></div></div>} />
          </section>
          ...
          <Table
            rows={weeklySessions}
            cols={["tester", "week", "mode", "status", "steps_completed", "outing_sentence", "result_mode", "selected_result", "email_status", "last_activity"]}
