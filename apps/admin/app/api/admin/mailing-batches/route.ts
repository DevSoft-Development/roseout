...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { data: batch, error: batchError } = await getAdminDatabaseClient()
    ...
    const { data: createdItems, error: itemError } = await getAdminDatabaseClient()
      .from("mailing_batch_items")
      .insert(itemRows)
      .select("id,location_id,business_name,tracking_token");
    if (itemError) {
      await getAdminDatabaseClient().from("mailing_batches").delete().eq("id", batch.id);
      throw itemError;
    }
    ...
    await Promise.all((createdItems || []).map(async (item) => {
      try {
        await ensureShortLink(getAdminDatabaseClient(), {
