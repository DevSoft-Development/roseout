...
export default async function KnowledgeBasePage({ searchParams }: Props) {
  ...
  const [categories, result] = await Promise.all([
    ...
    listKbArticles(admin.role, admin.user_id, {
      q: first(sp.q), category: first(sp.category), type: first(sp.type), template: first(sp.template), status: first(sp.status), visibility: first(sp.visibility), page: Number(first(sp.page) || 1), pageSize: 20,
    }),
  ]);
