...
export default async function MailingBatchPrintPage({
  ...
  const [{ data: batch, error: batchError }, { data: itemData, error: itemError }, { data: templateObjects, error: templateError }] = await Promise.all([
    getAdminDatabaseClient().from("mailing_batches").select("id,name,status").eq("id", id).maybeSingle(),
    getAdminDatabaseClient()
      ...
      .limit(1000),
    getAdminDatabaseClient().storage.from(BUCKET).list("", { limit: 100 }),
  ]);
  ...
  if (staging) {
    ...
    const { data: stagingObjects, error: stagingError } = await getAdminDatabaseClient().storage.from(BUCKET).list(stagingFolder, { limit: 100, search: stagingFile });
    ...
    postageUrlByItem.set(stagingItem.id, `${getAdminDatabaseClient().storage.from(BUCKET).getPublicUrl(stagingPath).data.publicUrl}?v=${Date.now()}`);
  }
  ...
  if (production) {
    ...
    const { data: purchaseRow, error: purchaseError } = await getAdminDatabaseClient()
      .from("mailing_batch_items")
    ...
    const { data: productionObjects, error: productionError } = await getAdminDatabaseClient().storage.from(BUCKET).list(productionFolder, { limit: 100, search: productionFile });
    ...
    postageUrlByItem.set(productionItem.id, `${getAdminDatabaseClient().storage.from(BUCKET).getPublicUrl(productionPath).data.publicUrl}?v=${Date.now()}`);
  }
  ...
  if (productionBatch) {
    ...
    const { data: purchaseRows, error: purchaseError } = await getAdminDatabaseClient()
      .from("mailing_batch_items")
    ...
    const productionFolder = `production-proofs/${id}`;
    const { data: productionObjects, error: productionError } = await getAdminDatabaseClient().storage.from(BUCKET).list(productionFolder, { limit: 1000 });
    ...
    for (const item of renderItems) {
      ...
      postageUrlByItem.set(item.id, `${getAdminDatabaseClient().storage.from(BUCKET).getPublicUrl(productionPath).data.publicUrl}?v=${Date.now()}`);
    }
  }

  const frontUrl = getAdminDatabaseClient().storage.from(BUCKET).getPublicUrl("claim-front").data.publicUrl;
  const backUrl = getAdminDatabaseClient().storage.from(BUCKET).getPublicUrl("claim-back").data.publicUrl;
  ...
  if (itemIds.length) {
    const { data: shortRows, error: shortError } = await getAdminDatabaseClient()
      .from("short_links")
