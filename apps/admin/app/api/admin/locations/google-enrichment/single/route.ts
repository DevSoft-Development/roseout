...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data: row, error: loadError } = await adminDb.from("locations").select(ADMIN_LOCATION_ENRICHMENT_FIELDS).eq("id", locationId).maybeSingle();
    ...
    const { error: updateError } = await adminDb.from("locations").update(finalUpdate).eq("id", locationId);
