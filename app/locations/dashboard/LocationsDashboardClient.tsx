...
import { isPubliclyVisible, type LocationVisibilityFields } from "@/lib/locationVisibility";
...
function OverviewDashboard({ location, summary, links, demoMode }: { location: LocationItem; summary?: DashboardSummary; links: Links; demoMode?: boolean }) {
  ...
  const hasRevenue = Boolean(summary?.revenueEstimate30d);
  ...
  return (
    <div className="space-y-5">
      ...
      <Panel title="Today" eyebrow="Live operations" action={<Link href={links.reserveDashboard} className="text-xs font-black text-[#ff2142]">Open Reserve</Link>}>
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          <Metric label="Reservations Today" value={summary?.reservationsToday ?? 0} note="Active bookings dated today" />
          <Metric label="Guests Seated" value={summary?.guestsSeated ?? 0} note="Checked in, seated, arrived, or in progress" />
          <Metric label="Available Tables / Spaces" value={summary?.openSpaces == null ? "—" : summary.openSpaces} note={summary?.openSpaces == null ? "Layout/capacity is not configured" : "Available now"} />
          <Metric label="Upcoming Reservations" value={summary?.upcomingReservations ?? 0} note="Future active bookings" />
        </div>
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          <Metric label="No-shows · 30d" value={summary?.noShows30d ?? 0} note="Reservations marked no-show" />
          <Metric label="Reservation Clicks" value={reservationClicks} note="Recorded reservation-link actions" />
          <Link href={links.reservations} className="flex min-h-[92px] items-center justify-between rounded-2xl border border-white/10 bg-white/[0.035] p-4 text-sm font-black text-white/75 hover:bg-white/[0.07]">View reservation history <ExternalLink size={15} /></Link>
        </div>
      </Panel>

      <Panel title="Guest Discovery" eyebrow="Measured engagement" action={<Link href={links.analytics} className="text-xs font-black text-[#ff2142]">View analytics</Link>}>
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
          <Metric label="Search Appearances" value={summary?.searchesThisMonth ?? 0} note={trendText(summary?.searchTrendPercent)} />
          <Metric label="Profile Views" value={summary?.profileViews30d ?? location.view_count ?? 0} note={trendText(summary?.profileViewsTrendPercent)} />
          <Metric label="Guest Clicks" value={summary?.guestClicks30d ?? location.click_count ?? 0} note={trendText(summary?.clickTrendPercent)} />
          <Metric label="Calls" value={summary?.calls30d ?? location.call_count ?? 0} note="Recorded call actions" />
          <Metric label="Reservation Clicks" value={reservationClicks} note="Recorded reservation actions" />
        </div>
      </Panel>

      <Panel title="Location Health" eyebrow="Profile readiness">
        <div className="grid gap-5 lg:grid-cols-[1fr_180px] lg:items-center">
          <div>
            <div className="mb-4 flex items-center gap-3"><IconBubble good={status.tone === "good"}><ShieldCheck size={18} /></IconBubble><div><p className={`text-xl font-black ${status.tone === "good" ? "text-emerald-300" : "text-amber-300"}`}>{status.label}</p><p className="text-sm font-semibold text-white/45">{status.detail}</p></div></div>
            <CheckRows location={location} />
          </div>
...
function CheckRows({ location }: { location: LocationItem }) { const checks = [["Public search visibility", isPubliclyVisible(location)], ["Profile identity", Boolean(location.display_name && (location.address || location.city))], ["Profile photo", Boolean(getLocationImage(location) || location.images?.length)], ["Claim record", Boolean(location.is_claimed || location.claimed || location.owner_user_id)]] as const; return <div className="space-y-2">{checks.map(([label, ok]) => <div key={label} className="flex items-center gap-2 text-sm font-semibold text-white/65"><span className={`grid h-5 w-5 place-items-center rounded-full ${ok ? "bg-emerald-400/20 text-emerald-300" : "bg-amber-400/20 text-amber-300"}`}>{ok ? "✓" : "!"}</span>{label}</div>)}</div>; }
...
function DemoBanner({ selected, links }: { selected: LocationItem | null; links: Links | null }) { return <section className="mb-4 rounded-3xl border border-rose-300/20 bg-rose-500/10 p-4"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-rose-100">Demo Mode</p><p className="mt-1 text-sm font-bold text-white/72">You are viewing {selected?.display_name || "the demo location"} as a location owner. KPI values remain tied to the summary data supplied for this location.</p></div><div className="flex flex-wrap gap-2"><Link href="/admin/dashboard/settings/demo-center" className="rounded-2xl bg-white px-4 py-2 text-xs font-black text-black">Return to Demo Center</Link>{links ? <Link href={links.publicPage} className="rounded-2xl border border-white/10 px-4 py-2 text-xs font-black text-white/80">Open Public Profile</Link> : null}</div></div></section>; }
