...
const RESERVATION_READ_ROLES = ["superadmin","admin","manager","editor","reviewer","ambassador","experience_team","viewer"] as const;
const SUPPORT_READ_ROLES = ["superadmin","admin","experience_team","viewer"] as const;

async function authorize(scope: CommunicationScope) {
  if (scope === "reservations") return requireAdminRole(RESERVATION_READ_ROLES);
  if (scope === "support") return requireAdminRole(SUPPORT_READ_ROLES);
  return requireAdminRole(CRM_READ_ROLES);
}
...
export async function GET(req: Request) {
  try {
    ...
    const { data: locations } = locationIds.length
      ? await db.from("locations").select("id,name").in("id", locationIds)
      : { data: [] as any[] };
