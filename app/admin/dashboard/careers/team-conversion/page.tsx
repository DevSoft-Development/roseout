...
export default async function Page() {
  await requireAdminRole(ADMIN_PAGE_ACCESS.careersTeamConversion);
  ...
  return (
    <AdminPageShell>
      <AdminPageHeader
        eyebrow="Careers CRM"
        title="Employees"
        subtitle="Provision hired candidates, verify Microsoft 365 readiness, and manage employee lifecycle access."
        actions={
          <>
            <AdminActionButton href="/admin/dashboard/careers">Overview</AdminActionButton>
            <AdminActionButton href="/admin/dashboard/careers/jobs/new" variant="primary">
              Create Job
            </AdminActionButton>
          </>
        }
          ...
          <tbody>
            {rows.map((r: any) => (
              <tr key={r.id} className="border-t border-white/10">
                ...
                <td className="p-3">
                  <AdminStatusBadge tone={getCareerStageTone(r.stage || r.status || r.compliance_status)}>
                    {r.stage
                      ? formatCareerStage(r.stage)
                      : r.status || r.compliance_status || "Active"}
                  </AdminStatusBadge>
                </td>
                <td className="p-3 text-white/60">
                  {r.email ||
                    ...
                    r.template_key ||
                    formatCareerDate(r.created_at || r.submitted_at || r.scheduled_at || r.converted_at)}
                </td>
                <td className="p-3">
                  <Link
                    className="font-black text-rose-200"
                    href={r.first_name ? `/admin/dashboard/careers/applications/${r.id}` : "/admin/dashboard/careers"}
                  >
                    Open
                  </Link>
                </td>
