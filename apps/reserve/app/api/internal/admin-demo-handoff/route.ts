...
export async function GET(request: NextRequest) {
  const publicOrigin = String(process.env.NEXT_PUBLIC_RESERVE_SITE_URL || process.env.RESERVE_SITE_URL || "https://reserve.theouthaven.com").replace(/\/$/, "");
  const token = request.nextUrl.searchParams.get("token");
  const payload = verifyAdminDemoHandoff(token);
  if (!payload) {
    return NextResponse.redirect(new URL("/reserve", publicOrigin), 302);
  }
