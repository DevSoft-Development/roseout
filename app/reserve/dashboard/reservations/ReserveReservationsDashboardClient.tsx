...
type LocationSort = "upcoming" | "total" | "pending" | "name";
...
export default function ReserveReservationsDashboardClient() {
  ...
  const [locationSort, setLocationSort] = useState<LocationSort>("upcoming");
  ...
  const sortedLocations = useMemo(() => {
    return [...locations].sort((a, b) => {
      if (locationSort === "name") return a.name.localeCompare(b.name);
      if (locationSort === "pending") return b.pending - a.pending;
      if (locationSort === "total") return b.total - a.total;

      const aTime = a.nextReservation
        ? new Date(a.nextReservation).getTime()
        : Number.MAX_SAFE_INTEGER;
      const bTime = b.nextReservation
        ? new Date(b.nextReservation).getTime()
        : Number.MAX_SAFE_INTEGER;

      return aTime - bTime || b.total - a.total || a.name.localeCompare(b.name);
    });
  }, [locations, locationSort]);
  ...
  }, [locationKind, search, selectedLocation]);
          ...
          <div className="relative z-10 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm leading-6 text-white/60">
                Bird’s-eye view of reservations across all restaurants and
                activities, with location filters and search for individual
                venues.
              </p>
            </div>
        ...
        <section className="mt-5 rounded-[1.75rem] border border-white/10 bg-[#120d0b] p-4 shadow-2xl">
          <div className="grid gap-3 lg:grid-cols-[360px_1fr_auto] lg:items-center">
            ...
            <div className="flex flex-wrap gap-2">
              {(["all", "restaurant", "activity"] as LocationKind[]).map(
                (kind) => (
                  <button
                    key={kind}
                    type="button"
            ...
            <div className="flex flex-wrap items-center justify-end gap-3">
              <select
                value={locationSort}
                onChange={(event) =>
                  setLocationSort(event.target.value as LocationSort)
                }
                className="h-10 rounded-full border border-white/10 bg-white/[0.07] px-3 text-xs font-black uppercase tracking-wide text-white outline-none focus:border-rose-300"
              >
                <option className="bg-[#120d0b]" value="upcoming">
                  Sort: Next booking
                </option>
                <option className="bg-[#120d0b]" value="total">
                  Sort: Most reservations
                </option>
                <option className="bg-[#120d0b]" value="pending">
                  Sort: Pending
                </option>
                <option className="bg-[#120d0b]" value="name">
                  Sort: A-Z
                </option>
              </select>
          ...
          <div className="mb-4 flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
            <div>
              ...
              <p className="mt-1 text-sm text-white/45">
                Search, sort, or select a venue to narrow the bird’s-eye
                location list.
              </p>
            </div>
            {selectedLocation && (
              <button
                ...
                className="rounded-full border border-rose-300/40 bg-rose-500/15 px-4 py-2 text-xs font-black uppercase tracking-wide text-rose-100 transition hover:bg-rose-500/25"
              >
                Clear selected: {selectedLocation.name}
              </button>
            )}
          </div>
              ...
              {sortedLocations.map((location) => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-[1.5rem] border p-4 text-left transition hover:-translate-y-0.5 hover:bg-black/35 ${
                      selected
                        ? "border-rose-300 bg-rose-500/15 shadow-lg shadow-rose-950/30"
                  ...
                  >
                    ...
                    <div className="mt-4 grid grid-cols-3 gap-2 text-sm">
                      ...
                      <div className="rounded-2xl bg-white/[0.06] p-3">
                        <p className="text-white/40">Upcoming</p>
                        <p className="text-xl font-black">
                          {location.upcoming}
                        </p>
                      </div>
                    ...
                    <div className="mt-3 grid gap-1 text-xs text-white/35">
                      <p>Next: {formatDateTime(location.nextReservation)}</p>
                      <p>Last: {formatDateTime(location.lastReservation)}</p>
                    </div>
                  </button>
                );
              })}
...
function LoadingState({ dark = false }: { dark?: boolean }) {
  return (
    <div className="flex min-h-[220px] items-center justify-center text-center">
      <div>
        <Loader2
          className={`mx-auto animate-spin ${dark ? "text-rose-600" : "text-rose-300"}`}
        />
        <p
          className={`mt-4 text-sm font-bold ${dark ? "text-black/50" : "text-white/45"}`}
        >
...
function EmptyState({ text, dark = false }: { text: string; dark?: boolean }) {
  return (
    <div className="flex min-h-[220px] items-center justify-center text-center">
      <div>
        <CalendarDays
          className={`mx-auto ${dark ? "text-black/25" : "text-white/20"}`}
          size={44}
        />
        <p
          className={`mt-4 text-lg font-black ${dark ? "text-black" : "text-white"}`}
        >
          {text}
        </p>
      </div>
...
function FilterButton({
  ...
  return (
    ...
    >
      ...
      <span
        className={`rounded-full px-2 py-1 text-xs font-black ${active ? "bg-white/20 text-white" : "bg-white/10 text-white/70"}`}
      >
        {count}
      </span>
    </button>
      ...
      <div className="flex flex-col justify-between gap-5 lg:flex-row">
        <div className="min-w-0">
          ...
          <div className="mt-4 text-sm leading-7 text-black/60">
            ...
            {[reservation.location_city, reservation.location_state].filter(
              Boolean,
            ).length > 0 && (
              <p>
                Location:{" "}
                {[reservation.location_city, reservation.location_state]
                  .filter(Boolean)
                  .join(", ")}
              </p>
            )}
