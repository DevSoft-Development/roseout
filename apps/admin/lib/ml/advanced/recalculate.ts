...
async function runLog(type:string){ const {data}=await getAdminDatabaseClient().from('advanced_ml_score_runs').insert({run_type:type,status:'running'}).select('id').maybeSingle(); return data?.id; }
async function finish(id:string|undefined, patch:any){ if(id) await getAdminDatabaseClient().from('advanced_ml_score_runs').update({completed_at:new Date().toISOString(),...patch}).eq('id',id); }
async function locs(o:RecalcOptions){ let q=getAdminDatabaseClient().from('locations').select('*').limit(o.limit||200); if(o.locationId) q=q.eq('id',o.locationId); const {data,error}=await q; if(error) throw error; return data||[]; }
...
export async function recalculateReviewIntelligence(o:RecalcOptions={}){
  ...
  try{
    for(const l of await locs(o)){
      const {data:reviews,error}=await getAdminDatabaseClient().from('location_reviews').select('*').eq('location_id',l.id).eq('status','approved').order('created_at',{ascending:false}).limit(500);
      ...
      if(!o.dryRun){ const {error:upsertError}=await getAdminDatabaseClient().from('location_review_ml_features').upsert(row,{onConflict:'location_id'}); if(upsertError) throw upsertError; }
      updated++;
    }
...
export async function recalculateMarketSpecific(o:RecalcOptions={}){
  ...
  try{
    ...
    const {data:rows,error}=await getAdminDatabaseClient().from('location_pair_ml_features').select('market,market_key,pair_distance_miles,distance_miles,impressions_30d,shown_count,clicks_30d,click_count,saves_30d,save_count,completed_outings_30d,complete_count,pair_score,pair_compatibility_score').limit(limit);
    if(error) throw error;
    ...
    for(const [marketKey,items] of groups){
      ...
      if(!o.dryRun){ const {error:upsertError}=await getAdminDatabaseClient().from('market_ml_features').upsert(row,{onConflict:'market_key'}); if(upsertError) throw upsertError; }
      updated++;
    }
...
export async function recalculatePairCompatibility(o:RecalcOptions={}){
  ...
  try{
    ...
    const [{data:pairs,error:pairError},{data:markets,error:marketError},{data:reviews,error:reviewError}]=await Promise.all([
      getAdminDatabaseClient().from('location_pair_ml_features').select('*').limit(limit),
      getAdminDatabaseClient().from('market_ml_features').select('*').limit(500),
      getAdminDatabaseClient().from('location_review_ml_features').select('location_id,overall_review_quality_score,review_confidence_score').limit(10000),
    ]);
    ...
    for(const pair of pairs||[]){
      ...
      if(!o.dryRun){ const {error}=await getAdminDatabaseClient().from('location_pair_ml_features').update(patch).eq('id',pair.id); if(error) throw error; }
      updated++;
    }
    await finish(id,{status:'completed',records_updated:updated}); return {ok:true,recordsUpdated:updated};
  }catch(e:any){ await finish(id,{status:'failed',errors:[e.message]}); return {ok:false,error:e.message}; }
