...
async function fetchLocations() {
  const full = await getAdminDatabaseClient()
    .from("locations")
  ...
  const safe = await getAdminDatabaseClient()
    .from("locations")
...
async function safeSelect(table: string, columns = "*", limit = 200) {
  const result = await getAdminDatabaseClient().from(table).select(columns).limit(limit);
