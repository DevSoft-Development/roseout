...
export async function getOrCreateWeeklyBetaSessionForTester(testerId: string) {
  ...
  const { data: tester } = await getAdminDatabaseClient()
    .from("beta_testers")
...
export async function getOrCreateWeeklyBetaSessionForUser(
  ...
  const { data: tester } = await getAdminDatabaseClient()
    ...
    .eq("user_id", userId)
    .maybeSingle();
  const query = getAdminDatabaseClient()
    .from("beta_test_sessions")
    .select("*")
    .eq("week_start_date", weekStart)
  ...
  const { data, error: insertError } = await getAdminDatabaseClient()
    .from("beta_test_sessions")
    .insert({
      user_id: userId,
...
export async function getOrCreateWeeklyBetaSessionsForActiveTesters() {
  ...
  const { data: testers } = await getAdminDatabaseClient()
    .from("beta_testers")
    .select("id,user_id,status")
...
export async function resetTestWeeklyBetaSession(sessionId: string) {
  const { data: session, error } = await getAdminDatabaseClient()
    .from("beta_test_sessions")
    .select("id,test_mode")
    .eq("id", sessionId)
    .maybeSingle();
  ...
  const { data: runs } = await getAdminDatabaseClient()
  ...
  await getAdminDatabaseClient()
    ...
    .eq("beta_session_id", sessionId)
    .eq("test_mode", true);
  const { data, error: updateError } = await getAdminDatabaseClient()
    .from("beta_test_sessions")
    .update({
      status: "not_started",
      completed_steps: [],
...
export async function deleteTestWeeklyBetaSession(sessionId: string) {
  const { data: session, error: sessionError } = await getAdminDatabaseClient()
    .from("beta_test_sessions")
    .select("id,test_mode")
    .eq("id", sessionId)
    .eq("test_mode", true)
  ...
  const { data: runs, error: runsError } = await getAdminDatabaseClient()
    .from("beta_search_runs")
    .select("id")
    .eq("beta_session_id", sessionId)
    .eq("test_mode", true);
