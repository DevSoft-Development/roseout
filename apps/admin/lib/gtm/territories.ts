...
export async function getGtmTerritorySummary(){
  const [{data:territories,error:territoryError},{data:links,error:linkError},{data:states,error:stateError}]=await Promise.all([
    getAdminDatabaseClient().from('crm_territories').select('id,name,borough,status,owner_user_id').eq('status','active').order('name'),
    getAdminDatabaseClient().from('crm_location_territories').select('location_id,territory_id,territory_name,neighborhood,borough'),
    getAdminDatabaseClient().from('gtm_location_state').select('location_id,association_level,gtm_status,opportunity_tier,opportunity_score,demand_score,suppressed'),
  ]);
