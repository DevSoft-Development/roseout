...
export async function buildSearchPlan({ input }: { input: SearchPlannerInput }): Promise<SearchPlan> {
  ...
  const anchored = Boolean(p.anchorName && /\b(restaurant|activity|dinner|food)\s+near\b/i.test(input.query));
  ...
  const plan: SearchPlan = {
    ...
    anchor: { requested: anchored, rawName: anchored ? p.anchorName : null, locationId: null, name: null, latitude: null, longitude: null },
