...
export default async function TheOutHavenLoungeMirrorPage() {
  ...
  const [reservations, bookableItems, qrCodes, analytics, leads, offers, vip, feedback, notifications, menuItems] =
    await Promise.all([
  ...
  const modules: ModuleState[] = [
    {
      ...
      description: "The same location profile surface used by real venues, opened only through authenticated demo context.",
    },
    {
      label: "Reservation booking",
      count: bookableItems,
      href: context(`/reserve/location/${encodeURIComponent(locationId)}`),
      description: "Use the real reservation booking route, availability logic, notifications, confirmation, and analytics.",
    },
    {
      label: "Reserve operations",
      count: reservations,
      href: context("/reserve/dashboard"),
      description: "Calendar, reservations, seating, tables/booths, status changes, and guest operations.",
    },
    {
      label: "Location dashboard",
      count: 1,
      href: context("/locations/dashboard"),
      description: "The real location command center with demo location context preserved.",
    },
    {
      label: "Menu and commerce",
      count: menuItems,
      href: context("/business/dashboard/menu"),
      description: "Menu, commerce pages, items, pricing, availability, and guest-facing offerings.",
    },
    {
      label: "QR tools",
      count: qrCodes,
      href: context("/business/dashboard/qr-codes"),
      description: "Profile, menu, reservation, VIP, and offer QR workflows using demo-tagged records.",
    },
    {
      label: "Analytics",
      count: analytics,
      href: context("/business/dashboard/analytics"),
      description: "Views, engagement, reservation, QR, and conversion events scoped to TheOutHaven Lounge.",
    },
    {
      label: "Marketing and offers",
      count: (leads ?? 0) + (offers ?? 0),
      href: context("/business/dashboard/marketing-studio"),
      description: "Demo-safe offers, leads, marketing workflows, and growth tools.",
    },
    {
      label: "VIP",
      count: vip,
      href: context("/business/dashboard/vip"),
      description: "VIP signup and guest relationship workflows using isolated demo data.",
    },
    {
      label: "Feedback and review",
      count: feedback,
      href: context(`/locations/restaurant/${encodeURIComponent(locationId)}/feedback`),
      description: "Use the real post-visit feedback path tied to the demo venue.",
    },
    {
      label: "Check-in",
      count: reservations,
      href: context(`/locations/restaurant/${encodeURIComponent(locationId)}/check-in`),
      description: "Exercise the real check-in surface against demo reservations and visit state.",
    },
    {
      label: "Notifications",
      count: notifications,
      href: context("/locations/dashboard"),
      description: "Reservation and location notification history remains isolated to safe demo records and recipients.",
    },
  ];
    ...
    <main className="min-h-screen bg-[#050607] px-4 pb-16 pt-24 text-white sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl space-y-6">
        <header className="rounded-[30px] border border-white/10 bg-gradient-to-br from-[#171016] via-[#0c0d12] to-black p-6 shadow-2xl shadow-black/30 sm:p-8">
          <p className="text-xs font-black uppercase tracking-[0.24em] text-rose-300">Internal full-location mirror</p>
          <h1 className="mt-3 text-3xl font-black tracking-tight sm:text-5xl">TheOutHaven Lounge</h1>
          <p className="mt-3 max-w-3xl text-sm font-semibold leading-6 text-white/60 sm:text-base">
            This hidden fixture uses the same production location, reservation, dashboard, menu, QR, analytics, marketing,
            check-in, and feedback surfaces as a real venue. It stays excluded from ordinary public search and is available
            only to signed-in Admin, Sales Ambassador, Partner Ambassador, and Support roles.
          </p>
            ...
            <article key={module.label} className="rounded-[24px] border border-white/10 bg-white/[0.035] p-5">
              <div className="flex items-start justify-between gap-3">
                ...
                <span className="rounded-full border border-white/10 bg-black/30 px-3 py-1 text-xs font-black text-white/60">
                  {module.count === null ? "Not installed" : module.count > 0 ? `${module.count} demo records` : "Ready / empty"}
                </span>
              </div>
              <p className="mt-3 text-sm font-semibold leading-6 text-white/50">{module.description}</p>
              <Link href={module.href} className="mt-5 inline-flex rounded-full border border-rose-300/25 bg-rose-500/10 px-4 py-2 text-sm font-black text-rose-100 hover:bg-rose-500/20">
                Open real flow
              </Link>
            </article>
          ))}
