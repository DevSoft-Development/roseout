...
export async function GET() {
  ...
  const [settings, analyticsResult, recentResult] = await Promise.all([
    ...
    supabaseAdmin.from("search_ranking_experiments").select("variant,market,rollout_percent,model_version,latency_ms,no_results,pair_count,metadata,created_at").order("created_at", { ascending: false }).limit(25),
  ]);
  return NextResponse.json({ settings, analytics: analyticsResult.data ?? [], recent: recentResult.data ?? [] });
}
...
export async function POST(request: NextRequest) {
  ...
  const { data, error: updateError } = await supabaseAdmin
    ...
    .upsert({
      ...
      shadow_test_enabled: Boolean(body.shadow_test_enabled),
