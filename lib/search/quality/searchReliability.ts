...
export function evaluateEngineCorrectness(args: {
  responseContractValid: boolean;
  wrongDomainCount: number;
  geographyLeakageCount: number;
  hardConstraintViolations: number;
  parserCorrect: boolean;
}) {
  return args.responseContractValid
    && args.wrongDomainCount === 0
    && args.geographyLeakageCount === 0
    && args.hardConstraintViolations === 0
    && args.parserCorrect;
}

export function acceptableOutcome(args: {
  engineCorrect: boolean;
  fulfilled: boolean;
  knownInventoryRequired: boolean;
  failureClass: SearchFailureClass | null;
}) {
