...
export default function PostcardTemplatePanel({ batchId }: { batchId: string }) {
  ...
  async function upload(side: "front" | "back", file: File | null) {
    ...
    try {
      ...
      setState(data.templates as TemplateState);
      setMessage(`${side === "front" ? "Front" : "Back"} production master saved. This version now applies to every mailing batch.`);
    } catch (error) {
    ...
    <section className="rounded-3xl border border-emerald-300/15 bg-emerald-500/[0.06] p-5">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-xl font-black text-emerald-50">6×4 postcard masters</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-emerald-100/65">
            Upload the finalized mailing side and marketing side separately. Each master must be a true 3:2 landscape image at 1800×1200 pixels or larger. Do not upload a stacked front/back preview. The artwork stays locked while the recipient address, tracking QR, permanent claim code, postage output, and matching sequence number remain dynamic.
          </p>
        </div>
        {state?.ready ? (
      ...
      <div className="mt-4 rounded-2xl border border-white/10 bg-black/20 p-4 text-xs leading-5 text-white/55">
        <strong className="text-white">Locked master rule:</strong> no sample business name/address, fake postage text, fake postal barcode, sample QR, sample claim code, or sequence number should be baked into the uploaded files. The renderer supplies those values for every mailing item.
      </div>
            ...
            <div key={side} className="overflow-hidden rounded-2xl border border-white/10 bg-black/20">
              <div className="flex items-center justify-between gap-3 border-b border-white/10 px-4 py-3">
                <div>
                  ...
                  <p className="mt-0.5 text-xs text-white/40">6 × 4 landscape · 1800 × 1200+ · 3:2</p>
                </div>
