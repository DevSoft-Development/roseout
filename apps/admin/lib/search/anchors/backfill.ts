...
export async function executeApprovedAnchorBackfill(runId: string, batchSize = 100) {
  const { data: run, error: runError } = await getAdminDatabaseClient()
  ...
  const { data: actions, error: actionsError } = await getAdminDatabaseClient()
  ...
  for (const action of (actions || []) as ActionRow[]) {
    try {
      if (["skip", "conflict", "manual_review"].includes(action.action_type)) {
        await getAdminDatabaseClient().from("search_anchor_sync_actions").update({ status: "skipped", executed_at: new Date().toISOString() }).eq("id", action.id);
        summary.skipped++;
        continue;
      }

      if (action.action_type === "create") {
        const { data, error } = await getAdminDatabaseClient().from("search_anchors").insert(action.proposed_values).select("id").single();
        if (error) throw error;
        await getAdminDatabaseClient().from("search_anchor_sync_actions").update({ anchor_id: data.id, previous_values: {}, status: "completed", executed_at: new Date().toISOString() }).eq("id", action.id);
        summary.created++;
      } else {
        ...
        const { error } = await getAdminDatabaseClient().from("search_anchors").update(patch).eq("id", action.anchor_id);
        if (error) throw error;
        await getAdminDatabaseClient().from("search_anchor_sync_actions").update({ previous_values: action.current_values, status: "completed", executed_at: new Date().toISOString() }).eq("id", action.id);
      ...
      summary.failed++;
      await getAdminDatabaseClient().from("search_anchor_sync_actions").update({ status: "failed", error_message: error?.message || String(error), executed_at: new Date().toISOString() }).eq("id", action.id);
    } finally {
      summary.processed++;
    }
  }

  const { count: remaining } = await getAdminDatabaseClient()
    .from("search_anchor_sync_actions")
