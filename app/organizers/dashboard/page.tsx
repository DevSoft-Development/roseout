...
import { CalendarDays, CheckCircle2, QrCode, TicketCheck, Users, Building2, Plus, ExternalLink } from "lucide-react";
...
type Tab = "overview" | "events" | "tickets" | "scanner" | "profile";
...
export default async function OrganizerDashboardPage({ searchParams }: { searchParams: Params }) {
  ...
  const [{ data: profile }, { data: events, error: eventsError }] = await Promise.all([
    ...
    supabaseAdmin
      .from("events")
      .select("id,title,category,venue_name,city,state,starts_at,ends_at,status,searchable,is_free,price_min,ticketing_enabled,capacity,image_url,created_at")
      .eq("organization_id", organizationId)
        ...
        <aside className="hidden border-r border-white/10 bg-[#08090a] p-4 lg:block">
          ...
          <div className="space-y-2">
            {organizationContext.organizations.map((item) => (
              <Link
                key={item.id}
                href={`/organizers/dashboard?organizationId=${encodeURIComponent(item.id)}`}
                className={`block rounded-2xl border p-3 transition ${item.id === organizationId ? "border-[#ff2142]/60 bg-[#e1062a]/15" : "border-white/10 bg-white/[.03] hover:bg-white/[.06]"}`}
              >
                <div className="flex items-start gap-3">
                  <div className="rounded-xl bg-white/[.06] p-2"><Building2 size={16} /></div>
                  <div className="min-w-0">
                    <p className="truncate text-sm font-black">{item.name}</p>
                    <p className="mt-1 text-[11px] text-white/45">{item.role} · {item.organizationType}</p>
                  </div>
                </div>
              </Link>
            ))}
        ...
        <section className="min-w-0">
          <header className="border-b border-white/10 bg-[#070809]/95 px-4 py-4 sm:px-6 lg:px-8">
            <div className="mx-auto flex max-w-[1760px] flex-wrap items-center justify-between gap-4">
              ...
              <Link href={tabHref(organizationId, "events")} className="inline-flex items-center gap-2 rounded-xl bg-[#e1062a] px-4 py-3 text-sm font-black shadow-lg shadow-[#e1062a]/20">
          ...
          <div className="border-b border-white/10 px-4 sm:px-6 lg:px-8">
            <div className="mx-auto flex max-w-[1760px] gap-2 overflow-x-auto pb-3 pt-2">
              {tabs.map(([id, label]) => (
                <Link key={id} href={tabHref(organizationId, id)} className={`shrink-0 rounded-full border px-4 py-2 text-[11px] font-black uppercase tracking-[0.12em] transition ${activeTab === id ? "border-[#ff2142]/70 bg-[#e1062a]/20 text-white" : "border-white/10 bg-white/[0.04] text-white/55 hover:bg-white/[0.08] hover:text-white"}`}>
                  {label}
                </Link>
              ))}
            </div>
          ...
          <div className="mx-auto max-w-[1760px] space-y-5 px-4 pb-8 pt-5 sm:px-6 lg:px-8">
            ...
            {activeTab === "overview" ? (
              <>
                ...
                <Panel eyebrow="Next up" title="Upcoming Events" description="The events you are actively preparing for.">
                  <EventCards events={upcoming.slice(0, 6)} organizationId={organizationId} ticketsByEvent={ticketsByEvent} />
                </Panel>
              </>
            ) : null}
            ...
            {activeTab === "events" ? (
              <>
                <Panel eyebrow="Create" title="Create an Event" description="Add the event first, then manage tickets and attendees from the same dashboard.">
                ...
                <Panel eyebrow="Manage" title="Your Events" description="Update status, open the public listing, manage attendees, or scan tickets.">
                  <EventCards events={eventRows} organizationId={organizationId} ticketsByEvent={ticketsByEvent} editable />
                </Panel>
              </>
            ) : null}
                        ...
                        {rows.length ? (
                          <div className="mt-4 grid gap-2 md:grid-cols-2 xl:grid-cols-3">
                            {rows.slice(0, 12).map((ticket) => (
                              <div key={ticket.id} className="rounded-xl border border-white/10 bg-black/20 p-3 text-sm">
                                <b>{ticket.attendee_name}</b><p className="text-xs text-white/45">{ticket.attendee_email}</p><p className={`mt-2 text-xs font-black uppercase ${ticket.status === "checked_in" ? "text-emerald-300" : "text-white/55"}`}>{ticket.status.replaceAll("_", " ")}</p>
                              </div>
                            ))}
                          </div>
                        ) : <p className="mt-3 text-sm text-white/40">No registrations yet.</p>}
                      </article>
                    );
                  })}
                </div>
            ...
            {activeTab === "scanner" ? (
              <Panel eyebrow="Door operations" title="Scan Tickets" description="Choose an event and open the camera scanner on the device being used at entry.">
                <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                  {upcoming.map((event) => (
                    <Link key={event.id} href={`/organizers/dashboard/events/${event.id}/scanner`} className="rounded-2xl border border-white/10 bg-white/[.035] p-5 transition hover:border-[#ff2142]/40 hover:bg-[#e1062a]/10">
                      <QrCode size={24} className="text-[#ff2142]" /><h3 className="mt-4 font-black">{event.title}</h3><p className="mt-1 text-xs text-white/45">{formatDate(event.starts_at)}</p><p className="mt-4 text-sm font-black text-[#ff5570]">Open scanner →</p>
                    </Link>
                  ))}
                </div>
              </Panel>
            ...
            {activeTab === "profile" ? (
              <Panel eyebrow="Organizer" title="Organization / Profile" description="The identity guests will associate with your events.">
                ...
                <div className="mt-4 rounded-2xl border border-white/10 bg-black/20 p-4 text-sm text-white/60">
                  <p><b className="text-white">Bio:</b> {profile?.bio || "Not added yet"}</p>
                  <p className="mt-2"><b className="text-white">Website:</b> {profile?.website || "Not added yet"}</p>
                  <p className="mt-2"><b className="text-white">Instagram:</b> {profile?.instagram || "Not added yet"}</p>
                </div>
              </Panel>
            ) : null}
          </div>
