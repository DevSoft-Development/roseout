...
async function fetchCRMRowsForInMemorySearch(sourceConfig: CRMSourceConfig, filter?: string) {
  ...
  for (let attempt = 0; attempt < 50; attempt += 1) {
    let queryBuilder = adminDb().from(source.name).select(CRM_SELECT).limit(5000);
...
export async function safeSelect(table: string, builder: (query: any) => any) {
  try {
    const { data, error } = await builder(adminDb().from(table));
    if (error) {
...
export async function safeUpdateLocationPhotos(
  ...
  const { data: currentLocation } = await adminDb()
    .from("locations")
    .select("*")
    .eq("id", locationId)
    .maybeSingle();
