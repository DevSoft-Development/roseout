...
export const listEffectiveAdminRolePolicies = cache(async (): Promise<EffectiveAdminRolePolicy[]> => {
...
export async function saveAdminRolePolicy(input: {
  ...
  const { error } = await getAdminDatabaseClient().from("admin_role_policies").upsert(
    {
      role: input.role,
...
export async function resetAdminRolePolicy(input: {
  ...
  const before = await getEffectiveAdminRolePolicy(input.role);
  const { error } = await getAdminDatabaseClient().from("admin_role_policies").delete().eq("role", input.role);
  if (error) throw error;
