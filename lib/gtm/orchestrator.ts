...
function productGapsFor(score:any,offering:boolean){const rows=[{key:'claim',status:score.claimed?'covered':'sell_now',reason:score.claimed?'Business claimed':'Business is unclaimed'},{key:'essentials',status:score.paid?'covered':score.claimed?'sell_now':'blocked',reason:score.paid?'Paid plan active':score.claimed?'Claimed without paid plan':'Claim required first'},{key:'reserve',status:score.hasReservation?'review':score.claimed?'sell_now':'blocked',reason:score.hasReservation?'Existing reservation path detected':'No online reservation capability'},{key:'website',status:score.hasWebsite?'review':score.claimed?'sell_now':'blocked',reason:score.hasWebsite?'Website present; review quality':'No website detected'},{key:'events_experiences',status:offering?'covered':score.claimed?'review':'blocked',reason:offering?'Active offering detected':'No active event or experience detected'}];return rows;}
