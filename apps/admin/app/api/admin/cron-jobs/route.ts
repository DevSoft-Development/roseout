...
export async function GET() {
  ...
  const [{ data: cronJobs, error }, { data: runs, error: runsError }, { data: pgSnapshot, error: pgError }] = await Promise.all([
    getAdminDatabaseClient().from("cron_jobs").select(CRON_JOB_FIELDS),
    getAdminDatabaseClient()
      ...
      .limit(5000),
    getAdminDatabaseClient().rpc("admin_get_pg_cron_snapshot"),
  ]);
  ...
  if (missing.length) {
    const { data: inserted, error: insertError } = await getAdminDatabaseClient().from("cron_jobs").insert(missing).select(CRON_JOB_FIELDS);
