...
type Probe = { ok:boolean; status:number|null; response_ms:number|null; error:string|null };
type Health = { score:number; live_url?:string|null; healthy:boolean; checks:HealthCheck[]; live?:{checked_at:string;site:Probe|null;sitemap:Probe|null;robots:Probe|null;reservation:Probe|null} };

export function WebsiteHealthPanel({ locationId }: { locationId:string }) {
  const [health,setHealth]=useState<Health|null>(null);
  const [loading,setLoading]=useState(true);
  const [refreshing,setRefreshing]=useState(false);
  async function load(){const response=await fetch(`/api/business/website/health?location_id=${encodeURIComponent(locationId)}`,{cache:"no-store"});const data=await response.json();if(response.ok)setHealth(data)}
  ...
  const site=health.live?.site;
  return <section className="mb-5 rounded-3xl border border-white/10 bg-white/[0.03] p-5 sm:p-6">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-[#ff2142]">Website Health</p><h2 className="mt-2 text-xl font-black">{health.score}/100</h2><p className="mt-2 text-sm text-white/55">Checks your published site, SEO files, booking path, and synced business content.</p>{site?<p className="mt-2 text-xs font-bold text-white/40">Live response: {site.status||"Unavailable"}{site.response_ms!=null?` · ${site.response_ms} ms`:""}</p>:null}</div><div className="flex items-center gap-2"><button type="button" onClick={refresh} disabled={refreshing} className="rounded-full border border-white/10 px-3 py-2 text-xs font-black text-white/70 disabled:opacity-40">{refreshing?"Checking…":"Check now"}</button><span className={`rounded-full px-3 py-2 text-xs font-black ${health.healthy?"bg-emerald-500/10 text-emerald-200":"bg-amber-500/10 text-amber-100"}`}>{health.healthy?"Healthy":"Needs attention"}</span></div></div>
