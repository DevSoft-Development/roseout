...
export async function syncApprovedSocialRecords(content: MarketingContentRow) {
  ...
  for (const platform of platforms) {
    ...
    if (socialPostId) {
      const { error } = await getAdminDatabaseClient().from("social_posts").update(socialPayload).eq("id", socialPostId);
      if (error) throw error;
    } else {
      const { data: created, error } = await getAdminDatabaseClient()
        .from("social_posts")
    ...
    let connectionQuery = getAdminDatabaseClient()
    ...
    if (content.auto_publish && content.publish_at && connection?.id) {
      const { data: activeJob } = await getAdminDatabaseClient()
      ...
      if (activeJob?.id) {
        ...
        await getAdminDatabaseClient().from("social_publish_jobs").insert({
...
export async function cancelPendingApprovalForEdit(content: MarketingContentRow, actor: TaskActor) {
  const { data: approvals } = await getAdminDatabaseClient()
  ...
  for (const approval of approvals || []) {
    ...
    if (approval.crm_task_id) {
      const { data: task } = await getAdminDatabaseClient()
        .from("crm_tasks")
...
export async function submitMarketingContentForApproval(
  ...
  await getAdminDatabaseClient().from("marketing_content_versions").upsert(
    { content_item_id: content.id, version, snapshot, created_by: actor.user_id },
    { onConflict: "content_item_id,version" },
  );

  const { data: approval, error: approvalError } = await getAdminDatabaseClient()
    .from("marketing_approvals")
    .insert({
      content_item_id: content.id,
      version,
...
export async function decideMarketingApproval(
  ...
  const content = await loadMarketingContent(contentId);
  const { data: approval, error } = await getAdminDatabaseClient()
    .from("marketing_approvals")
