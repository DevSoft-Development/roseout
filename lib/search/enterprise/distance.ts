...
export function walkingMinutesToMiles(minutes: number) { return Number((minutes / WALKING_MINUTES_PER_MILE).toFixed(2)); }

function clampedWalkingMinutes(minutes: number) {
  return Math.min(minutes, MAX_WALKING_DISTANCE_MINUTES);
}

function maxMilesForPreference(pref: PairingPreference, fallbackMiles: number, fallbackMinutes?: number) {
  ...
  if (finitePositive(pref.maxPairWalkingMinutes)) {
    candidates.push(walkingMinutesToMiles(clampedWalkingMinutes(pref.maxPairWalkingMinutes as number)));
  }
...
function maxMinutesForPreference(pref: PairingPreference, fallbackMinutes: number) {
...
export function isWalkablePair(restaurant: EnterpriseLocation, activity: EnterpriseLocation, preference?: PairingPreference | null): { isWalkable: boolean; warnings: string[]; pairDistanceMiles: number | null; pairWalkingMinutes: number | null } {
  ...
  if (pref.distanceMode === "walking") {
    const maxMiles = maxMilesForPreference(pref, 3, MAX_WALKING_DISTANCE_MINUTES);
    const maxMinutes = maxMinutesForPreference(pref, MAX_WALKING_DISTANCE_MINUTES);
    return { isWalkable: pairDistanceMiles <= maxMiles && pairWalkingMinutes != null && pairWalkingMinutes <= maxMinutes, warnings, pairDistanceMiles, pairWalkingMinutes };
  }
