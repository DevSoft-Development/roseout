...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.redirect(new URL("/admin/dashboard/security/devices?sync=queued", origin), 303);
  } catch (error) {
    console.error("Intune device action failed", error);
    return NextResponse.redirect(new URL("/admin/dashboard/security/devices?sync=failed", origin), 303);
  }
}
