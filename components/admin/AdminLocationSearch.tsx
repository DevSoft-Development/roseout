...
export default function AdminLocationSearch({ compact = false, className = "" }: Props) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/admin/locations/search?q=${encodeURIComponent(q)}&limit=10`);
    ...
    <div ref={rootRef} className={`relative ${className}`}>
      <div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-black/30 px-3 py-2">
        ...
        <input
          ...
          className={compact ? "w-64 bg-transparent text-xs text-white outline-none placeholder:text-white/35" : "w-full bg-transparent text-sm text-white outline-none placeholder:text-white/35"}
        />
      </div>
      {open && (
        <div className="absolute left-0 right-0 z-[180] mt-2 max-h-[34rem] overflow-y-auto rounded-3xl border border-white/10 bg-[#120d0b]/98 p-3 text-white shadow-[0_24px_80px_rgba(0,0,0,0.65)] backdrop-blur-2xl">
          {query.trim().length < 2 && <p className="rounded-2xl border border-dashed border-white/10 p-4 text-sm text-white/55">Type at least 2 characters to search locations.</p>}
          {loading && <p className="px-2 py-3 text-sm text-white/55">Searching locations…</p>}
          {!loading && query.trim().length >= 2 && results.length === 0 && <p className="rounded-2xl border border-dashed border-white/10 p-4 text-sm text-white/55">No locations matched your search.</p>}
          {results.map((item) => (
            <article key={item.id} className="mt-2 rounded-2xl border border-white/10 bg-white/[0.04] p-3">
              ...
              <AdminLocationQuickActions locationId={item.id} className="mt-3" compact />
            </article>
          ))}
        </div>
      )}
