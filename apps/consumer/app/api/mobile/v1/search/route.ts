...
function laneFor(planType: MobileSearchBody["planType"]) {
  return planType === "restaurant" ? "restaurant" : planType === "activity" ? "activity" : "auto";
}
...
function shapePlace(value: any, kind: "restaurant" | "activity") {
  ...
  return {
    ...
    matchReasons: Array.isArray(value?.matchReasonDetails)
      ...
      : Array.isArray(value?.matchReasons)
        ? value.matchReasons.filter((item: unknown) => typeof item === "string").map((label: string) => ({ type: "other", label: label.trim() })).filter((item: any) => item.label).slice(0, 4)
        : [],
