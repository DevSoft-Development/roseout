...
export default function EnterpriseCrmShell({ children }: { children: React.ReactNode }) {
  ...
  const groups = enterpriseCrmNavigation.reduce<Record<string, CrmNavItem[]>>((result, item) => {
    ...
    <div data-testid="crm-single-navigation-shell" className="min-w-0 bg-[#070707] text-[#f8f8fa]">
      <header className="sticky top-0 z-30 border-b border-white/10 bg-[#0c0c0e]/95 backdrop-blur">
        <div className="flex h-16 min-w-0 items-center gap-3 px-3 sm:px-4">
          ...
          <Link href="/admin/dashboard/crm?focus=search" className="ml-auto flex h-10 min-w-0 flex-1 items-center gap-2 rounded-xl border border-white/10 bg-[#18181d] px-3 text-sm text-zinc-400 hover:border-white/20 hover:text-zinc-200 sm:max-w-xl">
            <Search className="h-4 w-4 shrink-0" />
            <span className="truncate">Search CRM records</span>
          </Link>
          <Link href="/admin/dashboard/crm/tasks?create=task" className="inline-flex h-10 shrink-0 items-center gap-2 rounded-xl bg-[#ec0b5b] px-3 text-sm font-bold text-white hover:bg-[#ff206e]">
        ...
        <div className="flex min-w-0 items-center gap-2 border-t border-white/[0.06] px-3 py-2 sm:px-4">
          <nav aria-label="Primary CRM modules" className="flex min-w-0 flex-1 gap-1 overflow-x-auto pb-0.5">
            {primaryItems.map((item) => {
              ...
              return (
                <Link key={item.id} href={item.href} aria-current={active ? "page" : undefined} className={`inline-flex h-9 shrink-0 items-center gap-2 rounded-lg px-3 text-sm font-semibold transition ${active ? "bg-[#ec0b5b] text-white" : "text-zinc-300 hover:bg-white/[0.06] hover:text-white"}`}>
                  ...
                  <section key={group}>
                    ...
                    <div className="space-y-1">
                      {items.map((item) => {
                        ...
                        const active = isActive(pathname, item);
                        return (
                          <Link key={item.id} href={item.href} className={`flex items-center gap-2 rounded-lg px-2.5 py-2 text-sm ${active ? "bg-[#ec0b5b] text-white" : "text-zinc-300 hover:bg-white/[0.06] hover:text-white"}`}>
                            <Icon className="h-4 w-4" />
                            {item.label}
                          </Link>
                        );
