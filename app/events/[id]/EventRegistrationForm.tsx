...
export default function EventRegistrationForm({ eventId, isFree }: { eventId: string; isFree: boolean }) {
  ...
  if (!isFree) {
    return (
      <div className="mt-6 rounded-xl border border-amber-300/20 bg-amber-300/10 p-4 text-sm text-amber-100">
        Paid online ticket checkout is being enabled separately. This event is not accepting online ticket purchases yet.
      </div>
    );
  }
  ...
  return (
    <form onSubmit={register} className="mt-6 space-y-3 border-t border-white/10 pt-5">
      <div>
        <h3 className="font-bold">Get your ticket</h3>
        <p className="mt-1 text-xs leading-5 text-white/45">Your QR ticket will be shown immediately and sent by email. Add a mobile number to receive the secure ticket link by text too.</p>
      </div>
      ...
      <button disabled={loading} className="w-full rounded-xl bg-red-600 px-4 py-3 font-semibold text-white transition hover:bg-red-500 disabled:cursor-not-allowed disabled:opacity-60">
        {loading ? "Creating ticket…" : "Register & Get QR Ticket"}
      </button>
    </form>
  );
}
