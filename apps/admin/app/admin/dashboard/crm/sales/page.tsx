...
function cardTone(){
  return "border-[var(--admin-shell-border)] bg-[var(--admin-shell-card)] text-[var(--admin-shell-text)]";
}
function statusTone(status:string){
  if(status==="sell_now")return "border-[var(--admin-shell-accent-border)] bg-[var(--admin-shell-accent-soft)] text-[var(--admin-shell-accent)]";
  if(status==="review")return "border-[var(--admin-shell-border-strong)] bg-[var(--admin-shell-card-strong)] text-[var(--admin-shell-soft)]";
  if(status==="covered")return "border-emerald-500/20 bg-emerald-500/[.07] text-emerald-400";
  return "border-[var(--admin-shell-border)] bg-[var(--admin-shell-card-strong)] text-[var(--admin-shell-muted)]";
}
        ...
        return <details key={row.id} id={`location-${row.id}`} open={p.location_id===row.id} className="group overflow-hidden rounded-3xl border border-[var(--admin-shell-border)] bg-[var(--admin-shell-card)]">
          <summary className="cursor-pointer list-none p-5 transition hover:bg-[var(--admin-shell-soft)]">
            <div className="grid gap-4 xl:grid-cols-[minmax(0,1.4fr)_160px_210px_minmax(220px,.8fr)] xl:items-center">
              ...
              <div><small className="font-black uppercase tracking-wide text-[var(--admin-shell-accent)]">What to sell</small><p className="mt-1 font-black">{top?.label||"Review account"}</p><p className="mt-1 line-clamp-2 text-xs text-[var(--admin-shell-muted)]">{top?.reason}</p></div>
              <div><small className="font-black uppercase tracking-wide text-[var(--admin-shell-accent)]">Do next</small><p className="mt-1 text-sm font-black">{row.nextBestAction}</p><p className="mt-1 text-xs text-[var(--admin-shell-muted)]">{row.openOpportunities.length} open opportunities · {row.openTasks.length} follow-ups</p></div>
            </div>
          </summary>

          <div className="space-y-5 border-t border-[var(--admin-shell-border)] p-5">
            <section>
              <div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div><p className="text-xs font-black uppercase tracking-[.18em] text-[var(--admin-shell-accent)]">Customer journey</p><h3 className="mt-1 text-xl font-black">Where this location is in the relationship</h3></div><Link href={`/admin/dashboard/crm/${row.id}`} className="rounded-xl border border-[var(--admin-shell-border)] px-3 py-2 text-xs font-black">Full location record</Link></div>
              <Journey stage={row.lifecycleStage}/>
            </section>

            <section>
              <div className="mb-3"><p className="text-xs font-black uppercase tracking-[.18em] text-[var(--admin-shell-accent)]">Opportunity coverage</p><h3 className="mt-1 text-xl font-black">What to sell and why</h3><p className="mt-1 text-sm text-[var(--admin-shell-muted)]">Detected gaps stay recommendations until an ambassador qualifies them. Existing opportunities remain canonical CRM opportunities.</p></div>
              <div className="grid gap-3 xl:grid-cols-2">
                {row.recommendations.map((rec)=>{
                  const play=SALES_PLAYBOOKS[rec.key];
                  return <article key={rec.key} className={`rounded-2xl border p-4 shadow-sm ${cardTone()}`}>
                    <div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><h4 className="font-black">{rec.label}</h4><span className={`rounded-full border px-2 py-0.5 text-[10px] font-black uppercase ${statusTone(rec.status)}`}>{rec.status.replace("_"," ")}</span><span className="text-[10px] font-black uppercase text-[var(--admin-shell-muted)]">{rec.confidence} confidence</span></div><p className="mt-2 text-sm text-[var(--admin-shell-soft)]">{rec.reason}</p></div><b className="text-xl text-[var(--admin-shell-soft)]">{rec.priority}</b></div>
                    <div className="mt-3 flex flex-wrap gap-1">{rec.evidence.map((e)=><span key={e} className="rounded-full border border-[var(--admin-shell-border)] bg-[var(--admin-shell-card-strong)] px-2 py-1 text-[10px] font-bold text-[var(--admin-shell-muted)]">{e}</span>)}</div>
                    <details className="mt-4 rounded-xl border border-[var(--admin-shell-border)] bg-[var(--admin-shell-card-strong)] p-3"><summary className="cursor-pointer text-xs font-black text-[var(--admin-shell-text)]">How to sell this</summary><div className="mt-3 space-y-3 text-xs text-[var(--admin-shell-soft)]"><p><b>Opening:</b> {play.opener}</p><div><b>Discovery:</b><ul className="mt-1 list-disc space-y-1 pl-5">{play.discovery.map((q)=><li key={q}>{q}</li>)}</ul></div><p><b>Value:</b> {play.value}</p><p><b>Common objection:</b> {play.objection}</p><p><b>Response:</b> {play.response}</p></div></details>
                    <div className="mt-4 flex flex-wrap gap-2">
                      {rec.activeOpportunityId?<Link href={`/admin/dashboard/crm/opportunities/${rec.activeOpportunityId}`} className="rounded-xl bg-[var(--admin-shell-accent)] px-3 py-2 text-xs font-black text-white hover:bg-[var(--admin-shell-accent-hover)]">Open opportunity · {rec.stage||"active"}</Link>:rec.status!=="blocked"&&rec.status!=="covered"?<form action={qualifyProductOpportunityAction}><input type="hidden" name="location_id" value={row.id}/><input type="hidden" name="product" value={rec.key}/><button className="rounded-xl bg-[var(--admin-shell-accent)] px-3 py-2 text-xs font-black text-white hover:bg-[var(--admin-shell-accent-hover)]">Qualify opportunity</button></form>:null}
                    </div>
                  </article>;
                })}
              </div>
            ...
            <section className="grid gap-4 xl:grid-cols-3">
              <article className="rounded-2xl border border-[var(--admin-shell-border)] bg-[var(--admin-shell-soft)] p-4"><h3 className="font-black">Who to contact</h3>{row.contacts.length?<div className="mt-3 space-y-2">{row.contacts.map((c)=><div key={c.value} className="rounded-xl border border-[var(--admin-shell-border)] bg-[var(--admin-shell-card)] p-3"><b className="break-all text-sm">{c.value}</b><p className="mt-1 text-xs text-[var(--admin-shell-muted)]">{c.role||"Business contact"} · {c.verification||"discovered"} · confidence {c.confidence??"—"}</p></div>)}</div>:<p className="mt-3 text-sm text-[var(--admin-shell-muted)]">No verified/discovered business contact yet. Next action should be contact research.</p>}</article>
              <article className="rounded-2xl border border-[var(--admin-shell-border)] bg-[var(--admin-shell-soft)] p-4"><h3 className="font-black">Qualified opportunities</h3>{row.openOpportunities.length?<div className="mt-3 space-y-2">{row.openOpportunities.map((o:any)=><Link key={o.id} href={`/admin/dashboard/crm/opportunities/${o.id}`} className="block rounded-xl border border-[var(--admin-shell-border)] bg-[var(--admin-shell-card)] p-3 hover:border-[var(--admin-shell-accent-border)] hover:bg-[var(--admin-shell-card-strong)]"><b className="text-sm">{o.name}</b><p className="mt-1 text-xs text-[var(--admin-shell-muted)]">{String(o.pipeline_key||"sales").replaceAll("_"," ")} · {String(o.stage||"identified").replaceAll("_"," ")} · {o.amount?money(o.amount):"Value not set"}</p><p className="mt-1 text-xs">Next: {o.next_step||"Set next step"}</p></Link>)}</div>:<p className="mt-3 text-sm text-[var(--admin-shell-muted)]">No qualified opportunity yet. Use the product cards above to qualify a detected gap.</p>}</article>
