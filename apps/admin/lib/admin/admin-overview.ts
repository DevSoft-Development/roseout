...
async function readAdminOverviewLocally(): Promise<Record<string, any>> {
  ...
  const [
    ...
    getAdminDatabaseClient().from("restaurants").select("id", { count: "exact", head: true }),
    getAdminDatabaseClient().from("activities").select("id", { count: "exact", head: true }),
    getAdminDatabaseClient().from("location_reservations").select("id", { count: "exact", head: true }),
    ...
    getAdminDatabaseClient()
      ...
      .gte("created_at", thirtyDaysAgo)
      .limit(5000),
    getAdminDatabaseClient().from("experiences").select("id,price_per_person").limit(5000),
    getAdminDatabaseClient()
      .from("locations")
    ...
    getAdminDatabaseClient().from("location_ml_features").select("location_id", { count: "exact", head: true }),
    getAdminDatabaseClient().from("location_intent_ml_features").select("id", { count: "exact", head: true }),
    getAdminDatabaseClient().from("location_pair_ml_features").select("id", { count: "exact", head: true }),
    getAdminDatabaseClient().from("location_ml_score_runs").select("created_at").order("created_at", { ascending: false }).limit(1).maybeSingle(),
    getAdminDatabaseClient().from("business_websites").select("id", { count: "exact", head: true }),
    getAdminDatabaseClient().from("business_websites").select("id", { count: "exact", head: true }).eq("status", "live"),
    getAdminDatabaseClient().from("website_hosting_nodes").select("id", { count: "exact", head: true }),
    getAdminDatabaseClient().from("website_hosting_nodes").select("id", { count: "exact", head: true }).eq("status", "healthy"),
  ]);
...
async function readAdminOverviewUncached(): Promise<Record<string, any>> {
...
export async function readAdminOverview(): Promise<Record<string, any>> {
