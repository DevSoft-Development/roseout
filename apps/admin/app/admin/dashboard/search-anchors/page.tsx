...
export default async function SearchAnchorsAdminPage({
  ...
  const [{ data: anchors, count }, active, approved, pending, discoveries, linked] = await Promise.all([
    query,
    db.from("search_anchors").select("id", { count: "exact", head: true }).eq("is_active", true),
    db.from("search_anchors").select("id", { count: "exact", head: true }).eq("review_status", "approved"),
    db.from("search_anchors").select("id", { count: "exact", head: true }).eq("review_status", "pending_review"),
