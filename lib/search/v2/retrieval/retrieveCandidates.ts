...
function candidateFrom(location: any, request: ReturnType<typeof buildRetrievalRequests>[number], source: string): RetrievedCandidate {
  ...
  return {
    ...
    matchedRetrievalTerms: request.retrievalTerms.filter((term) => serialized.includes(term.toLowerCase())),
...
export async function retrieveCandidates({ plan, supabase, trace }: { plan: SearchPlan; supabase: SupabaseClient; trace: SearchTrace }): Promise<RetrievalResult> {
  ...
  const lanes = await Promise.all(requests.map(async (request) => {
    ...
    if (rollout.serveProfiles || rollout.shadowProfiles) {
      try {
        profileRows = await retrieveProfileLocations(supabase, request);
      } catch (error) {
        trace.decisions.push({ stage: "retrieval", decision: "profile_rpc_failed", reason: error instanceof Error ? error.message : "unknown profile retrieval failure" });
      }
  ...
  trace.decisions.push({
    stage: "retrieval",
    decision: "retrieval_source_selected",
    reason: `mode=${rollout.mode}, served=${trace.retrieval.servedSource}, profile=${trace.retrieval.profileCandidateCount}, legacy=${trace.retrieval.legacyCandidateCount}, fallback_domains=${trace.retrieval.fallbackDomains.join(",") || "none"}`,
  });
