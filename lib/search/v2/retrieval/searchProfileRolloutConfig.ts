...
function environmentConfig(): SearchProfileRolloutConfig {
  ...
  return {
    mode,
    canaryPercent: boundedPercent(process.env.SEARCH_PROFILE_CANARY_PERCENT, 5),
    killSwitch: false,
...
const readConfig = unstable_cache(
  async (): Promise<SearchProfileRolloutConfig> => {
    ...
    try {
      ...
      return {
        mode: validModes.has(value.mode as SearchProfileMode) ? (value.mode as SearchProfileMode) : fallback.mode,
        canaryPercent: boundedPercent(value.canaryPercent, fallback.canaryPercent),
...
export async function updateSearchProfileRolloutConfig(
  ...
  await supabaseAdmin.from("admin_audit_logs").insert({
    ...
    summary: "Search profile rollout configuration updated",
    metadata: {
      previous,
      next,
