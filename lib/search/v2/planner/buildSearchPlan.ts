...
export async function buildSearchPlan({ input }: { input: SearchPlannerInput }): Promise<SearchPlan> {
  ...
  const resolvedCity = coordinateFirstLocality ? null : geoRecord?.city ?? (geoRecord?.type === "city" ? geoRecord.name : place?.[1] ?? null);
  ...
  const plan: SearchPlan = {
    ...
    geo: { source: anchored ? "anchor" : place ? "explicit" : current ? "current_location" : "default_market", market: place?.[3] ?? input.market ?? null, city: resolvedCity, borough: geoRecord?.borough ?? (geoRecord?.type === "borough" ? geoRecord.name : place?.[2] ?? null), neighborhood: geoRecord?.type === "neighborhood" ? geoRecord.name : null, county: resolvedCounty, state: geoRecord?.state ?? "NY", latitude, longitude, radiusMiles, strictness: place ? "strict" : current ? "preferred" : "broad" },
