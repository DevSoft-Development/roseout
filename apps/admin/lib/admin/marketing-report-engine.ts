...
export async function runMarketingReport(config: MarketingReportConfig): Promise<MarketingReportResult> {
  ...
  const [analyticsResult, searchResult, locationEventResult, attributionResult, locationsResult, socialResult] = await Promise.all([
    getAdminDatabaseClient().from("analytics_events").select("event_name,event_type,canonical_event_name,page_path,session_id,anonymous_id,source,device_type,metadata,created_at").gte("created_at", earliest.toISOString()).lt("created_at", current.end.toISOString()).eq("is_bot", false).limit(20000),
    getAdminDatabaseClient().from("search_events").select("created_at,raw_query,normalized_query,search_type,primary_domain,city,borough,neighborhood").eq("source", "public_create_search").eq("route", "/api/generate").gte("created_at", earliest.toISOString()).lt("created_at", current.end.toISOString()).limit(20000),
    getAdminDatabaseClient().from("location_analytics_events").select("location_id,event_type,event_name,created_at").gte("created_at", earliest.toISOString()).lt("created_at", current.end.toISOString()).limit(20000),
    getAdminDatabaseClient().from("marketing_attribution_events").select("source,medium,campaign,event_type,content_item_id,social_post_id,occurred_at").gte("occurred_at", earliest.toISOString()).lt("occurred_at", current.end.toISOString()).limit(20000),
    getAdminDatabaseClient().from("locations").select("id,name,city,state,borough,neighborhood,location_type,cuisine,activity_type,category").limit(10000),
    getAdminDatabaseClient().from("social_post_metric_snapshots").select("social_post_id,provider,captured_at,views,reach,likes,comments,shares,saves,clicks").gte("captured_at", earliest.toISOString()).lt("captured_at", current.end.toISOString()).limit(20000),
  ]);
