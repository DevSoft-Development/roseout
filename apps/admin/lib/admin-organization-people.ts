...
export type AdminOrganizationPerson={userId:string;email:string;name:string;role:string};

function displayName(user:any,email:string){
  const candidate=[user?.user_metadata?.full_name,user?.user_metadata?.name,user?.user_metadata?.display_name].find((value)=>typeof value==="string"&&value.trim());
...
export async function listAdminOrganizationPeople():Promise<AdminOrganizationPerson[]>{
  ...
  return (adminRows||[]).map((row:any)=>{
    ...
    return {userId:row.user_id as string,email,name:displayName(user,email),role:String(row.role||"staff")} satisfies AdminOrganizationPerson;
