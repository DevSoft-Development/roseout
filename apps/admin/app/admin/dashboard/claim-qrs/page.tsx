...
export default async function AdminClaimQrPrintPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const [allCountResult, missingQrResult, missingCodeResult, unclaimedResult] = await Promise.all([
    adminDb.from("locations").select("id", { count: "exact", head: true }),
    adminDb.from("locations").select("id", { count: "exact", head: true }).or("qr_code_data_url.is.null,claim_qr_url.is.null"),
    adminDb.from("locations").select("id", { count: "exact", head: true }).is("claim_code", null),
  ...
  for (const row of data || []) {
    ...
    if (missingQr || hasLegacyClaimUrl) {
      ...
      await adminDb.from("locations").update(fields).eq("id", row.id);
  ...
  const locations: ClaimQrLocation[] = (data || []).map((row) => ({
    ...
    name: String(row.name || row.restaurant_name || row.activity_name || "Untitled location"),
    address: row.address,
    city: row.city,
    state: row.state,
