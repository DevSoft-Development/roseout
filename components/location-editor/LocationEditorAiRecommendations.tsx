...
export default function LocationEditorAiRecommendations({ context, form, onApply, targetSection }: { context: LocationEditorContext; form: Form; onApply?: (patch: Patch)=>void; targetSection?: string }) {
  ...
  async function suggest(){setBusy(true);setMessage("");try{const res=await fetch("/api/locations/optimize",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({id:context.effectiveLocationId,location_id:context.effectiveLocationId,type:"locations",...form})});const json=await res.json();if(!res.ok) throw new Error(json.error||"Suggestions are unavailable right now.");setSuggestions(json.suggestions||{});}catch(e:any){setMessage(e.message||"Suggestions are unavailable right now.");}finally{setBusy(false)}}
