...
export const metadata: Metadata = buildMetadata({
  ...
  description:
    "Plan dinner, activities, nightlife, and nearby experiences in one TheOutHaven outing across NYC and Long Island.",
  path: "/",
});
...
export default function HomePage() {
  return (
    <main className="min-h-dvh bg-[#050505] text-white">
      ...
      <section
        id="prelaunch"
        className="mx-auto grid w-full max-w-7xl gap-8 px-5 py-12 sm:px-6 lg:grid-cols-2 lg:px-8"
      >
        <div>
          ...
          <h2 className="mt-3 text-3xl font-black">
            TheOutHaven is preparing for launch.
          </h2>

          <p className="mt-4 text-white/62">
            Request early access. Launch coverage begins in NYC and Long
            Island.
          </p>
        </div>
  ...
  return (
    <aside className="min-w-0 rounded-[2rem] border border-white/10 bg-[radial-gradient(circle_at_top_right,rgba(225,6,42,.22),transparent_36%),rgba(255,255,255,.045)] p-5 shadow-2xl shadow-black/40">
      ...
      <div className="mt-5 space-y-3">
        <div className="rounded-[1.25rem] bg-black/35 p-4">
          <p className="text-sm text-white/50">Restaurant · Astoria</p>
          <h2 className="mt-1 text-xl font-black">Seafood dinner</h2>
        </div>

        <div className="rounded-[1.25rem] bg-black/35 p-4">
          <p className="text-sm text-white/50">Nightlife · Astoria</p>
          <h2 className="mt-1 text-xl font-black">Rooftop drinks</h2>
        </div>
      </div>

      <p className="mt-5 text-sm leading-6 text-white/62">
        About a 12-minute walk. Matches because both stops fit a relaxed
        dinner-plus-drinks plan in the same neighborhood.
      </p>
    </aside>
  );
}
