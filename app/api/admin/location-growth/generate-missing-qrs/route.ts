...
export async function POST(request: NextRequest) { const auth = await authorize(request); if (auth) return auth; const body = await request.json().catch(() => ({})); const result = await generateMissingLocationQrs(body.limit || 100); return NextResponse.json({ success: true, ...result }); }
