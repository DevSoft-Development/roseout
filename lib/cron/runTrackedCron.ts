...
async function sendConfiguredCronEmail(row: any, success: boolean, message: string, startedAt: string, finishedAt: string, durationMs: number, data: Record<string, unknown>, error?: string): Promise<EmailDelivery> {
...
async function persistEmailDelivery(runId: string | number | undefined, baseDetails: Record<string, unknown>, delivery: EmailDelivery, recipientsList?: string[]) {
  if (!runId) return;
  await supabaseAdmin
    .from("cron_job_runs")
    .update({
