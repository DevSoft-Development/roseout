...
function anchorCandidate(candidate: any) {
  return { id: candidate?.linkedLocationId ?? candidate?.linked_location_id ?? candidate?.id ?? null, name: candidate?.canonicalName ?? candidate?.canonical_name ?? candidate?.name ?? null };
}
...
async function resolvePlanAnchor(plan: SearchPlan, supabase: SupabaseClient): Promise<{ plan: SearchPlan; trace: AnchorResolutionTrace }> {
  ...
  const latitude = Number(anchor.latitude); const longitude = Number(anchor.longitude);
  ...
  return {
    plan: { ...plan, geo: { ...plan.geo, source: "anchor", latitude, longitude, radiusMiles: hardRadius ?? defaultRadius, strictness: plan.travel.constraint === "hard" ? "strict" : "preferred", city: null, borough: null, neighborhood: null, county: null }, anchor: { ...plan.anchor, locationId: String(resolvedLocationId), name: anchor.canonicalName ?? anchor.canonical_name ?? anchor.name, latitude, longitude } },
...
function recordCandidateStageDiagnostics({ trace, plan, retrieved, qualified, rawScored, scored }: { trace: ReturnType<typeof createSearchTrace>; plan: SearchPlan; retrieved: any; qualified: any[]; rawScored: { all: ScoredCandidate[]; restaurants: ScoredCandidate[]; activities: ScoredCandidate[] }; scored: { all: ScoredCandidate[]; restaurants: ScoredCandidate[]; activities: ScoredCandidate[] } }) {
  ...
  for (const candidate of allCandidates) {
    ...
    if (!candidate?.geoMatch?.accepted) rejectedCandidates.push({ locationId, desiredRole: candidate?.requestedRoles?.[0] ?? null, originalType: candidateOriginalType(candidate), assignedDomain: null, rejectedAtStage: "geo", rejectionReason: candidate?.geoMatch?.reason ?? "geo_not_accepted", matchedTerms: candidateTerms(candidate) });
    else if (locationId && !qualifiedIds.has(locationId)) rejectedCandidates.push({ locationId, desiredRole: candidate?.requestedRoles?.[0] ?? null, originalType: candidateOriginalType(candidate), assignedDomain: null, rejectedAtStage: "domain_assignment", rejectionReason: "no_qualified_requested_role", matchedTerms: candidateTerms(candidate) });
    else if (locationId && !taxonomyIds.has(locationId)) rejectedCandidates.push({ locationId, desiredRole: candidate?.requestedRoles?.[0] ?? null, originalType: candidateOriginalType(candidate), assignedDomain: null, rejectedAtStage: "taxonomy", rejectionReason: "taxonomy_or_scoring_excluded", matchedTerms: candidateTerms(candidate) });
    else if (locationId && !finalIds.has(locationId)) rejectedCandidates.push({ locationId, desiredRole: candidate?.requestedRoles?.[0] ?? null, originalType: candidateOriginalType(candidate), assignedDomain: null, rejectedAtStage: "final_domain", rejectionReason: "not_in_required_final_domain", matchedTerms: candidateTerms(candidate) });
  }
  ...
  const restaurantRequired = Boolean(plan.restaurant.required); const activityRequired = Boolean(plan.activity.required);
  ...
  const missingRestaurant = restaurantRequired && scored.restaurants.length === 0; const missingActivity = activityRequired && scored.activities.length === 0;
...
export async function searchV2(input: SearchPlannerInput & { supabase: SupabaseClient; rolloutOverride?: SearchProfileRolloutOverride }) {
  ...
  const deterministicPlan = await buildSearchPlan({ input: { ...input, requestId: trace.requestId } });
  const learned = await applyLearnedIntent({ plan: deterministicPlan, supabase: input.supabase });
  trace.decisions.push({ stage: "learned_intent", decision: learned.diagnostics.additions.length ? "safe_semantic_slots_enriched" : "deterministic_plan_preserved", reason: JSON.stringify(learned.diagnostics) });
  const anchorResult = await resolvePlanAnchor(learned.plan, input.supabase);
  ...
  const successful = (response as any).requestFulfilled !== false && ((response as any).counts?.restaurants ?? 0) + ((response as any).counts?.activities ?? 0) + ((response as any).counts?.pairs ?? 0) > 0;
  void rememberSuccessfulQuery({ plan, supabase: input.supabase, success: successful });
  return response;
}
