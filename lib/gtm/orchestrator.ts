...
export async function reconcileGtmLocation(locationId:string,{dryRun=false}:{dryRun?:boolean}={}){
  ...
  const lifecycle=lifecycleFor(l,metrics,score); const shouldFull=score.opportunityScore>=60||highIntent||email||score.claimed||score.paid; const suppressed=Boolean(l.do_not_contact); const associationLevel=shouldFull?'full':'lightweight';
