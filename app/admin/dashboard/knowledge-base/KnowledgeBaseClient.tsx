...
import { Archive, BookOpen, Copy, FileText, Globe, Lock, Pencil, Plus, Search, Sparkles } from "lucide-react";
...
function Chip({ children }: { children: React.ReactNode }) {
  return <span className="rounded-full border border-amber-200/15 bg-amber-300/10 px-3 py-1 text-xs font-bold text-amber-100">{children}</span>;
}

export default function KnowledgeBaseClient({ articles, categories, role, page, pageSize, count }: Props) {
  ...
  const update = (key: string, value: string) => {
    ...
    next.delete("page");
    router.push(`/admin/dashboard/knowledge-base?${next.toString()}`);
  };
  ...
  return (
    <main className="px-4 pb-16 pt-8 text-white sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl space-y-8">
        <section className="rounded-[2rem] border border-rose-200/15 bg-gradient-to-br from-[#21100f] via-[#120b0a] to-[#090706] p-6 shadow-[0_24px_80px_rgba(0,0,0,0.45)] md:p-8">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="mb-3 inline-flex items-center gap-2 rounded-full border border-amber-200/20 bg-amber-300/10 px-4 py-2 text-xs font-black uppercase tracking-[0.24em] text-amber-100"><BookOpen className="h-4 w-4" /> Knowledge Base</p>
              <h1 className="text-4xl font-black tracking-tight md:text-6xl">Team answers, scripts, and public help.</h1>
              <p className="mt-4 max-w-2xl text-base text-rose-100/70">Search approved internal guidance, reusable templates, and public help center content without leaving the admin console.</p>
            </div>
            <div className="flex flex-wrap gap-3">
              {canEdit && <Link href="/admin/dashboard/knowledge-base/new" className="inline-flex items-center gap-2 rounded-full bg-amber-200 px-5 py-3 text-sm font-black text-[#32130f]"><Plus className="h-4 w-4" /> New Article</Link>}
        ...
        <section className="grid gap-4 lg:grid-cols-[1fr_320px]">
          ...
          <div className="grid gap-3">
            <Link href="/admin/dashboard/knowledge-base/ai" className="rounded-3xl border border-amber-200/20 bg-amber-300/10 p-5"><Sparkles className="mb-3 h-6 w-6 text-amber-100" /><b>KB AI Assistant</b><p className="mt-1 text-sm text-rose-100/65">Ask questions answered only from approved content.</p></Link>
        ...
        {templates.length > 0 && <section><div className="mb-4 flex items-center justify-between"><h2 className="text-2xl font-black">Template Library</h2><Link href="/admin/dashboard/knowledge-base/templates" className="text-sm font-black text-amber-100">View all →</Link></div><div className="grid gap-4 md:grid-cols-3">{templates.map((article) => <ArticleCard key={article.id} article={article} canEdit={canEdit} compact />)}</div></section>}
