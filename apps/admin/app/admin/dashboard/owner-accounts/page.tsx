...
export default async function OwnerAccountsPage({
  ...
  let ownersQuery = getAdminDatabaseClient()
    .from("users")
      ...
      <div className="mx-auto max-w-6xl space-y-5">
        ...
        <section className="rounded-[1.75rem] border border-white/10 bg-[#120d0b] p-4 shadow-2xl">
          {!ownerAccounts.length ? (
            <p className="rounded-2xl border border-dashed border-white/10 p-8 text-center text-white/55">
            ...
            <div className="space-y-3">
              {ownerAccounts.map((owner) => (
                <div
                  key={owner.id}
                  className="grid gap-3 rounded-2xl border border-white/10 bg-white/[0.04] p-4 md:grid-cols-[1fr_auto] md:items-center"
                >
                  <div className="min-w-0">
                    <p className="truncate font-black text-white">
                      {owner.full_name || owner.email || "Unnamed owner"}
                    </p>
                    <p className="mt-1 truncate text-sm text-white/55">
                      {owner.email || "No email"}
                    </p>
                    <p className="mt-2 text-xs font-semibold uppercase tracking-[0.18em] text-white/35">
                      Joined {formatDate(owner.created_at)} · {owner.role || "owner"}
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-2 md:justify-end">
                    <Link
                      href={`/admin/dashboard/users/${owner.id}`}
                      className="rounded-full border border-white/10 bg-black/25 px-4 py-2 text-xs font-black text-white/75 transition hover:bg-white/10 hover:text-white"
                    >
                      View user
                    </Link>
                    {canImpersonate ? (
