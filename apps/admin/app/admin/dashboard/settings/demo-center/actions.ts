...
async function admin() {
  return requireAdminRole(["superadmin", "admin", "manager"]);
}
...
async function invokeDemoReservationFunction(
  ...
  const { data, error } = await getAdminDatabaseClient().functions.invoke(functionName, {
...
async function ensureDueDemoReminder(locationId: string) {
  let { data: reservation } = await getAdminDatabaseClient()
  ...
  if (!reservation?.id) {
    ...
    const retry = await getAdminDatabaseClient()
      .from("location_reservations")
      .select("id,location_id,status")
      .eq("location_id", locationId)
      .in("status", ["pending", "confirmed", "arrived", "seated", "checked_in"])
  ...
  const { data: existing } = await getAdminDatabaseClient()
...
async function updateLocationReservation(
  ...
  return safely(`reservation ${status}`, async () => {
    ...
    let result = await getAdminDatabaseClient()
      .from("location_reservations")
      .update(updates)
      .eq("id", id)
      .eq("location_id", location.id)
    ...
    if (result.error) {
      result = await getAdminDatabaseClient()
        .from("location_reservations")
        .update({ status })
        .eq("id", id)
        .eq("location_id", location.id)
