...
create or replace function public.retry_worker_job(p_job_id uuid,p_grant_attempt boolean default false,p_run_after timestamptz default now()) returns public.worker_jobs language plpgsql security definer set search_path=public as $$
declare
  v_job public.worker_jobs;
  v_previous_max_attempts integer;
  v_attempt_count integer;
  v_new_max_attempts integer;
begin
  select max_attempts, attempt_count into v_previous_max_attempts, v_attempt_count from public.worker_jobs where id=p_job_id for update;
  if v_previous_max_attempts is null then
    raise exception 'Worker job not found' using errcode='P0002';
  end if;
  if v_attempt_count >= v_previous_max_attempts and not coalesce(p_grant_attempt,false) then
    raise exception 'ATTEMPTS_EXHAUSTED: Worker job has used all allowed attempts' using errcode='P0001', hint='ATTEMPTS_EXHAUSTED';
  end if;
  v_new_max_attempts := case when coalesce(p_grant_attempt,false) and v_attempt_count >= v_previous_max_attempts then v_previous_max_attempts + 1 else v_previous_max_attempts end;
  update public.worker_jobs set status='queued', run_after=coalesce(p_run_after,now()), max_attempts=v_new_max_attempts, completed_at=null, lease_owner=null, lease_expires_at=null, updated_at=now() where id=p_job_id and status in ('failed','dead_letter','cancelled') returning * into v_job;
  if v_job.id is null then
    raise exception 'Worker job is not retryable' using errcode='P0001';
  end if;
  insert into public.worker_job_events(job_id,event_type,message,metadata,created_by) values(p_job_id,case when coalesce(p_grant_attempt,false) and v_new_max_attempts > v_previous_max_attempts then 'attempt_granted' else 'retried' end,'Job returned to queue',jsonb_build_object('previous_max_attempts',v_previous_max_attempts,'new_max_attempts',v_new_max_attempts,'attempt_count',v_attempt_count,'grant_attempt',coalesce(p_grant_attempt,false),'requested_run_after',p_run_after,'source','admin_workers_retry'),'retry_worker_job');
  return v_job;
end $$;
...
revoke all on function public.retry_worker_job(uuid,boolean,timestamptz) from anon, authenticated;
