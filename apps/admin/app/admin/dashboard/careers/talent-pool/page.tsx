...
export default async function CareersTalentPoolPage() {
  await requireAdminRole([...CAREERS_VIEW_ROLES]);
  const result = await getAdminDatabaseClient()
    .from("career_talent_pool")
    .select("id,candidate_name,email,tags,status,last_contacted_at")
  ...
  return (
    <main className="min-h-screen bg-[#090706] px-4 py-6 text-white sm:px-6 lg:px-8">
      <div className="mx-auto max-w-[1400px] space-y-6">
        ...
        {result.error ? <section className="rounded-2xl border border-rose-300/25 bg-rose-500/10 p-4 text-sm font-bold text-rose-100">Unable to load talent pool: {result.error.message}</section> : null}
        ...
        <section className="overflow-hidden rounded-3xl border border-white/10 bg-[#120d0b]">
          {rows.length ? <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-sm">
            <thead className="bg-white/[.03] text-left text-[10px] uppercase tracking-[.16em] text-white/40"><tr><th className="p-3">Candidate</th><th className="p-3">Status</th><th className="p-3">Email</th><th className="p-3">Tags</th><th className="p-3">Last Contacted</th></tr></thead>
            <tbody>{rows.map((row:any) => {
              const tone = getCareerStageTone(row.status);
              return <tr key={row.id} className="border-t border-white/10">
                <td className="p-3 font-black text-white">{row.candidate_name || row.id}</td>
                ...
                <td className="p-3 text-white/65">{row.email || "—"}</td>
                <td className="p-3 text-white/65">{Array.isArray(row.tags) ? row.tags.join(", ") || "—" : row.tags || "—"}</td>
                <td className="p-3 text-white/65">{formatCareerDate(row.last_contacted_at)}</td>
              </tr>;
            })}</tbody>
