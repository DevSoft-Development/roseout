export const ML_SCORE_VERSION = "ml_rank_v1";

export type LocationMlFeatureInput = {
  impressions_30d?: number | null;
  views_30d?: number | null;
  clicks_30d?: number | null;
  reservation_clicks_30d?: number | null;
  call_clicks_30d?: number | null;
  website_clicks_30d?: number | null;
  saves_30d?: number | null;
  completed_outings_30d?: number | null;
  negative_signals_30d?: number | null;
  ctr_30d?: number | null;
...
export function calculateLocationMlScore(input: LocationMlFeatureInput) {
  ...
  if (impressions === 0 && views === 0 && clicks === 0 && conversions === 0) return 0;
  ...
  if (impressions < 10) score *= 0.35;
  else if (impressions < 25) score *= 0.65;

  return Number(cap(score, 0, 100).toFixed(2));
}

export function calculateMlBoost(mlScore: number | null | undefined) {
