...
function pairingEligibility(response: any) {
  ...
  try {
    return JSON.parse(decision.reason);
  } catch {
    return { primaryFailure: decision.reason };
  }
}
...
function laneDiagnostics(response: any) {
  ...
  return (["restaurant", "activity"] as const).map((domain) => {
    const matching = calls.filter((call: any) =>
      (call.domain ?? (String(call.role).includes("restaurant") ? "restaurant" : "activity")) === domain,
    );
    return {
      domain,
      ...
      profileRetrieved: matching
        .filter((call: any) => String(call.reason).includes("canonical_profile"))
        .reduce((sum: number, call: any) => sum + Number(call.resultCount ?? 0), 0),
...
const isServedDomain = (value: unknown): value is ServedDomain => value === "restaurant" || value === "activity";
...
export function evaluateReplayCase(query: any, legacy: any, canonical: any, strictCanonical: any) {
  ...
  const enforceExactDomainSet = expectedDomains.length > 0;
  const unexpectedDomains = enforceExactDomainSet
    ? [...servedDomains].filter((domain) => !expectedDomains.includes(domain))
    : [];
  const wrongDomain = missingDomains.length > 0 || unexpectedDomains.length > 0 || slotMismatches.length > 0;
...
export function buildQueryFailureReport(query: any, canonical: any, strictCanonical: any, comparison: any) {
  ...
  const parsedDomains = [...new Set(calls.map((call: any) =>
    call.domain ?? (String(call.role).includes("restaurant") ? "restaurant" : "activity"),
  ))];
  ...
  return {
    ...
    unexpectedDomains: comparison.unexpectedDomains,
    profileTerms,
    fallbackReason: canonical.retrieval?.legacyFallbackUsed
...
function snapshot(response: any) {
  ...
  return {
    ...
    debug: {
      retrievalCalls: retrievalCalls(response),
      decisions: (Array.isArray(response?.debug?.decisions) ? response.debug.decisions : [])
        .filter((decision: any) => ["retrieval_domain", "pairing_eligibility"].includes(decision?.stage)),
    },
  };
}

async function failRun(runId: string, error: string, persisted: number, queryCount: number) {
  await supabaseAdmin
    .from("search_quality_replay_runs")
    .update({
      status: "failed",
      metrics: { persistenceError: error, persistedRowCount: persisted, queryCount },
...
export async function POST(request: Request) {
  ...
  if (source === "production_replay") {
    ...
    cases = (data ?? []).map((row: any) => ({
      ...
      category: "production",
      query: row.raw_query,
      expectations: { expectedDomains: [] },
    }));
  }

  const { data: run, error } = await supabaseAdmin
    .from("search_quality_replay_runs")
    .insert({ source, status: "running", query_count: cases.length, created_by: auth.adminUser!.user_id })
    .select("id")
    .single();
  ...
  for (const testCase of cases) {
    try {
      ...
      rows.push({
        run_id: run.id,
        source_search_id: testCase.sourceSearchId ?? null,
      ...
      rows.push({
        ...
        expectations: testCase.expectations,
        comparison: {
          error: runError instanceof Error ? runError.message : "Replay failed",
          contractFailure: true,
          failureReport: {
            query: testCase.query,
            expectedDomains: testCase.expectations?.expectedDomains ?? [],
            unexpectedDomains: [],
            passed: false,
          },
        },
  ...
  for (let index = 0; index < rows.length; index += BATCH_SIZE) {
    ...
    const { error: insertError } = await supabaseAdmin.from("search_quality_replay_items").insert(batch);
    if (insertError) {
      await failRun(run.id, insertError.message, persisted, cases.length);
  ...
  if (persisted !== cases.length) {
    const message = `Persisted ${persisted} of ${cases.length} replay items`;
    await failRun(run.id, message, persisted, cases.length);
    return NextResponse.json({
      success: false,
      runId: run.id,
      error: message,
  ...
  const metrics: SearchQualityMetrics = {
    ...
    pairedQuerySuccessRate: paired.length
      ? (paired.filter((row) => row.comparison?.pairedPass).length / paired.length) * 100
      : 100,
