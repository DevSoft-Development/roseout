...
function normalizedV2Telemetry(args: SearchEventLoggerArgs) {
  const metadata = args.metadata ?? {};
  const normalizedIntent = metadata.normalizedIntent ?? {};
  const debugParity = metadata.debugParity ?? {};
  const searchCoreVersion =
    safeText(normalizedIntent.searchCoreVersion, 40) ??
    safeText(metadata.searchCoreVersion, 40) ??
    safeText(debugParity.searchCoreVersion, 40) ??
    (safeText(metadata.search_system, 80)?.toLowerCase().includes("v2")
      ? "v2"
      : null);
  const fallbackUsed =
    safeBool(normalizedIntent.fallbackUsed) ??
    safeBool(metadata.fallbackUsed) ??
    safeBool(debugParity.fallbackUsed) ??
    false;
  const fallbackOutcome = safeText(
    normalizedIntent.fallbackOutcome ?? metadata.fallbackOutcome,
    80,
  );
  const fallbackType = safeText(
    normalizedIntent.fallbackType ?? metadata.fallbackType,
    120,
  );
  const fallbackReason = safeText(
    normalizedIntent.fallbackReason ?? metadata.fallbackReason,
    180,
  );
  const requestFulfilled =
    safeBool(normalizedIntent.requestFulfilled) ??
    safeBool(metadata.requestFulfilled);
  const partialResults =
    safeBool(normalizedIntent.partialResults) ??
    safeBool(metadata.partialResults) ??
    false;

  const fallbackParts = fallbackUsed
    ? [fallbackOutcome ?? "unknown", fallbackType, fallbackReason]
        .filter(Boolean)
        .map((value) => String(value).replaceAll("|", "/"))
    : [];
  const fallbackSummary = fallbackParts.length
    ? fallbackParts.join("|").slice(0, 500)
    : null;

  const issueCodes = new Set<string>();
  if (partialResults) issueCodes.add("partial_results");
  if (requestFulfilled === false) issueCodes.add("request_not_fulfilled");
  if (fallbackUsed) {
    issueCodes.add("fallback_used");
    if (fallbackType) issueCodes.add(`fallback_${fallbackType}`);
  }
...
export async function logSearchEvent(
  ...
  try {
    ...
    const restaurantCount =
      safeInteger(counts.restaurants ?? counts.restaurant_count) ?? 0;
    const activityCount =
      safeInteger(counts.activities ?? counts.activity_count) ?? 0;
    const pairCount = safeInteger(counts.pairs ?? counts.pair_count) ?? 0;
    const resultCount =
      safeInteger(counts.finalDisplayedResultCount) ??
      safeInteger(performance.result_count) ??
      restaurantCount + activityCount + pairCount;

    const mlIntent = classifySearchIntent(rawQuery || normalizedQuery || "");
    ...
    const v2 = normalizedV2Telemetry(args);
    ...
    const cleanedMetadata = cleanMetadata(
      {
        ...
        requestFulfilled:
          mixedWithoutPair ? false : v2.requestFulfilled ?? args.metadata?.requestFulfilled,
        partialResults:
          mixedPartial ? true : v2.partialResults || args.metadata?.partialResults === true,
        no_pairs_reason: noPairsReason,
        fallback_used: v2.fallbackUsed,
      },
      restaurantCount,
      activityCount,
    );
    ...
    const row = {
      ...
      county: safeText(geo.county, 120),
      zip_code: safeText(geo.zipCode ?? geo.zip_code ?? geo.postal_code, 12),
      geo_source: safeText(geo.geoSource ?? geo.geo_source ?? geo.source, 80),
      resolved_market: safeText(geo.resolvedMarket ?? geo.resolved_market ?? geo.market, 100),
      ...
      pair_candidates_evaluated: safeInteger(counts.pairCandidatesEvaluated),
      ...
      timing_ms: safeInteger(performance.total_ms ?? performance.timing_ms),
      ...
      no_pairs_reason: noPairsReason,
      search_core_version: v2.searchCoreVersion,
      v2_issue_codes: v2.issueCodes.length ? v2.issueCodes : null,
      v2_fallback_outcome: v2.fallbackSummary,
    ...
    console.info("[search-events] insert succeeded", {
      ...
      source: row.source,
      route: row.route,
      search_core_version: row.search_core_version,
      v2_fallback_outcome: row.v2_fallback_outcome,
    });
