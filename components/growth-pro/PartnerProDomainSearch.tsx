...
export default function PartnerProDomainSearch({ locationId, claimedDomain, onRegistered }: { locationId: string; claimedDomain?: string | null; onRegistered?: (domain: string) => void | Promise<void> }) {
  ...
  async function registerDomain(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (response.ok && data?.status === "active" && typeof data?.domain === "string" && onRegistered) {
        await onRegistered(data.domain);
      }
    } catch (cause) {
  ...
  return (
    <div className="space-y-5">
      <section className="rounded-3xl border border-rose-200/15 bg-white/[0.04] p-6">
        <p className="text-xs font-black uppercase tracking-[0.22em] text-rose-200">Essentials domain benefit</p>
        <h2 className="mt-3 text-2xl font-black">Find and register a new domain</h2>
        <p className="mt-2 text-sm leading-6 text-white/65">Choose one eligible standard domain for your location. The first year of registration is included with Essentials through TheOutHaven&apos;s registrar connection. Renewal after the first year is billed separately.</p>
        {claimedDomain ? <div className="mt-5 rounded-2xl border border-emerald-300/20 bg-emerald-500/10 p-4"><p className="text-xs font-black uppercase tracking-[0.18em] text-emerald-100/70">Included domain</p><p className="mt-2 text-xl font-black text-emerald-50">{claimedDomain}</p><p className="mt-1 text-sm text-emerald-100/70">This location has already used its one-time first-year domain benefit.</p></div> : null}
      </section>
