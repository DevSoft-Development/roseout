...
function formatAddedLocation(record: Record<string, unknown> | null, fallback: Record<string, unknown>, locationType: "restaurant" | "activity") {
  const source = record || fallback;
  const hours = source.opening_hours || source.current_opening_hours || source.regularOpeningHours || source.hours || source.weekday_text;
  return {
    id: source.id || null,
    name: source.name || source.restaurant_name || source.activity_name || fallback.name || null,
    ...
    category: source.primary_category || source.category || source.activity_type || source.primary_tag || source.cuisine || null,
    rating: source.rating || null,
    website: source.website || source.website_url || null,
    phone: source.phone || source.phone_number || null,
    hasPhoto: Boolean(source.image_url || source.main_image || source.photo_url || source.has_photo || source.has_photos),
...
async function upsertRestaurant(
  ...
  const row = await addClaimFields({
    ...
    status: "approved",
    market: normalizeMarketKey(marketValidation.correctedMarket || marketValidation.inferredMarket || options.requestedMarket || null),
  ...
  return { status: "imported" as const, validation: marketValidation, location: formatAddedLocation(restaurant, row, "restaurant") };
}
...
async function upsertActivity(
  ...
  return { status: "imported" as const, validation: marketValidation, location: formatAddedLocation(activity, row, "activity") };
}
...
export function resolveRequestedMarketForImport(options: GooglePlacesImportOptions = {}, candidateMarket?: MarketKey | string | null, queriesUsed?: string[]): ImportMarketResolution {
  ...
  const batch = options.batch || options.primaryTag || null;
  if (batch && /nyc|new york|brooklyn|queens|bronx|staten|long island|nassau|suffolk|jersey|nj|westchester|connecticut|\bct\b/i.test(batch)) {
    const batchMarket = inferMarketFromPlace({ query: batch, requestedArea: batch });
    if (normalizeMarketKey(batchMarket) !== "UNKNOWN") return { original: batch, resolved: normalizeMarketKey(batchMarket), source: "batch", confidence: "medium" };
  }
  const fallback = normalizeMarketKey(candidateMarket);
  if (fallback !== "UNKNOWN") return { original: String(candidateMarket), resolved: fallback, source: "candidate_inferred_market", confidence: "low" };
  return { original: areaText || null, resolved: "UNKNOWN", source: "unresolved", confidence: "low" };
}
...
function validateImportMarket(place: GooglePlace, options: GooglePlacesImportOptions = {}): MarketValidationResult {
  ...
  return validatePlaceForMarket({
    requestedMarket: resolveRequestedMarketForImport(options, inferMarketFromPlace({ requestedArea: options.requestedArea || options.areas, query: options.areas || undefined })).resolved,
...
export async function runGooglePlacesImport(options: GooglePlacesImportOptions = {}) {
  ...
  const meta = {
    ...
    duplicate_examples: [...restaurant.duplicate_examples, ...activity.duplicate_examples].slice(0, 10),
