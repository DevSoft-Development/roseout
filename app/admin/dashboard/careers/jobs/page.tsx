...
type CareerApplicationJobReference = {
  job_id: string | null;
};
...
export default async function Page() {
  ...
  if (jobs.length) {
    ...
    const applicationRows =
      (applications ?? []) as CareerApplicationJobReference[];

    counts = applicationRows.reduce<Record<string, number>>(
      (acc, application) => {
  ...
  return (
    <AdminPageShell>
      <AdminPageHeader
        ...
        actions={
          <>
            <AdminActionButton href="/admin/dashboard/careers">
              Overview
            </AdminActionButton>
            <AdminActionButton
              href="/admin/dashboard/careers/jobs/new"
              variant="primary"
            >
              Create Job
            </AdminActionButton>
      ...
      {jobs.length === 0 ? (
        <AdminEmptyState
          title="No career jobs have been created yet."
          body="Create your first public or private career posting for TheOutHaven."
          action={
            <AdminActionButton
              href="/admin/dashboard/careers/jobs/new"
              variant="primary"
                ...
                >
                  <td className="p-3">
                    <Link
                      ...
                      href={`/admin/dashboard/careers/jobs/${job.id}`}
                    >
                  ...
                  <td className="p-3">
                    <AdminStatusBadge tone={getJobStatusTone(job.status)}>
                      {formatValue(job.status)}
                    </AdminStatusBadge>
                  </td>
                  <td className="p-3 capitalize text-white/70">
                    {formatValue(job.visibility)}
                  </td>
                  <td className="p-3 font-black text-white">
                    {counts[job.id] ?? 0}
                  </td>
                  <td className="p-3 text-white/60">
                    {formatCareerDate(job.created_at)}
                  </td>
                  <td className="p-3">
                    <div className="flex flex-wrap gap-3">
                      ...
                      <Link
                        className="font-black text-rose-200"
                        href={`/admin/dashboard/careers/applications?jobId=${job.id}`}
                      >
                        Applications
                      </Link>
                    </div>
