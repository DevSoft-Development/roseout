...
export default async function AiTrustPage() {
  ...
  const [searches24h, healthIssues24h, verifiedReviews, openIncidents, recentIncidents] = await Promise.all([
    ...
    supabaseAdmin.from("ai_trust_incidents").select("id,incident_type,severity,surface,request_id,summary,status,created_at").order("created_at", { ascending: false }).limit(12),
  ]);
    ...
    <AdminPageShell>
      <AdminPageHeader
        ...
        actions={<>
          ...
          <AdminActionButton href="/admin/dashboard/trust">Verification</AdminActionButton>
        </>}
      />
          ...
          <div className="divide-y divide-white/10">
            {incidents.map((incident: any) => (
              <div className="grid gap-2 px-5 py-4 md:grid-cols-[160px_110px_1fr_180px]" key={incident.id}>
                ...
                <div><p className="font-bold text-white">{incident.summary}</p><p className="mt-1 text-xs text-white/35">{incident.surface || incident.incident_type}{incident.request_id ? ` · ${incident.request_id}` : ""}</p></div>
      ...
      <AdminSectionCard className="p-5">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            <Link className="rounded-xl border border-white/10 px-4 py-2 text-sm font-black text-white/70" href="/admin/dashboard/search-health">Search explanations</Link>
            <Link className="rounded-xl border border-white/10 px-4 py-2 text-sm font-black text-white/70" href="/admin/dashboard/marketing/promotions">Sponsored promotions</Link>
            <Link className="rounded-xl border border-white/10 px-4 py-2 text-sm font-black text-white/70" href="/admin/dashboard/security">Security</Link>
          </div>
        </div>
      </AdminSectionCard>
    </AdminPageShell>
