...
export async function ingestCommunityEvent(input: {
  ...
  if (input.externalMessageId) {
    const { data: duplicate } = await getAdminDatabaseClient()
  ...
  let conversationQuery = getAdminDatabaseClient()
  ...
  if (!conversationId) {
    const { data: created, error } = await getAdminDatabaseClient().from("social_community_conversations").insert({
      provider: input.provider,
    ...
    const { error } = await getAdminDatabaseClient().from("social_community_conversations").update({
      person_type: analysis.personType,
      intent: analysis.intent,
      occasion: analysis.occasion,
      area: analysis.area,
  ...
  if (["instagram", "facebook"].includes(input.provider) && canAutoHandle(settings, analysis, conversationType)) {
    try {
      ...
      await getAdminDatabaseClient().from("social_community_conversations").update({
        status: "needs_reply",
        metadata: {
