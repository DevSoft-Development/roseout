...
export async function POST(request: NextRequest) { const auth = await authorize(request); if (auth) return auth; const body = await request.json().catch(() => ({})); const result = await importNycRestaurants({ limit: body.limit || 1000, offset: body.offset || 0 }); return NextResponse.json({ success: true, ...result }); }
