...
type ImportCursor = { kind?: "restaurant" | "activity"; areaIndex?: number; queryIndex?: number; placeIndex?: number };
type Submitted = Quality & { type: ImportType; limit: number; maxQueries: number; batch: string; primaryTag: string; areas: string; market: Market; requestedMarket: Market; minRating: number; allowMarketCorrection: boolean; qualityPreset: Preset; cursor?: ImportCursor | null };
...
type DuplicateSkippedLocation = { name: string; address?: string; matchedBy?: string; existingId?: string; reason?: string };
...
function normalizeDuplicateSkipped(value: unknown): DuplicateSkippedLocation | null {
  if (!isRecord(value)) return null;
  const name = firstText(value, ["name", "existingName", "restaurant_name", "activity_name", "location_name", "place_name"]) || "Unknown location";
  return {
    name,
    address: firstText(value, ["address", "formatted_address", "vicinity", "street_address"]),
    matchedBy: firstText(value, ["matchedBy", "matched_by", "match", "reason_code"]),
    existingId: firstText(value, ["existingId", "existing_id", "id", "database_id"]),
    reason: firstText(value, ["reason", "message", "details"]),
  };
}
function getDuplicateSkippedLocations(result: unknown): DuplicateSkippedLocation[] {
  if (!isRecord(result)) return [];
  const candidates = [result.duplicatesSkipped, result.duplicate_examples, result.duplicates, isRecord(result.results) ? result.results.duplicatesSkipped : undefined, isRecord(result.details) ? result.details.duplicatesSkipped : undefined, isRecord(result.summary) ? result.summary.duplicatesSkipped : undefined];
  const list = candidates.find(Array.isArray);
  return Array.isArray(list) ? list.map(normalizeDuplicateSkipped).filter((location): location is DuplicateSkippedLocation => Boolean(location)) : [];
}
function yesNo(value: boolean) { return value ? "Yes" : "No"; }

export function friendlyImportError(result: Record<string, unknown> | null | undefined) {
  const raw = String(result?.raw || result?.error || "");
  if (result?.status === 504 || raw.includes("FUNCTION_INVOCATION_TIMEOUT")) {
    return {
      headline: "Google import took too long",
      explanation: "The server stopped the import before it could finish. No action is needed on the locations already saved. Run a smaller batch or use Continue Import after this fix.",
      nextSteps: [
        "Use one area at a time.",
        "Keep Max queries at 1–3.",
        "Keep Import limit at 3–5 for manual runs.",
        "Use Continue Import for larger imports.",
      ],
    };
  }
  return null;
...
export function buildImportSummary(result: Record<string, unknown> | null | undefined, submitted: Submitted | null): ImportSummary {
  ...
  const duplicateCount = Number(result?.duplicateCount ?? result?.skipped_duplicate ?? 0);
  const otherSkipped = Math.max(0, skipped - duplicateCount);
  ...
  let headline = result?.partial === true ? "Google import paused safely." : "Google import finished.";
  ...
  else if (result?.partial === true) { statusTone = "warning"; headline = "Google import paused safely before the server timeout."; }
  ...
  const duplicateDetailsMissing = duplicateCount > 0 && !Array.isArray(result?.duplicatesSkipped) && !Array.isArray(result?.duplicate_examples);
  const successExplanation = imported === 0 && duplicateCount > 0 && otherSkipped === 0 && failed === 0
    ? `Google import finished, but no new locations were added. The places found by Google already exist in your database, so they were skipped instead of being added again.${duplicateDetailsMissing ? " Duplicate details were not reported by the import route." : ""}`
    : `${result?.partial === true ? "Google import paused safely before the server timeout." : "Google import finished."} TheOutHaven checked ${checked || "the reported Google Places"} results. ${imported} new locations were added, ${duplicateCount} were skipped because they already exist, ${otherSkipped} were skipped because they did not meet quality requirements or market rules, and ${failed} failed.${duplicateDetailsMissing ? " Duplicate details were not reported by the import route." : ""} ${qualityText(q)} ${hoursNote}${missingHours}`;
  const explanation = statusTone === "error" ? `The server returned: ${errorMessage || status}. ${unauthorized ? "This usually means your admin session expired, your account does not have permission, or the route is missing the correct admin auth check." : "Review the advanced response below before retrying."}` : successExplanation;
  return { statusTone, headline, explanation, stats: [{ label: "Market", value: q.market }, { label: "Area searched", value: q.areas }, { label: "Location type", value: TYPE_LABELS[q.type] }, { label: "Category", value: titleize(q.batch) }, { label: "Google queries", value: String(Array.isArray(result?.queries_used) ? result.queries_used.length : q.maxQueries) }, { label: "Places checked", value: readCount(result, ["checked", "checkedCount", "total_found_from_google"]) }, { label: "Imported", value: readCount(result, ["imported", "importedCount"]) }, { label: "Duplicate skipped", value: readCount(result, ["duplicateCount", "skipped_duplicate"]) }, { label: "Other skipped", value: String(otherSkipped) }, { label: "Skipped", value: readCount(result, ["skipped", "skippedCount"]) }, { label: "Failed", value: readCount(result, ["failed", "failedCount"]) }, { label: "Skip reasons", value: skippedReasons }], qualityFilters: qualityText(q), nextSteps: imported > 0 ? ["Review the imported locations for images, hours display, tags, and public search readiness.", "Run another small batch only after checking duplicates and skipped reasons."] : ["Try a broader category, a lower minimum rating, or the Hours optional import preset if Google is missing hours.", "Check whether the places were already in your database or outside the selected market."] };
}
...
export function GoogleImportFormClient() {
  ...
  const [maxQueries, setMaxQueries] = useState("2"); const [limit, setLimit] = useState("5"); const [minRating, setMinRating] = useState("3.8");
  ...
  const [busy, setBusy] = useState(false); const [result, setResult] = useState<Record<string, unknown> | null>(null); const [submitted, setSubmitted] = useState<Submitted | null>(null); const [showSlowMessage, setShowSlowMessage] = useState(false); const [nextCursor, setNextCursor] = useState<ImportCursor | null>(null); const [partial, setPartial] = useState(false); const [completed, setCompleted] = useState(false); const [lastSubmittedBody, setLastSubmittedBody] = useState<Submitted | null>(null);
  const summary = useMemo(() => result ? buildImportSummary(result, submitted) : null, [result, submitted]);
  const friendlyError = useMemo(() => friendlyImportError(result), [result]);
  const addedLocations = useMemo(() => getAddedLocations(result), [result]);
  const duplicateSkippedLocations = useMemo(() => getDuplicateSkippedLocations(result), [result]);
  ...
  async function runImport(body: Submitted) { setBusy(true); setResult(null); setSubmitted(body); setLastSubmittedBody(body); try { const res = await fetch("/api/admin/run-google-import", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }); const text = await res.text(); let json: Record<string, unknown>; try { json = text ? JSON.parse(text) as Record<string, unknown> : {}; } catch { json = { raw: text }; } const merged: Record<string, unknown> = { status: res.status, ok: res.ok, ...json }; setResult(merged); const cursor = isRecord(merged.nextCursor) ? merged.nextCursor as ImportCursor : null; setNextCursor(cursor); setPartial(merged.partial === true); setCompleted(merged.completed === true); } catch (error) { setResult({ success: false, error: error instanceof Error ? error.message : String(error) }); setPartial(false); setCompleted(false); setNextCursor(null); } finally { setBusy(false); } }
  async function submit(e: FormEvent) { e.preventDefault(); if (busy) return; setNextCursor(null); setPartial(false); setCompleted(false); const batch = customCategory.trim() || category; const areas = customArea.trim() || area; const body: Submitted = { type, limit: Number(limit), maxQueries: Number(maxQueries), batch, primaryTag: batch, areas, market, requestedMarket: market, minRating: Number(minRating), ...quality, allowMarketCorrection, qualityPreset: preset, cursor: null }; await runImport(body); }
  async function continueImport() { if (busy || !lastSubmittedBody || !nextCursor) return; await runImport({ ...lastSubmittedBody, cursor: nextCursor }); }
  return <form onSubmit={submit} className="space-y-6">
    ...
    {type === "both" && (Number(maxQueries) > 3 || Number(limit) > 5) ? <div className="rounded-2xl border border-amber-300/25 bg-amber-500/10 p-4 text-sm font-bold text-amber-50">Larger imports will run in safe chunks so Vercel does not time out.</div> : null}
    ...
    {partial && nextCursor ? <section className="rounded-3xl border border-amber-300/30 bg-amber-500/10 p-5 shadow-2xl shadow-amber-950/10">
      <p className="text-xs font-black uppercase tracking-[0.28em] text-amber-200">Safe pause</p>
      <h3 className="mt-2 text-xl font-black text-white">Import paused safely before the server timeout.</h3>
      <p className="mt-2 text-sm font-bold leading-6 text-white/70">{Number(result?.checked || 0)} checked, {Number(result?.imported || 0)} imported, {Number(result?.duplicateCount || result?.skipped_duplicate || 0)} duplicates skipped.</p>
      <button type="button" disabled={busy} onClick={continueImport} className="mt-4 rounded-full bg-amber-300 px-5 py-3 text-sm font-black text-black disabled:cursor-not-allowed disabled:opacity-50">Continue Import</button>
    </section> : null}
    {friendlyError ? <section className="rounded-3xl border border-red-300/30 bg-red-500/10 p-5">
      <p className="text-xs font-black uppercase tracking-[0.28em] text-red-200">Import needs another run</p>
      <h3 className="mt-2 text-xl font-black text-white">{friendlyError.headline}</h3>
      <p className="mt-2 text-sm font-bold leading-6 text-white/70">{friendlyError.explanation}</p>
      <h4 className="mt-5 text-sm font-black text-white">Recommended next steps</h4>
      <ul className="mt-2 list-disc space-y-1 pl-5 text-sm font-bold text-white/65">{friendlyError.nextSteps.map((step) => <li key={step}>{step}</li>)}</ul>
      <details className="mt-5"><summary className="cursor-pointer text-sm font-black text-white">Advanced raw response</summary><pre className="mt-3 max-h-[360px] overflow-auto rounded-2xl bg-black/60 p-4 text-xs text-white/70">{JSON.stringify(result, null, 2)}</pre></details>
    </section> : null}
    {summary && !friendlyError ? <section className={`rounded-3xl border p-5 ${summary.statusTone === "error" ? "border-red-300/30 bg-red-500/10" : summary.statusTone === "warning" ? "border-amber-300/30 bg-amber-500/10" : "border-emerald-300/30 bg-emerald-500/10"}`}><h3 className="text-xl font-black">{summary.headline}</h3><p className="mt-2 text-sm font-bold leading-6 text-white/70">{summary.explanation}</p><div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{summary.stats.map((s) => <div key={s.label} className="rounded-2xl bg-black/30 p-3"><p className="text-xs font-black uppercase tracking-widest text-white/40">{s.label}</p><p className="mt-1 text-sm font-bold text-white/80">{s.value}</p></div>)}</div><h4 className="mt-5 text-sm font-black">Quality filters used</h4><p className="mt-2 text-sm font-bold text-white/65">{summary.qualityFilters}</p><h4 className="mt-5 text-sm font-black">What should I do next?</h4><ul className="mt-2 list-disc space-y-1 pl-5 text-sm font-bold text-white/65">{summary.nextSteps.map((n) => <li key={n}>{n}</li>)}</ul><section className="mt-5 rounded-3xl border border-white/10 bg-black/25 p-4"><h4 className="text-base font-black text-white">Locations added</h4>{addedLocations.length ? <div className="mt-4 overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="text-xs font-black uppercase tracking-widest text-white/40"><tr>{["Name", "Type", "Market", "City or area", "Address", "Category/cuisine/type", "Rating", "Website", "Phone", "Has photo", "Has business hours", "Database ID"].map((h) => <th key={h} className="whitespace-nowrap px-3 py-2">{h}</th>)}</tr></thead><tbody className="divide-y divide-white/10">{addedLocations.map((location, index) => <tr key={location.id || `${location.name}-${index}`} className="align-top text-white/70"><td className="min-w-[180px] px-3 py-3 font-black text-white">{location.id ? <Link className="text-rose-100 hover:text-white hover:underline" href={`/admin/dashboard/crm/${location.id}`}>{location.name}</Link> : location.name}</td><td className="px-3 py-3">{location.locationType || "—"}</td><td className="px-3 py-3">{location.market || "—"}</td><td className="px-3 py-3">{location.city || "—"}</td><td className="min-w-[220px] px-3 py-3">{location.address || "—"}</td><td className="px-3 py-3">{location.category || "—"}</td><td className="px-3 py-3">{location.rating || "—"}</td><td className="px-3 py-3">{location.website ? <a href={location.website} target="_blank" rel="noreferrer" className="text-rose-100 underline">Website</a> : "—"}</td><td className="px-3 py-3">{location.phone || "—"}</td><td className="px-3 py-3">{yesNo(location.hasPhoto)}</td><td className="px-3 py-3">{yesNo(location.hasHours)}</td><td className="px-3 py-3 font-mono text-xs text-white/45">{location.id || "—"}</td></tr>)}</tbody></table></div> : <div className="mt-3 rounded-2xl border border-amber-200/15 bg-amber-500/10 p-4 text-sm font-bold leading-6 text-amber-50"><p>No new locations were added during this run.</p><p className="mt-2 text-white/65">Try relaxing the quality preset, choosing a different category, increasing the query count, or selecting a nearby area.</p></div>}</section><section className="mt-5 rounded-3xl border border-white/10 bg-black/25 p-4"><h4 className="text-base font-black text-white">Duplicates skipped</h4>{duplicateSkippedLocations.length ? <div className="mt-4 overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="text-xs font-black uppercase tracking-widest text-white/40"><tr>{["Name", "Address", "Matched by", "Existing database ID", "Reason"].map((h) => <th key={h} className="whitespace-nowrap px-3 py-2">{h}</th>)}</tr></thead><tbody className="divide-y divide-white/10">{duplicateSkippedLocations.map((location, index) => <tr key={`${location.existingId || location.name}-${index}`} className="align-top text-white/70"><td className="min-w-[180px] px-3 py-3 font-black text-white">{location.name}</td><td className="min-w-[220px] px-3 py-3">{location.address || "—"}</td><td className="px-3 py-3">{location.matchedBy ? location.matchedBy.replace(/_/g, " ") : "—"}</td><td className="px-3 py-3 font-mono text-xs text-white/45">{location.existingId || "—"}</td><td className="min-w-[240px] px-3 py-3">{location.reason || "Skipped because this location already exists."}</td></tr>)}</tbody></table></div> : <div className="mt-3 rounded-2xl border border-white/10 bg-black/30 p-4 text-sm font-bold leading-6 text-white/65">No duplicate locations were detected during this run.</div>}</section><details className="mt-5"><summary className="cursor-pointer text-sm font-black text-white">Advanced raw import response</summary><pre className="mt-3 max-h-[360px] overflow-auto rounded-2xl bg-black/60 p-4 text-xs text-white/70">{JSON.stringify(result, null, 2)}</pre></details></section> : null}
  </form>;
}
