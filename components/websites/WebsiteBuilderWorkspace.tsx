...
const STEP_LABELS: Record<Step, string> = { style: "Design", content: "Pages & content", preview: "Preview", publish: "Publish" };
...
function startingPoint(customContent: Record<string, unknown>) {
...
export function WebsiteBuilderWorkspace({ initialWebsite, locationName }: { initialWebsite: Website; locationName: string }) {
  ...
  const liveUrl = website.live_url || (website.domain ? `https://${website.domain}` : website.platform_domain ? `https://${website.platform_domain}` : null);
  ...
  const generatedStartingPoint = startingPoint(website.custom_content || {});
  const direction = WEBSITE_DESIGN_DIRECTIONS.find((item) => item.id === selectedDirection);
  const isRedesign = Boolean(generatedStartingPoint || website.published_version);
  ...
  async function generateWebsite() {
    ...
    if (!response.ok || !data.website) {
      setMessage(data?.error || "We could not create this design right now.");
      return;
    }
  ...
  async function loadPreview() {
    ...
    const response = await fetch("/api/business/website/preview", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ location_id: website.location_id, site_title: siteTitle, sections, theme: website.theme, custom_content: website.custom_content }),
    });
    const data = await response.json().catch(() => ({}));
    setPreviewLoading(false);
  ...
  return <div className="space-y-5">
    <section className="overflow-hidden rounded-3xl border border-white/10 bg-white/[0.04]">
      ...
      <div className="grid grid-cols-4 border-b border-white/10">
        {STEP_ORDER.map((item, index) => {
          ...
          const complete = index < stepIndex || (item === "style" && Boolean(generatedStartingPoint));
          return <button key={item} type="button" onClick={() => void goToStep(item)} className={`px-2 py-4 text-center text-[11px] font-black sm:text-sm ${active ? "bg-white/[0.08] text-white" : "text-white/45 hover:bg-white/[0.03]"}`}>
    ...
    {step === "preview" ? <section className="rounded-3xl border border-white/10 bg-black/25 p-5 sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-[#f5b700]">Step 3 of 4</p><h3 className="mt-2 text-2xl font-black">Preview before you publish</h3><p className="mt-2 text-sm text-white/60">Check desktop, iPad/tablet, and phone layouts before going live.</p></div><div className="flex flex-wrap rounded-full border border-white/10 bg-white/[0.03] p-1">{(["desktop", "tablet", "mobile"] as Device[]).map((item) => <button key={item} type="button" onClick={() => setDevice(item)} className={`rounded-full px-4 py-2 text-xs font-black capitalize ${device === item ? "bg-white/10" : "text-white/45"}`}>{item}</button>)}</div></div>
      <div className="mt-5 overflow-auto rounded-2xl border border-white/10 bg-[#0b0b0d] p-3"><div className="mx-auto overflow-hidden rounded-xl bg-white transition-all" style={{ height: 720, width: DEVICE_WIDTH[device], maxWidth: "100%" }}>{previewLoading ? <div className="flex h-full items-center justify-center text-sm font-black text-black/50">Building your preview…</div> : previewHtml ? <iframe title="Website preview" srcDoc={previewHtml} className="h-full w-full border-0" sandbox="allow-scripts allow-same-origin allow-popups allow-popups-to-escape-sandbox" /> : <div className="flex h-full items-center justify-center text-sm font-black text-black/50">Preview unavailable.</div>}</div></div>
    ...
    {step === "publish" ? <section className="rounded-3xl border border-white/10 bg-black/25 p-5 sm:p-6">
      ...
      <p className="mt-2 max-w-2xl text-sm leading-6 text-white/60">We&apos;ll publish the version you just reviewed. Your connected business information can continue updating without rebuilding the website from scratch.</p>
    ...
    {website.last_publish_status === "failed" && website.last_error ? <p className="rounded-2xl border border-red-300/20 bg-red-500/10 px-4 py-3 text-sm text-red-100">We couldn&apos;t publish your latest changes. Try again, or contact support if this keeps happening.</p> : null}
  </div>;
}
