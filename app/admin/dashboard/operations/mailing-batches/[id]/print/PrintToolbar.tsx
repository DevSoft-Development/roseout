...
export default function PrintToolbar({
  ...
  const stagingQuery = staging && stagingItemId ? `&staging=1&item=${encodeURIComponent(stagingItemId)}` : "";
    ...
    <div className="print:hidden sticky top-0 z-50 border-b border-white/10 bg-[#080706]/95 px-4 py-3 text-white backdrop-blur">
      <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {staging ? <span className="rounded-full border border-amber-300/30 bg-amber-500/15 px-3 py-1.5 text-xs font-black uppercase tracking-[0.12em] text-amber-100">Staging test</span> : null}
          {[
            ["duplex", "Duplex"],
            ["fronts", "Fronts only"],
            ["backs", "Backs only"],
            ...
            <Link
              key={value}
              href={`${base}?mode=${value}${stagingQuery}`}
        ...
        <div className="flex items-center gap-3">
          ...
          <button type="button" onClick={() => window.print()} className="rounded-lg bg-emerald-300 px-4 py-2 text-sm font-black text-black">{staging ? "Print staging test" : "Print"}</button>
        </div>
      </div>
    </div>
  );
