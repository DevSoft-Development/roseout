...
export default function BetaCommandCenter({ assignments, weekStart, giveawayStatus, feedbackCount, testMode = false }: { assignments: Assignment[]; weekStart: string; giveawayStatus?: string | null; feedbackCount: number; profileComplete: boolean; testMode?: boolean }) {
  ...
  async function choose(item: any, type: "single_location" | "paired_outing" | "none", flags: Record<string, boolean>) { setBusy(true); setError(null); setNotice(null); try { if (type !== "none" && !runId) { throw new Error("Search results are still being prepared. Please run the search again before selecting a result."); } const savedSelection = await persist("selection", { beta_search_run_id: runId, chosen_result_type: type, selected_none: type === "none", result: item, result_type: type, was_saved: false, ...flags }); const nextStep = Math.max(step, weekNumber === 3 || weekNumber === 4 ? 3 : 4); setSelected(type === "none" ? { none: true } : item); setStep((current) => Math.max(current, weekNumber === 3 || weekNumber === 4 ? 3 : 4)); setNotice(type === "none" ? "Thanks — we recorded that none of the results fit your search." : type === "paired_outing" ? "Your beta pairing choice was recorded." : "Your beta choice was recorded."); scrollToNextBetaStep(nextStep); return savedSelection; } catch (e: any) { setError(e.message || "We could not save that choice."); } finally { setBusy(false); } }
  async function saveItem(item: any, type: "single_location" | "paired_outing") { setBusy(true); setError(null); setNotice(null); try { if (!runId) { throw new Error("Search results are still being prepared. Please run the search again before saving a result."); } await persist("selection", { beta_search_run_id: runId, chosen_result_type: type, selected_none: false, result: item, result_type: type, was_saved: true, was_selected: false }); setSaved((current) => current.some((entry) => isSameItem(entry, item)) ? current : [...current, item]); setNotice(type === "paired_outing" ? "Pairing bookmarked for beta review." : "Bookmarked for beta review."); } catch (e: any) { setError(e.message || "We could not save that result."); } finally { setBusy(false); } }
