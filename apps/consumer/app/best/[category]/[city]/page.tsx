...
export default async function BestCategoryCityPage({ params }: { params: Params }) {
  ...
  const categoryLabel = titleCaseSlug(category);
  const cityLabel = titleCaseSlug(city);
  const locations = await loadSeoLocations({ city, category });
    ...
    <main className="min-h-screen bg-[#fff8f3] text-[#1b1210]">
      ...
      <section className="mx-auto max-w-6xl px-5 py-16 sm:px-8">
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {locations.map((location) => <Link key={location.id} href={locationHref(location)} className="rounded-[2rem] bg-white p-4 shadow-sm"><img src={location.main_image || location.image_url || "/placeholder.jpg"} alt={locationCardName(location)} className="h-44 w-full rounded-[1.5rem] object-cover" /><h2 className="mt-4 text-xl font-black">{locationCardName(location)}</h2><p className="mt-1 text-sm font-bold text-black/45">{[location.city, location.state].filter(Boolean).join(", ")}</p></Link>)}
        </div>
      </section>
    </main>
  );
