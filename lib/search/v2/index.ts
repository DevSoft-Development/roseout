...
function anchorCandidate(candidate: any) { return { id: candidate?.linkedLocationId ?? candidate?.linked_location_id ?? candidate?.id ?? null, name: candidate?.canonicalName ?? candidate?.canonical_name ?? candidate?.name ?? null }; }
async function resolvePlanAnchor(plan: SearchPlan, supabase: SupabaseClient): Promise<{ plan: SearchPlan; trace: AnchorResolutionTrace }> {
  if (!plan.anchor.requested || !plan.anchor.rawName) return { plan, trace: { status: "not_requested", requested: false, rawName: null, resolvedLocationId: null, requiresClarification: false, candidateCount: 0, candidates: [], diagnostics: null } };
  ...
  const baseTrace = { requested: true, rawName: plan.anchor.rawName, resolvedLocationId: null, requiresClarification: false, candidateCount: candidates.length, candidates, diagnostics: (resolution as any).diagnostics ?? null };
  if (resolution.status === "ambiguous") return { plan, trace: { ...baseTrace, status: "clarification_required", requiresClarification: true } };
  if (resolution.status === "not_found" || !resolution.anchor) return { plan, trace: { ...baseTrace, status: "not_found" } };
  if (resolution.status === "missing_coordinates") return { plan, trace: { ...baseTrace, status: "missing_coordinates" } };
  const anchor = resolution.anchor; const latitude = Number(anchor.latitude); const longitude = Number(anchor.longitude);
  const resolvedLocationId = anchor.linkedLocationId ?? anchor.linked_location_id ?? anchor.id ?? null;
  if (!resolvedLocationId) return { plan, trace: { ...baseTrace, status: "not_found" } };
  if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) return { plan, trace: { ...baseTrace, status: "missing_coordinates", resolvedLocationId: String(resolvedLocationId) } };
  ...
  return { plan: { ...plan, geo: { ...plan.geo, source: "anchor", latitude, longitude, radiusMiles: hardRadius ?? defaultRadius, strictness: plan.travel.constraint === "hard" ? "strict" : "preferred", city: null, borough: null, neighborhood: null, county: null }, anchor: { ...plan.anchor, locationId: String(resolvedLocationId), name: anchor.canonicalName ?? anchor.canonical_name ?? anchor.name, latitude, longitude } }, trace: { ...baseTrace, status: "resolved", resolvedLocationId: String(resolvedLocationId), candidateCount: Math.max(1, candidates.length) } };
}
...
export function enforceRequestedDomains(plan: SearchPlan, scored: { all: ScoredCandidate[]; restaurants: ScoredCandidate[]; activities: ScoredCandidate[] }) {
  const restaurants = plan.restaurant.required ? scored.restaurants : []; const activities = plan.activity.required ? scored.activities : []; const allowed = new Set([...restaurants, ...activities]);
  return { all: scored.all.filter((candidate) => allowed.has(candidate)), restaurants, activities };
}
function retrievedLocation(candidate: any) { return candidate?.location ?? candidate?.candidate?.location ?? candidate?.candidate?.candidate?.location ?? null; }
function candidateLocationId(candidate: any) { const id = retrievedLocation(candidate)?.id; return id == null ? null : String(id); }
function candidateOriginalType(candidate: any) { const location = retrievedLocation(candidate); return location?.location_type ?? location?.type ?? location?.primary_category ?? location?.activity_type ?? null; }
function candidateTerms(candidate: any) { const terms = candidate?.matchedRetrievalTerms ?? candidate?.candidate?.matchedRetrievalTerms ?? candidate?.candidate?.candidate?.matchedRetrievalTerms; return Array.isArray(terms) ? terms.map(String) : []; }
function recordCandidateStageDiagnostics({ trace, plan, retrieved, qualified, rawScored, scored }: { trace: ReturnType<typeof createSearchTrace>; plan: SearchPlan; retrieved: any; qualified: any[]; rawScored: { all: ScoredCandidate[]; restaurants: ScoredCandidate[]; activities: ScoredCandidate[] }; scored: { all: ScoredCandidate[]; restaurants: ScoredCandidate[]; activities: ScoredCandidate[] } }) {
  ...
  const qualifiedIds = new Set(qualified.map(candidateLocationId).filter(Boolean)); const taxonomyIds = new Set(rawScored.all.map(candidateLocationId).filter(Boolean));
  const finalRestaurantIds = new Set(scored.restaurants.map(candidateLocationId).filter(Boolean)); const finalActivityIds = new Set(scored.activities.map(candidateLocationId).filter(Boolean)); const finalIds = new Set([...finalRestaurantIds, ...finalActivityIds]);
  const rejectedCandidates: CandidateStageRejection[] = [];
  for (const candidate of allCandidates) { const locationId = candidateLocationId(candidate); if (!candidate?.geoMatch?.accepted) rejectedCandidates.push({ locationId, desiredRole: candidate?.requestedRoles?.[0] ?? null, originalType: candidateOriginalType(candidate), assignedDomain: null, rejectedAtStage: "geo", rejectionReason: candidate?.geoMatch?.reason ?? "geo_not_accepted", matchedTerms: candidateTerms(candidate) }); else if (locationId && !qualifiedIds.has(locationId)) rejectedCandidates.push({ locationId, desiredRole: candidate?.requestedRoles?.[0] ?? null, originalType: candidateOriginalType(candidate), assignedDomain: null, rejectedAtStage: "domain_assignment", rejectionReason: "no_qualified_requested_role", matchedTerms: candidateTerms(candidate) }); else if (locationId && !taxonomyIds.has(locationId)) rejectedCandidates.push({ locationId, desiredRole: candidate?.requestedRoles?.[0] ?? null, originalType: candidateOriginalType(candidate), assignedDomain: null, rejectedAtStage: "taxonomy", rejectionReason: "taxonomy_or_scoring_excluded", matchedTerms: candidateTerms(candidate) }); else if (locationId && !finalIds.has(locationId)) rejectedCandidates.push({ locationId, desiredRole: candidate?.requestedRoles?.[0] ?? null, originalType: candidateOriginalType(candidate), assignedDomain: null, rejectedAtStage: "final_domain", rejectionReason: "not_in_required_final_domain", matchedTerms: candidateTerms(candidate) }); }
  trace.candidateStages = { profileCandidates: trace.retrieval.profileCandidateCount, rawProfileCandidates: trace.retrieval.profileCandidateCount, rawLegacyCandidates: trace.retrieval.legacyCandidateCount, geoEligibleCandidates: geoEligibleCandidates.length, domainAssignedCandidates: qualified.length, taxonomyEligibleCandidates: rawScored.all.length, publishableCandidates: rawScored.all.length, finalRestaurantCandidates: scored.restaurants.length, finalActivityCandidates: scored.activities.length, rejectedCandidates };
  const restaurantRequired = Boolean(plan.restaurant.required); const activityRequired = Boolean(plan.activity.required); const rawRestaurantEvidence = trace.retrievalCalls.filter((call) => call.domain === "restaurant").reduce((sum, call) => sum + call.resultCount, 0); const rawActivityEvidence = trace.retrievalCalls.filter((call) => call.domain === "activity").reduce((sum, call) => sum + call.resultCount, 0); const missingRestaurant = restaurantRequired && scored.restaurants.length === 0; const missingActivity = activityRequired && scored.activities.length === 0; const noRawEvidenceForMissingDomains = (!missingRestaurant || rawRestaurantEvidence === 0) && (!missingActivity || rawActivityEvidence === 0); const noCandidateEvidence = allCandidates.length === 0 && trace.retrieval.profileCandidateCount === 0 && trace.retrieval.legacyCandidateCount === 0; const confirmedGap = (missingRestaurant || missingActivity) && noRawEvidenceForMissingDomains && noCandidateEvidence; const supportedMarket = Boolean(plan.geo.city || plan.geo.borough || plan.geo.neighborhood || plan.geo.county || plan.geo.source === "anchor" || plan.geo.source === "current_location");
