...
const EXPECTED_EMPTY_OUTCOMES = new Set(["expected_constraint_no_pair", "clarification_required", "anchor_not_found"]);
...
export function resultCounts(result: any) {
  ...
  const restaurantCards = asArray(result?.restaurantCards ?? result?.restaurants); const activityCards = asArray(result?.activityCards ?? result?.activities); const pairCards = asArray(result?.pairCards ?? result?.pairs);
  const pairRestaurants = uniquePairVenueCount(pairCards, "restaurant"); const pairActivities = uniquePairVenueCount(pairCards, "activity");
...
export function buildSummary(index: number, query: string, engine: QaEngine, result: any, elapsedMs: number, extraWarnings: string[] = [], caughtError?: unknown) {
  ...
  const outcome = publicOutcomeOf(result); const anchorResolution = anchorResolutionOf(result); const expectedEmpty = Boolean(outcome && EXPECTED_EMPTY_OUTCOMES.has(outcome));
  const needsRestaurant = Boolean(intent?.needsRestaurant ?? intent?.restaurant?.required); const needsActivity = Boolean(intent?.needsActivity ?? intent?.activity?.required);
  const suspiciousFlags: string[] = [];
  if (["slow", "critical"].includes(speedStatus)) suspiciousFlags.push("slow"); if (speedStatus === "critical") suspiciousFlags.push("critical_speed"); if (parserSource?.toLowerCase().includes("llm")) suspiciousFlags.push("llm_used"); if (fallbackUsed) suspiciousFlags.push("deterministic_fallback");
  if (!counts.displayed && !expectedEmpty) suspiciousFlags.push("no_results"); if (errors.length) suspiciousFlags.push("errors"); if (warnings.length) suspiciousFlags.push("warnings"); if (engine === "compare") suspiciousFlags.push("engine_comparison");
  if (needsRestaurant && needsActivity && counts.pairs === 0 && outcome !== "expected_constraint_no_pair") suspiciousFlags.push("mixed_no_pairs");
  const ok = errors.length === 0 && (expectedEmpty || (Boolean(result?.success ?? result?.searchV2?.success) && !(needsRestaurant && needsActivity && counts.pairs === 0)));
  return {
    index, query, ok, engine, outcome, expectedOutcome: expectedEmpty,
    anchorResolutionStatus: stringOrNull(anchorResolution?.status), requiresClarification: Boolean(anchorResolution?.requiresClarification), resolvedAnchorLocationId: stringOrNull(anchorResolution?.resolvedLocationId), anchorCandidateCount: numberOrNull(anchorResolution?.candidateCount),
    normalized_search_type: stringOrNull(mode), primary_domain: stringOrNull(domain), restaurant_count: counts.restaurants, activity_count: counts.activities, pair_count: counts.pairs,
    fallback_pair_count: Number(result?.fallback_pair_count ?? debug?.fallbackPairCount ?? 0), fallbackPairsUsedAsPrimary: Boolean(result?.fallbackPairsUsedAsPrimary ?? debug?.fallbackPairsUsedAsPrimary),
    primaryResultType: stringOrNull(result?.primaryResultType ?? debug?.primaryResultType ?? result?.searchV2?.displayMode), timing_ms: totalMs, speed_status: speedStatus, intentParserSource: parserSource, fastPathMatched,
    fastPathReason: stringOrNull(debug?.fastPathReason ?? parserReasons.join("; ")), llm_ms: numberOrNull(performance?.llm_ms ?? performance?.llmMs), rpc_ms: numberOrNull(performance?.rpc_ms ?? performance?.rpcMs ?? result?.searchV2?.timing?.retrievalMs ?? result?.timing?.retrievalMs),
    intent_parse_ms: numberOrNull(performance?.intent_parse_ms ?? performance?.intentParseMs ?? result?.searchV2?.timing?.plannerMs ?? result?.timing?.plannerMs), ranking_ms: numberOrNull(performance?.ranking_ms ?? performance?.rankingMs ?? result?.searchV2?.timing?.scoringMs ?? result?.timing?.scoringMs),
    result_count: counts.displayed, no_results_reason: stringOrNull(debug?.noResultsReason ?? result?.fallback?.reason ?? result?.searchV2?.fallback?.reason), no_pairs_reason: stringOrNull(debug?.noPairsReason), warnings, errors, suspiciousFlags: [...new Set(suspiciousFlags)],
    activityTerms: strings(debug?.activityTerms ?? intent?.activity?.categories ?? []), restaurantTerms: strings(debug?.restaurantTerms ?? [...asArray(intent?.restaurant?.cuisines), ...asArray(intent?.restaurant?.foods), ...asArray(intent?.restaurant?.features)]), needsRestaurant, needsActivity,
    maxWalkingMinutes: numberOrNull(plan?.pairing?.maxWalkingMinutes), maxDrivingMinutes: numberOrNull(plan?.pairing?.maxDrivingMinutes), maxDistanceMiles: numberOrNull(plan?.pairing?.maxDistanceMiles),
  };
}
