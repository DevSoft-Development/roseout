...
export async function POST(req: Request) {
  ...
  for (const recipient of recipients) {
    ...
    try {
      ...
      sent += 1;
      logs.push({ campaign_id: campaignId, subscriber_id: recipient.id || null, user_id: recipient.user_id || null, channel: "sms", provider: "twilio", recipient_phone: to, status: result.status || "queued", provider_response: { sid: result.sid }, attempted_at: attemptedAt, sent_at: nowIso() });
