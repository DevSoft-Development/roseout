...
export default function TeamWorkSessionClient({ profile, allowedWorkTypes, activeSession: initialActive, recentSessions }: Props) {
  ...
  async function submit(action: "start" | "stop") {
    ...
    try {
      ...
      setMessage(error instanceof Error ? error.message : "Could not update work session.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return <div className="grid gap-6 lg:grid-cols-[1fr_0.8fr]">
    <section className="rounded-[2rem] border border-white/10 bg-[#111] p-5 shadow-2xl sm:p-7">
      ...
      <h2 className="mt-2 text-3xl font-black">Clock In / Clock Out</h2>
      ...
      {activeSession ? <div className="mt-5 rounded-3xl border border-emerald-400/25 bg-emerald-500/10 p-5">
        ...
        <h3 className="mt-2 text-2xl font-black">{labelize(activeSession.work_type)}</h3>
        <p className="mt-1 text-sm font-bold text-emerald-100/75">Started {new Date(activeSession.clock_in_at).toLocaleString()}</p>
        <textarea value={userNotes} onChange={(e) => setUserNotes(e.target.value)} rows={3} className="mt-4 w-full rounded-2xl border border-white/10 bg-black px-4 py-3 text-sm font-bold text-white" placeholder="Optional clock-out notes" />
        ...
        <select value={workType} onChange={(e) => setWorkType(e.target.value)} className="w-full rounded-full border border-white/10 bg-black px-4 py-3 text-sm font-bold text-white">{allowedWorkTypes.map((type) => <option key={type} value={type}>{labelize(type)}</option>)}</select>
        <textarea value={userNotes} onChange={(e) => setUserNotes(e.target.value)} rows={3} className="w-full rounded-2xl border border-white/10 bg-black px-4 py-3 text-sm font-bold text-white" placeholder="Optional notes" />
    ...
    <aside className="rounded-[2rem] border border-white/10 bg-white/[0.06] p-5 sm:p-7">
      ...
      <div className="mt-4 space-y-3">{recentSessions.map((session) => <div key={session.id} className="rounded-2xl border border-white/10 bg-black/30 p-4"><p className="font-black">{labelize(session.work_type)}</p><p className="mt-1 text-xs font-bold text-white/45">{new Date(session.clock_in_at).toLocaleString()} · {labelize(session.status)} · {labelize(session.approval_status || "pending_review")}</p></div>)}</div>
    </aside>
  </div>;
}
