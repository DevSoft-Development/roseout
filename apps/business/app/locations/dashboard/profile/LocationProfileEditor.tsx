...
import { Building2, CheckCircle2, Clock3, Eye, ImageIcon, MapPin, Phone, Save, Search } from "lucide-react";
...
export default function LocationProfileEditor({
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!response.ok || !result.location) throw new Error(result.error || "We could not load this profile.");
        ...
        if (!cancelled) setMessage(error instanceof Error ? error.message : "We could not load this profile.");
      } finally {
  ...
  const completion = useMemo(() => {
    const checks = [
      ...
      form.hours || form.operating_hours,
    ];
  ...
  async function save() {
    ...
    try {
      const response = await fetch("/api/locations/edit-context", {
        ...
        body: JSON.stringify({ type: locationType, id: canonicalId || locationId, payload }),
      });
      const result = await response.json();
  ...
  if (loading) {
    return <main className="min-h-screen bg-[#050607] p-6 text-white"><div className="mx-auto max-w-5xl animate-pulse rounded-3xl border border-white/10 bg-white/[0.04] p-8"><div className="h-5 w-40 rounded bg-white/10" /><div className="mt-4 h-10 w-80 max-w-full rounded bg-white/10" /><div className="mt-8 h-64 rounded-3xl bg-white/5" /></div></main>;
  }
      ...
      <div className="sticky top-0 z-30 border-b border-white/10 bg-[#050607]/95 px-4 py-4 backdrop-blur-xl sm:px-6 lg:px-8">
        <div className="mx-auto flex max-w-6xl flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-2xl text-sm font-semibold text-white/45">Update the information guests use to understand, find, and contact your location.</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <Link href={publicHref} className="rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-2.5 text-sm font-black text-white/75 hover:bg-white/[0.08]">Preview profile</Link>
            <button type="button" onClick={save} disabled={saving || !hasChanges} className="inline-flex items-center gap-2 rounded-2xl bg-gradient-to-r from-[#e1062a] to-[#ff2142] px-5 py-2.5 text-sm font-black shadow-lg shadow-[#ff1654]/20 disabled:cursor-not-allowed disabled:opacity-40"><Save size={16} /> {saving ? "Saving..." : hasChanges ? "Save changes" : "Saved"}</button>
          </div>
        </div>
      </div>

      <div className="mx-auto max-w-6xl space-y-5 px-4 py-6 sm:px-6 lg:px-8">
        {demoMode ? <div className="rounded-2xl border border-amber-300/20 bg-amber-400/10 px-4 py-3 text-sm font-bold text-amber-100">Demo mode is active. Changes affect the demo location only.</div> : null}
        {message ? <div className={`rounded-2xl border px-4 py-3 text-sm font-bold ${message.toLowerCase().includes("success") ? "border-emerald-300/20 bg-emerald-400/10 text-emerald-100" : "border-white/10 bg-white/[0.05] text-white/75"}`}>{message}</div> : null}

        <section className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_280px]">
          <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-[#111722] to-[#090c12] p-5 sm:p-6">
            <div className="flex items-start gap-4"><div className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-[#e1062a]/15 text-[#ff6b86]"><Building2 size={22} /></div><div><h2 className="text-xl font-black">Profile essentials</h2><p className="mt-1 text-sm font-semibold text-white/45">Start here. These are the fields guests notice most.</p></div></div>
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              <Field label="Business name" value={form.name} onChange={(value) => update("name", value)} placeholder="Your business name" />
              <Field label={locationType === "restaurants" ? "Cuisine" : "Activity type"} value={locationType === "restaurants" ? form.cuisine : form.activity_type} onChange={(value) => update(locationType === "restaurants" ? "cuisine" : "activity_type", value)} placeholder={locationType === "restaurants" ? "Italian, Caribbean, Steakhouse..." : "Bowling, pottery, escape room..."} />
            ...
            <div className="mt-4"><Field label="Short description" value={form.short_description} onChange={(value) => update("short_description", value)} placeholder="A quick sentence that tells guests what makes your location special" maxLength={180} /><p className="mt-2 text-right text-xs font-bold text-white/30">{form.short_description.length}/180</p></div>
            <div className="mt-4"><TextArea label="Full description" value={form.description} onChange={(value) => update("description", value)} placeholder="Tell guests what to expect, what you are known for, and what kind of experience you offer." /></div>
          </div>

          <aside className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
            <p className="text-xs font-black uppercase tracking-[0.18em] text-white/35">Profile strength</p><p className="mt-3 text-4xl font-black">{completion}%</p><div className="mt-4 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-[#ff2142]" style={{ width: `${completion}%` }} /></div>
            <p className="mt-4 text-sm font-semibold leading-6 text-white/45">Complete the basics, contact info, image, and hours so guests have fewer reasons to leave your profile.</p>
            <div className="mt-5 space-y-2 text-sm font-bold text-white/65"><Check label="Business name" done={Boolean(form.name)} /><Check label="Contact details" done={Boolean(form.phone || form.website)} /><Check label="Address" done={Boolean(form.address && form.city)} /><Check label="Description" done={Boolean(form.short_description || form.description)} /><Check label="Photo" done={Boolean(image)} /><Check label="Hours" done={Boolean(form.hours || form.operating_hours)} /></div>
          </aside>
        </section>
        ...
        <Section icon={<Clock3 size={20} />} title="Business hours" description="Set your weekly schedule here, including late-night hours that close after midnight.">
          <LocationEditorHoursPanel
            value={form.operating_hours || form.hours}
            onChange={(operatingHours, summary) => {
              setForm((current) => ({ ...current, operating_hours: operatingHours, hours: summary }));
              setMessage("");
            }}
          />
          {form.hours ? <div className="mt-4 rounded-2xl border border-white/10 bg-black/20 p-4"><p className="text-xs font-black uppercase tracking-[0.14em] text-white/35">Public hours summary</p><pre className="mt-2 whitespace-pre-wrap font-sans text-sm font-semibold leading-6 text-white/65">{form.hours}</pre></div> : null}
        </Section>

        <Section icon={<Eye size={20} />} title="Visibility" description="Control whether this location can appear in TheOutHaven discovery."><button type="button" onClick={() => update("is_searchable", !form.is_searchable)} className="flex w-full items-center justify-between gap-4 rounded-2xl border border-white/10 bg-black/20 p-4 text-left"><div><p className="font-black">Show this location in search</p><p className="mt-1 text-sm font-semibold text-white/40">Turn this off only if the location should temporarily stop appearing in discovery.</p></div><span className={`relative h-7 w-12 shrink-0 rounded-full transition ${form.is_searchable ? "bg-[#e1062a]" : "bg-white/15"}`}><span className={`absolute top-1 h-5 w-5 rounded-full bg-white transition ${form.is_searchable ? "left-6" : "left-1"}`} /></span></button></Section>

        <section className="flex flex-col gap-3 rounded-3xl border border-white/10 bg-white/[0.035] p-5 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2 text-sm font-black"><Search size={16} className="text-[#ff6b86]" /> Need more control?</div><p className="mt-1 text-sm font-semibold text-white/40">Advanced admin fields remain available without cluttering the owner profile.</p></div><Link href={advancedHref} className="shrink-0 rounded-2xl border border-white/10 bg-white/[0.05] px-4 py-2.5 text-sm font-black text-white/75 hover:bg-white/[0.08]">Advanced editor</Link></section>

        <div className="flex justify-end pb-8"><button type="button" onClick={save} disabled={saving || !hasChanges} className="inline-flex items-center gap-2 rounded-2xl bg-gradient-to-r from-[#e1062a] to-[#ff2142] px-6 py-3 text-sm font-black shadow-lg shadow-[#ff1654]/20 disabled:cursor-not-allowed disabled:opacity-40"><Save size={16} />{saving ? "Saving..." : hasChanges ? "Save changes" : "Everything is saved"}</button></div>
      </div>
    </main>
  );
}
