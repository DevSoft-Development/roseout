...
export async function buildSearchPlan({ input }: { input: SearchPlannerInput }): Promise<SearchPlan> {
  ...
  const latitude = current?.latitude ?? (useDefaultMarketCoordinates ? DEFAULT_MARKET_CENTER.latitude : null);
  const longitude = current?.longitude ?? (useDefaultMarketCoordinates ? DEFAULT_MARKET_CENTER.longitude : null);
  const radiusMiles = current?.radiusMiles ?? (place ? 8 : DEFAULT_MARKET_CENTER.radiusMiles);
  const plan: SearchPlan = {
    ...
    geo: { source: anchored ? "anchor" : place ? "explicit" : current ? "current_location" : "default_market", market: place?.[3] ?? input.market ?? null, city: place?.[1] ?? null, borough: place?.[2] ?? null, neighborhood: place?.[1] ?? null, county: place?.[4] ?? null, state: "NY", latitude, longitude, radiusMiles, strictness: place ? "strict" : current ? "preferred" : "broad" },
    ...
    confidence: { overall: place && (restaurantRequired || activityRequired) ? .96 : .85, mode: .95, restaurant: restaurantRequired ? .95 : .9, activity: activityRequired ? .95 : .9, geo: place || current || useDefaultMarketCoordinates ? .95 : .7 }, parser: { source: "deterministic", reasons: ["explicit taxonomy, mode, sequence, and geography signals resolved"] },
  };
