...
export async function GET() {
  ...
  const [{ data: sections, error: sectionError }, { data: items, error: itemError }] = await Promise.all([
    getAdminDatabaseClient().from("discover_sections").select("*").order("sort_order", { ascending: true }),
    getAdminDatabaseClient().from("discover_items").select("*").order("section_id").order("sort_order", { ascending: true }),
  ]);
...
export async function POST(req: Request) {
  ...
  if (kind === "section") {
    ...
    const { data, error } = await getAdminDatabaseClient().from("discover_sections").insert({
      id,
  ...
  const { data, error } = await getAdminDatabaseClient().from("discover_items").insert({
...
export async function PATCH(req: Request) {
  ...
  if (kind === "section") {
    ...
    const { data, error } = await getAdminDatabaseClient().from("discover_sections").update(updates).eq("id", id).select("*").single();
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
  ...
  const { data, error } = await getAdminDatabaseClient().from("discover_items").update(updates).eq("id", id).select("*").single();
  if (error) return NextResponse.json({ error: error.message }, { status: 400 });
...
export async function DELETE(req: Request) {
  ...
  const { error } = await getAdminDatabaseClient().from(table).delete().eq("id", id);
  if (error) return NextResponse.json({ error: error.message }, { status: 400 });
