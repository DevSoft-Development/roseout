...
import { addDomainToVercelProject } from "@/lib/domains/vercel-project-domain";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ ok: true, domain, verified: Boolean(result?.verified), verification: result?.verification || [] });
  } catch (error) {
