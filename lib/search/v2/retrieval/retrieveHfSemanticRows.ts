  ...
  try {
    ...
    const perDomain = await Promise.all(domains.map(async (domain) => {
      const { data, error } = await supabase.rpc("match_hf_location_search_embeddings", {
        p_query_embedding: embedding, p_expected_domain: domain, p_market_key: plan.geo.market,
        p_match_count: Math.max(10, Math.min(120, Number(process.env.SEARCH_HF_SEMANTIC_MATCH_COUNT || 80))),
        p_min_similarity: Number(process.env.SEARCH_HF_SEMANTIC_MIN_SIMILARITY || 0.50), p_embedding_version: runtimeConfig.embeddingVersion,
    ...
    let menuRows: any[] = [];
    if (runtimeConfig.menuMode !== "disabled" && foodIntent) {
      const { data, error } = await supabase.rpc("match_hf_location_menu_items", {
        p_query_embedding: embedding, p_market_key: plan.geo.market, p_match_count: 60,
        p_min_similarity: Number(process.env.SEARCH_HF_MENU_MIN_SIMILARITY || 0.58), p_embedding_version: runtimeConfig.embeddingVersion,
      });
      if (!error) menuRows = (data ?? []) as any[];
    }
    const menuByLocation = new Map<string, any>();
    for (const row of menuRows) {
      const key = String(row.location_id); const current = menuByLocation.get(key);
    ...
    const { data: locationRows, error: locationError } = ids.length ? await supabase.from("locations").select("*").in("id", ids) : { data: [] as any[], error: null };
    ...
    for (const lane of perDomain) {
      ...
      for (const match of lane.rows) {
        const location = byId.get(String(match.location_id)); if (!location) continue;
        const similarity = Number(match.similarity ?? 0); const semanticSimilarity = Number(match.semantic_similarity ?? similarity);
        const rawFoodSimilarity = match.food_similarity == null ? null : Number(match.food_similarity); const foodSimilarity = rawFoodSimilarity != null && Number.isFinite(rawFoodSimilarity) ? rawFoodSimilarity : null;
        const menu = menuByLocation.get(String(match.location_id));
        const decorated = { ...location, hf_semantic_similarity: similarity, hf_general_similarity: semanticSimilarity, hf_food_similarity: foodSimilarity, hf_menu_similarity: menu ? Number(menu.similarity) : null, hf_menu_item_name: menu?.item_name ?? null, hf_menu_source: menu?.source ?? null, hf_semantic_model_version: runtimeConfig.embeddingVersion };
        for (const request of laneRequests) { items.push({ location: decorated, request, similarity, semanticSimilarity, foodSimilarity, menuSimilarity: menu ? Number(menu.similarity) : null, menuItem: menu?.item_name ?? null }); seen.add(`${String(location.id)}:${request.desiredRole}`); }
      }
    }
    if (runtimeConfig.menuMode === "enabled" && foodIntent) {
      ...
      for (const [locationId, menu] of menuByLocation) {
        const location = byId.get(locationId); if (!location) continue;
        for (const request of restaurantRequests) {
          const key = `${locationId}:${request.desiredRole}`; if (seen.has(key)) continue;
          const similarity = Number(menu.similarity ?? 0);
          const decorated = { ...location, hf_semantic_similarity: similarity, hf_general_similarity: null, hf_food_similarity: similarity, hf_menu_similarity: similarity, hf_menu_item_name: menu.item_name, hf_menu_source: menu.source, hf_semantic_model_version: runtimeConfig.embeddingVersion };
          items.push({ location: decorated, request, similarity, semanticSimilarity: similarity, foodSimilarity: similarity, menuSimilarity: similarity, menuItem: menu.item_name }); seen.add(key);
        }
      }
    }
