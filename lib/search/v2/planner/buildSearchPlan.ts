...
const SOFT_RESTAURANT_PREFERENCES = new Set(["family_friendly", "casual", "romantic", "group_friendly", "quiet", "relaxed"]);
...
function resolveTravelPolicy(query: string, parsedWalkMinutes: number | null | undefined) {
  ...
  const driveSentence = q.match(new RegExp(`\\b(?:drive|driving|car ride)(?:\\s+between\\s+[^,.]+?)?(?:\\s+should)?(?:\\s+not)?\\s+(?:take|last|be)(?:\\s+more than|\\s+over|\\s+longer than|\\s+under|\\s+within)?\\s*${DURATION}\\s*[- ]?\\s*(?:minute|minutes|min)\\b`));
  const travelMinutes = q.match(new RegExp(`\\b(?:within|under|no more than)\\s*${DURATION}\\s*[- ]?\\s*(?:minute|minutes|min)\\s+of\\s+travel\\b`));
  const explicitWalkMinutes = parsedWalkMinutes ?? numericToken(walkAfter?.[1]);
  const explicitDriveMinutes = numericToken(driveAfter?.[1] ?? driveBefore?.[1] ?? driveSentence?.[1]);
  ...
  const hard = Boolean(hasWalkMinutes || hasDriveMinutes || explicitMiles || ambiguousTravelMinutes);
  ...
  const maxDrivingMinutes = hasDriveMinutes ? Number(explicitDriveMinutes) : mode === "driving" && ambiguousTravelMinutes != null ? ambiguousTravelMinutes : null;
