...
export function DesignDirectionPicker({ locationId }: { locationId: string }) {
  ...
  const direction = useMemo(() => WEBSITE_DESIGN_DIRECTIONS.find((item) => item.id === selected) || null, [selected]);

  async function match() {
    setBusy(true); setMessage(""); setDirectionSaved(false); setGenerated(null); setViewMode("edit");
  ...
  async function generateInitialWebsite() {
    ...
    setGenerated(data.generated || null); setVersion(typeof data.version === "number" ? data.version : null); setViewMode("preview");
    setMessage("Your initial website build is ready. Preview it, then edit anything you want without using a redesign.");
  }
  ...
  const previewDark = direction?.theme.contrast === "dark";
  const previewEditorial = direction?.theme.typography === "editorial" || direction?.theme.typography === "classic";
  const previewCompact = direction?.theme.density === "compact";
  ...
  return <section className="rounded-3xl border border-white/10 bg-black/25 p-5">
    ...
    {matches.length ? <div className="mt-5 space-y-3">{matches.map((match, index) => { const item = WEBSITE_DESIGN_DIRECTIONS.find((candidate) => candidate.id === match.id); if (!item) return null; return <label key={match.id} className="block rounded-2xl border border-white/10 bg-white/[0.04] p-4"><div className="flex gap-3"><input type="radio" name="design-direction" checked={selected === match.id} onChange={() => { setSelected(match.id); setDirectionSaved(false); setGenerated(null); setViewMode("edit"); }} /><div><p className="font-black">{index === 0 ? "Best match — " : "Alternative — "}{item.name}</p><p className="mt-1 text-sm text-white/60">{item.summary}</p></div></div></label>; })}
    ...
    {generated ? <div className="mt-6 overflow-hidden rounded-3xl border border-white/10 bg-white/[0.04]">
      <div className="flex flex-col gap-3 border-b border-white/10 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        <div><p className="text-xs font-black uppercase tracking-[0.18em] text-rose-200">Website workspace{version ? ` · Version ${version}` : ""}</p><p className="mt-1 text-xs text-white/45">Preview uses your selected Design Direction. Live business facts and real photos are supplied separately from canonical location data.</p></div>
        <div className="flex gap-2"><button type="button" onClick={() => setViewMode("preview")} className={`rounded-full px-4 py-2 text-xs font-black ${viewMode === "preview" ? "bg-white text-black" : "bg-white/10 text-white"}`}>Preview</button><button type="button" onClick={() => setViewMode("edit")} className={`rounded-full px-4 py-2 text-xs font-black ${viewMode === "edit" ? "bg-white text-black" : "bg-white/10 text-white"}`}>Edit</button></div>
      </div>

      {viewMode === "preview" ? <div className="bg-neutral-200 p-3 sm:p-5">
        <div className={`mx-auto min-h-[620px] max-w-5xl overflow-hidden rounded-[28px] shadow-2xl ${previewDark ? "bg-[#111111] text-white" : "bg-[#fbfaf7] text-[#171717]"}`}>
          <header className={`flex items-center justify-between border-b px-6 py-5 ${previewDark ? "border-white/10" : "border-black/10"}`}><div><p className="text-sm font-black tracking-wide">{direction?.name || "Your website"}</p><p className={`text-xs ${previewDark ? "text-white/45" : "text-black/45"}`}>Live location identity</p></div><nav className={`hidden gap-5 text-xs font-bold sm:flex ${previewDark ? "text-white/65" : "text-black/60"}`}><span>About</span><span>Hours</span><span>Gallery</span><span>Contact</span></nav></header>
          <main>
            <section className={`${previewCompact ? "px-6 py-14 sm:px-10" : "px-6 py-20 sm:px-12 sm:py-28"}`}><p className={`text-xs font-black uppercase tracking-[0.22em] ${previewDark ? "text-white/45" : "text-black/45"}`}>{direction?.summary}</p><h2 className={`mt-5 max-w-4xl leading-[0.96] ${previewEditorial ? "font-serif text-5xl sm:text-7xl" : "text-5xl font-black sm:text-7xl"}`}>{generated.hero.heading}</h2><p className={`mt-6 max-w-2xl text-base leading-7 ${previewDark ? "text-white/65" : "text-black/60"}`}>{generated.hero.subheading}</p><button type="button" className={`mt-8 rounded-full px-6 py-3 text-sm font-black ${previewDark ? "bg-white text-black" : "bg-black text-white"}`}>{generated.hero.ctaLabel}</button></section>
            <section className={`border-t px-6 py-14 sm:px-12 ${previewDark ? "border-white/10 bg-white/[0.03]" : "border-black/10 bg-black/[0.025]"}`}><p className={`text-xs font-black uppercase tracking-[0.22em] ${previewDark ? "text-white/45" : "text-black/45"}`}>About</p><h3 className={`mt-3 ${previewEditorial ? "font-serif text-3xl" : "text-3xl font-black"}`}>{generated.about.heading}</h3><p className={`mt-4 max-w-3xl text-sm leading-7 ${previewDark ? "text-white/65" : "text-black/60"}`}>{generated.about.body}</p></section>
            <section className="grid gap-px sm:grid-cols-3">{["Hours", "Reservations", "Contact"].map((label) => <div key={label} className={`border-t p-6 sm:min-h-36 ${previewDark ? "border-white/10 bg-white/[0.02]" : "border-black/10 bg-white"}`}><p className="text-sm font-black">{label}</p><p className={`mt-2 text-xs leading-5 ${previewDark ? "text-white/45" : "text-black/45"}`}>Live from your location dashboard.</p></div>)}</section>
          </main>
        </div>
      </div> : <div className="space-y-6 p-5">
        <div className="flex justify-end"><button type="button" onClick={saveEdits} disabled={busy} className="rounded-full bg-[#f5b700] px-5 py-2.5 text-sm font-black text-black disabled:opacity-40">{busy ? "Saving..." : "Save edits"}</button></div>
