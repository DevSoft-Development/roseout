...
export async function loadWebsiteProductionVerification(): Promise<WebsiteVerificationRow[]> {
  const { data: websites, error } = await getAdminDatabaseClient()
  ...
  const [{ data: locations }, { data: nodes }, { data: replicas }] = await Promise.all([
    locationIds.length
      ? getAdminDatabaseClient().from("locations").select("id,name,title,included_domain_name,included_domain_status,included_domain_connection_status,included_domain_renewal_due_at").in("id", locationIds)
      : Promise.resolve({ data: [] as any[] }),
    nodeIds.length
      ? getAdminDatabaseClient().from("website_hosting_nodes").select("id,name,role,status,public_ip,deploy_url,last_health_check_at,cpu_percent,memory_percent,disk_percent").in("id", nodeIds)
      : Promise.resolve({ data: [] as any[] }),
    (websites || []).length
      ? getAdminDatabaseClient().from("website_hosting_replicas").select("website_id,node_id,version,status,synced_at,last_error").in("website_id", (websites || []).map((row) => row.id))
      : Promise.resolve({ data: [] as any[] }),
  ]);
