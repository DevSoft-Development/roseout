...
export async function getMenuPage(locationId: string, commercePageId?: string | null) {
  ...
  const { data } = await getAdminDatabaseClient().from("location_commerce_pages").select("*").eq("location_id", locationId).eq("page_type", "menu").order("sort_order", { ascending: true }).limit(1).maybeSingle();
  return data as Record<string, any> | null;
}
...
export async function ensureMenuPage(locationId: string, title = "Menu", commercePageId?: string | null) {
  ...
  const { data, error } = await getAdminDatabaseClient().from("location_commerce_pages").insert({ location_id: locationId, page_type: "menu", title, status: "draft", is_active: false }).select("*").single();
  if (error) throw error;
  return data as Record<string, any>;
}

async function readMenuRows(locationId: string, pageId: string, publicOnly = false) {
  const sectionsQuery = getAdminDatabaseClient().from("location_commerce_sections").select("*").eq("location_id", locationId).eq("commerce_page_id", pageId).order("sort_order", { ascending: true });
  const itemsQuery = getAdminDatabaseClient().from("location_commerce_items").select("*").eq("location_id", locationId).eq("commerce_page_id", pageId).order("sort_order", { ascending: true });
...
async function findPublicMenuLocation(locationIdOrSlug: string) {
  for (const column of ["id", "source_id", "source_location_id", "slug"] as const) {
    try {
      const { data, error } = await getAdminDatabaseClient().from("locations").select("*").eq(column, locationIdOrSlug).maybeSingle();
...
export async function getPublicLocationMenu(locationIdOrSlug: string, allowDraftPreview = false, commercePageId?: string | null) {
  ...
  let query = getAdminDatabaseClient().from("location_commerce_pages").select("*").eq("location_id", String(location.id));
...
export async function saveLocationMenu(locationId: string, input: SaveLocationMenuInput, actorContext: MenuActorContext, method: "POST" | "PATCH" | "DELETE" = "PATCH") {
  ...
  if (method === "POST") {
    ...
    if (body.action === "create_section") {
      ...
      const { error } = await getAdminDatabaseClient().from("location_commerce_sections").insert({ location_id: locationId, commerce_page_id: page.id, page_id: page.id, name: title, title, description: String(body.description || "").trim() || null, sort_order: Number(body.sort_order ?? 0), is_active: body.is_active !== false }); if (error) throw error;
    ...
    if (["publish_page", "unpublish_page", "update_page"].includes(String(body.action))) {
      ...
      const { error } = await getAdminDatabaseClient().from("location_commerce_sections").update({ title, name: title, description: String(body.description || "").trim() || null, is_active: body.is_active !== false, updated_at: new Date().toISOString() }).eq("id", body.section_id).eq("commerce_page_id", page.id).eq("location_id", locationId); if (error) throw error;
      ...
      const { data: section } = await getAdminDatabaseClient().from("location_commerce_sections").select("id,commerce_page_id").eq("id", sectionId).eq("location_id", locationId).maybeSingle();
      if (!section || String(section.commerce_page_id) !== String(page.id)) throw new MenuValidationError("Section not found on this page");
      const { error } = await getAdminDatabaseClient().from("location_commerce_items").update({ section_id: sectionId, name, description: String(body.description || "").trim() || null, price_cents: price, price_label: String(body.price_label || "").trim() || null, price: body.price_label || (price != null ? `$${(price / 100).toFixed(2)}` : null), image_url: cleanNullableUrl(body.image_url), tags: body.tags || [], is_available: body.is_available !== false, is_featured: body.is_featured === true, updated_at: new Date().toISOString() }).eq("id", body.item_id).eq("commerce_page_id", page.id).eq("location_id", locationId); if (error) throw error;
    }
    for (const [i, id] of (body.section_ids || []).entries()) await getAdminDatabaseClient().from("location_commerce_sections").update({ sort_order: i }).eq("id", id).eq("commerce_page_id", page.id).eq("location_id", locationId);
    for (const [i, id] of (body.item_ids || []).entries()) await getAdminDatabaseClient().from("location_commerce_items").update({ sort_order: i }).eq("id", id).eq("commerce_page_id", page.id).eq("location_id", locationId);
  } else {
    const page = await getMenuPage(locationId, commercePageId); if (!page) throw new MenuValidationError("Menu or package page not found");
    if (body.action === "delete_item") await getAdminDatabaseClient().from("location_commerce_items").delete().eq("id", body.item_id).eq("commerce_page_id", page.id).eq("location_id", locationId);
    else { await getAdminDatabaseClient().from("location_commerce_items").delete().eq("section_id", body.section_id).eq("commerce_page_id", page.id).eq("location_id", locationId); await getAdminDatabaseClient().from("location_commerce_sections").delete().eq("id", body.section_id).eq("commerce_page_id", page.id).eq("location_id", locationId); }
  }
