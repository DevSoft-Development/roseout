...
function redact(value: unknown, max = 8000) {
  return String(value ?? "").replace(SENSITIVE, "[redacted]").slice(0, max);
}

function normalizeError(value: unknown) {
  const error = value instanceof Error ? value : new Error(typeof value === "string" ? value : "Unhandled server error");
  const digestValue = value && typeof value === "object" && "digest" in value ? (value as { digest?: unknown }).digest : null;
  ...
  try {
    ...
    await fetch(`${supabaseUrl.replace(/\/$/, "")}/rest/v1/platform_error_events`, {
      ...
      body: JSON.stringify({
        ...
        metadata: { digest: redact(digest, 200), route_type: context.routeType, router_kind: context.routerKind, render_source: context.renderSource || null, revalidate_reason: context.revalidateReason || null },
      }),
    });
  } catch {
