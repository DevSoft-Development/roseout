  ...
  return (
    <section className="reserve-card rounded-2xl p-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <Link href={settingsHref} className="reserve-soft rounded-full px-3 py-2 text-xs font-black">{vocab.floorView}</Link>
      </div>
      {floorResources.length ? (
        <div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-3 2xl:grid-cols-4">
          {floorResources.map((r) => {
            ...
            return (
              <button type="button" key={r.id || r.layout_item_id || resourceName(r)} disabled={disabled} onClick={() => state.reservation ? onReservationSelect?.(state.reservation) : onResourceSelect?.(r)} className={`min-h-[96px] rounded-2xl border p-3 text-center transition-colors disabled:cursor-not-allowed disabled:opacity-45 ${statusStyles[state.status] || statusStyles.Open} ${assigningReservation && state.available ? "ring-2 ring-emerald-500/55" : ""}`}>
                ...
                <span className="mt-2 inline-flex whitespace-nowrap rounded-full border border-current/20 px-2 py-0.5 text-[10px] font-black leading-none">{stateLabel(state.status, vocab)}</span>
                {state.reservation ? (
                  <p className="mt-2 truncate text-xs font-bold text-white">{getReservationGuestName(state.reservation)} · {vocab.partyLabel} {state.reservation.party_size || "—"}</p>
