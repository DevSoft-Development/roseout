...
export default async function CentralDashboardPage() {
  ...
  const {
    totalLocations,reservations,todayReservations,upcomingReservations,activeEvents,activeExperiences,eventOrders,eventTickets,eventSalesCents,
    eventPlatformRevenueCents,experienceBookingCount,experienceGuests,experienceEstimatedValueCents,activePaidLocations,mrrCents,
    subscriptionCollected30dCents,trackedPlatformRevenue30dCents,openTickets,mlScored,mlIntentRows,mlPairRows,mlLastRunCreatedAt,
    generatedSites,liveGeneratedSites,hostingNodes,healthyHostingNodes,
  } = overview;
  ...
  return (
    <AdminPageShell>
      <AdminPageHeader
        ...
        subtitle="Monitor operations, partners, search health, claims, websites, infrastructure, and growth."
        badge={<AdminStatusBadge tone={openTickets ? "amber" : "green"}>{openTickets ? `${openTickets} open tickets` : "Operations clear"}</AdminStatusBadge>}
      ...
      <AdminKpiGrid>
        <AdminKpiCard label="Total locations" value={format(totalLocations)} helper="Restaurants + activities" />
        <AdminKpiCard label="Generated sites" value={format(generatedSites)} helper={`${format(liveGeneratedSites)} live`} />
        <AdminKpiCard label="Hosting nodes" value={format(hostingNodes)} helper={`${format(healthyHostingNodes)} healthy`} />
        <AdminKpiCard label="Reservations" value={format(reservations)} helper="All-time records" />
        <AdminKpiCard label="Today" value={format(todayReservations)} helper="Reservations today" />
        <AdminKpiCard label="Open tickets" value={format(openTickets)} helper="Support requiring attention" />
      </AdminKpiGrid>

      <section className="overflow-hidden rounded-3xl border border-white/10 bg-[#120d0b]">
        <div className="border-b border-white/10 px-5 py-4"><p className="text-[10px] font-black uppercase tracking-[.2em] text-rose-200">Marketplace + financial pulse</p><h2 className="mt-1 text-lg font-black">Business at a glance</h2></div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-5">
          {pulse.map(([label,value,detail,href]) => <Link key={label} href={href} className="min-h-[94px] border-b border-white/10 px-4 py-3 transition hover:bg-white/[.045] sm:border-r"><p className="text-[10px] font-black uppercase tracking-[.14em] text-white/40">{label}</p><p className="mt-1.5 text-xl font-black">{value}</p><p className="mt-1 text-[11px] font-semibold text-white/35">{detail}</p></Link>)}
        </div>
      </section>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
        <section className="grid min-w-0 gap-4 md:grid-cols-2">
          {groups.map(([title,desc,href,status]) => <Link key={title} href={href} className="rounded-[1.5rem] border border-white/10 bg-white/[.04] p-5 shadow-xl shadow-black/20 transition hover:border-rose-200/30 hover:bg-white/[.065]"><span className="inline-flex rounded-full border border-rose-200/20 bg-rose-500/10 px-3 py-1 text-xs font-black text-rose-50">{status}</span><h3 className="mt-4 text-xl font-black">{title}</h3><p className="mt-2 text-sm leading-6 text-white/55">{desc}</p>{title === "Machine Learning" && mlLastRunCreatedAt ? <p className="mt-2 text-xs font-bold text-white/40">Last run {new Date(mlLastRunCreatedAt).toLocaleDateString()}</p> : null}</Link>)}
        </section>
        <section className="rounded-3xl border border-white/10 bg-[#120d0b] p-5"><h2 className="text-xl font-black">Priority tasks</h2><p className="mt-1 text-sm text-white/55">Operational queues that usually need daily attention.</p><div className="mt-4 grid gap-2">{tasks.map(([label,href]) => <Link key={label} href={href} className="flex items-center justify-between rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-sm font-bold text-white/75 hover:border-rose-200/30 hover:text-white"><span>{label}</span><span className="text-rose-200">Open</span></Link>)}</div></section>
      </div>
    </AdminPageShell>
  );
}
