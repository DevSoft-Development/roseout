  ...
  return (
    <main className="space-y-6 p-4 sm:p-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--admin-shell-muted)]">Marketing · Content</p>
          <h1 className="text-3xl font-semibold">{item.title}</h1>
          <p className="mt-1 text-sm text-[var(--admin-shell-muted)]">Status: {item.status.replaceAll("_", " ")} · Approval: {item.approval_status.replaceAll("_", " ")} · Version {item.current_version}</p>
        </div>
        <div className="flex flex-wrap gap-2">
          {canPublish && currentVersionApproved ? <MarketingPublishNowButton contentId={item.id} /> : null}
          {item.approval_status === "pending" && canApprove ? <Link href={`/admin/dashboard/marketing/content/${item.id}/review`} className="min-h-11 rounded-xl bg-[var(--admin-shell-accent)] px-4 py-2.5 text-sm font-semibold text-white">Open review</Link> : null}
