...
async function templateState() {
  const { data: objects, error } = await getAdminDatabaseClient().storage.from(BUCKET).list("", { limit: 100 });
  if (error) throw error;
  const names = new Set((objects || []).map((item) => item.name));
  const front = getAdminDatabaseClient().storage.from(BUCKET).getPublicUrl("claim-front").data.publicUrl;
  const back = getAdminDatabaseClient().storage.from(BUCKET).getPublicUrl("claim-back").data.publicUrl;
  return {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { error: uploadError } = await getAdminDatabaseClient().storage.from(BUCKET).upload(storagePath, normalizedFile, {
