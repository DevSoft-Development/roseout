create table if not exists public.search_result_impressions (
  ...
  result_type text not null check (result_type in ('restaurant','activity','pair','matched_location')),
...
create unique index if not exists search_result_impressions_dedupe_idx
  on public.search_result_impressions (
    ...
    coalesce(activity_location_id::text, '')
  );
create index if not exists search_result_impressions_search_idx on public.search_result_impressions(search_id);
create index if not exists search_result_impressions_created_idx on public.search_result_impressions(created_at desc);
create index if not exists search_result_impressions_location_idx on public.search_result_impressions(location_id);
create index if not exists search_result_impressions_pair_idx on public.search_result_impressions(restaurant_location_id, activity_location_id);
...
create index if not exists search_negative_feedback_pair_idx on public.search_negative_feedback(restaurant_location_id, activity_location_id);
