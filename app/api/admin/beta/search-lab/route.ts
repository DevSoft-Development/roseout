...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const responseBody = {
      ...
      restaurantRpcTimedOut: debug?.restaurantRpcTimedOut ?? false,
      restaurantRpcTimeoutMs: debug?.restaurantRpcTimeoutMs ?? null,
      restaurantRpcFallbackUsed: debug?.restaurantRpcFallbackUsed ?? false,
      restaurantRpcFallbackReason: debug?.restaurantRpcFallbackReason ?? null,
      boroughStrictnessApplied: debug?.boroughStrictnessApplied ?? false,
      requestedBorough: debug?.requestedBorough ?? null,
      inBoroughResultCount: debug?.inBoroughResultCount ?? null,
      outOfBoroughResultCount: debug?.outOfBoroughResultCount ?? null,
      outOfBoroughSuppressedCount: debug?.outOfBoroughSuppressedCount ?? 0,
      outOfBoroughRecoveryAllowed: debug?.outOfBoroughRecoveryAllowed ?? false,
      outOfBoroughRecoveryReason: debug?.outOfBoroughRecoveryReason ?? null,
      featureRelaxed: debug?.featureRelaxed ?? false,
      featureRelaxedReason: debug?.featureRelaxedReason ?? null,
