      ...
      {result && (
        <div className="mt-4 rounded-2xl border border-white/10 bg-black/25 p-4 text-sm text-white/70">
          ...
          {result.error ? <p className="mt-2 text-rose-200">{result.error}</p> : (
            <p className="mt-2">Requested {result.requestedLimit}; processed {result.processed}; updated {result.updated}; skipped {result.skipped}; failed {result.failed}; retry later {result.retryLater}; duration {result.durationMs}ms.</p>
          )}
        </div>
      )}
    </AdminSectionCard>
