...
import { stripeRequestViaIntegrationApi } from "../_shared/aws-integration.ts";
...
Deno.serve(async (request: Request) => {
  ...
  if (!workerSecret) return json({ success: false, error: "WORKER_INTERNAL_SECRET is not configured" }, 500);
  if (!supabaseUrl || !serviceRoleKey) return json({ success: false, error: "Supabase runtime credentials are not configured" }, 500);
  ...
  const { data: locations, error: readError } = await supabase
    ...
    .limit(limit);
  if (readError) return json({ success: false, error: `Unable to read billing locations: ${readError.message}`, durationMs: Date.now() - startedAt }, 500);
  ...
  for (const location of locations ?? []) {
    ...
    try {
      ...
      const drift = String(location.subscription_plan || "") !== String(patch.subscription_plan)
      ...
      if (drift) drifted += 1;
      const { error: updateError } = await supabase.from("locations").update(patch).eq("id", location.id);
  ...
  if (!expiredError && expiredGrace?.length) {
    ...
    const { error } = await supabase.from("locations").update({ subscription_status: "past_due", updated_at: nowIso }).in("id", ids);
    if (!error) graceExpired = ids.length;
  }
  console.log("billing-reconciliation completed", { source: "aws_edge_runtime", stripeProvider: "aws_integration_api", scanned, updated, drifted, graceExpired, failed, durationMs: Date.now() - startedAt });
  return json({ success: failed === 0, source: "aws_edge_runtime", stripeProvider: "aws_integration_api", scanned, updated, drifted, graceExpired, failed, failures: failures.slice(0, 10), durationMs: Date.now() - startedAt }, failed === scanned && scanned > 0 ? 500 : 200);
});

async function fetchStripeSubscription(subscriptionId: string): Promise<StripeSubscription> {
  return stripeRequestViaIntegrationApi<StripeSubscription>({
    apiVersion: "v1",
    mode: "live",
    method: "GET",
...
function normalizeBillingStatus(value?: string | null): string {
  ...
  if (["inactive", "trialing", "active", "past_due", "grace_period", "canceled", "comped", "incomplete", "incomplete_expired", "unpaid", "paused"].includes(clean)) return clean;
  return "inactive";
}
function toIso(seconds?: number | null): string | null { return seconds ? new Date(seconds * 1000).toISOString() : null; }
function addDays(value: Date, days: number): string { return new Date(value.getTime() + days * 86_400_000).toISOString(); }
function secureCompare(left: string, right: string): boolean { if (!left || !right || left.length !== right.length) return false; let difference = 0; for (let index = 0; index < left.length; index += 1) difference |= left.charCodeAt(index) ^ right.charCodeAt(index); return difference === 0; }
