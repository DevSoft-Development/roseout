...
export default async function TicketOrdersPage({ searchParams }: { searchParams: Params }) {
  ...
  const [{ data: eventOrders, error: eventError }, { data: experienceBookings, error: experienceError }] = await Promise.all([
    adminDb.from("event_ticket_orders").select("id,event_id,purchaser_name,purchaser_email,quantity,status,email_delivery_status,sms_delivery_status,payment_status,currency,total_cents,platform_fee_cents,organizer_net_estimate_cents,provider_account_id,provider_payment_intent_id,created_at").order("created_at", { ascending: false }).limit(300),
    adminDb.from("experience_bookings").select("id,experience_id,customer_name,customer_email,party_size,status,email_delivery_status,sms_delivery_status,payment_status,amount_cents,checked_in_count,created_at").order("created_at", { ascending: false }).limit(300),
  ]);
  ...
  const [{ data: events }, { data: experiences }, { data: tickets }] = await Promise.all([
    eventIds.length ? adminDb.from("events").select("id,title,location_id,organization_id").in("id", eventIds) : Promise.resolve({ data: [] as any[] }),
    experienceIds.length ? adminDb.from("experiences").select("id,title,location_id,organization_id,currency").in("id", experienceIds) : Promise.resolve({ data: [] as any[] }),
    eventIds.length ? adminDb.from("event_tickets").select("order_id,status").in("event_id", eventIds) : Promise.resolve({ data: [] as any[] }),
  ]);
  ...
  return <AdminPageShell>
      <AdminPageHeader
        ...
        title="Ticket Orders"
        subtitle="Unified operational ledger for Event ticket orders and Experience bookings. Stripe remains the payment source of truth while this workspace tracks fulfillment, delivery, payment, and check-in state."
        badge={<AdminStatusBadge tone={problem ? "amber" : "green"}>{problem ? `${problem} need attention` : "Order health clear"}</AdminStatusBadge>}
        actions={<AdminActionButton href="/admin/dashboard">Admin Overview</AdminActionButton>}
      />
      <AdminKpiGrid>
        <AdminKpiCard label="Orders" value={rows.length} helper="Events + experiences" />
        <AdminKpiCard label="Paid gross" value={money(gross)} helper="Paid orders" />
        <AdminKpiCard label="Platform fees" value={money(fees)} helper="Tracked event fees" />
        <AdminKpiCard label="Needs attention" value={problem} helper="Failed, refunded, or disputed" />
      </AdminKpiGrid>
