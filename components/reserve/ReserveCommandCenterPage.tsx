...
function ReserveCommandCenterContent() {
  ...
  useEffect(()=>{ const nextTab = searchParams.get("tab") || "today"; if(validTabs.has(nextTab)) setActiveTab(nextTab); const nextDate = searchParams.get("date"); if(nextDate) setSelectedDate(nextDate); const nextStatus = searchParams.get("status"); if(nextStatus) setStatusFilter(nextStatus); }, [searchParams]);
  ...
  const topActions = <><Link className="reserve-soft rounded-full px-4 py-2 text-sm font-black" aria-disabled={!locationId} title={locationId ? "Open public booking page" : "Select a location before opening the booking page."} href={getBookingLink(locationId, locationType) || "#"}>Booking page <ExternalLink className="inline" size={14}/></Link><Link className="reserve-soft rounded-full px-4 py-2 text-sm font-black" title={locationId ? "Open embed page" : "Select a location before opening the embed."} href={getEmbedLink(locationId) || "#"}>Embed</Link><Link className="reserve-soft rounded-full px-4 py-2 text-sm font-black" title={locationId ? "Open QR tools" : "QR tools are not configured for this location yet."} href={getQrLink(locationId) || "#"}><QrCode className="inline" size={14}/> QR Code</Link><ReserveQuickActionButton disabled={!locationId} title={!locationId ? "Select a location before creating a reservation." : undefined} onClick={()=>setModal("reservation")}><Plus size={14} className="inline"/> New Reservation</ReserveQuickActionButton><ReserveQuickActionButton disabled={!locationId} title={!locationId ? "Select a location before adding a walk-in." : undefined} onClick={()=>setModal("walkin")}><UserPlus size={14} className="inline"/> Walk-in</ReserveQuickActionButton></>;
