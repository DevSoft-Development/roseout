...
export default async function CommunityPage({ searchParams }: { searchParams: Promise<{ conversation?: string }> }) {
  ...
  const [messageResult, journeyResult] = selectedId ? await Promise.all([
    supabaseAdmin.from("social_community_messages").select("id,direction,sender_type,body,status,created_at").eq("conversation_id", selectedId).order("created_at"),
    supabaseAdmin.from("analytics_events").select("id,event_name,event_type,query,source,occurred_at,created_at,metadata").eq("metadata->>social_conversation_id", selectedId).order("occurred_at", { ascending: true }).limit(100),
  ...
  return <main className="min-h-screen bg-[#090706] px-4 py-6 text-white sm:px-6 lg:px-8"><div className="mx-auto max-w-[1500px] space-y-5">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.25em] text-rose-300">Social Manager</p><h1 className="mt-2 text-4xl font-semibold">Community</h1><p className="mt-2 text-white/55">Messages, customer opportunities, business leads, creator interest, and conversations that need a person.</p></div><Link href="/admin/dashboard/marketing/social-manager" className="rounded-full border border-white/15 px-5 py-3 text-sm font-semibold">Back to Social Manager</Link></div>
