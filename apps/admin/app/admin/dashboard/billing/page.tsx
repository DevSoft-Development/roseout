...
export default async function BillingPage() {
  ...
  return <AdminPageShell>
    <AdminPageHeader
      eyebrow="Commerce · Stripe Operations"
      title="Billing Command Center"
      subtitle="Real subscription lifecycle, Stripe events, collections, upcoming renewals, and failed payment data."
      badge={<AdminStatusBadge tone={sourceError ? "amber" : metrics.pastDueLocations ? "amber" : "green"}>{sourceError ? "Billing read warning" : metrics.pastDueLocations ? `${metrics.pastDueLocations} past due` : "Billing operations healthy"}</AdminStatusBadge>}
      actions={<AdminActionButton href="/admin/dashboard/plans">Open Plans</AdminActionButton>}
    />
