...
type MarketingUserRow = { id: string; phone?: string | null; marketing_opt_in?: boolean | null };
type PreferenceRow = { phone?: string | null; sms_opt_in?: boolean | null; sms_opted_out_at?: string | null };
type Recipient = { id?: string | null; user_id?: string | null; phone?: string | null; sms_opt_in?: boolean | null; sms_opted_out_at?: string | null };
...
async function loadSmsRecipients(): Promise<Recipient[]> {
  const [subscribersResult, usersResult, preferencesResult] = await Promise.all([
    supabaseAdmin.from("marketing_subscribers").select("id,user_id,phone,sms_opt_in,sms_opted_out_at").eq("sms_opt_in", true).is("sms_opted_out_at", null).not("phone", "is", null).limit(1000),
    supabaseAdmin.from("users").select("id,phone,marketing_opt_in").eq("marketing_opt_in", true).not("phone", "is", null).limit(1000),
    supabaseAdmin.from("user_marketing_preferences").select("phone,sms_opt_in,sms_opted_out_at").eq("sms_opt_in", false).not("phone", "is", null).limit(1000),
  ]);

  const optedOutPhones = new Set(((preferencesResult.data || []) as PreferenceRow[]).filter((row) => row.sms_opted_out_at || row.sms_opt_in === false).map((row) => row.phone).filter((phone): phone is string => Boolean(phone)));
...
export async function POST(req: Request) {
  ...
  for (const recipient of recipients) {
    ...
    try {
      ...
      failed += 1;
      logs.push({ campaign_id: campaignId, subscriber_id: recipient.id || null, user_id: recipient.user_id || null, channel: "sms", provider: "telnyx", recipient_phone: to, status: "failed", error_message: sendError instanceof Error ? sendError.message : "SMS send failed", attempted_at: attemptedAt });
    }
  }
