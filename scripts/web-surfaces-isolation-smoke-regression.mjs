...
requireText(authOrigin, 'surface !== "business"', 'Only the isolated Business surface should override the normal consumer auth origin.');
requireText(authOrigin, 'request.headers.get("x-forwarded-host")', 'Business callback origin must respect the forwarded request hostname behind the ALB.');
requireText(authOrigin, 'request.headers.get("x-forwarded-proto")', 'Business callback origin must preserve HTTPS behind the ALB.');
requireText(authOrigin, '? "/login"', 'Business auth failures must return to the Business login page.');
