...
export async function sendCommunityReply(input: {
  ...
  const { data: conversation, error: conversationError } = await getAdminDatabaseClient()
  ...
  const [{ data: contact }, { data: inbound }, { data: connection }] = await Promise.all([
    getAdminDatabaseClient().from("social_community_contacts").select("external_user_id").eq("id", conversation.contact_id).maybeSingle(),
    getAdminDatabaseClient().from("social_community_messages").select("external_message_id").eq("conversation_id", conversationId).eq("direction", "inbound").order("created_at", { ascending: false }).limit(1).maybeSingle(),
    getAdminDatabaseClient().from("marketing_social_connections").select("id,provider_account_id,status,token_expires_at,granted_scopes").eq("scope", "platform").eq("provider", conversation.provider).eq("status", "connected").order("updated_at", { ascending: false }).limit(1).maybeSingle(),
  ]);
  ...
  const { error: messageError } = await getAdminDatabaseClient().from("social_community_messages").insert({
  ...
  const { error: conversationUpdateError } = await getAdminDatabaseClient().from("social_community_conversations").update({
