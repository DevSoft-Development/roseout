...
async function loadHealthyCandidates(role: "primary" | "failover", excludeNodeId?: string | null) {
  const { data: nodes, error } = await getAdminDatabaseClient()
...
export async function allocateLightsailWebsiteNode(locationId: string, rawDomain?: string | null) {
  ...
  if (existing?.hosting_node_id) {
    const { data: existingNode, error: nodeError } = await getAdminDatabaseClient()
      .from("website_hosting_nodes")
  ...
  if (existing) {
    const { data: website, error } = await getAdminDatabaseClient()
      .from("business_websites")
  ...
  const { data: website, error: insertError } = await getAdminDatabaseClient()
    .from("business_websites")
    .insert({
  ...
  if (insertError) {
    if (insertError.code === "23505") {
      const { data: collided } = await getAdminDatabaseClient().from("business_websites").select(selectFields).eq("location_id", locationId).maybeSingle();
