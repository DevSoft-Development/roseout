...
export type ReserveLayoutManagerProps = {
  backHref?: string;
  adminMode?: boolean;
  createMode?: boolean;
...
function defaultForm(
  ...
  nextSpot: { left: number; top: number },
): FormState {
...
export default function ReserveLayoutManager({
  backHref = "/reserve/dashboard",
  adminMode = false,
  createMode = false,
  initialLocationId = "",
  initialLocationType,
  ...
  const [locationType, setLocationType] = useState<LocationType>(
    initialLocationType || "restaurant",
  );
  ...
  const [form, setForm] = useState<FormState>(
    defaultForm(null, { left: 24, top: 24 }),
  );
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "layout_items" },
        () => loadLayout(false),
      )
      .on(
        "postgres_changes",
  ...
  async function loadLayout(showLoader = true) {
    try {
      ...
      if (initialLocationId) {
        ...
        if (
          matchedInitialLocation?.type &&
          matchedInitialLocation.type !== locationType
        ) {
          setLocationType(matchedInitialLocation.type);
        } else if (
          !matchedInitialLocation &&
          initialLocationType &&
          initialLocationType !== locationType
        ) {
  ...
  async function saveArea(override?: Partial<FormState>) {
    ...
    const durationMinutes = Number(
      values.duration === 0 ? values.customDuration : values.duration,
    );
    try {
      setSaving("area");
      setError("");
  ...
  function continueDrag(event: React.PointerEvent<HTMLButtonElement>) {
    ...
    const rect = canvas.getBoundingClientRect();
    const left = clamp(
      event.clientX - rect.left - drag.offsetLeft,
      8,
      CANVAS_WIDTH - CARD_WIDTH - 8,
    );
    const top = clamp(
      event.clientY - rect.top - drag.offsetTop,
      8,
      CANVAS_HEIGHT - CARD_HEIGHT - 8,
    );
    setItems((current) =>
      current.map((item) =>
        item.id === drag.id ? { ...item, layout_x: left, layout_y: top } : item,
      ),
    );
  }
  ...
  const completedSteps = [
    ...
    visibleItems.some((item) => itemCapacity(item) > 0),
    visibleItems.some((item) => itemDuration(item) > 0),
    visibleItems.some((item) => statusFromItem(item) === "available"),
  ...
  const content = (
    <div
      className={
        embedded ? "space-y-5" : "mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8"
      }
    >
      {!embedded ? (
      ...
      {!embedded ? (
        ...
        <section className="reserve-card rounded-[1.5rem] p-5">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-4">
            <div className="reserve-soft rounded-2xl p-3">
              <p className="text-xs reserve-muted">Total spaces</p>
            ...
            {scopedInitialLocation ? (
              ...
              filteredLocations.slice(0, 9).map((location) => (
                <button
                  key={location.id}
                  onClick={() => {
                    if (scopedInitialLocation) return;
                ...
                >
                  ...
                  <p className="mt-1 text-sm text-white/55">
                    {[location.city, location.state]
                      .filter(Boolean)
                      .join(", ") || "Address details not listed"}
                  </p>
                  {location.owner_email ? (
                    <p className="mt-1 text-xs font-bold text-white/40">
                      Owner: {location.owner_email}
                    </p>
                  ) : null}
                </button>
              ))
        ...
        <section className="mt-6 rounded-[2rem] border border-white/10 bg-white/[0.04] p-5">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              {selectedLocation.owner_email ? (
                <p className="mt-1 text-sm text-white/50">
                  Owner: {selectedLocation.owner_email}
                </p>
              ) : null}
            </div>
            <select
              ...
              className="rounded-2xl border border-white/10 bg-black px-4 py-3 text-sm font-black text-white outline-none"
            >
      ...
      {error ? (
        <div className="mt-5 rounded-2xl border border-red-400/40 bg-red-950/60 p-4 text-sm font-bold text-red-100">
          {error}
        </div>
      ) : null}
      {message ? (
        <div className="mt-5 rounded-2xl border border-emerald-400/40 bg-emerald-950/40 p-4 text-sm font-bold text-emerald-100">
          {message}
        </div>
      ) : null}
          ...
          <div className="mt-5 space-y-4">
            ...
            <label className="space-y-2 block">
              ...
              <textarea
                ...
                placeholder="Near window, VIP only, wheelchair accessible, etc."
            ...
            {loading ? (
              ...
              <div className="overflow-x-auto p-4">
                <div
                  ...
                  className="relative rounded-[1.5rem] border border-neutral-200 bg-[radial-gradient(circle_at_1px_1px,rgba(0,0,0,0.08)_1px,transparent_0)] [background-size:24px_24px]"
                >
  ...
  return <main className="min-h-screen bg-black text-white">{content}</main>;
}
