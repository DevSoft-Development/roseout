...
function buildCronDigestText(digest: CronDigest, options: { hours: number; debug?: boolean }) {
  const lines = [
    ...
    `- Reservation cron runs: ${digest.reservationCronRuns.length}`,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: jobRow } = await supabase.from("cron_jobs").select("is_active").eq("job_key", "admin-cron-digest-email").maybeSingle();
    if (jobRow?.is_active === false) {
      await logCronJobRun(supabase, { job_name: "admin-cron-digest-email", function_name: "admin-cron-digest-email", route_path: "supabase/functions/admin-cron-digest-email", source: "digest_self", status: "skipped", started_at: startedAt, finished_at: new Date().toISOString(), duration_ms: timer(), skipped_count: 1, message: "admin-cron-digest-email skipped because job is paused.", metadata: { reason: "job_disabled" } });
      return ok({ success: true, skipped: true, reason: "job_disabled" });
    }
    ...
    const digest: CronDigest = {
      ...
      reservationCronRuns: cron.filter((row: RunRow) => ["reservation-daily-digest", "reservation-status-cleanup", "reservation-reminder-cron"].includes(String(row.job_key || row.job_name || row.function_name))),
      recentCronRuns: cron,
      importRuns: imports,
    };
    ...
    if (body.sendEmail !== false) {
      ...
      emailResult = to
        ? await sendEmail({
            to,
            subject: "TheOutHaven overnight cron activity digest",
    ...
    await logCronJobRun(supabase, {
      ...
      function_name: "admin-cron-digest-email", route_path: "supabase/functions/admin-cron-digest-email", description: "Emails admins a cron and import monitoring digest.", schedule_hint: "pg_cron: 45 6 * * *",
      source: "digest_self",
      status: "success",
    ...
    await logCronJobRun(supabase, { job_name: "admin-cron-digest-email", function_name: "admin-cron-digest-email", route_path: "supabase/functions/admin-cron-digest-email", description: "Emails admins a cron and import monitoring digest.", schedule_hint: "pg_cron: 45 6 * * *", source: "digest_self", status: "failed", started_at: startedAt, finished_at: new Date().toISOString(), duration_ms: timer(), failed_count: 1, error_message: message, metadata: { selfDigest: true, authSource } });
