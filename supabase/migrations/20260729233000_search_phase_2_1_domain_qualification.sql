...
as $$
  ...
  where l.active = true
    ...
    and (
      ...
      or (
        p_domain = 'restaurant'
        and p.supported_domains @> array['restaurant']::text[]
        and coalesce(p.is_verified, false) = true
      )
      or (
        p_domain = 'activity'
        and p.supported_domains @> array['activity']::text[]
        and coalesce(p.is_verified, false) = true
        and (
...
comment on function public.enterprise_search_profile_locations(text,text,text[],text,text,text,text,text,text,double precision,double precision,double precision,integer) is
  'Canonical profile retrieval with strict primary-domain authority and verified dual-domain eligibility.';
