...
export const metadata: Metadata = buildMetadata({
  title: "Plan the Full Outing",
  description:
    "TheOutHaven helps people plan restaurants, activities, lounges, date plans, birthdays, brunch, weekend ideas, and local experiences across NYC, Long Island, Northern New Jersey, and Connecticut.",
  path: "/",
});
...
const occasionCards = [
  {
    icon: "♡",
    title: "Date Plans",
    description: "Food, atmosphere, and something memorable to do together.",
    prompt: "date plan with food and something fun after",
  },
  {
    icon: "✦",
    title: "Birthday Plans",
    description: "Celebration-ready places with room for the next stop.",
    prompt: "birthday plans with food and an activity",
  },
  {
    icon: "✨",
    title: "Girls’ Plans",
    description: "Social food, drinks, activities, and the right energy.",
    prompt: "girls plans with food drinks and activities",
  },
  {
    icon: "☀",
    title: "Brunch Plans",
  ...
  },
  {
    ...
    prompt: "rooftop views with food drinks or activities",
  },
  {
    icon: "◐",
    title: "Food + Hookah",
    description: "Start with food, then keep the plan relaxed nearby.",
    prompt: "food and hookah lounge nearby",
  },
  {
    icon: "◆",
    title: "After Work Plans",
    description: "A polished reset with drinks, bites, or something casual.",
    prompt: "after work drinks bites or activity",
  },
  {
    icon: "○",
    title: "Weekend Ideas",
    description: "Casual food, activities, and low-pressure plans.",
    prompt: "casual food and relaxed weekend activity",
  },
];

const searchExamples = [
  "brunch and something fun in Brooklyn",
  "rooftop views and drinks in Manhattan",
  "birthday plans with food and activities",
  "family-friendly activities near Queens",
];
...
const planningSteps = [
  {
    ...
    copy: "Search by food, activity, occasion, neighborhood, borough, city, or what you want to do next.",
  },
  {
    title: "Get matched options",
    copy: "TheOutHaven helps surface restaurants, lounges, activities, rooftops, brunch spots, and local experiences that fit the full plan.",
  },
  {
      ...
      <div className="space-y-7">
        ...
        <div className="space-y-5">
          <h1 className="max-w-4xl text-5xl font-black tracking-[-0.05em] text-white sm:text-7xl lg:text-8xl">
            Plan the full outing, not just one stop.
          </h1>
          <p className="max-w-2xl text-lg leading-8 text-white/70 sm:text-xl">
            Find restaurants, lounges, rooftops, activities, brunch spots, and
            local experiences matched by vibe, area, occasion, and what you want
            to do next.
          </p>
        </div>
        ...
        <div className="rounded-[2rem] border border-white/10 bg-black/35 p-4 shadow-2xl shadow-black/30 backdrop-blur sm:p-5">
          ...
          >
            <input
              type="text"
              name="prompt"
              placeholder="Try: brunch, rooftop views, and something fun in Queens"
...
function OutingPreviewCard() {
  const rows = [
    {
      title: "Start with the vibe",
      copy: "Choose brunch, dinner, drinks, family fun, a date plan, a birthday, or something casual.",
    },
    {
      title: "Add the next stop",
      copy: "Pair food with rooftops, lounges, hookah, bowling, comedy, views, museums, or something chill.",
    },
    {
      title: "Match the area",
      copy: "Keep the plan nearby with smarter area, distance, and vibe matching.",
    },
  ];
    ...
    <aside className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.04] p-5 shadow-2xl shadow-black/40 sm:p-6 lg:ml-auto lg:max-w-[34rem]">
      ...
      <div className="relative rounded-[1.5rem] border border-white/10 bg-black/35 p-5 backdrop-blur">
        ...
        <h2 className="mt-3 max-w-sm text-3xl font-black tracking-tight text-white sm:text-4xl">
          Food, activities, and experiences that fit the plan
        </h2>
        ...
        <div className="mt-5 flex flex-col gap-2 rounded-3xl border border-white/10 bg-[#e1062a]/10 p-3 text-xs font-black uppercase tracking-[0.18em] text-white/80 sm:flex-row sm:items-center sm:justify-between">
          <span>Built for complete outings</span>
          <span className="text-[#ff8a9a]">Less searching, more going</span>
        </div>
      </div>
    </aside>
  );
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-white/65">
            Start with the moment. TheOutHaven helps shape the food, the next
            stop, and the overall vibe.
          </p>
        </div>
      </div>
  ...
  return (
    <section className="rounded-[2rem] border border-white/10 bg-white/[0.035] p-6 shadow-2xl shadow-black/25 sm:p-8">
      <div className="mb-6">
        ...
        <p className="mt-2 max-w-3xl text-sm leading-6 text-white/65">
          Made for complete outings — food, activities, experiences, and the
          mood around it.
        </p>
      </div>
...
function FeaturedPlaces({ locations }: { locations: HomeLocation[] }) {
  return (
    <section>
      <div className="mb-6">
        <h2 className="text-3xl font-black tracking-tight sm:text-4xl">
          Outing-worthy places
        </h2>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-white/65">
          A rotating mix of restaurants, activities, lounges, rooftops, brunch
          spots, and local experiences to inspire your next plan.
        </p>
      </div>
      {locations.length ? (
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="rounded-[1.5rem] border border-white/10 bg-white/[0.035] p-6 text-sm leading-6 text-white/65">
          Fresh outing-worthy places are being curated. Start with a prompt and
  ...
  return (
    <section className="rounded-[2rem] border border-white/10 bg-[radial-gradient(circle_at_20%_20%,rgba(225,6,42,.30),transparent_40%),linear-gradient(135deg,#180807,#080303)] p-6 shadow-2xl shadow-black/25 sm:p-8 lg:p-10">
      ...
      <p className="mt-3 max-w-3xl text-sm leading-7 text-white/70 sm:text-base">
        ...
        activity you want to include — and TheOutHaven will help turn it into a
        plan.
      </p>
      <div className="mt-6">
        <Link
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-center">
        <div>
          ...
          <h2 className="mt-3 max-w-3xl text-3xl font-black tracking-tight sm:text-4xl">
            Own a restaurant, lounge, activity space, or local experience?
          </h2>
          <p className="mt-3 max-w-3xl text-sm leading-7 text-white/70 sm:text-base">
            Get discovered by people planning birthdays, brunch, date plans,
            weekend ideas, family plans, group outings, and local experiences
            near you.
          </p>
        </div>
  ...
  return (
    <section className="rounded-[2rem] border border-[#e1062a]/30 bg-[#e1062a]/10 p-6 text-center shadow-2xl shadow-black/25 sm:p-8 lg:p-10">
      ...
      <p className="mx-auto mt-3 max-w-2xl text-sm leading-7 text-white/72 sm:text-base">
        Tell us the vibe, area, and occasion. We’ll help you find places and
        experiences that fit the full plan.
      </p>
...
function buildFeaturedLocations(locations: HomeLocation[]) {
  ...
  const activities = rotate(
    ranked.filter((location) => !isRestaurant(location)),
    3,
  );
  const featured = rotate(
    ranked.filter((location) => location.featured),
    5,
  );
    ...
    <article className="group flex h-full min-h-[382px] flex-col overflow-hidden rounded-[1.6rem] border border-white/10 bg-zinc-950/80 p-3 shadow-2xl shadow-black/30 transition duration-300 hover:-translate-y-1 hover:border-white/20">
      ...
      <div className="mt-4 flex flex-1 flex-col px-1 pb-1">
        ...
        <p className="mt-1 line-clamp-1 text-xs font-semibold text-white/50">
          {[
            getCuisine(location) || location.activity_type,
            ratingLabel(location),
          ]
            .filter(Boolean)
...
function occasionHref(title: string, prompt: string) {
  const landingPages: Record<string, string> = {
    ...
    "Rooftop Plans": "/explore/rooftop-restaurants",
    "Food + Hookah": "/explore/hookah-lounges",
  };
...
function matchingLocations(locations: HomeLocation[], keywords: string[]) {
  return locations
    .filter((location) => hasAny(location, keywords))
