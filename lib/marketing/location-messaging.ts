...
export async function materializeLocationMessagingCampaign(input: {
  ...
  if (channel === "sms") {
    ...
    const { data: latestCredit, error: creditError } = await supabaseAdmin
    ...
    const remaining = Math.max(0, Number(latestCredit?.credits_remaining || 0));
    if (audience.length > remaining) {
      throw new Error(`This SMS campaign needs ${audience.length} credits, but only ${remaining} remain for the current billing period.`);
    }
  }
