...
export default async function ReserveOperationsPage() {
  ...
  const rows = locations.map((location) => {
    ...
    return { location, reservations: locReservations.length, active: active.length, seatedCovers: seated.reduce((sum, row) => sum + Math.max(1, Number(row.party_size || 1)), 0), waiting: locWaitlist.length, attention, pacing, assignmentMode: locSettings.assignment_mode || "balanced", healthy: attention === 0 && pacing === 0 };
