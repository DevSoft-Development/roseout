...
export async function runCrmSequenceAutomation(triggerSource="cron"):Promise<AutomationRunResult>{
  ...
  if(runError)throw normalizeAutomationError(runError);
  ...
  try{
    ...
    await supabaseAdmin.from("crm_automation_runs").update({status:"failed",finished_at:result.finishedAt,error_summary:[{code:error.code,message:error.message,category:error.category}]}).eq("id",run.id);
    throw error;
  }
}
