...
s = regex_once(
    s,
    r"fence_roles\(\) \{.*?\n\}\n\nunfence_roles\(\) \{.*?\n\}\n\ncandidate_sql\(\) \{",
    lambda _: new_helpers,
    'failback fence helpers',
)
...
s = regex_once(
    s,
    r"  roles_sql=\"select rolname,coalesce\(array_to_string\(rolconfig,','\),''\) rolconfig from pg_roles where rolname in \('authenticator','supabase_auth_admin','supabase_storage_admin'\) order by rolname;\"\n  query_ref \"\$VIRGINIA_REF\" \"\$roles_sql\" \"\$TMP/virginia-fence\.json\"\n  test .*?\n  jq -e .*?\n    echo 'Virginia is not fully fenced from application/Auth/Storage writes\.' >&2\n    exit 1\n  \}\n\}",
    lambda _: new_verify,
    'failback Virginia fence prerequisite',
)

subscriber_anchor = "    grant pg_create_subscription to ${FAILBACK_SUBSCRIBER_ROLE};\n"
subscriber_new = subscriber_anchor + "    alter role ${FAILBACK_SUBSCRIBER_ROLE} set default_transaction_read_only=off;\n"
