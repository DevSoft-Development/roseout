...
const subjects:Record<BetaReminderType,string>={weekly_start:"Your weekly TheOutHaven beta test is ready",midweek_nudge:"Your weekly TheOutHaven beta test is ready",daily_incomplete:"Your weekly TheOutHaven beta test is ready",friday_final:"Your weekly TheOutHaven beta test is ready",completed_weekly_goal:"Thank you — you completed your weekly beta test"};
type EmailInput={name?:string;completed:number;required?:number};
function reminderBody(input:EmailInput){const required=input.required??5;const dashboard=buildSiteUrl("/user/dashboard/beta");return `Hi ${input.name||"there"},
...
Completing your weekly beta test helps you stay eligible for the $100 Beta Tester Reward, along with any required social follow/tag verification.
...
TheOutHaven Team`;}
export async function sendBetaReminderEmail({testerId,reminderType}:{testerId:string;reminderType:BetaReminderType}){const weekStart=getCurrentWeekStart(); if(!(await shouldSendBetaReminder(testerId,reminderType,weekStart)))return {status:"skipped"}; const {data:tester}=await supabaseAdmin.from("beta_testers").select("*").eq("id",testerId).maybeSingle(); if(!tester?.email)return {status:"skipped"}; const links:any[]=[]; const completed=Number(tester.weekly_completed_tests||0); const subject=subjects[reminderType]; const isCompleted=reminderType==="completed_weekly_goal"; const mailBody=isCompleted?completedBody(tester.name):reminderBody({name:tester.name,completed,required:tester.weekly_required_tests||5}); const inserted=await supabaseAdmin.from("beta_email_reminders").insert({tester_id:testerId,email:tester.email,reminder_type:reminderType,subject,status:"pending",week_start:weekStart,weekly_required_tests:tester.weekly_required_tests||5,weekly_completed_tests:completed,incomplete_task_count:links.length,task_links:links}).select("id").single(); const result=await sendRawBrandedEmail({to:tester.email,department:"support",subject,heading:isCompleted?"Weekly beta tasks completed":subject,body:mailBody,cta:{label:"Continue Weekly Beta Test",url:buildSiteUrl("/user/dashboard/beta")},replyTo:"support@theouthaven.com"}); const status=result.status==="error"?"failed":result.status==="sent"?"sent":"skipped"; await supabaseAdmin.from("beta_email_reminders").update({status,sent_at:status==="sent"?new Date().toISOString():null,error_message:result.error??null}).eq("id",inserted.data?.id); return {status};}
