...
export async function GET(request: Request){
  ...
  const ctx=await resolveEditableLocationContext({userId:user.id,locationId:url.searchParams.get("locationId"),adminLocationId:url.searchParams.get("adminLocationId"),demoLocationId:url.searchParams.get("demoLocationId"),sourceId:url.searchParams.get("sourceId"),type:url.searchParams.get("type"),demo:url.searchParams.get("demo")==="1",fromDemoCenter:url.searchParams.get("fromDemoCenter")==="1"});
