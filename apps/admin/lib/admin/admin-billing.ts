...
export type AdminBillingSnapshot = Omit<
  CoreAdminBillingResponse,
  "success" | "upcomingRows" | "pastDueRows" | "recentEvents" | "trialRows"
> & {
...
const LOCATION_SELECT =
  "id,name,restaurant_name,activity_name,owner_email,claimed_by_email,subscription_plan,subscription_status,subscription_amount_cents,subscription_interval,next_billing_date,current_period_end,trial_ends_at,stripe_customer_id,stripe_subscription_id,last_payment_failed_at,billing_grace_ends_at,canceled_at,created_at";
...
function normalizePaymentEvent(row: AdminBillingRow): AdminBillingRow {
  ...
  const object =
    payload?.data?.object && typeof payload.data.object === "object"
      ? payload.data.object
      : {};
  const metadata =
  ...
  return {
    ...
    amount_paid_cents: Number(
      object.amount_paid ?? object.amount_total ?? object.amount_received ?? 0,
    ),
...
async function localAdminBillingSnapshot(): Promise<AdminBillingSnapshot> {
  ...
  const [{ data, error }, logsResult] = await Promise.all([
    db.from("locations").select(LOCATION_SELECT).limit(1000),
    db
      .from("payment_logs")
      .select(
  ...
  const locations = error
    ? ([] as AdminBillingRow[])
    : ((data || []) as unknown as AdminBillingRow[]);
  const logs = logsResult.error
    ? ([] as AdminBillingRow[])
    : ((logsResult.data || []) as unknown as AdminBillingRow[]).map(
        normalizePaymentEvent,
      );

  const activePaid = locations.filter(
    (row) =>
      ["active", "grace_period", "comped"].includes(
        String(row.subscription_status || ""),
      ) && isBusinessProPlan(row.subscription_plan),
  );
  const trialing = locations.filter(
    (row) => row.subscription_status === "trialing",
  );
  const pastDue = locations.filter((row) =>
    ["past_due", "unpaid"].includes(String(row.subscription_status || "")),
  );
  const canceledThisMonth = locations.filter(
    (row) => row.canceled_at && new Date(row.canceled_at) >= monthStart,
  );
  const mrrCents = activePaid.reduce((sum, row) => {
    const value = amount(row);
    return (
      sum +
      (["year", "annual"].includes(String(row.subscription_interval || ""))
        ? Math.round(value / 12)
        : value)
    );
  }, 0);
  ...
  const trialRows = trialing
    .filter(
      (row) =>
        row.trial_ends_at && new Date(row.trial_ends_at) <= thirty,
    )
    .slice(0, 20);
  ...
  const metrics: CoreAdminBillingMetrics = {
    ...
    pastDueEstimatedCents: pastDue.reduce(
      (sum, row) => sum + amount(row),
      0,
    ),
  };
...
export async function readAdminBillingSnapshot(): Promise<AdminBillingSnapshot> {
  try {
    ...
    console.warn(
      "[admin-billing] Core API unavailable; using local Admin DB fallback",
      error,
    );
