  ...
  return <AdminPageShell>
    ...
    <div className="grid gap-5 xl:grid-cols-[340px_1fr]">
      ...
      <div className="space-y-4">
        ...
        <AdminSectionCard className="p-5"><h2 className="text-xl font-black">Internal notes</h2><p className="mt-1 text-xs text-white/40">Keep notes job-related. Do not record protected characteristics, medical information, or accommodation details here.</p><div className="mt-3 grid gap-2">{notes.length ? notes.map((note) => <p className="rounded-xl bg-black/20 p-3 text-sm text-white/70" key={note.id}>{note.note}</p>) : <p className="text-sm text-white/45">No internal notes.</p>}</div></AdminSectionCard>
