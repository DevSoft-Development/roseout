...
export function normalizeNycEvent(input: unknown): NormalizedEvent {
  ...
  const startsAt = firstString(row, ["starts_at", "start_datetime", "datetime"]) ?? combineDateAndTime(startDate, startTime);
  ...
  const endsAt = firstString(row, ["ends_at", "end_datetime"]) ?? combineDateAndTime(endDate, endTime);
  ...
  return normalizeCanonicalEvent({
    ...
    category: firstString(row, ["category", "event_category", "type"]),
    subcategory: firstString(row, ["subcategory", "event_subcategory"]),
    venueName: firstString(row, ["venue_name", "venue", "location_name", "park_name"]),
    address: firstString(row, ["address", "location", "street_address"]),
    city: firstString(row, ["city", "borough"]),
    ...
    allDay: !startTime && !firstString(row, ["starts_at", "start_datetime", "datetime"]),
    isFree: Boolean(free),
    status: "scheduled",
    searchable: true,
