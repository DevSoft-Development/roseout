...
export default function AdminCrmWorkspace({ businesses, empty, pageStart, pageEnd, total, pagination }: { businesses: Row[]; empty: React.ReactNode; pageStart: number; pageEnd: number; total: number; pagination: React.ReactNode }) {
  ...
  return (
    <div className={`grid min-w-0 gap-5 ${panelOpen ? "xl:grid-cols-[minmax(0,1fr)_360px]" : "xl:grid-cols-1"}`}>
      <AdminDataTableShell>
        <table className="w-full min-w-[980px] text-left text-sm">
          <thead className="text-xs uppercase tracking-[0.18em] text-white/45"><tr>{["Location","Market","Status / Stage","Readiness","Analytics","Last Activity","Next Action"].map((h)=><th key={h} className="px-3 py-3 font-black">{h}</th>)}</tr></thead>
          <tbody>
            {businesses.map((b) => {
