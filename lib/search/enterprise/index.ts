...
import { DEFAULT_MARKET, getDefaultMarketTier, isDefaultMarketAppliedGeo } from "./default-market";
import { createPairingDebug, createSearchPairs, getPairCityState, getPairGeoPriority } from "./pairing";
import { formatDistanceFromRestaurant, getPairDistanceMiles, getRawWalkingMinutes, getSafeWalkingMinutes, userAskedForWalking } from "./distance";
...
export async function runEnterpriseSearch(query: string, options?: EnterpriseSearchOptions): Promise<EnterpriseSearchResult> {
  ...
  try {
    ...
    const defaultMarketApplied = isDefaultMarketAppliedGeo(intent.geo);
    const countTier = (items: EnterpriseLocation[], tier: number) =>
      items.filter((item) => getDefaultMarketTier(item) === tier).length;
    const countOutOfDefaultMarket = (items: EnterpriseLocation[]) =>
      items.filter((item) => getDefaultMarketTier(item) > 1).length;
    ...
    const pairDisplayLabels = pairs.map((pair) =>
    ...
    const walkingRequested = userAskedForWalking(intent.pairingPreference);
