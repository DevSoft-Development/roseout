...
export async function getGtmPriorityQueue(limit=25){
  ...
  const {data,error}=await db.from('gtm_location_state').select('location_id,crm_account_id,association_level,gtm_status,opportunity_score,opportunity_tier,demand_score,contactability_score,activation_score,next_best_action,next_best_action_type,last_signal_at,score_explanation,locations!inner(id,name,business_name,restaurant_name,activity_name,city,borough,county,neighborhood,phone,website,is_claimed)').eq('association_level','full').eq('suppressed',false).in('opportunity_tier',['hot','warm']).order('opportunity_score',{ascending:false}).limit(limit);
...
export async function getGtmCommandCenter(){
  const db=getAdminDatabaseClient();
  const {data,error}=await db.from('gtm_location_state').select('association_level,gtm_status,opportunity_tier,opportunity_score,demand_score,activation_score,suppressed,crm_account_id');
  if(error) throw error;
  const rows=data||[];
  return {
