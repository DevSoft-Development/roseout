...
exports.isDateNightDinnerIntent = isDateNightDinnerIntent;
exports.hasExplicitCafeDessertIntent = hasExplicitCafeDessertIntent;
exports.isCafeBakeryDessertQuickBiteOnly = isCafeBakeryDessertQuickBiteOnly;
exports.isSportsWatchIntent = isSportsWatchIntent;
exports.isSportsWatchComboEligible = isSportsWatchComboEligible;
exports.isStrongSameVenueMatch = isStrongSameVenueMatch;
exports.sameVenueSearchTerms = sameVenueSearchTerms;
exports.scoreSameVenueAttributeMatch = scoreSameVenueAttributeMatch;
exports.scoreSingleVenueWithMatch = scoreSingleVenueWithMatch;
exports.scoreRestaurantOutingFit = scoreRestaurantOutingFit;
exports.scoreRestaurantQuality = scoreRestaurantQuality;
exports.scoreActivityQuality = scoreActivityQuality;
exports.explainRejection = explainRejection;
exports.filterRestaurantResults = filterRestaurantResults;
exports.filterActivityResults = filterActivityResults;
exports.rankRestaurantResults = rankRestaurantResults;
exports.rankActivityResults = rankActivityResults;
const geo_taxonomy_1 = require("./geo-taxonomy");
const distance_1 = require("./distance");
const taxonomy_1 = require("./taxonomy");
const lowLevel_1 = require("../lowLevel");
const locationRanking_1 = require("../../ml/locationRanking");
const loadAdvancedMlFeatures_1 = require("../../ml/advanced/loadAdvancedMlFeatures");
const normalize_intent_1 = require("./normalize-intent");
function compactRecordText(r) {
    return (0, taxonomy_1.textForRecord)(r).replaceAll("_", " ").replaceAll("-", " ");
}
function fieldText(r, fields) {
    return fields
        .map((field) => {
        const value = r[field];
...
function isDateNightDinnerIntent(intent) {
    const text = [
        intent.rawQuery,
        intent.occasion,
        intent.timeContext,
        ...
        .join(" ")
        .toLowerCase();
    return (/\b(date night|dinner date|romantic date|romantic dinner|anniversary|couples night|night out)\b/.test(text) ||
        (/\bdate\b/.test(text) && /\b(night|dinner|romantic)\b/.test(text)));
}
function hasExplicitCafeDessertIntent(intent) {
    const text = [
        intent.rawQuery,
        ...
        .join(" ")
        .toLowerCase();
    return /\b(coffee|cafe|café|bakery|bake shop|pastry|dessert|ice cream|yogurt|frozen yogurt|bagel|donut|doughnut|cake|cupcake|cookies|sweets|bubble tea|tea|smoothie|juice|milkshake)\b/.test(text);
}
function isCafeBakeryDessertQuickBiteOnly(r) {
    ...
    const cafeDessertSignal = /\b(cafe|café|coffee|coffee shop|bakery|bake shop|pastry|patisserie|dessert|ice cream|yogurt|frozen yogurt|bagel|bagels|donut|doughnut|cake|cupcake|cookies|sweets|bubble tea|tea shop|smoothie|juice bar|milkshake)\b/.test(text);
    const fullDinnerSignal = /\b(full service|table service|dining room|dinner service|restaurant|steakhouse|seafood|italian restaurant|sushi|mediterranean|american restaurant|new american|thai restaurant|mexican restaurant|caribbean restaurant|latin restaurant|french restaurant|greek restaurant|tapas|wine bar|cocktail bar|lounge|bistro|brasserie|supper club|reservation|reservations|resy|open table)\b/.test(text);
    return cafeDessertSignal && !fullDinnerSignal;
}
function isSportsWatchIntent(intent) {
    const text = String(intent.rawQuery ?? "")
        .toLowerCase()
        .replaceAll("_", " ")
        .replaceAll("-", " ");
    return (0, normalize_intent_1.hasSportsWatchIntent)(text);
}
function userAskedToWatchSportsGame(intent) {
    return isSportsWatchIntent(intent);
}
function sportsWatchRecordSignal(r) {
    const text = fieldText(r, [
        "name",
        "activity_name",
        "primary_category",
...
function userAskedForRelaxedNoClub(intent) {
...
function relaxedActivityRecordSignal(r) {
    const text = fieldText(r, [
        "name",
        "activity_name",
        "primary_category",
...
function isNightlifeOnlyForSportsWatch(r) {
    const text = fieldText(r, [
        "name",
        "activity_name",
        "primary_category",
...
function isBarPubForSportsWatch(r) {
    const text = fieldText(r, [
        "name",
        "activity_name",
        "primary_category",
...
function userExplicitlyAskedForTheater(intent) {
    const text = [
        intent.rawQuery,
        ...
        .join(" ")
        .toLowerCase();
    return /\b(theater|theatre|broadway|off-broadway|show|play|musical|cinema|movie|movies)\b/.test(text);
}
function userAskedForHookah(intent) {
    return /\b(hookah|shisha|hookah lounge|hookah bar)\b/i.test(intent.rawQuery);
}
function isHookahRecord(record) {
    return /\b(hookah|shisha)\b/i.test((0, taxonomy_1.textForRecord)(record));
}
function userAskedForHardNightlife(rawQuery) {
    return /\b(nightlife|bar|club|dance club|dancing|live dj|dj|speakeasy|cocktails|drinks|rooftop lounge)\b/i.test(rawQuery);
}
function isHardNightlifeRecord(record) {
    const text = (0, taxonomy_1.textForRecord)(record).toLowerCase();
...
function isTheaterRecord(r) {
    ...
    return /\b(theater|theatre|broadway|off-broadway|performing arts|cinema|movie theater)\b/.test(text);
}
function explicitLocationType(r) {
    return String(r.location_type ?? r.source_table ?? r.type ?? "")
        .toLowerCase()
        .replaceAll("_", " ")
        .replaceAll("-", " ");
}
function isPlaceOfWorshipRecord(r) {
    ...
    return taxonomy_1.PLACE_OF_WORSHIP_TERMS.some((term) => combined.includes(term.toLowerCase()));
}
function shouldHidePlaceOfWorship(r, intent) {
    return (isPlaceOfWorshipRecord(r) && !(0, taxonomy_1.userAskedForPlaceOfWorship)(intent.rawQuery));
}
function isClearlyActivityOnly(r) {
    const typeText = explicitLocationType(r);
    const categoryText = fieldText(r, [
        "primary_category",
        "activity_type",
    ...
    if (hasRestaurantName || hasCuisine)
        return false;
    return (/\bactivity\b|\bactivities\b|\bexperience\b|\bentertainment\b/.test(typeText) ||
...
function matchesAnyAlternativeGroup(record, groups) {
    const validGroups = (groups ?? []).filter((group) => group.length >= 2);
    if (!validGroups.length)
        return true;
    return validGroups.some((group) => (0, taxonomy_1.termMatchesRecord)(record, group));
}
function isRestaurantLike(r) {
    const typeText = explicitLocationType(r);
    const categoryText = fieldText(r, [
        "primary_category",
        "cuisine",
    ...
    const strongRestaurantSignal = Boolean(r.restaurant_name) ||
        Boolean(r.cuisine) ||
    ...
    const foodSignal = /\bsteak\b|\bseafood\b|\bsushi\b|\bitalian\b|\bmexican\b|\bcaribbean\b|\bthai\b|\bindian restaurant\b|\bchinese restaurant\b|\bjapanese restaurant\b|\bkorean restaurant\b|\bbrunch\b|\bdinner menu\b|\bfood\b|\bcuisine\b/.test(fullText);
    const loungeWithFood = /\blounge\b|\bcocktail bar\b|\bwine bar\b/.test(categoryText) &&
        /\bfood\b|\bdining\b|\brestaurant\b|\bmenu\b|\bdinner\b|\bbrunch\b|\bcuisine\b/.test(fullText);
    const sportsWatchFoodVenue = /\bsports bar\b|\bbar and grill\b|\bgastropub\b|\bpub\b|\btavern\b/.test(categoryText) &&
        /\bwings?\b|\bchicken wings\b|\bfood\b|\bmenu\b|\bgrill\b|\bburgers?\b/.test(fullText);
    if (isClearlyActivityOnly(r))
        return false;
    return strongRestaurantSignal || foodSignal || loungeWithFood || sportsWatchFoodVenue;
}
function isSportsWatchComboEligible(record, intent) {
    ...
    const locationType = String(record.location_type ?? record.source_table ?? "").toLowerCase();
    const reasons = [];
    ...
    const restaurantSignal = locationType.includes("restaurant") ||
    ...
    const sportsWatchSignal = sportsWatchRecordSignal(record) > 0 ||
    ...
    if (cigarOnly || hookahOnly || cocktailLoungeOnly || nightlifeOnly) {
        if (cigarOnly)
            reasons.push("cigar_lounge_only");
        if (hookahOnly)
            reasons.push("hookah_lounge_only");
        if (cocktailLoungeOnly)
            reasons.push("cocktail_lounge_only");
        if (nightlifeOnly)
            reasons.push("generic_nightlife_only");
        return { status: "reject", eligible: false, reasons, restaurantSignal, foodSignal, sportsWatchSignal, barSignal, sportsBarSignal, barAndGrillSignal, pubTavernSignal, locationType, combined, disqualifyingLoungeOnly: true };
    }
    if (sportsBarSignal && foodSignal)
        reasons.push("sports_bar_with_food");
    if (barAndGrillSignal && foodSignal)
        reasons.push("bar_and_grill_with_food");
    if (restaurantSignal && barSignal && sportsWatchSignal)
        reasons.push("restaurant_bar_with_game_watch");
    if (pubTavernSignal && sportsWatchSignal)
        reasons.push("pub_tavern_with_sports_watch");
    if (reasons.length > 0) {
        return { status: "pass", eligible: true, reasons, restaurantSignal, foodSignal, sportsWatchSignal, barSignal, sportsBarSignal, barAndGrillSignal, pubTavernSignal, locationType, combined, disqualifyingLoungeOnly: false };
    }
    if (restaurantSignal && barSignal && foodSignal)
        reasons.push("restaurant_bar_with_food");
    if (sportsBarSignal)
        reasons.push("sports_bar_soft_without_explicit_food");
    if (barAndGrillSignal)
        reasons.push("bar_and_grill_soft_without_explicit_tv");
    if (pubTavernSignal && (foodSignal || sportsWatchSignal))
        reasons.push("pub_tavern_soft_combo");
    if (reasons.length > 0) {
        return { status: "demote", eligible: true, reasons, restaurantSignal, foodSignal, sportsWatchSignal, barSignal, sportsBarSignal, barAndGrillSignal, pubTavernSignal, locationType, combined, disqualifyingLoungeOnly: false };
    }
    return { status: "reject", eligible: false, reasons: ["missing_combo_sports_food_bar_signals"], restaurantSignal, foodSignal, sportsWatchSignal, barSignal, sportsBarSignal, barAndGrillSignal, pubTavernSignal, locationType, combined, disqualifyingLoungeOnly: false };
}
function sportsWatchFoodEligibility(record) {
    return isSportsWatchComboEligible(record);
}
function sportsWatchFoodScore(record, intent) {
    if (!(0, normalize_intent_1.isSportsWatchFoodSameVenueIntent)(intent.rawQuery))
        return 0;
    const eligibility = sportsWatchFoodEligibility(record);
    let score = 0;
    if (/\bsports bar\b/.test(eligibility.combined))
        score += 140;
    if (/\bbar and grill\b|\bgastropub\b/.test(eligibility.combined))
        score += 100;
    if (/\bpub\b|\btavern\b/.test(eligibility.combined))
        score += 70;
    if (/\bwings?\b|\bchicken wings\b/.test(eligibility.combined))
        score += 110;
    if (/\btvs?\b|\bscreens?\b|\bbig screens?\b/.test(eligibility.combined))
        score += 90;
    if (/\bknicks\b|\bbasketball\b|\bgame watch\b|\bwatch party\b|\blive sports\b|\bgame day\b/.test(eligibility.combined))
        score += 80;
    if (/\bfood\b|\bdinner\b|\bbar food\b|\bmenu\b/.test(eligibility.combined))
        score += 45;
    if (eligibility.locationType.includes("activity") && !eligibility.eligible)
        score -= 500;
    if (eligibility.disqualifyingLoungeOnly)
        score -= 350;
    return score;
}
function isActivityLike(r) {
    const typeText = explicitLocationType(r);
    const categoryText = fieldText(r, [
        "primary_category",
        "activity_type",
    ...
    const fullText = compactRecordText(r);
    const hasRestaurantOnlySignals = Boolean(r.restaurant_name || r.cuisine || r.cuisine_type) &&
        !r.activity_name &&
        !r.activity_type;
    if (hasRestaurantOnlySignals)
        return false;
    return Boolean(r.activity_name ||
        r.activity_type ||
...
function hasSingleVenueWithSportsSignal(terms) {
...
const SAME_VENUE_SECONDARY_SYNONYMS = {
    hookah: [
        "hookah",
        "shisha",
        "hookah lounge",
...
const SECONDARY_ATTRIBUTE_TAXONOMY = {
    hookah: {
        explicit: ["hookah"],
...
function secondaryBuckets(secondaryCandidates) {
...
function strengthFromMatches(matched, buckets) {
    const has = (terms) => matched.some((term) => terms.includes(term));
    if (has(buckets.explicitSecondaryTerms))
        return "explicit";
    if (has(buckets.strongSecondarySynonyms))
        return "strong_synonym";
    if (has(buckets.supportingSecondaryTerms))
        return "supporting";
    if (has(buckets.genericSecondaryTerms))
        return "generic";
    return "none";
}
function isStrongSameVenueMatch(record, intent) {
...
function matchTermsInFields(record, terms, fields) {
    const matchedTerms = [];
    const matchedFields = [];
...
function sameVenueSearchTerms(intent) {
    const singleVenue = (0, taxonomy_1.detectSingleVenueWithIntent)(intent.rawQuery);
    const sameVenuePreferred = Boolean(intent.sameVenuePreferred) || singleVenue.matched;
    ...
    const primaryCandidates = uniqLowerTerms([
        ...
        ...SAME_VENUE_PRIMARY_FOOD_TERMS.filter((term) => intent.rawQuery.toLowerCase().includes(term)),
    ]);
    const connectorSplit = String(intent.rawQuery ?? "")
        .toLowerCase()
        .split(/\b(?:with|and|plus|has|have|serving|serves|offering|offers|featuring|features|including|includes)\b/);
    const afterWith = connectorSplit.slice(1).join(" ");
    const secondaryCandidates = uniqLowerTerms([
        ...(intent.secondaryAttributeTerms ?? []),
        ...SAME_VENUE_SECONDARY_ATTRIBUTE_TERMS.filter((term) => afterWith.includes(term)),
    ]);
    ...
    const expandedSecondaryAttributeTerms = uniqLowerTerms([
        ...
        ...secondaryCandidates.flatMap((term) => SAME_VENUE_SECONDARY_SYNONYMS[term] ?? [term]),
    ]);
    return {
...
function scoreSameVenueAttributeMatch(record, intent) {
    const terms = sameVenueSearchTerms(intent);
    if (!terms.primaryFoodTerms.length ||
        ...
        return {
            ...
            secondaryMatchStrength: "none",
        };
    }
    const primary = matchTermsInFields(record, terms.primaryFoodTerms, SAME_VENUE_PRIMARY_FIELDS);
    const secondary = matchTermsInFields(record, terms.expandedSecondaryAttributeTerms, SAME_VENUE_SECONDARY_FIELDS);
    const namePrimary = matchTermsInFields(record, terms.primaryFoodTerms, [
        "name",
        "restaurant_name",
        "activity_name",
    ]).matchedTerms.length > 0;
    const nameSecondary = matchTermsInFields(record, terms.expandedSecondaryAttributeTerms, [
        "name",
        "restaurant_name",
        "activity_name",
    ]).matchedTerms.length > 0;
    const docPrimary = matchTermsInFields(record, terms.primaryFoodTerms, ["search_document"])
        .matchedTerms.length > 0;
    const docSecondary = matchTermsInFields(record, terms.expandedSecondaryAttributeTerms, [
        "search_document",
    ]).matchedTerms.length > 0;
    ...
    const secondaryStrongMatched = secondaryMatchStrength === "explicit" ||
        secondaryMatchStrength === "strong_synonym";
    const secondarySupportingMatched = secondaryMatchStrength === "supporting" || secondaryMatchStrength === "generic";
    const primaryGenericOnly = primaryMatched && primary.matchedTerms.every((term) => ["food", "restaurant", "dinner", "lunch", "brunch", "bar", "lounge", "drinks", "cocktails"].includes(term));
    let score = 0;
    ...
    if (primaryMatched && secondaryMatched) {
        ...
        if (secondaryStrongMatched) {
            if (namePrimary && nameSecondary)
                score += 120;
            if (docPrimary && docSecondary)
                score += 120;
            if (primary.matchedFields.some((f) => [
                "cuisine",
                "cuisine_type",
                "primary_category",
                "primary_tag",
                ...
                score += 80;
            if (secondary.matchedFields.some((f) => ["description", "semantic_search_text"].includes(f)))
                score += 40;
        }
    }
    ...
    record.sameVenuePrimaryMatched = primaryMatched;
    record.sameVenueSecondaryMatched = secondaryMatched;
    record.sameVenuePrimaryTermsMatched = primary.matchedTerms;
    record.sameVenueAttributeTermsMatched = secondary.matchedTerms;
    record.sameVenueSecondaryStrongMatched = secondaryStrongMatched;
    record.sameVenueSecondarySupportingMatched = secondarySupportingMatched;
    record.sameVenueAttributeMatchStrength = secondaryMatchStrength;
    record.phase2IntentMatchStrength = secondaryMatchStrength;
    record.sameVenuePrimaryFieldsMatched = primary.matchedFields;
    record.sameVenueSecondaryFieldsMatched = secondary.matchedFields;
    record.sameVenueScore = score;
    record.sameVenueBoostApplied = score > 0;
    record.sameVenueRankingReason = reason;
    record.matchedFields = uniqLowerTerms([
...
function scoreSingleVenueWithMatch(record, intent) {
    const singleVenue = (0, taxonomy_1.detectSingleVenueWithIntent)(intent.rawQuery);
    ...
    const venueMatched = singleVenue.venueTerms.length === 0 ||
        (0, taxonomy_1.termMatchesRecord)(record, singleVenue.venueTerms);
    const foodMatched = singleVenue.foodTerms.length === 0 ||
        (0, taxonomy_1.termMatchesRecord)(record, singleVenue.foodTerms);
    const featureMatched = singleVenue.featureTerms.length === 0 ||
        (0, taxonomy_1.termMatchesRecord)(record, singleVenue.featureTerms);
    const sportsMatched = hasSingleVenueWithSportsSignal([
        ...singleVenue.venueTerms,
        ...singleVenue.featureTerms,
    ]) && sportsWatchRecordSignal(record) > 0;
    let score = 0;
    if (venueMatched && singleVenue.venueTerms.length)
        score += 70;
    if (foodMatched && singleVenue.foodTerms.length)
        score += 70;
    if (featureMatched && singleVenue.featureTerms.length)
        score += 40;
    if (sportsMatched)
        score += 25;
    if (singleVenue.foodTerms.length && venueMatched && !foodMatched)
        score -= 60;
    if (singleVenue.venueTerms.length && foodMatched && !venueMatched)
        score -= 60;
    if (isClearlyActivityOnly(record) && !isRestaurantLike(record))
        score -= 80;
    if (singleVenue.featureTerms.length &&
        isRestaurantLike(record) &&
        !venueMatched &&
        !featureMatched)
        score -= 80;
    record.singleVenueWithScore = score;
    record.singleVenueWithVenueMatched = venueMatched;
    record.singleVenueWithFoodMatched = foodMatched;
    record.singleVenueWithFeatureMatched = featureMatched;
    record.singleVenueWithDualMatched = Boolean(venueMatched && (foodMatched || featureMatched));
    return {
        score,
        venueMatched,
        foodMatched,
...
function userHasSelfCareIntent(intent) {
    const text = intentText(intent);
    return SELF_CARE_INTENT_TERMS.some((term) => text.includes(term));
}
function isNonWellnessPriorityIntent(intent) {
    const text = intentText(intent);
    return NON_WELLNESS_PRIORITY_TERMS.some((term) => text.includes(term));
}
function wellnessIntentAdjustment(r, intent, domain) {
    if (domain !== "activity" || !(0, lowLevel_1.isWellnessActivity)(r))
        return 0;
    if (userHasSelfCareIntent(intent))
        return 180;
    if (isNonWellnessPriorityIntent(intent))
        return -220;
    return 0;
}
function hasLocationPhotos(r) {
    const images = r.images ?? r.gallery_images;
    const hasImageArray = Array.isArray(images)
        ? images.filter(Boolean).length > 0
        : typeof images === "string" &&
            images.trim().length > 2 &&
            !["[]", "null"].includes(images.trim().toLowerCase());
    return Boolean(r.has_photos === true ||
        r.image_url ||
        r.main_image ||
        r.photo_url ||
        r.primary_photo_url ||
...
function normalizedPublicVisibilityTier(r) {
    return String(r.public_visibility_tier ?? "")
        .toLowerCase()
        .replaceAll("_", " ");
}
function normalizedCurationTier(r) {
    return String(r.curation_tier ?? "")
        .toLowerCase()
        .replaceAll("_", " ");
}
function normalizedVisibilityTier(r) {
    return `${normalizedPublicVisibilityTier(r)} ${normalizedCurationTier(r)}`.trim();
}
function normalizedQualityStatus(r) {
    return String(r.quality_status ??
...
function userAskedForCasualQuickOrChicken(intent) {
...
function userAskedForChicken(intent) {
    return /\b(chicken|wings|fried chicken|hot chicken)\b/i.test(intent.rawQuery);
}
function userAskedForQuickService(intent) {
...
function baseQualitySignals(r) {
    ...
    const reviewCount = Number(r.review_count ?? r.reviewCount ?? r.total_reviews ?? 0);
    ...
    const curated = Boolean(r.is_featured ||
        r.featured ||
        r.is_curated ||
        r.approved) || /\b(featured|premium|curated|editor|approved)\b/.test(visibility);
...
function hasReservationSignal(r, text) {
    return (Boolean(r.reservation_url ||
        r.reservation_link ||
        r.booking_url ||
        r.external_reservation_url ||
        r.reservation_enabled ||
        r.reservations_url) || RESERVATION_DINING_RE.test(text));
}
function scoreRestaurantOutingFit(r, intent) {
    const reasons = [];
    const penalties = [];
    ...
    const suppressCafeDessertForDateNight = dateNightDinnerIntent &&
    ...
    const ambianceSignal = outingDescriptionMatch ||
        /\b(romantic|elegant|ambiance|ambience|date night|upscale|lounge|supper club|brasserie|fine dining)\b/.test(`${categoryTypeText} ${descriptionTagText}`);
    if (/\b(featured|premium|curated)\b/.test(signals.curationTier) ||
        Boolean(r.is_featured || r.featured || r.is_curated)) {
        score += 25;
    ...
    if (outingCategoryMatch) {
        if (suppressCafeDessertForDateNight) {
            qualityReason(penalties, "removed dining boost for cafe/bakery/dessert-only date-night result", 0);
        }
        else {
            score += 20;
    ...
    if (outingDescriptionMatch) {
        if (suppressCafeDessertForDateNight) {
            qualityReason(penalties, "removed date-night boost for cafe/bakery/dessert-only result", 0);
        }
        else {
            score += 15;
    ...
    if (suppressCafeDessertForDateNight) {
        score -= 80;
        qualityReason(penalties, "cafe/bakery/dessert-only suppressed for date-night dinner intent", -80);
    }
    const priority = Number(r.default_market_priority ?? r.market_priority);
    if (priority === 0) {
        score += 8;
    ...
    if (!askedCasual &&
        ...
        !strongOutingSignal) {
        score -= 15;
        qualityReason(penalties, "name contains eats without upscale/full-service signal", -15);
    }
    ...
    if (!askedCasual &&
        ...
        score -= 30;
        qualityReason(penalties, "fast casual/quick service/counter service category", -30);
    }
    if (!askedQuickService && TAKEOUT_FIRST_RE.test(categoryTypeText)) {
        score -= 30;
        qualityReason(penalties, "delivery/takeout/catering/ghost kitchen category", -30);
    }
    if (!askedCasual &&
        (r.is_chain === true ||
            /\b(chain|utility|low priority)\b/.test(`${signals.visibility} ${signals.text}`))) {
        score -= 15;
    ...
    if (isGenericRestaurantQualityIntent(intent) &&
        !reservationSignal &&
        !strongOutingSignal) {
        score -= 10;
        qualityReason(penalties, "missing reservation/dining/full-service signal", -10);
    }
    if (!signals.hasPhotos) {
        score -= 15;
    ...
    if (askedCasual &&
        /\b(casual|quick|fast casual|quick service|chicken|wings|fried chicken|hot chicken|pizza|slice|deli)\b/.test(signals.text)) {
        score += 20;
        qualityReason(reasons, "requested casual/quick/specific food fit", 20);
    }
    r.restaurantOutingFitScore = score;
    r.restaurantOutingFitReasons = reasons;
    r.restaurantOutingFitPenalties = penalties;
    return { score, reasons, penalties };
}
function scoreRestaurantQuality(r, intent) {
    ...
    r.restaurantQualityScore = score;
    r.restaurantQualityReasons = reasons;
    r.restaurantQualityPenalties = penalties;
    return { score, outingFitScore: outingFit.score, reasons, penalties };
}
function scoreActivityQuality(r, intent) {
    const reasons = [];
    const penalties = [];
    const signals = baseQualitySignals(r);
    let score = 0;
    ...
    if (sportsWatchIntent && isNightlifeOnlyForSportsWatch(r)) {
        score -= 55;
        qualityReason(penalties, "nightlife/rooftop-only result for sports-watch query", -55);
    }
    if (!sportsWatchIntent &&
        !relaxedNoClubIntent &&
        /\b(rooftop|roof top|rooftop bar|rooftop lounge|terrace|skyline|views?|roof deck)\b/.test(signals.text)) {
        score += 30;
    ...
    if (/\b(live dj|dj)\b/.test(signals.text) &&
    ...
    const restaurantOnly = Boolean(r.restaurant_name || r.cuisine || r.cuisine_type) &&
        !/\b(rooftop|roof top|bar|lounge|cocktail|speakeasy|nightlife|terrace)\b/.test(signals.text);
    if (rooftopDrinksRequested && restaurantOnly) {
        score -= 35;
        qualityReason(penalties, "restaurant-only activity without rooftop/bar relevance", -35);
    }
    if (/\b(rooftop bars? nyc|best rooftop bars|top rooftop bars|rooftop bars list|guide to rooftop)\b/.test(signals.text)) {
        score -= 25;
    ...
    r.activityQualityScore = score;
    r.activityQualityReasons = reasons;
    r.activityQualityPenalties = penalties;
    return { score, reasons, penalties };
}
...
function isCuratedIntent(intent) {
    const t = intentText(intent);
    return (CURATED_TERMS.some((term) => t.includes(term)) &&
        !UTILITY_TERMS.some((term) => t.includes(term)));
}
function chainPenalty(r, intent) {
    if (!isCuratedIntent(intent))
        return 0;
    const isChain = r.is_chain === true ||
        String(r.brand_type || "").toLowerCase() === "chain" ||
        String(r.curation_tier || "").toLowerCase() === "utility";
    return isChain ? 500 : 0;
}
function rooftopMatch(r) {
    return /rooftop|roof top|terrace|patio|outdoor|skyline|view|roof deck/.test((0, taxonomy_1.textForRecord)(r));
}
function explainRejection(record, intent, domain) {
    if ((0, geo_taxonomy_1.shouldExcludeByGeo)(record, intent.geo))
        return "wrong_geo";
    ...
    if (domain === "activity" &&
        (0, normalize_intent_1.hasRelaxedActivityIntent)(intent.rawQuery) &&
        !userAskedForHardNightlife(intent.rawQuery) &&
        isHardNightlifeRecord(record)) {
        return "hard_nightlife_not_relaxed";
    }
    if (domain === "restaurant" && (0, normalize_intent_1.isSportsWatchFoodSameVenueIntent)(intent.rawQuery)) {
        const comboEligibility = isSportsWatchComboEligible(record, intent);
        record.sportsWatchComboEligibility = comboEligibility.status;
        record.sportsWatchComboEligibilityReasons = comboEligibility.reasons;
    ...
    const hasMealOnlyFallback = domain === "restaurant" &&
        Boolean(intent.timeContext || intent.restaurantIntent.mealTerms.length) &&
        specificRestaurantTerms.some((term) => ["chicken", "wings", "fried chicken", "hot chicken"].includes(term.toLowerCase()));
    if (domain === "restaurant" &&
        !(0, normalize_intent_1.isSportsWatchFoodSameVenueIntent)(intent.rawQuery) &&
        specificRestaurantTerms.length > 0 &&
        !(0, taxonomy_1.termMatchesRecord)(record, specificRestaurantTerms) &&
        !hasMealOnlyFallback) {
...
function filterRestaurantResults(results, intent) {
    return results.filter((r) => !explainRejection(r, intent, "restaurant"));
}
function filterActivityResults(results, intent) {
    return results.filter((r) => !explainRejection(r, intent, "activity"));
}
function isFoodForwardRestaurantSearch(intent) {
    const text = [
        intent.rawQuery,
        ...
        .replaceAll("_", " ")
        .replaceAll("-", " ");
    return /\b(lunch|dinner|brunch|breakfast|food|restaurant|dining|eat|chicken|wings|fried chicken|hot chicken|seafood|sushi|pizza|tacos|burger|burgers|steak|pasta|ramen|bbq|barbecue|lobster|crab|shrimp|oyster|oysters|raw bar)\b/.test(text);
}
function activityNightlifeRestaurantPenalty(record, intent) {
    if (!isFoodForwardRestaurantSearch(intent))
        return 0;
    ...
    const isActivityRecord = /\b(activity|activities)\b/.test(String(record.location_type ?? "").toLowerCase()) ||
        /\b(activity|activities)\b/.test(String(record.source_table ?? "").toLowerCase()) ||
        /\b(activity|activities)\b/.test(String(record.type ?? "").toLowerCase());
    const isNightlifeTyped = /\b(nightlife|hookah|shisha|lounge|club|nightclub|night club|cigar|karaoke|speakeasy)\b/.test(text);
    if (!isActivityRecord && !isNightlifeTyped)
        return 0;
    const hasRealRestaurantIdentity = Boolean(record.restaurant_name ||
        record.food_type ||
        record.menu_url ||
        String(record.primary_category ?? "")
            .toLowerCase()
            .includes("restaurant"));
    const hasSpecificFoodMatch = /\b(chicken|wings|fried chicken|hot chicken|seafood|sushi|pizza|tacos|burger|burgers|steak|pasta|ramen|bbq|barbecue|lobster|crab|shrimp|oyster|oysters|raw bar)\b/.test(text);
    if (hasRealRestaurantIdentity && hasSpecificFoodMatch)
        return -35;
    if (hasRealRestaurantIdentity)
        return -125;
    if (isActivityRecord && isNightlifeTyped)
        return -500;
    if (isActivityRecord)
...
function relevance(r, intent, domain) {
    const geo = (0, geo_taxonomy_1.scoreGeoMatch)(r, intent.geo);
    const dist = (0, distance_1.getLocationDistanceMiles)(r, intent.geo);
    if (dist != null)
        r.distance_miles = Number(dist.toFixed(2));
    r.distance_score = (0, distance_1.scoreDistance)(r, intent.geo);
    const terms = domain === "restaurant"
        ? [
            ...intent.restaurantIntent.foodTerms,
            ...intent.restaurantIntent.cuisineTerms,
            ...intent.restaurantIntent.mealTerms,
    ...
    const termScore = terms.reduce((s, t) => {
        const matched = (0, taxonomy_1.termMatchesRecord)(r, [t]);
        if (!matched)
            return s;
    ...
    const alternativeScore = domain === "restaurant"
        ? (intent.restaurantIntent.alternativeGroups ?? []).some((group) => (0, taxonomy_1.termMatchesRecord)(r, group))
            ? 45
            : 0
        : (intent.activityIntent.alternativeGroups ?? []).some((group) => (0, taxonomy_1.termMatchesRecord)(r, group))
            ? 45
            : 0;
    const domainScore = domain === "restaurant"
        ? isRestaurantLike(r)
            ? 80
            : -80
        : isActivityLike(r)
            ? 80
            : -80;
    const generic = (0, taxonomy_1.isGenericMealIntent)(intent.restaurantIntent) && domain === "restaurant"
        ? 30
        : 0;
    const domainQuality = domain === "restaurant"
        ? scoreRestaurantQuality(r, intent).score
        : scoreActivityQuality(r, intent).score;
    const legacySingleVenueScore = domain === "restaurant" ? scoreSingleVenueWithMatch(r, intent).score : 0;
    const sameVenueAttributeScore = domain === "restaurant" ? scoreSameVenueAttributeMatch(r, intent).score : 0;
    const singleVenueWithScore = legacySingleVenueScore + sameVenueAttributeScore;
    const quality = Number(r.theouthaven_score ?? r.quality_score ?? 0) +
        Number(r.rating ?? 0) * 2 +
        Math.min(Number(r.review_count ?? 0) / 100, 10) +
        domainQuality +
        singleVenueWithScore;
    const restaurantFoodActivityPenalty = domain === "restaurant" ? activityNightlifeRestaurantPenalty(r, intent) : 0;
    ...
    r.restaurant_food_activity_penalty = restaurantFoodActivityPenalty;
    r.match_score =
        (r.term_score ?? 0) + domainScore + geo + restaurantFoodActivityPenalty;
    return ((r.match_score ?? 0) +
        (r.term_score ?? 0) +
        (r.geo_score ?? 0) +
        (r.domain_score ?? 0) +
        (r.distance_score ?? 0) +
        ...
        ((r.ml_boost = (0, locationRanking_1.calculateMlBoost)(r.ml_score)), r.ml_boost ?? 0) +
        ((r.advanced_ml = (0, loadAdvancedMlFeatures_1.calculateAdvancedMlRankingAdjustments)({ ...r.advanced_ml_features, ...r }, intent)),
            r.advanced_ml.advancedMlBoost ?? 0));
}
function rankRestaurantResults(results, intent) {
    return filterRestaurantResults(results, intent).sort((a, b) => relevance(b, intent, "restaurant") - relevance(a, intent, "restaurant"));
}
function rankActivityResults(results, intent) {
    results.forEach((result) => scoreActivityQuality(result, intent));
    return filterActivityResults(results, intent).sort((a, b) => relevance(b, intent, "activity") - relevance(a, intent, "activity"));
}
