...
function countsOf(result: any) {
  ...
  return {
    ...
    activities,
    pairs,
    displayed: cards || restaurants + activities + pairs,
  };
}
...
function buildSummary(index: number, query: string, result: any, elapsedMs: number, caughtError?: unknown) {
  ...
  const performance = result?.searchPerformance ?? debug?.performance ?? {};
  ...
  const acceptance = evaluateSearchAcceptanceContracts({
    result: { ...result, query },
    errors,
    warnings,
    counts,
  ...
  const totalMs =
    numberOrNull(performance?.total_ms ?? performance?.totalMs ?? result?.timing_ms) ??
    elapsedMs;
  ...
  return {
    ...
    normalized_search_type: stringOrNull(
      intent?.searchType ?? result?.search_type ?? result?.searchType,
    ),
    primary_domain: stringOrNull(
      intent?.primaryDomain ?? result?.primary_domain ?? result?.primaryDomain,
    ),
    ...
    intentParserSource: stringOrNull(
      debug?.intentParserSource ?? intent?.intentParserSource,
    ),
    rawActivityCandidateCount: numberOrNull(debug?.rawActivityCandidateCount),
    pairCandidatesEvaluated: numberOrNull(
      debug?.searchTelemetry?.pairCandidatesEvaluated ??
        result?.searchTelemetry?.pairCandidatesEvaluated,
    ),
    warnings,
    errors,
    suspiciousFlags: [...new Set(suspiciousFlags)],
    needsRestaurant: Boolean(intent?.needsRestaurant),
    needsActivity: Boolean(intent?.needsActivity),
  };
}
...
function createQaPublicController(adminUser: { user_id: string; email?: string | null }) {
  return createPublicSearchController({
    getIdentity: async () => ({
      user: { id: adminUser.user_id, email: adminUser.email ?? null },
      guestId: null,
...
async function runPublicQaSearch(
  ...
  if (!response.ok && !payload.success) {
    throw new Error(payload?.error?.message ?? payload?.error ?? `Public search failed (${response.status}).`);
  }
  return payload;
}
