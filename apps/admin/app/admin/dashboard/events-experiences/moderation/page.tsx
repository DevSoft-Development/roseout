...
export default async function EventsExperiencesModerationPage({ searchParams }: { searchParams: Params }) {
  ...
  const { data: cases, error: casesError } = await getAdminDatabaseClient()
    .from("fraud_cases")
  ...
  const [eventsResult, experiencesResult, eventSignalsResult, experienceSignalsResult, eventActionsResult, experienceActionsResult] = await Promise.all([
    eventIds.length
      ? getAdminDatabaseClient().from("events").select("id,title,description,category,image_url,venue_name,address,city,state,location_id,organization_id,status,searchable,starts_at,ends_at,is_free,price_min,price_max,ticketing_enabled,capacity").in("id", eventIds)
      : Promise.resolve({ data: [], error: null }),
    experienceIds.length
      ? getAdminDatabaseClient().from("experiences").select("id,title,description,category,image_url,venue_name,address,city,state,location_id,organization_id,status,searchable,duration_minutes,min_party_size,max_party_size,price_per_person").in("id", experienceIds)
      : Promise.resolve({ data: [], error: null }),
    eventIds.length
      ? getAdminDatabaseClient().from("fraud_signals").select("id,subject_id,rule_key,signal_type,severity,score_delta,observed_at").eq("subject_type", "event").in("subject_id", eventIds).order("observed_at", { ascending: false }).limit(300)
      : Promise.resolve({ data: [], error: null }),
    experienceIds.length
      ? getAdminDatabaseClient().from("fraud_signals").select("id,subject_id,rule_key,signal_type,severity,score_delta,observed_at").eq("subject_type", "experience").in("subject_id", experienceIds).order("observed_at", { ascending: false }).limit(300)
      : Promise.resolve({ data: [], error: null }),
    eventIds.length
      ? getAdminDatabaseClient().from("fraud_actions").select("id,subject_id,action_type,reason,created_at").eq("subject_type", "event").in("subject_id", eventIds).order("created_at", { ascending: false }).limit(300)
      : Promise.resolve({ data: [], error: null }),
    experienceIds.length
      ? getAdminDatabaseClient().from("fraud_actions").select("id,subject_id,action_type,reason,created_at").eq("subject_type", "experience").in("subject_id", experienceIds).order("created_at", { ascending: false }).limit(300)
      : Promise.resolve({ data: [], error: null }),
  ]);
  ...
  const [locationsResult, organizationsResult] = await Promise.all([
    locationIds.length
      ? getAdminDatabaseClient().from("locations").select("id,name,city,state").in("id", locationIds)
      : Promise.resolve({ data: [], error: null }),
    organizationIds.length
      ? getAdminDatabaseClient().from("organizations").select("id,name").in("id", organizationIds)
      : Promise.resolve({ data: [], error: null }),
  ]);
