...
async function attachLocations(rows: any[]) {
  ...
  const { data, error } = ids.length
    ? await getAdminDatabaseClient().from("locations").select(LOCATION_FIELDS).in("id", ids)
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (body.action === "scan") {
      ...
      const { data: found, error: findError } = await getAdminDatabaseClient().rpc("oh_find_live_location_duplicates_fast", { p_limit: limit });
    ...
    if (body.action === "merge") {
      const { data, error } = await getAdminDatabaseClient().rpc("oh_merge_live_location_duplicate", {
    ...
    if (body.action === "ignore" || body.action === "not_duplicate") {
      const { data, error } = await getAdminDatabaseClient().rpc("oh_ignore_live_location_duplicate", {
