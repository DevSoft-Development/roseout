...
function isServed(row: Row) {
  return ["completed", "seated", "checked_in", "arrived"].includes(
    String(row.status || "").toLowerCase(),
  );
}

function isWalkIn(row: Row) {
  return String(row.source || row.channel || "")
    .toLowerCase()
    .includes("walk");
}
...
export async function GET(request: NextRequest) {
  ...
  const { data: location, error: locationError } = await supabase
    .from("locations")
    .select("id, source_id, source_location_id, legacy_source_id, canonical_location_id, owner_user_id")
    .eq("id", locationId)
    .maybeSingle();
  ...
  const currentRows = active.filter((row) =>
    inRange(row.created_at || reservationDate(row), currentStart, end),
  );
  const previousRows = active.filter((row) =>
    inRange(row.created_at || reservationDate(row), previousStart, currentStart),
  );

  const currentVip = (vipSignups || []).filter((row) =>
    inRange(row.created_at, currentStart, end),
  ).length;
  const previousVip = (vipSignups || []).filter((row) =>
    inRange(row.created_at, previousStart, currentStart),
  ).length;

  const current = {
    reservations: currentRows.length,
    guestsServed: currentRows
      .filter(isServed)
