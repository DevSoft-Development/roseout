...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: application, error: applicationError } = await getAdminDatabaseClient()
    ...
    const { data: interview, error: interviewError } = await getAdminDatabaseClient()
    ...
    const stageResult = await getAdminDatabaseClient().rpc("career_set_application_stage", {
      p_application_id: id,
    ...
    if (sendEmail && candidateEmail) {
      ...
      await getAdminDatabaseClient().from("career_email_events").insert({
        application_id: id,
