...
export async function POST(_request: Request, context: { params: Promise<{ runId: string }> }) {
  ...
  if (!new Set(["superadmin", "admin"]).has(admin.role)) return Response.json({ success: false, error: "Forbidden." }, { status: 403 });

  try {
    const { runId } = await context.params;
    const { data: run, error } = await getAdminDatabaseClient()
    ...
    const { error: updateError } = await getAdminDatabaseClient()
      .from("search_anchor_sync_runs")
      .update({ status: "approved", approved_by: admin.user_id ?? null })
      .eq("id", runId);
