      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.hero}>
          ...
          <AppText muted style={styles.body}>
            {mode === "signin" ? "Your saved places, plans, favorites, and OUTings are waiting." : "Save what you love, keep your plans together, and get timely OUTing updates."}
          </AppText>
        </View>
        ...
        <View style={styles.form}>
          ...
          {mode === "signup" ? (
            <>
              ...
              <Field label="Birth month" hint="Month only — we do not need your full birthday.">
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.months}>
                  {MONTHS.map((month, index) => {
                    ...
                    return (
                      <Pressable key={month} onPress={() => setBirthMonth(index + 1)} style={[styles.month, { borderColor: selected ? theme.colors.accent : theme.colors.borderStrong, backgroundColor: selected ? theme.colors.accentSoft : theme.colors.surface }]}>
                        <AppText variant="caption" accent={selected}>{month}</AppText>
                      </Pressable>
                    );
                  })}
                </ScrollView>
              ...
              <Pressable accessibilityRole="checkbox" accessibilityState={{ checked: smsConsent }} onPress={() => setSmsConsent((value) => !value)} style={[styles.consent, { borderColor: theme.colors.borderStrong, backgroundColor: theme.colors.surface }]}>
                <View style={[styles.checkbox, { borderColor: smsConsent ? theme.colors.accent : theme.colors.borderStrong, backgroundColor: smsConsent ? theme.colors.accent : "transparent" }]}>
                  {smsConsent ? <AppText variant="caption" style={{ color: theme.colors.onAccent }}>✓</AppText> : null}
                </View>
                <AppText muted style={styles.consentText}>{SMS_TERMS}</AppText>
              </Pressable>

              <AppText variant="caption" muted style={styles.legal}>
                By creating an account, you agree to TheOutHaven Terms and acknowledge the Privacy Policy. SMS consent is optional.
              </AppText>
            </>
          ) : null}
          ...
          <Button disabled={!valid || busy || verifying} onPress={() => void startVerification()}>
            {busy ? "Working…" : verifying ? "Complete verification…" : mode === "signin" ? "Verify & Sign In" : "Verify & Create Account"}
          </Button>
...
const styles = StyleSheet.create({
  page: { flex: 1 },
  content: { paddingHorizontal: 22, paddingTop: 22, paddingBottom: 48, gap: 20 },
  hero: { gap: 8, marginTop: 8 },
  body: { lineHeight: 22 },
  segment: { flexDirection: "row", padding: 4, borderRadius: 18, backgroundColor: "rgba(255,255,255,0.035)" },
  ...
  months: { gap: 8, paddingVertical: 2 },
  month: { minWidth: 54, height: 42, borderRadius: 21, borderWidth: 1, alignItems: "center", justifyContent: "center" },
  consent: { flexDirection: "row", gap: 12, borderWidth: 1, borderRadius: 18, padding: 14, alignItems: "flex-start" },
  checkbox: { width: 24, height: 24, borderRadius: 7, borderWidth: 1, alignItems: "center", justifyContent: "center", flexShrink: 0 },
