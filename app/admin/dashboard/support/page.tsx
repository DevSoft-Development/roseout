...
export default async function AdminSupportPage() {
  ...
  const [supportTickets, departments] = await Promise.all([
      ...
      <div className="mx-auto max-w-[1400px]">
        <section className="rounded-[2rem] border border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(225,29,72,0.22),transparent_34%),linear-gradient(135deg,#170b0b,#090706_58%,#14100c)] p-6 shadow-2xl">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-white/60">
            Route tickets by department, reassign ownership to admins, and reply
            from the dashboard or by email.
          </p>
        ...
        <section className="mt-6 grid gap-4 md:grid-cols-4">
          <Metric label="Total tickets" value={supportTickets.length} />
          <Metric label="Open" value={openTickets.length} />
          <Metric label="Departments" value={departments.length} />
          <Metric
            label="Unassigned"
            value={
              supportTickets.filter((ticket) => !ticket.assigned_admin_email)
                .length
            }
          />
        </section>
        ...
        <section className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {departments.map((department) => {
            const count = supportTickets.filter(
              (ticket) =>
                (ticket.department_route || "general") === department.slug,
            ).length;
            return (
              <Link
                key={department.slug}
                href={`/admin/dashboard/support/department/${department.slug}`}
                className="rounded-[1.5rem] border border-white/10 bg-white/[0.06] p-4 transition hover:-translate-y-0.5 hover:bg-white/[0.1]"
              >
                <p className="text-xs font-black uppercase tracking-[0.22em] text-white/40">
                  Department
                </p>
                <h3 className="mt-2 text-xl font-black">{department.name}</h3>
        ...
        <TicketList tickets={supportTickets} title="Latest tickets" />
