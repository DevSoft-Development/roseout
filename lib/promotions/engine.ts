...
function pacingAllows(campaign: PromotionCampaign, now = Date.now()) {
  if (!campaign.starts_at || !campaign.ends_at) return true;
  ...
  const targetSpend = Math.max(1, Math.round(campaign.total_budget_cents * elapsed));
  const pacingBuffer = Math.max(100, Math.round(campaign.total_budget_cents * 0.05));
  return campaign.spent_cents <= targetSpend + pacingBuffer;
}
...
export async function getDeliverablePromotionCampaigns(placement: PromotionPlacement) {
  ...
  const campaigns = (data || []).filter((row: any) => activeNow(row as PromotionCampaign) && pacingAllows(row as PromotionCampaign)) as PromotionCampaign[];
  const eligible: PromotionCampaign[] = [];
  for (const campaign of campaigns) {
    if (campaign.status === "scheduled") {
      await supabaseAdmin.from("promotion_campaigns").update({ status: "active", activated_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq("id", campaign.id).eq("status", "scheduled");
...
function idOf(value: any) { return value?.id == null ? null : String(value.id); }
function campaignForLocation(campaigns: PromotionCampaign[], value: any) { const id = idOf(value); return id ? campaigns.find((campaign) => campaign.location_id === id) || null : null; }
function sponsoredFields(campaign: PromotionCampaign) { return { sponsored: true, isSponsored: true, is_sponsored: true, placement_type: "sponsored", sponsor_id: campaign.id, promotion_campaign_id: campaign.id }; }
...
export async function applySearchPromotions<T extends Record<string, any>>(payload: T): Promise<T> {
  ...
  if (Array.isArray(root.pairs)) {
    const pairs = root.pairs.map((pair: any) => {
      const campaign = campaignForLocation(campaigns, pair?.restaurant) || campaignForLocation(campaigns, pair?.activity);
      if (!campaign) return pair;
      return { ...pair, restaurant: annotateLocation(pair.restaurant), activity: annotateLocation(pair.activity), ...sponsoredFields(campaign) };
    });
