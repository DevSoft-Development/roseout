...
export default function CuratedReviewClient({ anchors }: Props) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-[var(--admin-shell-border)] bg-[var(--admin-shell-card)] p-4">
        <button type="button" onClick={toggleAllPending} disabled={!pendingIds.length || working} className="rounded-xl border border-[var(--admin-shell-border)] px-4 py-2 text-sm font-semibold text-[var(--admin-shell-text)] disabled:opacity-50">
        ...
        {status && <p className="w-full text-sm text-[var(--admin-shell-muted)]">{status}</p>}
      </div>

      <div className="overflow-hidden rounded-2xl border border-[var(--admin-shell-border)] bg-[var(--admin-shell-card)]">
            ...
            <tbody>
              {anchors.map((anchor) => (
                <tr key={anchor.id} className="border-t border-[var(--admin-shell-border)] align-top hover:bg-[var(--admin-shell-soft)]">
                  ...
                  <td className="px-4 py-4"><p className="font-medium text-[var(--admin-shell-text)]">{anchor.canonical_name}</p><p className="mt-1 text-xs text-[var(--admin-shell-muted)]">{anchor.anchor_type}</p></td>
                  <td className="px-4 py-4 text-[var(--admin-shell-text)]"><p>{anchor.city ?? "—"}, {anchor.state ?? "—"}</p><p className="mt-1 text-xs text-[var(--admin-shell-muted)]">{anchor.market ?? "Unassigned market"}</p></td>
                  <td className="px-4 py-4 text-[var(--admin-shell-text)]">{Number(anchor.latitude).toFixed(5)}, {Number(anchor.longitude).toFixed(5)}</td>
              ...
              {!anchors.length && <tr><td colSpan={5} className="px-6 py-12 text-center text-[var(--admin-shell-muted)]">No curated anchors found.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
