...
for (const source of [
  careersApplicationDetailPage,
  careersWorkflowRoute,
  careersScorecardRoute,
  careersScheduleInterviewRoute,
  careersInterviewSessionRoute,
]) {
  if (!source.includes(careersApplicationManageRoles)) {
    throw new Error("Careers application-detail slice must preserve careersApplicationsManage role access.");
  }
  if (
    source.includes("@/lib/admin-auth") ||
    source.includes("@/lib/admin-permissions") ||
...
if (
  !careersApplicationDetailPage.includes("@theouthaven/auth/admin-session") ||
  !careersApplicationDetailPage.includes("@theouthaven/db/admin-client") ||
  !careersApplicationDetailPage.includes("@/lib/supabase/relations") ||
  !careersHiringWorkflow.includes("/workflow") ||
  !careersHiringWorkflow.includes("/scorecard") ||
  !careersHiringWorkflow.includes("/schedule-interview") ||
  !careersInterviewSessionPanel.includes("/interview-session")
) {
  throw new Error("Careers application detail must preserve isolated data access and hiring workflow endpoints.");
}
if (
  !careersScheduleInterviewRoute.includes("@/lib/careers/interview-guide") ||
  !careersScheduleInterviewRoute.includes("@/lib/email/send") ||
  !careersScheduleInterviewRoute.includes("@/lib/microsoft-365/calendar") ||
  !careersScheduleInterviewRoute.includes("@/lib/sms/telnyx") ||
  !careersInterviewGuide.includes("buildFallbackInterviewGuide") ||
  !careersCalendar.includes("@theouthaven/db/admin-client") ||
  !careersSms.includes("sendTelnyxSmsViaIntegrationApi") ||
  !adminIntegrationApi.includes("sendTelnyxSmsViaIntegrationApi")
) {
...
for (const source of [careersOnboardRoute, careersOffboardRoute]) {
  if (
    !source.includes("@theouthaven/db/admin-client") ||
    !source.includes('["superadmin", "admin"]') ||
    !source.includes('functions.invoke("career-workflow"')
  ) {
    throw new Error("Careers lifecycle APIs must preserve isolated Admin authorization and career-workflow invocation.");
  }
}
if (
  !careersLifecycleActions.includes("/onboard") ||
  !careersLifecycleActions.includes("/offboard") ||
  !careersRelations.includes("normalizeSingleRelation")
) {
