      ...
      {reservations.map((r) => {
        ...
        return (
          <button
            ...
            key={r.id}
            onClick={() => onSelect(r)}
            className={`reserve-timeline-row group relative w-full overflow-hidden rounded-2xl border bg-[var(--reserve-card-strong)] px-3 py-3 text-left transition hover:border-[var(--reserve-border-strong)] ${selected ? "border-[var(--reserve-primary)]/50 shadow-[0_0_0_1px_rgba(225,6,42,.16),0_10px_28px_rgba(0,0,0,.22)]" : "border-[var(--reserve-border)]"}`}
          >
            ...
            <div className="reserve-timeline-grid grid gap-3">
              <div className="shrink-0 pl-2">
                <p className="whitespace-nowrap text-sm font-black">{formatReservationTime(r.reservation_time)}</p>
                <p className="whitespace-nowrap text-[11px] reserve-muted">{duration(r)}m</p>
              </div>
              ...
              >
                {canTextReady(r) && onTableReady && (
                  <ReserveQuickActionButton disabled={updatingId === r.id} onClick={() => onTableReady(r)}>
                    {updatingId === r.id ? "Sending…" : "Table ready"}
                  </ReserveQuickActionButton>
                )}
                <ReserveQuickActionButton
                  disabled={updatingId === r.id || !!action.disabledReason || !action.targetStatus}
                  title={action.disabledReason}
                  onClick={() => action.targetStatus && onStatus(r, action.targetStatus)}
                >
                  {updatingId === r.id ? "Updating…" : action.label}
                </ReserveQuickActionButton>
                {onAssign && <ReserveQuickActionButton onClick={() => onAssign(r)}>Assign</ReserveQuickActionButton>}
                <span className="grid h-[31px] w-[31px] shrink-0 place-items-center rounded-full border border-[var(--reserve-border)] text-[var(--reserve-muted)]">
