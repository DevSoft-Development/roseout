...
with candidates as (
  ...
  where l.deleted_at is null
    ...
    and (
      nullif(trim(coalesce(l.main_image, '')), '') is not null
      or nullif(trim(coalesce(l.image_url, '')), '') is not null
      or coalesce(jsonb_array_length(to_jsonb(l.images)), 0) > 0
      or coalesce(jsonb_array_length(to_jsonb(l.gallery_images)), 0) > 0
    )
), scored as (
  select c.*,
