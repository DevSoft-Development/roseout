...
serve(async (req) => {
  ...
  const counters = { scanned: 0, operational: 0, temporarily_closed: 0, permanently_closed: 0, future_opening: 0, unspecified: 0, failed: 0, hidden_now: 0, googleApi: "aws_integration_api", results: [] as Array<Record<string, unknown>> };
  for (const row of rows ?? []) {
    counters.scanned += 1;
    try {
      const place = await googlePlaceDetailsViaIntegrationApi<Record<string, unknown>>(String(row.google_place_id));
      const status = String(place.businessStatus || "BUSINESS_STATUS_UNSPECIFIED");
      const checkedAt = new Date().toISOString();
      const { data: updated, error: updateError } = await supabase.from("locations").update({ google_business_status: status, google_business_status_checked_at: checkedAt }).eq("id", row.id).select("id,is_searchable,is_hidden,publish_ready").single();
      ...
      counters.results.push({ id: row.id, status, hidden: updated?.is_hidden === true });
    } catch (error) {
      counters.failed += 1;
