...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const rawQuery = String(
      body.query || body.input || body.prompt || "",
    ).trim();
    const simulatedLocation =
      body.simulateCurrentLocation === true
        ? {
            userLatitude: body.testLatitude,
            userLongitude: body.testLongitude,
            latitude: body.testLatitude,
            longitude: body.testLongitude,
          }
        : {};
    ...
    const outingTiming = {
      ...parseOutingDateTime(query),
      ...(debug?.normalizedIntent
        ? {
            outingDateLabel: debug.normalizedIntent.outingDateLabel,
            outingTimeLabel: debug.normalizedIntent.outingTimeLabel,
            outingDateTimeText: debug.normalizedIntent.outingDateTimeText,
            outingTimeConfidence: debug.normalizedIntent.outingTimeConfidence,
            parsedDateText: debug.normalizedIntent.parsedDateText,
            parsedTimeText: debug.normalizedIntent.parsedTimeText,
            parsedDateTimeISO: debug.normalizedIntent.parsedDateTimeISO,
          }
        : {}),
    };
    ...
    const publicCounts = {
      ...rawCounts,
      cards: restaurantCards.length + activityCards.length,
    };
    ...
    const marketFiltering = {
      explicitMarketRequested:
        debug?.explicitMarketRequested ??
        normalizedRequest.debugParity.explicitMarketRequested,
      resolvedMarket:
        debug?.resolvedMarket ?? normalizedRequest.debugParity.resolvedMarket,
      requestedMarket:
        debug?.requestedMarket ?? normalizedRequest.debugParity.requestedMarket,
      allowedMarkets:
        debug?.allowedMarkets ?? normalizedRequest.debugParity.allowedMarkets,
      geoSource: normalizedRequest.debugParity.geoSource,
      userLocationReceived:
        normalizedRequest.userLatitude != null &&
        normalizedRequest.userLongitude != null,
      ...
      restaurantCandidatesBeforeMarketFilter:
        debug?.restaurantCandidatesBeforeMarketFilter ?? restaurantCards.length,
      restaurantCandidatesAfterMarketFilter: restaurantCards.length,
      activityCandidatesBeforeMarketFilter:
        debug?.activityCandidatesBeforeMarketFilter ?? activityCards.length,
    ...
    const debugParity = {
      ...
      searchType:
        ...
        normalizedRequest.debugParity.searchType,
      wantsPairing:
        debug?.normalizedIntent?.wantsPairing ??
        normalizedRequest.debugParity.wantsPairing,
      needsRestaurant:
        debug?.normalizedIntent?.needsRestaurant ??
        normalizedRequest.debugParity.needsRestaurant,
      needsActivity:
        debug?.normalizedIntent?.needsActivity ??
        normalizedRequest.debugParity.needsActivity,
    ...
    const responseBody = {
      ...
      debugParity,
      marketFiltering,
      mlAvailable: Boolean(debug?.mlSearchDebug),
      mlDebug: debug?.mlSearchDebug ?? {
        mlAvailable: false,
        reason:
          "ML debug unavailable. Confirm Phase 1/Phase 2 migrations are applied and recalculation routes have run.",
      },
      intentClassification:
        debug?.mlSearchDebug?.intentClassification ??
        debug?.mlPhase2Intent ??
        null,
      mlImpactSummary: debug?.mlSearchDebug
        ? {
            mlEnabled: debug.mlSearchDebug.mlEnabled,
            phase1Enabled: debug.mlSearchDebug.phase1Enabled,
            phase2Enabled: debug.mlSearchDebug.phase2Enabled,
            resultsWithMlBoostCount:
              debug.mlSearchDebug.resultsWithMlBoostCount,
            resultOrderChangedByMl: debug.mlSearchDebug.resultOrderChangedByMl,
            maxMlBoostApplied: debug.mlSearchDebug.maxMlBoostApplied,
            averageMlBoostApplied: debug.mlSearchDebug.averageMlBoostApplied,
            mlUnavailableReason: debug.mlSearchDebug.mlUnavailableReason,
          }
        : null,
      source:
        (result as any).source ||
        (isEdgeCreateSearchEnabled() ? "edge" : "legacy"),
      searchBackendUsed,
      ...
      fastPathMatched: Boolean(
        ...
        (debug?.parser_source === "fast_parser" ||
          debug?.parser_source === "fast_path"),
      ),
      ...
      fallbackUsed: Boolean(
        debug?.restaurantRecoveryUsed ||
        debug?.activityRecoveryUsed ||
        debug?.edge_error,
      ),
    ...
    if ((result as any).source === "edge" || debug?.source === "edge") {
      void logSearchHealthEvent({
        ...
        debugMode: true,
        forceLog:
          body?.logSearchHealth === true ||
          body?.debug === true ||
          body?.debug?.logSearchHealth === true,
      });
    }
