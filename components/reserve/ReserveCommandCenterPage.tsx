...
import { getReserveActionLinks, getReserveDashboardUrl } from "@/lib/reservations/reserveLinks";
...
function normalizeType(value: string | null | undefined) { const type = String(value || "").toLowerCase(); if (!type) return ""; return type === "activities" ? "activity" : type; }
...
function ReserveCommandCenterContent() {
  ...
  const loadedLocationType = normalizeType(adminSummary?.location?.location_type || adminSummary?.location?.source_table || adminSummary?.location?.type);
  const locationType = suppliedLocationType || loadedLocationType || "location";
  const vocab = getReserveVocabulary(locationType, resources[0]?.item_type || resources[0]?.type);
  function dashboardHref(tab = activeTab, section?: string){ return getReserveDashboardUrl(tab, section, { adminLocationId: adminLocationId||undefined, locationId: !adminLocationId?locationId:undefined, type: suppliedLocationType || loadedLocationType || undefined, date:selectedDate }); }
  function actionContext(){ return { adminLocationId: adminLocationId || undefined, type: suppliedLocationType || loadedLocationType || undefined }; }
  const isDemoLocation = Boolean(adminSummary?.location?.is_demo || adminSummary?.location?.demo_key || searchParams.get("demo") === "1");
  const actionLinks = getReserveActionLinks({ locationId, locationType, adminLocationId: adminLocationId || undefined, isDemo: isDemoLocation });
  ...
  async function loadAll(options: { silent?: boolean } = {}){
    ...
    try {
      const params = new URLSearchParams({ filter:"upcoming" }); if(locationId){ params.set("locationId", locationId); if(suppliedLocationType || loadedLocationType) params.set("type", locationType); if(adminLocationId) params.set("adminLocationId", adminLocationId); }
  ...
  async function assignResource(reservation:Reservation, resource:any){
    ...
    if(!reservation?.id){ setMessage({ tone:"error", text:`Select a reservation before using ${vocab.assignResource.toLowerCase()}.` }); return; }
    if(!resourceId(resource) && !resourceName(resource)){ setMessage({ tone:"error", text:`Choose a valid ${vocab.resource.toLowerCase()}.` }); return; }
    if(!state.available){ setMessage({ tone:"error", text:`That ${vocab.resource.toLowerCase()} is already unavailable for this reservation time.` }); return; }
    setUpdatingId(reservation.id); setMessage(null);
    try {
      const response = await fetch("/api/reserve/portal/assign-resource", { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ reservation_id: reservation.id, location_id: reservation.location_id, location_type: reservation.location_type, ...resourceAssignmentPayload(resource), seat_after_assign: true, adminLocationId: adminLocationId || undefined }) });
      const data = await response.json(); if(!response.ok || !data.success) { const details = data || {}; const message = details.error || `We could not use ${vocab.assignResource.toLowerCase()}. Please try another ${vocab.resource.toLowerCase()}.`; const error = new Error(details.debugId && adminLocationId ? `${message} Debug ID: ${details.debugId}` : message) as Error & { details?: any }; error.details = details; throw error; }
      setReservations((prev)=>prev.map((r)=>r.id===reservation.id?data.reservation:r));
      setSelectedId(reservation.id); setAssigningReservationId(""); setAssignmentDiagnostics(null); setMessage({ tone:"success", text:`${vocab.resource} assigned. ${vocab.customer} seated.` });
      try { await loadAll({ silent:true }); } catch {
        // Keep assignment success visible; do not reopen assign mode when refresh fails.
      }
    } catch(error){ const raw = error instanceof Error ? error.message : ""; const details = (error as any)?.details || {}; const text = raw || friendlyError(error, `We could not use ${vocab.assignResource.toLowerCase()}. Please try another ${vocab.resource.toLowerCase()}.`); if(details.debugId && adminLocationId){ setAssignmentDiagnostics({ debugId: details.debugId, reservationId: reservation.id, selectedResourceLabel: resourceName(resource), selectedResourceId: resourceId(resource), selectedResourceSource: resourceAssignmentPayload(resource).resource_source, locationId: reservation.location_id || locationId, message: details.error || text, errorCode: details.debug?.code, errorMessage: details.debug?.message, payloadPreview: details.debug?.payloadPreview }); } setMessage({ tone:"error", text }); }
    finally { setUpdatingId(""); }
  }

  async function sendTableReady(reservation:Reservation){
    if(!hasAssignedReservationResource(reservation)){ setSelectedId(reservation.id); setAssigningReservationId(reservation.id); setMessage({ tone:"warning", text:`${vocab.chooseResource} before sending a ${vocab.readyAction.toLowerCase()} text.` }); return; }
    if(!reservation.customer_phone){ setMessage({ tone:"warning", text:`Add a phone number before sending a ${vocab.readyAction.toLowerCase()} text.` }); return; }
    setUpdatingId(reservation.id); setMessage(null);
    try {
      ...
      const data = await response.json(); if(!response.ok) throw new Error(data.error || `We could not send the ${vocab.readyAction.toLowerCase()} text.`);
      ...
      setSelectedId(reservation.id); setMessage({ tone:"success", text:`${vocab.readyAction} text sent to ${getReservationGuestName(reservation)}.` }); await loadAll({ silent:true });
    } catch(error){ setMessage({ tone:"error", text:friendlyError(error, `We could not send the ${vocab.readyAction.toLowerCase()} text.`) }); }
    finally { setUpdatingId(""); }
  }
  ...
  async function notifyWaitlist(entry:any){
    ...
    try {
      ...
      const data = await response.json(); if(!response.ok) throw new Error(data.error || `We could not send the ${vocab.resource.toLowerCase()} offered text.`);
      setWaitlist((prev)=>prev.map((item)=>item.id===entry.id?data.waitlist:item));
      setMessage({ tone:"success", text:`${vocab.resource} offered text sent.` }); await loadAll({ silent:true });
    } catch(error){ setMessage({ tone:"error", text:friendlyError(error, `We could not send the ${vocab.resource.toLowerCase()} offered text.`) }); }
    finally { setUpdatingId(""); }
  }
  ...
  const filtered = dayReservations.filter((r)=> (statusFilter === "all" || r.status === statusFilter) && `${getReservationGuestName(r)} ${r.customer_name||""} ${r.guest_name||""} ${r.name||""} ${r.customer_phone||""} ${r.customer_email||""} ${r.special_request||""}`.toLowerCase().includes(search.toLowerCase()));
  ...
  return <ReserveCommandCenterShell locationName={locationName} locationId={locationId} locationType={locationType} activeTab={activeTab} activeSection={activeSection} onTabChange={switchTab} actions={topActions} setupEnabled={setupEnabled} userLabel={adminLocationId ? "Admin location mode" : "Owner workspace"} actingContext={actionContext()}>
    ...
    {assigningReservation && <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4"><div className="reserve-card max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-[2rem] p-5">{assignmentDiagnostics?.debugId && adminLocationId && <details className="mb-4 rounded-2xl border border-amber-300/30 bg-amber-300/10 p-3 text-sm"><summary className="cursor-pointer font-black text-amber-100">Assignment diagnostics</summary><div className="mt-2 space-y-1 text-amber-50/90"><p>Debug ID: {assignmentDiagnostics.debugId}</p><p>Reservation: {assignmentDiagnostics.reservationId}</p><p>{vocab.resource}: {assignmentDiagnostics.selectedResourceLabel || "Selected space"} ({assignmentDiagnostics.selectedResourceId || "label-only"})</p><p>Source: {assignmentDiagnostics.selectedResourceSource || "manual_label"}</p><p>Location: {assignmentDiagnostics.locationId}</p><p>Message: {assignmentDiagnostics.message}</p>{assignmentDiagnostics.errorCode&&<p>Error code: {assignmentDiagnostics.errorCode}</p>}{assignmentDiagnostics.errorMessage&&<p>Error message: {assignmentDiagnostics.errorMessage}</p>}{assignmentDiagnostics.payloadPreview&&<div><p className="font-black">Payload preview:</p><ul className="ml-4 list-disc"><li>bookable_item_id: {assignmentDiagnostics.payloadPreview.bookable_item_id || "null"}</li><li>bookable_item_name: {assignmentDiagnostics.payloadPreview.bookable_item_name || "Selected space"}</li><li>bookable_item_type: {assignmentDiagnostics.payloadPreview.bookable_item_type || "space"}</li></ul></div>}</div></details>}<div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase reserve-muted">{vocab.assignResource}</p><h2 className="mt-1 text-2xl font-black">{getReservationGuestName(assigningReservation)} · {vocab.partyLabel} {assigningReservation.party_size}</h2><p className="mt-1 text-sm reserve-muted">Unavailable {vocab.resourcePlural.toLowerCase()} are disabled for this reservation time.</p></div><button type="button" onClick={()=>setAssigningReservationId("")} className="reserve-soft rounded-full px-3 py-1 text-sm font-black">Close</button></div><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{resources.map((resource)=>{const state=getFloorSnapshotState(resource, dayReservations);return <button key={resourceId(resource) || resourceName(resource)} type="button" disabled={!state.available || updatingId===assigningReservation.id} onClick={()=>assignResource(assigningReservation, resource)} className="reserve-soft rounded-2xl p-4 text-left disabled:cursor-not-allowed disabled:opacity-50"><p className="text-lg font-black">{resourceName(resource)}</p><p className="text-sm reserve-muted">Capacity {resourceCapacity(resource)||"—"}</p><p className="mt-3 text-xs font-black">{state.status === "Seated" ? vocab.seatedStatus : state.status} · {state.available ? "Available" : "Unavailable"}</p>{state.reservation&&<p className="mt-1 truncate text-xs reserve-muted">{getReservationGuestName(state.reservation)} · {vocab.partyLabel} {state.reservation.party_size||"—"}</p>}</button>})}</div></div></div>}
    {modal && <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4"><form onSubmit={(event)=>submitCreate(event, modal)} className="reserve-card w-full max-w-xl rounded-[2rem] p-5"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase reserve-muted">{modal === "waitlist" ? "Waitlist" : modal === "walkin" ? "Walk-in" : "New reservation"}</p><h2 className="mt-1 text-2xl font-black">{modal === "waitlist" ? "Add to Waitlist" : modal === "walkin" ? "Add Walk-in" : "Create Reservation"}</h2></div><button type="button" onClick={()=>setModal(null)} className="reserve-soft rounded-full px-3 py-1 text-sm font-black">Close</button></div><div className="mt-5 grid gap-3 sm:grid-cols-2"><label className="text-sm font-bold">{vocab.customer} name<input name="guestName" required={modal!=="walkin"} placeholder={modal==="walkin"?`Walk-in ${vocab.customer}`:`${vocab.customer} name`} className="reserve-soft mt-1 w-full rounded-2xl px-4 py-3"/></label><label className="text-sm font-bold">{vocab.partySizeLabel}<input name="partySize" required type="number" min="1" defaultValue="2" className="reserve-soft mt-1 w-full rounded-2xl px-4 py-3"/></label>{modal!=="walkin" && <><label className="text-sm font-bold">Phone<input name="phone" placeholder="Phone" className="reserve-soft mt-1 w-full rounded-2xl px-4 py-3"/></label><label className="text-sm font-bold">Email<input name="email" type="email" placeholder="Email" className="reserve-soft mt-1 w-full rounded-2xl px-4 py-3"/></label></>}<label className="text-sm font-bold">Date<input name="date" type="date" required defaultValue={selectedDate} className="reserve-soft mt-1 w-full rounded-2xl px-4 py-3"/></label><label className="text-sm font-bold">Time<input name="time" type="time" required defaultValue={modal==="walkin" ? new Date().toTimeString().slice(0,5) : "19:00"} className="reserve-soft mt-1 w-full rounded-2xl px-4 py-3"/></label>{modal==="reservation" && <label className="text-sm font-bold">Duration<input name="duration" type="number" min="15" step="15" defaultValue="90" className="reserve-soft mt-1 w-full rounded-2xl px-4 py-3"/></label>}<label className="text-sm font-bold sm:col-span-2">Notes<textarea name="notes" rows={3} placeholder="Optional notes" className="reserve-soft mt-1 w-full rounded-2xl px-4 py-3"/></label></div><button disabled={submitting} className="reserve-primary mt-5 w-full rounded-full px-5 py-3 font-black disabled:opacity-60">{submitting ? "Saving…" : modal === "waitlist" ? "Add to Waitlist" : modal === "walkin" ? "Add Walk-in" : "Create Reservation"}</button></form></div>}
    ...
    {activeTab === "today" && <div className="grid gap-4 xl:grid-cols-[minmax(430px,0.42fr)_minmax(620px,1fr)]"><section className="reserve-card rounded-2xl p-4"><div className="mb-3 flex items-center justify-between"><div><p className="text-xs font-black uppercase reserve-muted">Reservation timeline</p><h2 className="text-xl font-black">{filtered.length} bookings</h2></div></div>{loading ? <ReserveEmptyState title="Loading reservations…" message="We’re checking the live reservation list."/> : filtered.length ? <ReserveTimeline vocabulary={vocab} reservations={filtered} selectedId={selected?.id} onSelect={(r)=>setSelectedId(r.id)} onStatus={updateStatus} onAssign={(r)=>{setSelectedId(r.id);setAssigningReservationId(r.id);}} onTableReady={sendTableReady} updatingId={updatingId}/> : <ReserveEmptyState title="No reservations for this day." message="New bookings and changes will appear here automatically."/>}</section><div className="space-y-4"><ReserveFloorSnapshot vocabulary={vocab} resources={resources} reservations={dayReservations} settingsHref={dashboardHref("settings","layout")} assigningReservation={assigningReservation} onResourceSelect={(resource)=>assigningReservation&&assignResource(assigningReservation,resource)} onReservationSelect={(r)=>setSelectedId(r.id)}/><div className="grid gap-4 2xl:grid-cols-2"><ReserveGuestDetails vocabulary={vocab} reservation={selected} onStatus={updateStatus} onAssign={(r)=>{setSelectedId(r.id);setAssigningReservationId(r.id);}} onTableReady={sendTableReady} updatingId={updatingId} onRefresh={()=>loadAll({ silent:true })}/><ReserveWaitlistPanel vocabulary={vocab} entries={waitlist} onAdd={()=>setModal("waitlist")} onOffer={notifyWaitlist} onViewAll={()=>switchTab("waitlist")} updatingId={updatingId}/></div></div></div>}
    {activeTab === "floor" && <ReserveFloorSnapshot vocabulary={vocab} resources={resources} reservations={dayReservations} settingsHref={dashboardHref("settings","layout")} assigningReservation={assigningReservation} onResourceSelect={(resource)=>assigningReservation&&assignResource(assigningReservation,resource)} onReservationSelect={(r)=>setSelectedId(r.id)}/>} {activeTab === "waitlist" && <ReserveWaitlistPanel vocabulary={vocab} entries={waitlist} onAdd={()=>setModal("waitlist")} onOffer={notifyWaitlist} onViewAll={()=>switchTab("waitlist")} updatingId={updatingId}/>} {activeTab === "guests" && <section className="reserve-card rounded-[2rem] p-5"><h2 className="text-2xl font-black">Guests</h2><ReserveTimeline vocabulary={vocab} reservations={filtered} selectedId={selected?.id} onSelect={(r)=>setSelectedId(r.id)} onStatus={updateStatus} onAssign={(r)=>{setSelectedId(r.id);setAssigningReservationId(r.id);}} onTableReady={sendTableReady} updatingId={updatingId}/></section>} {activeTab === "calendar" && <section className="reserve-card rounded-[2rem] p-5"><h2 className="text-2xl font-black">Calendar volume</h2><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{Array.from(new Set(reservations.map(r=>r.reservation_date))).slice(0,12).map(d=><button key={d} onClick={()=>{setSelectedDate(d);switchTab('today')}} className="reserve-soft rounded-2xl p-4 text-left"><CalendarDays size={18}/><p className="mt-2 font-black">{d}</p><p className="text-sm reserve-muted">{reservations.filter(r=>r.reservation_date===d).length} reservations</p></button>)}</div></section>} {activeTab === "settings" && <section className="reserve-card rounded-[2rem] p-5"><h2 className="text-2xl font-black">Reservation setup</h2><p className="mt-1 text-sm reserve-muted">Use these setup sections for the selected Reserve location.</p><div className="mt-5 grid gap-4 lg:grid-cols-[260px_1fr]"><div className="space-y-2">{[["layout",`Layout & ${vocab.resourcePlural}`],["hours","Hours & Capacity"],["reminders","Reminders"],["deposits","Deposit & Policies"],["booking","Booking page"],["embed","Embed"],["qr","QR code"]].map(([section,label])=><Link key={section} href={dashboardHref("settings", section)} className={`block rounded-2xl px-4 py-3 text-sm font-black ${activeSection === section ? "reserve-primary" : "reserve-soft"}`}>{label}</Link>)}</div><div className="reserve-soft rounded-[1.5rem] p-5">{activeSection === "layout" ? <div><h3 className="text-xl font-black">Layout & {vocab.resourcePlural}</h3>{resources.length ? <div className="mt-4 grid gap-3 sm:grid-cols-2">{resources.map((resource)=><div key={resourceId(resource) || resourceName(resource)} className="rounded-2xl bg-black/10 p-4"><p className="font-black">{resource.label || resource.item_name || "Resource"}</p><p className="text-sm reserve-muted">Capacity {resource.capacity || resource.capacity_max || resource.capacity_min || "—"}</p></div>)}</div> : <p className="mt-3 font-bold">No bookable spaces are set up yet.</p>}<Link href={`/reserve/dashboard/location-layout${locationId ? `?adminLocationId=${encodeURIComponent(locationId)}&type=${encodeURIComponent(locationType)}` : ""}`} className="mt-4 inline-block reserve-primary rounded-full px-4 py-2 text-sm font-black">Add spaces</Link></div> : activeSection === "hours" ? <div><h3 className="text-xl font-black">Hours & Capacity</h3><p className="mt-3 font-bold">Reservation hours are not configured here yet. Use the location hours settings.</p><Link href={locationId ? `/admin/dashboard/crm/${encodeURIComponent(locationId)}` : "/admin/dashboard/crm"} className="mt-4 inline-block reserve-primary rounded-full px-4 py-2 text-sm font-black">Open location hours settings</Link></div> : activeSection === "reminders" ? <div><h3 className="text-xl font-black">Reminders</h3><p className="mt-3 font-bold">Reservation reminders are handled by the existing reminder system.</p></div> : activeSection === "deposits" ? <div><h3 className="text-xl font-black">Deposit & Policies</h3><p className="mt-3 font-bold">Deposits are not configured for this location yet.</p></div> : activeSection === "booking" ? <div><h3 className="text-xl font-black">Booking page</h3><p className="mt-3 break-all text-sm reserve-muted">{actionLinks.bookingHref || "Select a location to generate the booking page link."}</p>{locationId && <div className="mt-4 flex flex-wrap gap-2"><Link href={actionLinks.bookingHref} className="reserve-primary rounded-full px-4 py-2 text-sm font-black">Open booking page</Link><button onClick={()=>navigator.clipboard?.writeText(`${window.location.origin}${actionLinks.bookingHref}`)} className="reserve-soft inline-flex h-10 items-center gap-1 rounded-full px-3 text-xs font-black">Copy booking page link</button></div>}</div> : activeSection === "embed" ? <div><h3 className="text-xl font-black">Embed</h3><p className="mt-3 break-all text-sm reserve-muted">{actionLinks.embedHref || "Select a location to generate the embed link."}</p>{locationId && <><code className="mt-4 block overflow-x-auto rounded-2xl bg-black/20 p-4 text-xs">{`<iframe src="${typeof window !== "undefined" ? window.location.origin : ""}${actionLinks.embedHref}" title="TheOutHaven reservations"></iframe>`}</code><div className="mt-4 flex flex-wrap gap-2"><Link href={actionLinks.embedPreviewHref || actionLinks.embedHref} className="reserve-primary rounded-full px-4 py-2 text-sm font-black">Open embed preview</Link><button onClick={()=>navigator.clipboard?.writeText(`<iframe src="${window.location.origin}${actionLinks.embedHref}" title="TheOutHaven reservations"></iframe>`)} className="reserve-soft inline-flex h-10 items-center gap-1 rounded-full px-3 text-xs font-black">Copy embed code</button></div></>}</div> : activeSection === "qr" ? <div><h3 className="text-xl font-black">QR code</h3>{actionLinks.qrHref ? <Link href={actionLinks.qrHref} className="mt-4 inline-block reserve-primary rounded-full px-4 py-2 text-sm font-black">Open QR tools</Link> : <p className="mt-3 font-bold">QR tools are not configured for this location yet.</p>}</div> : <div><h3 className="text-xl font-black">Settings</h3><p className="mt-3 font-bold">Choose a setup section to manage reservation operations.</p></div>}</div></div></section>}
