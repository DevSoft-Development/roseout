...
export default function LocationEditorMobileNav({ links, activeSectionId = "overview", onSectionSelect }: { links: LocationEditorLinks; activeSectionId?: string; onSectionSelect?: (sectionId: typeof cleanEditorHashNav[number]["sectionId"]) => void }) {
        ...
        <div id="location-editor-mobile-menu" className="fixed inset-0 z-50 lg:hidden" role="dialog" aria-modal="true" aria-label="Location editor menu">
          ...
          <aside className="relative z-10 h-full w-[min(86vw,340px)] overflow-y-auto border-r border-white/10 bg-[#050607] px-4 py-5 shadow-2xl">
            <div className="mb-5 flex items-start justify-between gap-3 px-3">
              <div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center overflow-hidden rounded-2xl bg-[#e1062a] ring-1 ring-white/10"><Image src="/icon.svg" alt="TheOutHaven icon" width={32} height={32} className="h-8 w-8 object-contain" unoptimized /></div><div><p className="text-lg font-black text-white">TheOutHaven</p><p className="text-xs font-bold text-white/45">Location Editor</p></div></div>
                ...
                <div key={section}>
                  ...
                  <div className="mt-2 grid gap-1">
                    {items.map((item) => {
                      ...
                      return "sectionId" in item ? <a key={`${section}-${item.label}`} href={item.href} onClick={(event) => { event.preventDefault(); onSectionSelect?.(item.sectionId); setOpen(false); }} className={`rounded-2xl px-3 py-2.5 text-left text-sm font-bold transition hover:bg-white/[0.06] hover:text-white ${active ? "border border-[#ff2142]/35 bg-[#e1062a]/25 text-white" : "text-white/70"}`}>{item.label}</a> : <Link key={`${section}-${item.label}`} href={item.href} onClick={() => setOpen(false)} className={`rounded-2xl px-3 py-2.5 text-sm font-bold transition hover:bg-white/[0.06] hover:text-white ${active ? "border border-[#ff2142]/35 bg-[#e1062a]/25 text-white" : "text-white/70"}`}>{item.label}</Link>;
                    })}
                  </div>
                </div>
              ))}
