...
async function paidCallsToday(jobKey: string) {
  ...
  const { count } = await getAdminDatabaseClient()
...
export async function recordGoogleOperation(
  ...
  const jobKey = clean(context.jobKey) || "unknown";
  try {
    await getAdminDatabaseClient().from("google_places_usage_events").insert({
      job_key: jobKey,
      operation,
      sku: config.sku,
...
export async function readIdSearchCache(query: string, regionCode = "US") {
  ...
  void getAdminDatabaseClient().from("google_places_search_id_cache").update({ last_hit_at: now }).eq("cache_key", cacheKey);
...
export async function writeIdSearchCache(query: string, regionCode: string, placeIds: string[], ttlDays = 14, metadata: Record<string, unknown> = {}) {
  ...
  await getAdminDatabaseClient().from("google_places_search_id_cache").upsert({
    cache_key: cacheKey,
...
export async function writeCandidateMemory(input: {
  ...
  await getAdminDatabaseClient().from("google_places_candidate_memory").upsert({
...
async function emitThresholdAlerts(summary: GoogleBudgetSummary) {
  ...
  for (const threshold of ALERT_THRESHOLDS) {
    ...
    try {
      ...
      await getAdminDatabaseClient().from("google_places_budget_alerts").update({ email_sent: Boolean((delivery as any)?.sent), email_error: (delivery as any)?.error || null }).eq("id", data.id);
    } catch (error) {
      await getAdminDatabaseClient().from("google_places_budget_alerts").update({ email_error: error instanceof Error ? error.message.slice(0, 500) : String(error).slice(0, 500) }).eq("id", data.id);
    }
  }
}
...
export async function getGoogleCostControlAdminSnapshot() {
  ...
  const [eventsResult, jobsResult, alertsResult] = await Promise.all([
    getAdminDatabaseClient().from("google_places_usage_events").select("job_key,operation,paid,blocked,cache_hit,estimated_unit_cost_usd,occurred_at").gte("occurred_at", start.toISOString()).order("occurred_at", { ascending: false }).limit(1000),
    getAdminDatabaseClient().from("google_places_job_budgets").select("job_key,daily_paid_call_limit,priority,enabled,notes").order("job_key"),
    getAdminDatabaseClient().from("google_places_budget_alerts").select("billing_month,threshold_pct,spend_usd,credits_remaining_usd,operating_mode,email_sent,created_at").order("created_at", { ascending: false }).limit(20),
  ]);
