...
export default async function DemoReservationPreview({ params }: Props) {
  ...
  const [{ data: session }, { data: locations = [] }] = await Promise.all([
    getAdminDatabaseClient().from("crm_demo_sessions").select("*").eq("id", sessionId).maybeSingle(),
    getAdminDatabaseClient().from("crm_demo_session_locations").select("*").eq("demo_session_id", sessionId).order("created_at"),
  ]);
  if (!session) notFound();
