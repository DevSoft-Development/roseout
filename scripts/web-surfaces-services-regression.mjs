...
requireText(template, "Name: AWS_PLATFORM_JOB_GATEWAY_URL", "Admin ECS task must receive the canonical credential gateway URL directly from its runtime secret.");
requireText(template, "Name: AWS_PLATFORM_JOB_GATEWAY_SECRET", "Admin ECS task must receive the credential gateway secret directly from its runtime secret.");
requireText(template, "ValueFrom: !Sub '${AdminAppEnvSecretArn}:AWS_PLATFORM_JOB_GATEWAY_URL::'", "Admin gateway URL must come from the isolated Admin runtime secret.");
requireText(template, "ValueFrom: !Sub '${AdminAppEnvSecretArn}:AWS_PLATFORM_JOB_GATEWAY_SECRET::'", "Admin gateway secret must come from the isolated Admin runtime secret.");
