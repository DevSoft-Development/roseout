...
export default async function LocationBillingPage({ searchParams }: { searchParams?: SearchParams }) {
  ...
  const message = params.retention === "accepted"
    ? "Your loyalty discount is active and your cancellation was stopped."
    : params.retention === "already_used"
      ...
      : params.cancellation === "scheduled"
        ? `Cancellation is scheduled for ${formatDate(location.current_period_end)}.`
        : null;
  ...
  return <div className="space-y-6 text-white">
    <section className="rounded-3xl border border-white/10 bg-[linear-gradient(135deg,rgba(225,6,42,0.12),rgba(255,255,255,0.03))] p-6 sm:p-8">
      ...
      <div className="mt-3 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div><h1 className="text-3xl font-black sm:text-4xl">{getLocationName(location, "Your location")}</h1><p className="mt-2 max-w-2xl text-sm font-bold leading-6 text-white/55">Manage this location’s Partner Pro subscription, billing cycle, payment method, and cancellation directly from the location workspace.</p></div>
    ...
    <section className="rounded-3xl border border-white/10 bg-white/[0.04] p-6">
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {!location.stripe_subscription_id ? <>
          ...
          {location.cancel_at_period_end ? <form action="/api/business/billing/change-plan" method="POST"><input type="hidden" name="location_id" value={location.id}/><input type="hidden" name="action" value="reactivate"/><button className="w-full rounded-2xl border border-emerald-400/30 px-5 py-4 text-sm font-black text-emerald-100 hover:bg-emerald-500/10">Keep Partner Pro</button></form> : <Link href="/locations/dashboard/billing/cancel" className="flex items-center justify-center rounded-2xl border border-rose-400/30 px-5 py-4 text-center text-sm font-black text-rose-100 hover:bg-rose-500/10">Review cancellation</Link>}
        </>}
      </div>
      {location.cancel_at_period_end ? <p className="mt-5 rounded-2xl border border-amber-300/25 bg-amber-500/10 p-4 text-sm font-bold text-amber-100">Your paid features remain active until {formatDate(location.current_period_end)}. You can reverse the cancellation above before then.</p> : null}
    </section>
  </div>;
}
