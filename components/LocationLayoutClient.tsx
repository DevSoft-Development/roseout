  ...
  return (
    <main className="min-h-screen bg-[#090706] px-4 pb-10 pt-4 text-white sm:px-6 lg:px-8">
      <div className="mx-auto max-w-[1560px]">
        ...
        <section className="mt-5 grid gap-4 rounded-[1.5rem] border border-white/10 bg-white/[0.06] p-4 lg:grid-cols-[0.8fr_1fr_1fr_0.8fr]">
        ...
        <section className={`mt-5 grid gap-5 ${hostessMode ? "xl:grid-cols-[1fr_390px]" : "xl:grid-cols-[1fr_390px_360px]"}`}>
          <div className="rounded-[2rem] border border-white/10 bg-[#f8f3ef] p-5 text-[#1b1210] shadow-2xl">
            ...
            {loading ? (
              ...
              <div className="relative mt-6 min-h-[560px] overflow-auto rounded-[1.5rem] border border-dashed border-black/15 bg-[linear-gradient(rgba(0,0,0,0.05)_1px,transparent_1px),linear-gradient(90deg,rgba(0,0,0,0.05)_1px,transparent_1px)] bg-[size:48px_48px] p-4">
                <div className="relative h-[720px] min-w-[900px]">
          ...
          {!hostessMode ? (
            <aside className="rounded-[2rem] border border-white/10 bg-[#120d0b] p-5 shadow-2xl">
              ...
              <div className="mt-5 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3"><label className="space-y-2"><span className="text-xs font-black uppercase tracking-[0.2em] text-white/45">Capacity</span><input type="number" min="1" value={form.capacity} onChange={(event) => setForm((current) => ({ ...current, capacity: Number(event.target.value) }))} className="w-full rounded-2xl border border-white/10 bg-black/40 px-4 py-3 text-sm font-bold text-white outline-none" /></label><label className="space-y-2"><span className="text-xs font-black uppercase tracking-[0.2em] text-white/45">Status</span><select value={form.status} onChange={(event) => setForm((current) => ({ ...current, status: event.target.value, is_active: !["blocked", "maintenance"].includes(event.target.value) }))} className="w-full rounded-2xl border border-white/10 bg-black/40 px-4 py-3 text-sm font-bold text-white outline-none">{itemStatuses.map((status) => <option key={status} value={status}>{prettyLabel(status)}</option>)}</select></label></div>
                <div className="grid grid-cols-2 gap-3"><label className="space-y-2"><span className="text-xs font-black uppercase tracking-[0.2em] text-white/45">X</span><input type="number" value={form.layout_x} onChange={(event) => setForm((current) => ({ ...current, layout_x: Number(event.target.value) }))} className="w-full rounded-2xl border border-white/10 bg-black/40 px-4 py-3 text-sm font-bold text-white outline-none" /></label><label className="space-y-2"><span className="text-xs font-black uppercase tracking-[0.2em] text-white/45">Y</span><input type="number" value={form.layout_y} onChange={(event) => setForm((current) => ({ ...current, layout_y: Number(event.target.value) }))} className="w-full rounded-2xl border border-white/10 bg-black/40 px-4 py-3 text-sm font-bold text-white outline-none" /></label></div>
                <div className="grid grid-cols-3 gap-3"><label className="space-y-2"><span className="text-xs font-black uppercase tracking-[0.2em] text-white/45">W</span><input type="number" min="1" value={form.layout_width} onChange={(event) => setForm((current) => ({ ...current, layout_width: Number(event.target.value) }))} className="w-full rounded-2xl border border-white/10 bg-black/40 px-4 py-3 text-sm font-bold text-white outline-none" /></label><label className="space-y-2"><span className="text-xs font-black uppercase tracking-[0.2em] text-white/45">H</span><input type="number" min="1" value={form.layout_height} onChange={(event) => setForm((current) => ({ ...current, layout_height: Number(event.target.value) }))} className="w-full rounded-2xl border border-white/10 bg-black/40 px-4 py-3 text-sm font-bold text-white outline-none" /></label><label className="space-y-2"><span className="text-xs font-black uppercase tracking-[0.2em] text-white/45">Rotate</span><input type="number" step="15" value={form.rotation} onChange={(event) => setForm((current) => ({ ...current, rotation: Number(event.target.value) }))} className="w-full rounded-2xl border border-white/10 bg-black/40 px-4 py-3 text-sm font-bold text-white outline-none" /></label></div>
                ...
                <div className="grid grid-cols-3 gap-2"><button onClick={() => selectedItem && saveItem("duplicate_layout_item")} disabled={!selectedItem} className="rounded-2xl border border-white/10 bg-white/[0.07] px-3 py-3 text-xs font-black text-white/70 disabled:opacity-40"><Copy className="mx-auto mb-1 h-4 w-4" />Duplicate</button><button onClick={() => setForm((current) => ({ ...current, rotation: Number(current.rotation || 0) + 15 }))} className="rounded-2xl border border-white/10 bg-white/[0.07] px-3 py-3 text-xs font-black text-white/70"><RotateCw className="mx-auto mb-1 h-4 w-4" />Rotate</button><button onClick={() => selectedItem && deleteItem(selectedItem)} disabled={!selectedItem} className="rounded-2xl border border-red-400/20 bg-red-500/10 px-3 py-3 text-xs font-black text-red-200 disabled:opacity-40"><Trash2 className="mx-auto mb-1 h-4 w-4" />Delete</button></div>
              </div>
            </aside>
          ) : null}
        </section>
