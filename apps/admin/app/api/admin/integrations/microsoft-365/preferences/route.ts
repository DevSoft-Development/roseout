...
export async function POST(request: NextRequest) {
  ...
  const emailMode =
    form.get("email_sync_mode") === "all"
      ? "all"
      : "crm_related_only";

  const calendarDirection = [
    "microsoft_to_theouthaven",
    "theouthaven_to_microsoft",
    "two_way",
  ].includes(String(form.get("calendar_sync_direction")))
  ...
  const taskDirection = [
    ...
    "theouthaven_to_microsoft",
    "two_way",
  ].includes(String(form.get("task_sync_direction")))
    ...
    .upsert(
      {
        ...
        include_internal_mail: checked(
          form,
          "include_internal_mail",
        ),
        sync_attachments: checked(form, "sync_attachments"),
        queue_unmatched_email: checked(
          form,
          "queue_unmatched_email",
        ),
  ...
  return NextResponse.redirect(
    new URL(
      "/admin/dashboard/settings/microsoft-365?saved=1",
      request.url,
    ),
    303,
  );
