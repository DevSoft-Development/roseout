...
export async function GET() {
  ...
  try {
    ...
    const { data: locationClaims, error: locationError } = await supabaseAdmin
      ...
      .select("id, user_id, location_id, location_name, location_type, request_type, address, city, state, zip_code, owner_name, owner_email, owner_phone, notes, status, verification_status, ownership_evidence_type, ownership_evidence_detail, ownership_attested, match_status, confidence_score, plan_interest, plan_interval, role_at_business, matched_location_snapshot, claim_code, submitted_at, created_at")
      .order("created_at", { ascending: false });
