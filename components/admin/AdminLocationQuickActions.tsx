...
import { appendReserveActingContext, getReserveActionLinks, getReserveDashboardUrl, getReservePublicProfileUrl } from "@/lib/reservations/reserveLinks";
...
export default function AdminLocationQuickActions({ locationId, className = "", compact = false, locationType = "restaurant" }: Props) {
  ...
  const actionLinks = getReserveActionLinks({ locationId, locationType: locationType || "restaurant", adminLocationId: locationId });
  ...
  const actions = [
    ...
    { label: "Booking Page", href: actionLinks.bookingHref },
