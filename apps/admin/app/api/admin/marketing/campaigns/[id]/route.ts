...
export async function GET(_req: Request, context: RouteContext) {
  ...
  const { data, error: fetchError } = await getAdminDatabaseClient()
...
export async function PATCH(req: Request, context: RouteContext) {
  ...
  if (needsPublicSlug) {
    const { data: existing } = await getAdminDatabaseClient().from("marketing_campaigns").select("public_slug").eq("id", id).maybeSingle();
...
export async function DELETE(_req: Request, context: RouteContext) {
  ...
  const { id } = await context.params;
  const { error: deleteError } = await getAdminDatabaseClient().from("marketing_campaigns").delete().eq("id", id);
