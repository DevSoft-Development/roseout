...
function enforceHardDistanceRows(
  rows: any[],
  plan: SearchPlan,
  trace: SearchTrace,
  stage: string,
...
async function retrieveLegacyAtSharedLevel(
  ...
  for (const level of levels) {
    const rows = await Promise.all(
      requests.map(async (request) => ({
        ...
        rows: enforceHardDistanceRows(
          await retrieveUnifiedLocations(supabase, request, 60, trace, {
            allowBroaderGeo: false,
            forcedGeoLevel: level,
          }),
          plan,
          trace,
          "legacy_shared_geo",
    ...
    const hasRestaurant = rows.some(
      ({ request, rows }) => retrievalDomain(request.desiredRole) === "restaurant" && rows.length > 0,
    );
    const hasActivity = rows.some(
      ({ request, rows }) => retrievalDomain(request.desiredRole) === "activity" && rows.length > 0,
    );
    ...
    trace.decisions.push({
      stage: "paired_geo_scope",
      decision: viable ? "shared_geo_level_succeeded" : "shared_geo_level_incomplete",
      reason: `level=${level}, restaurant=${hasRestaurant}, activity=${hasActivity}`,
    });
    if (viable) return { level, rows };
    if (
      plan.fallback.allowPartial &&
      rowCount > 0 &&
      (!bestPartial ||
        coveredDomains > bestPartial.coveredDomains ||
        (coveredDomains === bestPartial.coveredDomains && rowCount > bestPartial.rowCount))
    ) {
  ...
  if (bestPartial) {
    trace.decisions.push({
      stage: "paired_geo_scope",
      decision: "best_partial_geo_level_preserved",
      reason: `level=${bestPartial.level}, covered_domains=${bestPartial.coveredDomains}, rows=${bestPartial.rowCount}`,
    });
    return { level: bestPartial.level, rows: bestPartial.rows };
  }
  return {
    level: null as GeoLevel | null,
    rows: requests.map((request) => ({ request, rows: [] as any[] })),
  };
}
...
export async function retrieveCandidates({
  ...
  if (paired && !strictNoFallback && !rollout.serveProfiles) {
    sharedLegacy = await retrieveLegacyAtSharedLevel(supabase, requests, plan, trace);
  }
  ...
  const lanes = await Promise.all(
    requests.map(async (request) => {
      ...
      if (rollout.serveProfiles || rollout.shadowProfiles) {
        try {
          profileRows = enforceHardDistanceRows(
            await retrieveProfileLocations(
              supabase,
              request,
              60,
              ...
              (attempt) => {
                trace.decisions.push({
                  ...
                  decision: attempt.error
                    ? "profile_attempt_failed"
                    : attempt.resultCount
                      ? "profile_attempt_succeeded"
                      : "profile_attempt_empty",
                  reason: JSON.stringify(attempt),
                });
              },
            ),
            plan,
      ...
      const legacyAllowed =
        !strictNoFallback &&
        (!rollout.serveProfiles || rollout.shadowProfiles || profileMissingForLane);

      if (legacyAllowed) {
        const shared = sharedLegacy?.rows.find((lane) => lane.request === request);
        const rawLegacyRows = shared
          ? shared.rows
          : await retrieveUnifiedLocations(supabase, request, 60, trace, {
              allowBroaderGeo:
                candidateOriginDistanceIsHard(plan) ? false : plan.fallback.allowBroaderGeo,
            });
      ...
      const useFallback =
        !strictNoFallback &&
        rollout.serveProfiles &&
        profileMissingForLane &&
        legacyRows.length > 0;

      if (useFallback) {
        trace.retrieval.legacyFallbackUsed = true;
        trace.retrieval.fallbackDomains = [
          ...new Set([...trace.retrieval.fallbackDomains, domain]),
        ];
        trace.decisions.push({
          stage: "retrieval",
          decision: "missing_profile_lane_legacy_recovery",
          reason: JSON.stringify({
            domain,
            desiredRole: request.desiredRole,
            profileCandidateCount: profileRows.length,
            legacyCandidateCount: legacyRows.length,
            requestedAreaRadiusMiles: request.geo.radiusMiles,
            pairMaxWalkingMinutes: plan.pairing.maxWalkingMinutes,
            pairMaxDistanceMiles: plan.pairing.maxDistanceMiles,
          }),
        });
      }

      const servedRows = rollout.serveProfiles
        ? profileRows.length
          ? profileRows
          : strictNoFallback
            ? []
      ...
      trace.retrievalCalls.push({
        ...
        reason: sharedLegacy?.level
          ? `legacy_shared_geo_${sharedLegacy.level}`
          : strictNoFallback && rollout.serveProfiles && profileRows.length === 0
      ...
      return servedRows.map((location) =>
        candidateFrom(
          location,
          request,
  ...
  for (const item of lanes.flat()) {
    const lane = item.requestedRoles.some((role) => retrievalDomain(role) === "restaurant")
      ? "restaurant"
      : "activity";
    const key = `${lane}:${String(item.location.id)}`;
    const previous = byLaneAndId.get(key);
    if (previous) {
      previous.retrievalSources = [
        ...new Set([...previous.retrievalSources, ...item.retrievalSources]),
      ];
      previous.requestedRoles = [
        ...new Set([...previous.requestedRoles, ...item.requestedRoles]),
      ];
      previous.matchedRetrievalTerms = [
        ...new Set([...previous.matchedRetrievalTerms, ...item.matchedRetrievalTerms]),
      ];
    } else {
  ...
  trace.retrieval.servedSource = strictNoFallback
    ? "canonical_profile"
    : trace.retrieval.legacyFallbackUsed
      ? "mixed"
      : rollout.serveProfiles
        ? "canonical_profile"
        : "legacy";
