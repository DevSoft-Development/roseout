...
export async function getSearchHealthDashboardData(filters: SearchHealthFilters) {
  let searches = getAdminDatabaseClient()
    .from("search_events")
    .select(
      [
        "id",
  ...
  let issues = getAdminDatabaseClient()
  ...
  let trendQuery = getAdminDatabaseClient()
    .from("search_events")
    .select(
  ...
  const [searchResult, issueResult, kpiResult, trendResult] = await Promise.all([
    searches,
    issues,
    getAdminDatabaseClient().rpc("admin_search_health_kpis", {
      p_from: filters.from,
      p_to: filters.to,
