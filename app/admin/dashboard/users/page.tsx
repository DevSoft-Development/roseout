...
async function updateUserRole(formData: FormData) {
  ...
  redirect(
    `/admin/dashboard/users?q=${encodeURIComponent(q)}&role=${encodeURIComponent(currentRole)}`
  );
}
      ...
      <div className="mx-auto max-w-[1500px]">
        <section className="relative overflow-hidden rounded-[1.75rem] border border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(225,29,72,0.22),transparent_35%),linear-gradient(135deg,#160b0b,#090706_55%,#140f0a)] p-6 shadow-2xl">
          ...
          <div className="relative z-10 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            ...
            <div className="flex flex-wrap gap-3">
              <Link
                href="/admin/dashboard/users?role=all"
                className="rounded-full border border-white/10 bg-white/[0.07] px-6 py-3 text-sm font-black text-white/75 transition hover:bg-white hover:text-black"
              >
                {currentAdmin.role === "superuser"
                  ? "Edit superusers & down"
                  : "Edit admins & down"}
              </Link>
        ...
        <section className="mt-5 rounded-[1.75rem] border border-white/10 bg-[#f8f3ef] text-[#1b1210] shadow-2xl">
          <div className="flex flex-col gap-3 border-b border-black/10 bg-[#fffaf6] p-4 md:flex-row md:items-center md:justify-between">
            ...
            <Link href="/admin/dashboard/users/new" className="rounded-full bg-[#1b1210] px-4 py-2 text-xs font-black text-white">
              + Add User
            </Link>
          </div>
                ...
                return (
                  <div key={user.id} className="rounded-[1.5rem] border border-black/10 bg-white p-4 shadow-sm transition hover:shadow-xl">
                    <div className="grid gap-4 xl:grid-cols-[1fr_300px_370px] xl:items-center">
                      <div>
                        <Link href={`/admin/dashboard/users/${user.id}`} className="truncate font-black hover:text-rose-700">
                      ...
                      <div className="flex flex-wrap gap-2 xl:justify-end">
                        {canManage ? (
                          <Link
                            ...
                            className="rounded-full border border-black/10 bg-white px-4 py-3 text-xs font-black text-[#1b1210] transition hover:bg-rose-50"
                          >
                            Edit
                          </Link>
                        ) : (
