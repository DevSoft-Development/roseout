...
const refundMigration = read('supabase/migrations_archive/pre_virginia_baseline/20260822201331_stripe_e2e_refund_audit.sql');
const depositMigration = read('supabase/migrations_archive/pre_virginia_baseline/20260812110000_stripe_connect_reservation_deposits.sql');
...
const checks = [
  ...
  ['webhook resolves subscription id', webhook, /o\?\.object === "subscription"/, /eq\("stripe_subscription_id", subscriptionId\)/],
  ['webhook only upgrades subscription checkout sessions', webhook, /metadata\.plan === "business_pro" \|\| metadata\.source === "business_billing"/, /subscription_plan: "business_pro"/],
  ...
  ['organizer Connect return is synchronized', organizerOnboard, /api\/organizers\/stripe-connect\/return/, organizerReturn, /retrieveConnectAccountState/, /connectStateUpdate/, connectStatus, /stripe_connect_charges_enabled/, /stripe_connect_payouts_enabled/],
  ...
  ['Connect migration defaults deposits off', depositMigration, /deposits_enabled set default false/, /locations_deposit_opt_in_check/],
