...
import { getReserveBookingUrl, getReserveDashboardUrl, getReserveEmbedUrl, getReserveQrUrl, reserveQuery } from "@/lib/reservations/reserveLinks";
...
function ReserveCommandCenterContent() {
  ...
  function switchTab(tab:string){ if(!validTabs.has(tab)) tab="today"; setActiveTab(tab); const q=reserveQuery({ adminLocationId: adminLocationId||undefined, locationId: !adminLocationId?locationId:undefined, type: locationType, tab, date:selectedDate }); router.replace(`/reserve/dashboard${q}`, { scroll:false }); }
