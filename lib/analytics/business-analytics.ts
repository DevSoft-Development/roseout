...
type TrackLocationAnalyticsEventInput = {
  ...
  trafficType?: "production" | "internal_test" | null;
  isTestEvent?: boolean;
...
async function updateDailyAnalytics(input: TrackLocationAnalyticsEventInput, now: Date) {
  ...
  const revenue = input.eventType === "reservation_completed" ? safeNumber(input.metadata?.amount_paid) : 0;
  const { data: existing } = await supabaseAdmin.from("location_daily_analytics").select("*").eq("location_id", input.locationId).eq("analytics_date", analyticsDate).maybeSingle();
  const reservationCompletions = safeNumber(existing?.reservation_completions) + inc.reservation_completions;
  ...
  const row = {
    ...
    search_appearances: safeNumber(existing?.search_appearances) + inc.search_appearances,
    search_clicks: safeNumber(existing?.search_clicks) + inc.search_clicks,
    directions_clicks: safeNumber(existing?.directions_clicks) + inc.directions_clicks,
    ...
    reservation_starts: safeNumber(existing?.reservation_starts) + inc.reservation_starts,
    reservation_completions: reservationCompletions,
    reservation_cancellations: safeNumber(existing?.reservation_cancellations) + inc.reservation_cancellations,
  ...
  if (existing?.id) {
    await supabaseAdmin.from("location_daily_analytics").update(row).eq("id", existing.id).throwOnError();
    return;
  }
...
async function updateHourlyAnalytics(input: TrackLocationAnalyticsEventInput, now: Date) {
  ...
  const { data: existing } = await supabaseAdmin.from("location_hourly_analytics").select("*").eq("location_id", input.locationId).eq("day_of_week", dayOfWeek).eq("hour_of_day", hourOfDay).maybeSingle();
  const row = {
    location_id: input.locationId,
  ...
  if (existing?.id) {
    await supabaseAdmin.from("location_hourly_analytics").update(row).eq("id", existing.id).throwOnError();
    return;
  }
...
async function updateCustomerInsights(input: TrackLocationAnalyticsEventInput, now: Date) {
  ...
  const { data: existing } = await supabaseAdmin.from("location_customer_insights").select("*").eq("location_id", input.locationId).eq("user_id", input.userId).maybeSingle();
  const isReservation = input.eventType === "reservation_completed";
  const isCancellation = input.eventType === "reservation_cancelled";
  const isVisit = ["profile_view", "search_click", "reservation_started"].includes(input.eventType);
  ...
  const row = {
    ...
    metadata: { ...(typeof existing?.metadata === "object" && existing.metadata ? existing.metadata : {}), ...(input.metadata || {}) },
    updated_at: now.toISOString(),
  };
  if (existing?.id) {
    await supabaseAdmin.from("location_customer_insights").update(row).eq("id", existing.id).throwOnError();
    return;
  }
  await supabaseAdmin.from("location_customer_insights").insert({ ...row, first_seen_at: now.toISOString(), visit_count: Math.max(row.visit_count, 1) }).throwOnError();
}
...
export function isBusinessPro(locationOrBusiness: Record<string, unknown> | null | undefined) {
  const values = [locationOrBusiness?.plan, locationOrBusiness?.business_plan, locationOrBusiness?.subscription_plan, locationOrBusiness?.pricing_plan, locationOrBusiness?.tier, locationOrBusiness?.subscription_tier].filter(Boolean).map((value) => String(value).toLowerCase());
  const status = String(locationOrBusiness?.subscription_status || locationOrBusiness?.plan_status || "").toLowerCase();
  return values.some((value) => ["pro", "premium", "business_pro"].includes(value)) || (values.some((value) => value.includes("pro")) && status !== "cancelled");
}
