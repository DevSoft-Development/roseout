...
export default async function SocialManagerPage() {
  ...
  const [conversationResult, opportunityResult, growthResult, scheduledResult, approvalResult] = await Promise.all([
    supabaseAdmin.from("social_community_conversations").select("id,status,person_type,intent,area,timing,opportunity_strength,opportunity_score,risk_level,last_message_at,social_community_contacts(username,display_name)").order("last_message_at", { ascending: false }).limit(100),
    supabaseAdmin.from("social_growth_opportunities").select("id,provider,title,summary,area,timing,opportunity_strength,score,status").eq("status", "new").order("score", { ascending: false }).limit(12),
    supabaseAdmin.from("social_growth_daily_snapshots").select("followers_gained,followers_lost,reach,profile_visits,website_visits,outing_searches,business_leads,paid_customers,new_mrr,top_theme,top_area").gte("snapshot_date", since.slice(0, 10)),
    supabaseAdmin.from("marketing_content_items").select("id", { count: "exact", head: true }).eq("status", "scheduled"),
    supabaseAdmin.from("marketing_approvals").select("id", { count: "exact", head: true }).eq("status", "pending"),
  ]);
  ...
  return <main className="min-h-screen bg-[#090706] px-4 py-6 text-white sm:px-6 lg:px-8">
    <div className="mx-auto max-w-[1500px] space-y-6">
      <section className="rounded-[2rem] border border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(225,29,72,0.28),transparent_34%),linear-gradient(135deg,#170b0b,#090706_58%,#14100c)] p-6 sm:p-8">
        ...
        <div className="mt-6 flex flex-wrap gap-3"><Link href="/admin/dashboard/marketing/content" className="rounded-full bg-rose-600 px-5 py-3 text-sm font-semibold">Create Post</Link><Link href="/admin/dashboard/marketing/community" className="rounded-full border border-white/15 px-5 py-3 text-sm font-semibold">Review Messages</Link><Link href="/admin/dashboard/marketing/growth" className="rounded-full border border-white/15 px-5 py-3 text-sm font-semibold">View Growth</Link><Link href="/admin/dashboard/marketing/social-manager/settings" className="rounded-full border border-white/15 px-5 py-3 text-sm font-semibold">AI Settings</Link></div>
      </section>
