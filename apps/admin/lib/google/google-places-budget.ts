...
export function normalizeGooglePlacesBudget(
  value: unknown,
): GooglePlacesBudgetConfig {
  const raw =
    value && typeof value === "object"
      ? (value as Record<string, unknown>)
      : {};

  let hardCapUsd = money(
    raw.hardCapUsd,
    DEFAULT_GOOGLE_PLACES_BUDGET.hardCapUsd,
  );
  let softCapUsd = money(
    raw.softCapUsd,
    Math.min(DEFAULT_GOOGLE_PLACES_BUDGET.softCapUsd, hardCapUsd),
  );
  let targetUsd = money(
    raw.targetUsd,
    Math.min(DEFAULT_GOOGLE_PLACES_BUDGET.targetUsd, softCapUsd),
  );
  ...
  return {
    ...
    softCapUsd,
    hardCapUsd,
    creditBalanceUsd: money(
      raw.creditBalanceUsd,
      DEFAULT_GOOGLE_PLACES_BUDGET.creditBalanceUsd,
    ),
    openingSpendUsd: money(
      raw.openingSpendUsd,
      DEFAULT_GOOGLE_PLACES_BUDGET.openingSpendUsd,
    ),
    openingSpendMonth: month(raw.openingSpendMonth),
    minimumCreditReserveUsd: money(
      raw.minimumCreditReserveUsd,
      DEFAULT_GOOGLE_PLACES_BUDGET.minimumCreditReserveUsd,
    ),
