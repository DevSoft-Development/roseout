...
export const knownCronSources: KnownCronSource[] = [
  ...
  { job_key: "admin-cron-digest-email", source: "edge_function", route_path: "supabase/functions/admin-cron-digest-email", schedule_detected: true, schedule_hint: "pg_cron: 45 6 * * *", logger_expected: true, notes: "Production overnight cron activity digest Edge Function." },
  ...
  { job_key: "reservation-daily-digest", source: "edge_function", route_path: "supabase/functions/reservation-daily-digest", schedule_detected: true, schedule_hint: "pg_cron: 30 6 * * *", logger_expected: true, notes: "Production reservation daily digest Edge Function." },
