...
export type OutingRow = Record<string, any> & { location_id?: string | null; source_location_id?: string | null; status?: string | null; saved_at?: string | null; completed_at?: string | null; completed_no_feedback_at?: string | null; attendance_confirmed_at?: string | null; rating?: number | null; matched_vibe?: boolean | null; would_go_again?: boolean | null; contact_method?: string | null; reservation_type?: string | null; created_at?: string | null };
...
export function buildAnalyticsSummary(events: AnalyticsEventRow[], outings: OutingRow[]) { const names=events.map(normalizeEventName); const completed = outings.filter(o=>o.status==="completed"||!!o.completed_at||!!o.attendance_confirmed_at); const completedNoFeedback = outings.filter(o=>o.status==="completed_no_feedback"||!!o.completed_no_feedback_at); const savedPlans = Math.max(names.filter(n=>["plan_saved","guest_plan_saved","outing_plan_created","guest_plan_created"].includes(n)).length, outings.filter(o=>!!o.saved_at).length); const starts = Math.max(names.filter(n=>["outing_started","plan_saved","guest_plan_saved","outing_plan_created","guest_plan_created"].includes(n)).length, savedPlans); const outboundClicks=names.filter(n=>n.startsWith("outing_")&&n.endsWith("_clicked")).length; return { profile_views: events.filter(e=>["profile_view","location_profile_view","profile_viewed"].includes(normalizeEventName(e))).length, search_appearances: events.filter(e=>["search_appearance","location_impression","search_match"].includes(normalizeEventName(e))).length, search_clicks: events.filter(e=>["search_click","location_click","restaurant_click","activity_click"].includes(normalizeEventName(e))).length, reserve_clicks: events.filter(e=>["reserve_clicked","reservation_clicked","external_reservation_clicked","reservation_started","outing_reservation_clicked"].includes(normalizeEventName(e))).length, directions_clicks: events.filter(e=>["directions_click","outing_directions_clicked"].includes(normalizeEventName(e))).length, phone_clicks: events.filter(e=>["phone_click","phone_clicked","outing_phone_clicked"].includes(normalizeEventName(e))).length, website_clicks: events.filter(e=>["website_click","outing_website_clicked"].includes(normalizeEventName(e))).length, call_clicks: events.filter(e=>["call_clicked","phone_click","phone_clicked","outing_phone_clicked"].includes(normalizeEventName(e))).length, saved_plans: savedPlans, outbound_clicks: outboundClicks, completion_signals: completed.length + completedNoFeedback.length, outing_starts: starts, completed_outings: completed.length + completedNoFeedback.length, average_rating: average(completed.map(o=>o.rating)), matched_vibe_percentage: pct(completed.filter(o=>o.matched_vibe===true).length, completed.length), would_go_again_percentage: pct(completed.filter(o=>o.would_go_again===true).length, completed.length), plan_conversion_rate: pct(savedPlans, events.filter(e=>normalizeEventName(e).includes("search")).length), link_click_rate: pct(outboundClicks, savedPlans), completion_rate: pct(completed.length + completedNoFeedback.length, starts), action_rate: pct(events.filter(e=>["reserve_clicked","call_clicked","reservation_started","phone_click","outing_reservation_clicked","outing_phone_clicked"].includes(normalizeEventName(e))).length, events.filter(e=>["profile_view","location_profile_view","profile_viewed"].includes(normalizeEventName(e))).length) }; }

export const buildDailySeries = (events: AnalyticsEventRow[]) => Object.values(events.reduce((a,e)=>{
  const d=String(e.created_at||"").slice(0,10)||"Unknown";
  const n=normalizeEventName(e);
  const row=(a[d]=a[d]||{date:d,events:0,profile_views:0,search_appearances:0,search_clicks:0,reserve_clicks:0,directions_clicks:0,phone_clicks:0,website_clicks:0,call_clicks:0});
  row.events++;
  if (["profile_view","location_profile_view","profile_viewed"].includes(n)) row.profile_views++;
  if (["search_appearance","location_impression","search_match"].includes(n)) row.search_appearances++;
  if (["search_click","location_click","restaurant_click","activity_click"].includes(n)) row.search_clicks++;
  if (["reserve_clicked","reservation_clicked","external_reservation_clicked","reservation_started","outing_reservation_clicked"].includes(n)) row.reserve_clicks++;
  if (["directions_click","outing_directions_clicked"].includes(n)) row.directions_clicks++;
  if (["phone_click","phone_clicked","outing_phone_clicked"].includes(n)) { row.phone_clicks++; row.call_clicks++; }
  if (["call_clicked"].includes(n)) row.call_clicks++;
  if (["website_click","outing_website_clicked"].includes(n)) row.website_clicks++;
  return a;
},{} as Record<string, any>)).sort((a:any,b:any)=>String(a.date).localeCompare(String(b.date)));
