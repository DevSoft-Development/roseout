...
export async function GET() {
  ...
  const { data: websites, error } = await adminDb
    .from("business_websites")
    .select(
      "id,location_id,site_title,domain,platform_domain,status,deployment_status,last_publish_status,published_version,published_at,created_at,updated_at",
    )
  ...
  if (error) {
    ...
    return NextResponse.json(
      { ok: false, error: "Unable to load generated websites." },
      { status: 500 },
    );
  }
    ...
    if (locationError) {
      ...
      locationNames = new Map(
        (locations || []).map((location) => [
          String(location.id),
          String(
            location.name || location.title || "Untitled location",
          ),
        ]),
      );
    }
  ...
  return NextResponse.json({
    ...
    websites: (websites || []).map((row) => ({
      ...row,
      location_name:
        locationNames.get(String(row.location_id)) ||
        row.site_title ||
        "Untitled location",
    })),
  });
...
export async function DELETE(request: NextRequest) {
  ...
  if (!websiteId || !locationId) {
    return NextResponse.json(
      { ok: false, error: "Choose a location website to delete." },
      { status: 400 },
    );
  }
  if (confirmation !== "DELETE") {
    return NextResponse.json(
      {
        ok: false,
        error: "Type DELETE to confirm this website reset.",
      },
      { status: 400 },
    );
  }

  const { data: website, error: lookupError } = await adminDb
    .from("business_websites")
    .select(
      "id,location_id,site_title,domain,platform_domain,last_publish_status,published_version",
    )
    .eq("id", websiteId)
  ...
  if (lookupError) {
    ...
    return NextResponse.json(
      { ok: false, error: "Unable to verify this website." },
      { status: 500 },
    );
  }
  if (!website) {
  ...
  if (website.last_publish_status === "publishing") {
    return NextResponse.json(
      {
        ok: false,
        error: "Wait for the current publish to finish before deleting this website.",
      },
      { status: 409 },
    );
  }
  ...
  if (deleteError) {
    ...
    return NextResponse.json(
      { ok: false, error: "Unable to delete this location website." },
      { status: 500 },
    );
  }
