...
export default function ReserveLargeGroupSettings({ locationId }: { locationId: string }) {
  ...
  async function load() {
    ...
    if (!response.ok) throw new Error(payload.error || "Unable to load large-group settings.");
    setData(payload);
  ...
  if (!form) return <p className="reserve-muted text-sm">Loading large-group settings…</p>;
  ...
  async function save() {
    ...
    try {
      ...
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error || "Unable to save large-group settings.");
      setData(payload);
      setForm(payload.location);
      setMessage({ tone: "success", text: "Large-group booking settings saved." });
    } catch (error) {
  ...
  return <div className="space-y-5">
    <section className="reserve-card rounded-[2rem] p-5">
      <p className="text-xs font-black uppercase reserve-muted">Large Group Booking</p>
      <div className="mt-2 flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black">Live large-party reservations</h1>
          <p className="mt-1 max-w-2xl text-sm reserve-muted">When enabled, guests see live inventory on the public location Reserve flow and on your hosted website. A confirmed group booking uses the same capacity and Live Tables inventory as regular Reserve bookings.</p>
        </div>
        <label className="reserve-soft flex items-center gap-3 rounded-full px-4 py-3 text-sm font-black">
          <input type="checkbox" checked={Boolean(form.large_group_booking_enabled)} onChange={(event) => set("large_group_booking_enabled", event.target.checked)} /> Accept large groups
        </label>
      </div>
    </section>
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <section className="reserve-card rounded-[2rem] p-5">
        <h2 className="text-lg font-black">Group size & timing</h2>
      ...
      <section className="reserve-card rounded-[2rem] p-5">
        ...
        {form.large_group_prix_fixe_mode === "required" ? <p className="mt-3 rounded-xl border border-amber-300/20 bg-amber-300/10 p-3 text-xs font-bold text-amber-100">If the group package requires payment for the meal/package itself, create or convert it to a Group Dining Experience. Reserve only handles the table booking and optional booking deposit.</p> : null}
      </section>
    </div>

    <section className="reserve-card rounded-[2rem] p-5">
      <h2 className="text-lg font-black">Large-group payment protection</h2>
      <p className="mt-1 text-sm reserve-muted">Regular reservations should use card guarantees. Large groups can additionally require a real booking deposit.</p>
      ...
      {form.large_group_payment_mode === "deposit" ? <p className={`mt-3 rounded-xl border p-3 text-xs font-bold ${data?.stripeReady ? "border-emerald-300/20 bg-emerald-300/10 text-emerald-100" : "border-rose-300/20 bg-rose-300/10 text-rose-100"}`}>{data?.stripeReady ? `Stripe is ready. Guests will pay ${depositPreview} before the large-group booking is secured.` : "Stripe onboarding is not complete. Deposit mode cannot be saved until charges and payouts are enabled."}</p> : null}
    </section>

    <section className="reserve-card rounded-[2rem] p-5">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-lg font-black">Regular reservation card guarantee</h2><p className="mt-1 max-w-2xl text-sm reserve-muted">No money is charged when the guest books. The saved card can only be charged under the location’s disclosed late-cancellation/no-show policy.</p></div><label className="reserve-soft flex items-center gap-3 rounded-full px-4 py-3 text-sm font-black"><input type="checkbox" checked={Boolean(form.reservation_guarantee_enabled)} onChange={(e) => set("reservation_guarantee_enabled", e.target.checked)} /> Card guarantee</label></div>
      <div className="mt-4 grid gap-3 md:grid-cols-3">
        ...
        <label className="text-xs font-bold">Late-cancel fee<select className={input} value={form.reservation_late_cancel_fee_type || "flat"} onChange={(e) => set("reservation_late_cancel_fee_type", e.target.value)}><option value="flat">Flat fee</option><option value="per_person">Per person</option></select><input className={input} type="number" min="0" step="1" value={dollars(form.reservation_late_cancel_fee_cents)} onChange={(e) => set("reservation_late_cancel_fee_cents", cents(e.target.value))} /></label>
        <label className="text-xs font-bold">No-show fee<select className={input} value={form.reservation_no_show_fee_type || "flat"} onChange={(e) => set("reservation_no_show_fee_type", e.target.value)}><option value="flat">Flat fee</option><option value="per_person">Per person</option></select><input className={input} type="number" min="0" step="1" value={dollars(form.reservation_no_show_fee_cents)} onChange={(e) => set("reservation_no_show_fee_cents", cents(e.target.value))} /></label>
      </div>
    </section>
    ...
    <button type="button" onClick={save} disabled={saving} className="reserve-primary rounded-full px-5 py-3 text-sm font-black disabled:opacity-50">{saving ? "Saving…" : "Save large-group settings"}</button>
  </div>;
}
