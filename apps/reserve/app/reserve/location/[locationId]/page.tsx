...
import type { SeatingPreference } from "@/lib/reservations/seatingPreference";
...
type SeatingChoice = "" | SeatingPreference;
type SeatingInventory = { dining: boolean; bar: boolean };
...
export default function ReserveLocationPage() {
  ...
  const [seatingPreference, setSeatingPreference] = useState<SeatingChoice>("");
  const [seatingInventory, setSeatingInventory] = useState<SeatingInventory>({ dining: false, bar: false });
  ...
  const slotMap = useMemo(() => new Map(currentSlots.map((slot) => [slot.time, slot])), [currentSlots]);
  const availableSlots = useMemo(
    () => filteredTimes.map((time) => slotMap.get(time)).filter((slot): slot is Slot => Boolean(slot)),
    [filteredTimes, slotMap],
  );
  const visibleSlots = showAllTimes ? availableSlots : availableSlots.slice(0, INITIAL_VISIBLE_TIMES);
  const hiddenTimeCount = Math.max(0, availableSlots.length - INITIAL_VISIBLE_TIMES);
  ...
  const showSeatingChoice = seatingInventory.dining && seatingInventory.bar;
  ...
  async function filterTimes(nextSlots: Slot[], preference: SeatingChoice, quiet = false) {
    const times = nextSlots.map((slot) => slot.time);
    if (!times.length) {
      setFilteredTimes([]);
      setSeatingInventory({ dining: false, bar: false });
      return;
    }
    try {
      if (quiet) setChecking(true); else setSeatingLoading(true);
      setError("");
      ...
      const inventory = {
        dining: Boolean(data.dining_inventory),
        bar: Boolean(data.bar_inventory),
      };
      setSeatingInventory(inventory);

      let resolvedPreference: SeatingChoice = preference;
      if (!inventory.dining && inventory.bar) resolvedPreference = "bar";
      if (inventory.dining && !inventory.bar) resolvedPreference = "dining";
      if (!inventory.dining && !inventory.bar) resolvedPreference = "";

      if (!preference && inventory.dining && inventory.bar) {
        setSeatingPreference("");
        setFilteredTimes([]);
      } else {
        setSeatingPreference(resolvedPreference);
        setFilteredTimes(Array.isArray(data.available_times) ? data.available_times : []);
      }
    } catch (err: any) {
      setFilteredTimes([]);
  ...
  async function loadData(quiet = false) {
    try {
      ...
      setItems(nextItems);
      await filterTimes(nextSlots, "", quiet);
    } catch (err: any) {
  ...
  function chooseSeating(preference: SeatingPreference) {
    setSelectedTime("");
    setShowAllTimes(false);
    setSeatingPreference(preference);
    void filterTimes(currentSlots, preference);
  }

  function continueToBooking(time: string) {
    if (!seatingPreference) return;
    const query = new URLSearchParams({
      type: locationType,
      date,
      partySize: String(partySize),
        ...
        <div className="mx-auto max-w-7xl px-4 pb-16 sm:px-6 lg:px-8">
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_420px] lg:items-start">
            ...
            <section className="rounded-3xl border border-white/10 bg-zinc-950 p-5 shadow-2xl lg:sticky lg:top-24">
              {loading ? <div className="flex min-h-[360px] items-center justify-center"><div className="text-center"><Loader2 className="mx-auto animate-spin text-red-400" size={32} /><p className="mt-4 text-sm font-bold text-white/55">Loading reservation options...</p></div></div> : (
                <div className="space-y-5">
                  ...
                  <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      {(checking || seatingLoading) && <span className="inline-flex items-center gap-1 text-xs font-bold text-red-300"><RefreshCw size={12} className="animate-spin" /> Checking</span>}
                    </div>
                    {showSeatingChoice ? (
                      <>
                        <p className="mt-2 text-xs leading-5 text-white/45">Choose your seating area first. Available times below will match that choice. Exact table or stool placement stays with the venue.</p>
                        <div className="mt-3 grid grid-cols-3 gap-2">
                          {([['any', 'No preference'], ['dining', 'Table seating'], ['bar', 'Bar seating']] as [SeatingPreference, string][]).map(([value, label]) => (
                            <button key={value} type="button" onClick={() => chooseSeating(value)} aria-pressed={seatingPreference === value} className={`rounded-xl border px-3 py-3 text-xs font-black transition ${seatingPreference === value ? "border-red-500 bg-red-600 text-white" : "border-white/10 bg-black/25 text-white/70 hover:border-red-500/50"}`}>{label}</button>
                          ))}
                        </div>
                      </>
                    ) : seatingInventory.bar && !seatingInventory.dining ? (
                      <p className="mt-2 text-xs leading-5 text-white/55">Bar seating is the available seating type for this party size. Available times are filtered automatically.</p>
                    ) : seatingInventory.dining && !seatingInventory.bar ? (
                      <p className="mt-2 text-xs leading-5 text-white/55">Table seating is the available seating type for this party size. Available times are filtered automatically.</p>
                    ) : (
                      <p className="mt-2 text-xs leading-5 text-white/45">No reservable seating is currently configured for this party size.</p>
                    )}
                  </div>

                  <div>
                    <div className="flex items-center justify-between gap-3"><p className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.2em] text-white/45"><Clock size={15} /> Available times</p></div>
                    {showSeatingChoice && !seatingPreference ? (
                      <div className="mt-3 rounded-2xl border border-white/10 bg-white/[0.035] p-4 text-sm font-semibold text-white/55">Choose a seating preference to see matching times.</div>
                    ) : availableSlots.length ? (
                      <><div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3">{visibleSlots.map((slot) => <button key={slot.time} type="button" onClick={() => setSelectedTime(slot.time)} className={`rounded-xl border px-3 py-3 text-center text-sm font-black transition ${selectedTime === slot.time ? "border-red-500 bg-red-600 text-white" : "border-white/10 bg-white/[0.05] text-white/75 hover:border-red-500/50 hover:bg-red-500/10"}`}>{slot.label}</button>)}</div>{hiddenTimeCount > 0 && <button type="button" onClick={() => setShowAllTimes((value) => !value)} className="mt-3 w-full rounded-xl border border-white/10 px-4 py-3 text-xs font-black text-white/60 transition hover:text-white">{showAllTimes ? "Show fewer times" : `Show ${hiddenTimeCount} more times`}</button>}</>
                    ) : seatingPreference || !showSeatingChoice ? (
                      <div className="mt-3 rounded-2xl border border-yellow-500/20 bg-yellow-500/10 p-4 text-sm font-semibold text-yellow-100">No times are available for this seating choice on this date. Try another seating preference, date, or party size.</div>
                    ) : null}
                  </div>

                  <button type="button" disabled={!selectedTime || !seatingPreference || seatingLoading || checking} onClick={() => continueToBooking(selectedTime)} className="w-full rounded-full bg-red-600 p-4 font-black text-white transition hover:bg-red-500 disabled:cursor-not-allowed disabled:opacity-50">
                    Continue with {slotMap.get(selectedTime)?.label || "selected time"}
                  </button>
                  <p className="text-center text-xs leading-5 text-white/35">Times are filtered by your selected seating preference and live availability. Your reservation is not held until you complete the next step.</p>
                </div>
              )}
            </section>
          </div>
