...
export async function GET() {
  ...
  const [{ data: campaigns, error }, { data: events }, { data: attributions }, { data: ledger }] = await Promise.all([
    supabaseAdmin.from("promotion_campaigns").select("*").order("created_at", { ascending: false }).limit(500),
    supabaseAdmin.from("promotion_events").select("campaign_id,event_type,amount_cents").order("occurred_at", { ascending: false }).limit(50000),
    supabaseAdmin.from("promotion_attributions").select("campaign_id,conversion_type,attributed_revenue_cents").order("attributed_at", { ascending: false }).limit(50000),
    supabaseAdmin.from("promotion_ledger_entries").select("campaign_id,entry_type,amount_cents").order("created_at", { ascending: false }).limit(50000),
  ]);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });

  const locationIds = [...new Set((campaigns || []).map((row: any) => String(row.location_id)))];
  const { data: locations } = locationIds.length ? await supabaseAdmin.from("locations").select("id,name,restaurant_name,activity_name,city,state").in("id", locationIds) : { data: [] as any[] };
...
export async function PATCH(request: Request) {
  ...
  const { data: campaign } = await supabaseAdmin.from("promotion_campaigns").select("id,funded_at,starts_at,total_budget_cents,spent_cents").eq("id", id).maybeSingle();
  ...
  const { error } = await supabaseAdmin.from("promotion_campaigns").update({ status, paused_at: status === "paused" ? new Date().toISOString() : null, updated_at: new Date().toISOString() }).eq("id", id);
