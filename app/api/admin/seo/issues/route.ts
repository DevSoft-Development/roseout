...
export async function GET(){const a=await requireAdminApiRole(ADMIN_PAGE_ACCESS.seoTools); if(a.error)return a.error; const {data,error}=await supabaseAdmin.from('seo_audit_issues').select('*').order('created_at',{ascending:false}).limit(300); if(error) return NextResponse.json({error:error.message},{status:500}); return NextResponse.json({issues:data||[]});}
