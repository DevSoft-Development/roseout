...
import type { EmailCta, EmailDepartment, RenderedEmail } from "./types";
...
export async function sendRawBrandedEmail(params: { to?: string | string[] | null; subject: string; heading?: string; preview?: string; body: string; cta?: EmailCta; department?: EmailDepartment | string; replyTo?: string; cc?: string | string[]; bcc?: string | string[] }) {
  const department = (params.department || "account") as EmailDepartment;
  const rendered = renderBrandedEmail({ department, subject: params.subject, preview: params.preview || params.subject, heading: params.heading || params.subject, intro: params.body, sections: [], cta: params.cta });
