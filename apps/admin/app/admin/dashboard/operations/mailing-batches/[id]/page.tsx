  ...
  return (
    <AdminPageShell>
      <AdminPageHeader
        ...
        title={batch.name}
        subtitle={`Created ${fmt(batch.created_at)} · Planned mail date ${batch.planned_mail_date || "not set"}${batch.notes ? ` · ${batch.notes}` : ""}`}
        badge={<AdminStatusBadge tone={String(batch.status) === "completed" ? "green" : "blue"}>{String(batch.status).replaceAll("_", " ")}</AdminStatusBadge>}
        actions={
          <>
      ...
      <AdminKpiGrid>
        {[
          ...
          <AdminKpiCard key={String(label)} label={String(label)} value={value as number} helper={String(detail)} />
        ))}
      </AdminKpiGrid>
