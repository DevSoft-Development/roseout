...
export async function updateRankingRolloutSettings(
  ...
  const { error: baseError } = await getAdminDatabaseClient()
    .from("search_ranking_rollout_settings")
    .upsert({
      id: true,
  ...
  const { error: controlsError } = await getAdminDatabaseClient()
    .from("app_settings")
    .upsert({
  ...
  const { error: auditError } = await getAdminDatabaseClient()
...
export async function logRankingExperiment(input: {
  ...
  await getAdminDatabaseClient().from("search_ranking_experiments").insert({
