...
async function requireNativeEvent(id: string) {
  const existing = await getAdminDatabaseClient().from("events").select("id,source_kind").eq("id", id).maybeSingle();
...
export async function createNativeEventAction(formData: FormData) {
  ...
  const { data: event, error } = await getAdminDatabaseClient()
    .from("events")
    .insert({
      ...fields,
      source_kind: "native",
  ...
  const { error: sourceError } = await getAdminDatabaseClient().from("event_sources").insert({
    event_id: event.id,
    provider: "native",
  ...
  if (sourceError) {
    await getAdminDatabaseClient().from("events").delete().eq("id", event.id);
    throw sourceError;
  }
