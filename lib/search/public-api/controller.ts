...
export async function handleGeneratePost(
  ...
  try {
    ...
    const oneTimePersonalizationOff = body?.disablePersonalization === true || body?.disable_personalization === true;
    let personalizationAllowed = !oneTimePersonalizationOff;
    let personalizationConsentReason = oneTimePersonalizationOff ? "one_time_opt_out" : "default";
    if (personalizationAllowed && searchIdentity.user?.id) {
      try {
        const preferences = await getPersonalizationPreferences(searchIdentity.user.id);
        personalizationAllowed = preferences.personalizationEnabled;
        personalizationConsentReason = preferences.personalizationEnabled ? "user_enabled" : "user_disabled";
      } catch {
        personalizationAllowed = false;
        personalizationConsentReason = "preference_lookup_failed";
      }
    }
    const currentPersonalizationMode = personalizationAllowed ? globalPersonalizationMode : "disabled";
