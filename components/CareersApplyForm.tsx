...
function isResumeRequired(job: Job) { return Boolean(job.resume_required || job.requires_resume); }
function validateResumeFile(file: File) { const name = file.name.toLowerCase(); const hasAllowedExtension = ALLOWED_RESUME_EXTENSIONS.some((ext) => name.endsWith(ext)); if (!ALLOWED_RESUME_TYPES.has(file.type) || !hasAllowedExtension) return "Please upload a PDF, DOC, or DOCX resume."; if (file.size > MAX_RESUME_SIZE) return "Please upload a resume smaller than 5 MB."; return ""; }
function Field({ label, name, type = "text", required, placeholder, value, onChange }: { label: string; name: string; type?: string; required?: boolean; placeholder?: string; value?: string; onChange?: (value: string) => void }) { return <label className="grid gap-2 text-sm font-bold">{label}<input name={name} type={type} required={required} placeholder={placeholder} value={value} onChange={onChange ? (event) => onChange(event.target.value) : undefined} className="rounded-xl border border-white/10 bg-black/30 p-3 text-white placeholder:text-white/30" /></label>; }
...
export default function CareersApplyForm({ job, questions, showMarketingLinks, showSchoolCredit }: { job: Job; questions: Question[]; showMarketingLinks: boolean; showSchoolCredit: boolean }) {
  ...
  const uploadResume = useCallback(async (file: File) => {
    setResume(null); setFileName(file.name); const validation = validateResumeFile(file); if (validation) { setUploadMessage(validation); return; }
    setUploading(true); setUploadMessage("Uploading resume…"); const formData = new FormData(); formData.set("resume", file);
    try { const response = await fetch("/api/careers/upload-resume", { method: "POST", body: formData }); const data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data.error || "Resume upload failed. You can try again or add a resume link."); setResume(data); setUploadMessage("Resume uploaded."); }
    catch (error) { setUploadMessage(error instanceof Error ? error.message : "Resume upload failed. You can try again or add a resume link."); }
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setSubmitting(true); const form = new FormData(event.currentTarget); const payload = Object.fromEntries(form.entries());
    Object.assign(payload, { resumeUrl: resume?.storagePath || resumeLink.trim() || "", resumeStoragePath: resume?.storagePath || "", resumeOriginalName: resume?.originalName || "", resumeMimeType: resume?.mimeType || "", resumeSizeBytes: resume?.sizeBytes || "", turnstileToken });
    try { const response = await fetch("/api/careers/apply", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) }); const data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data.error || "We could not submit your application. Please try again."); router.push("/careers/apply/success"); }
    catch (error) { setMessage(error instanceof Error ? error.message : "We could not submit your application. Please try again."); setTurnstileToken(""); setTurnstileKey((current) => current + 1); }
  ...
  return <form onSubmit={submit} className="mt-8 grid gap-4 rounded-3xl border border-white/10 bg-[#101012] p-5">
    ...
    <section className="rounded-2xl border border-white/10 bg-white/[0.04] p-4"><h2 className="font-black">Portfolio and work links</h2><p className="mt-1 text-xs text-white/65">Optional. Share only links you want considered for job-related work or experience.</p><div className="mt-4 grid gap-4 sm:grid-cols-2"><Field name="linkedinUrl" label="LinkedIn URL" type="url" /><Field name="portfolioUrl" label="Portfolio URL" type="url" /><Field name="websiteUrl" label="Website URL" type="url" /><Field name="socialHandle" label="Professional / work social handle" placeholder="@yourhandle" /></div></section>
    ...
    {showMarketingLinks ? <div className="rounded-2xl bg-white/[0.04] p-4"><h2 className="font-black">Marketing / social work</h2><Field name="marketingPortfolioUrl" label="Additional work portfolio URL" type="url" /></div> : null}
    ...
    <section className="rounded-2xl border border-blue-300/20 bg-blue-500/[0.07] p-4"><h2 className="font-black text-blue-100">Equal employment opportunity & accommodations</h2><p className="mt-2 text-sm leading-6 text-white/65">TheOutHaven evaluates applicants using job-related qualifications and does not discriminate in employment decisions based on race, color, religion, sex, pregnancy, sexual orientation, gender identity, national origin, age, disability, genetic information, or any other status protected by applicable law.</p><p className="mt-2 text-sm leading-6 text-white/65">If you need a reasonable accommodation for the application or interview process because of a disability, medical condition, or sincerely held religious belief, contact <a className="font-black text-blue-100 underline" href="mailto:support@theouthaven.com?subject=Applicant%20Accommodation%20Request">support@theouthaven.com</a>. Accommodation requests are handled separately from candidate evaluation and are not scored.</p><p className="mt-2 text-xs leading-5 text-white/45">Please do not include medical or disability information in your application answers unless it is necessary to request an accommodation.</p></section>
