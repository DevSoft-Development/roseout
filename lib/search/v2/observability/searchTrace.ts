export type RetrievalCallTrace = { role:string; domain?:"restaurant"|"activity"; retrievalTerms?:string[]; categories?:string[]; cuisines?:string[]; foods?:string[]; features?:string[]; reason:string; durationMs:number; resultCount:number };
export type PairingRejectionReason = "distance_exceeded"|"missing_coordinates"|"market_mismatch"|"walkability_constraint"|"schedule_open_hours_conflict"|"same_venue_constraint"|"insufficient_domain_candidates"|"other";
export type FinalEligiblePairTrace = { restaurantId:string; activityId:string; distanceMiles:number|null; walkingMinutes:number|null; geoTier:string };
export type RejectedPairTrace = { restaurantId:string|null; activityId:string|null; reason:PairingRejectionReason; detail:string; distanceMiles:number|null; walkingMinutes:number|null };
export type PairingDebugTrace = {
  ...
  allCandidatePairsExceededTravelLimit:boolean; primaryFailure:string|null;
};
export type CandidateStageRejection = { locationId:string|null; desiredRole:string|null; originalType:string|null; assignedDomain:"restaurant"|"activity"|null; rejectedAtStage:"geo"|"domain_assignment"|"taxonomy"|"publishability"|"final_domain"; rejectionReason:string; matchedTerms:string[] };
...
export type SearchTimingKey = "intentParsingMs"|"plannerMs"|"restaurantRetrievalMs"|"activityRetrievalMs"|"retrievalMs"|"roleAssignmentMs"|"rankingMs"|"scoringMs"|"pairingMs"|"fallbackMs"|"validationMs"|"responseAdaptationMs"|"serializationMs"|"totalMs";
export type SearchTrace = {
  ...
  candidateStages:CandidateStagesTrace; inventoryAudit:InventoryAuditTrace|null; anchorResolution:AnchorResolutionTrace; pairingDebug:PairingDebugTrace|null;
