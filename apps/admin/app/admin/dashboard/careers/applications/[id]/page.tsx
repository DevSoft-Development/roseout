...
export default async function ApplicationDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const results = await Promise.all([
    ...
    getAdminDatabaseClient().from("career_application_answers").select("id,question_label,answer_text").eq("application_id", id),
    getAdminDatabaseClient().from("career_application_notes").select("id,note,created_at").eq("application_id", id).order("created_at", { ascending: false }),
    getAdminDatabaseClient().from("career_application_stage_history").select("id,created_at,from_stage,to_stage").eq("application_id", id).order("created_at", { ascending: false }),
    getAdminDatabaseClient().from("career_interviews").select("id,status,scheduled_at,meeting_type").eq("application_id", id).order("scheduled_at", { ascending: false }),
    getAdminDatabaseClient().from("career_offers").select("id,status,created_at,start_date,accepted_at,compensation_text").eq("application_id", id).order("created_at", { ascending: false }),
    getAdminDatabaseClient().from("career_content_tests").select("id,status,score").eq("application_id", id),
    getAdminDatabaseClient().from("career_team_conversions").select("id,company_email,provisioning_status,offboarding_status").eq("application_id", id).maybeSingle(),
    getAdminDatabaseClient().from("career_application_scorecards").select("id,communication_score,experience_score,role_fit_score,availability_score,professionalism_score,market_knowledge_score,overall_score,recommendation,notes,created_at").eq("application_id", id).order("created_at", { ascending: false }).limit(1).maybeSingle(),
  ]);
  ...
  if (application.job_id) {
    const profileResult = await getAdminDatabaseClient().from("career_job_provisioning_profiles").select("id").eq("job_id", application.job_id).eq("is_active", true).maybeSingle();
