...
it("prioritizes sports bars with TVs over rooftop lounges for game-watch searches", () => {
  ...
  intent.restaurantIntent = {
    foodTerms: [], mealTerms: [], vibeTerms: [], cuisineTerms: [], featureTerms: [], categoryTerms: [], negativeTerms: [], alternativeGroups: [],
  } as any;
  intent.geo = {
    raw: "Harlem", city: "New York", state: "NY", borough: "Manhattan", county: "New York County", region: null,
    aliases: ["Harlem", "harlem", "Manhattan", "New York", "New York County", "NY"], latitude: 40.888, longitude: -73.954,
    radiusMiles: 3, neighborhood: "Harlem", geoStrictness: "strict",
  } as any;

  const ranked = rankActivityResults([
    { id: "rooftop", name: "Republica Restaurant Rooftop & Lounge", primary_category: "rooftop lounge", description: "rooftop cocktails skyline lounge nightlife", rating: 4.6, review_count: 600, image_url: "x.jpg", latitude: 40.87, longitude: -73.95 },
    { id: "sports", name: "Harlem Sports Bar & Grill", primary_category: "sports bar", description: "sports bar with TVs, big screens, live NBA games, Knicks watch party", rating: 4.2, review_count: 120, image_url: "x.jpg", latitude: 40.888, longitude: -73.954 },
  ] as any, intent);
...
it("penalizes nightlife-only clubs for sports-watch searches", () => {
  ...
  intent.needsActivity = true;
  intent.wantsPairing = false;
  intent.activityIntent = { activityTerms: ["bar", "watch knicks game", "sports bar"], categoryTerms: ["sports bar"], featureTerms: ["tv"], vibeTerms: [], negativeTerms: [], alternativeGroups: [] } as any;
  const club = { id: "club", name: "DJ Night Club", primary_category: "dance club", description: "live dj dancing nightlife", rating: 4.8, review_count: 1000, image_url: "x.jpg", latitude: 40.888, longitude: -73.954 } as any;
...
it("keeps rooftop boosts for rooftop-drinks searches", () => {
  ...
  const rooftop = { id: "rooftop", name: "Harlem Rooftop Lounge", primary_category: "rooftop lounge", description: "rooftop cocktails skyline lounge nightlife", rating: 4.6, review_count: 600, image_url: "x.jpg", latitude: 40.888, longitude: -73.954 } as any;
