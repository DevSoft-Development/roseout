...
function normalizeGoogleHours(value: any) {
  ...
  const descriptions = Array.isArray(value.weekdayDescriptions)
    ...
    : Array.isArray(value.weekday_descriptions) ? value.weekday_descriptions : [];
...
async function discoverReservation(website: string) {
  ...
  for (const path of DISCOVERY_PATHS) {
    ...
    try {
      ...
      const matches = extractLinks(await response.text(), url)
...
function errorText(error: unknown) {
  ...
  try { return JSON.stringify(error).slice(0, 1000); }
  catch { return String(error).slice(0, 1000); }
}
  ...
  for (const row of candidates as any[]) {
    ...
    try {
      ...
      await supabase.from("locations").update({
        ...
        gap_repair_error: errorText(error),
        gap_repair_last_checked_at: new Date().toISOString(),
