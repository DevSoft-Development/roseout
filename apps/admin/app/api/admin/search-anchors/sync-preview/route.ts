...
export async function POST(request: Request) {
  ...
  if (!new Set(["superadmin", "admin", "manager"]).has(admin.role)) return Response.json({ success: false, error: "Forbidden." }, { status: 403 });
  ...
  try {
    ...
    const preview = await buildSearchAnchorSyncPreview(getAdminDatabaseClient(), scope);
    const { data: run, error: runError } = await getAdminDatabaseClient()
      ...
      .insert({
        ...
        requested_by: admin.user_id ?? null,
    ...
    if (preview.actions.length) {
      ...
      for (let start = 0; start < actionRows.length; start += ACTION_INSERT_CHUNK_SIZE) {
        ...
        const { error: actionsError } = await getAdminDatabaseClient()
    ...
    if (savedRunId) {
      await getAdminDatabaseClient().from("search_anchor_sync_runs").delete().eq("id", savedRunId);
    }
