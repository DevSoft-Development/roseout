-- Align item status constraints with the enrichment runner's real terminal states.
-- The runner legitimately writes `unchanged` and `skipped`; rejecting those
-- values leaves claimed rows stranded in `processing` while run counters move.

alter table public.location_enrichment_run_items
  drop constraint if exists location_enrichment_run_items_status_check;

alter table public.location_enrichment_run_items
  add constraint location_enrichment_run_items_status_check
  check (status in (
    'pending',
    'processing',
    'review',
