  ...
  async function requestUserLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        setLocationLoading(false);
        setLocationStatus("Location added to your next search.");
        setError("");
      },
      (geoError) => {
        setLocationSaved(false);
          ...
          >
            <div className="min-w-0">
              ...
              <div className="group/search relative overflow-hidden rounded-2xl bg-black">
                ...
                {!input && (
                  <div className="pointer-events-none absolute left-3 top-3.5 z-10 flex max-w-[calc(100%-1.5rem)] items-center truncate text-sm font-semibold leading-6 text-white sm:left-4 sm:top-4 sm:text-base sm:leading-7">
                    <span className="truncate">
                      {typedPlaceholder || "Tell TheOutHaven what you want..."}
                    </span>
                    {typedPlaceholder ? (
                      <span className="ml-0.5 h-5 w-[2px] shrink-0 animate-pulse rounded-full bg-[#e1062a] sm:h-6" />
                    ) : null}
                  </div>
                )}
