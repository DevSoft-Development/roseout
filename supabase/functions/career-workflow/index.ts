...
import {
  getMicrosoftAppReadinessViaIntegrationApi,
  microsoftAppGraphViaIntegrationApi,
} from "../_shared/aws-integration.ts";
...
const allowedRoles = new Set(["manager", "editor", "reviewer", "ambassador", "experience", "partner_ambassador", "experience_team", "viewer"]);
...
function normalizeEmailPart(value: string) { return value.normalize("NFKD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9]+/g, ".").replace(/^\.+|\.+$/g, "").replace(/\.{2,}/g, ".").slice(0, 48); }
function emailCandidates(firstName: string, lastName: string) {
  const first = normalizeEmailPart(firstName); const last = normalizeEmailPart(lastName);
...
async function logEvent(db: any, input: Record<string, unknown>) { await db.from("career_employee_lifecycle_events").insert(input); }

async function graph(path: string, init: RequestInit = {}) {
  const body = typeof init.body === "string" ? init.body : undefined;
  return microsoftAppGraphViaIntegrationApi<any>({
    path,
    method: (String(init.method || "GET").toUpperCase() as "GET" | "POST" | "PUT" | "PATCH" | "DELETE"),
    body,
    credentialSet: "provisioning",
  });
}
...
async function createTemporaryAccessPass(microsoftUserId: string) {
  const payload = await graph(`/users/${encodeURIComponent(microsoftUserId)}/authentication/temporaryAccessPassMethods`, {
    method: "POST",
    body: JSON.stringify({ lifetimeInMinutes: TAP_LIFETIME_MINUTES, isUsableOnce: true }),
  });
  const value = clean(payload?.temporaryAccessPass);
  if (!value) throw new Error("Microsoft created a Temporary Access Pass method but did not return the one-time pass value.");
  return {
    value,
...
async function createOrGetAuthUser(db: any, companyEmail: string, fullName: string, role: string) {
  ...
  if (!user) {
    const created = await db.auth.admin.createUser({ email: companyEmail, password: randomPassword(), email_confirm: true, user_metadata: { full_name: fullName }, app_metadata: { employee_role: role } });
...
async function resolveLicenseSku(profileSku: unknown) {
  const configured = clean(profileSku);
  if (configured) return configured;
  const readiness = await getMicrosoftAppReadinessViaIntegrationApi();
  return clean(readiness.licenseSku);
}

async function onboard(db: any, actorUserId: string, body: any) {
  const applicationId = clean(body.applicationId);
  if (!applicationId) throw new Error("applicationId is required.");
  const { data: application, error: appError } = await db.from("career_applications").select("id,job_id,first_name,last_name,email,phone,stage,status,career_jobs(id,title,department,slug),career_offers(id,status,start_date,accepted_at)").eq("id", applicationId).maybeSingle();
  ...
  if (application.stage !== "hired" && !acceptedOffer) throw new Error("Only hired candidates or candidates with an accepted offer can be onboarded.");
  const { data: profile } = await db.from("career_job_provisioning_profiles").select("*").eq("job_id", application.job_id).eq("is_active", true).maybeSingle();
  ...
  let microsoftTap: { value: string; methodId: string | null; lifetimeInMinutes: number; startDateTime: string | null } | null = null;

  try {
    let microsoftUser: any = null;
    try {
      microsoftUser = await graph(`/users/${encodeURIComponent(companyEmail)}`);
    } catch {
      microsoftUser = await graph("/users", {
        method: "POST",
        body: JSON.stringify({ accountEnabled: true, displayName: fullName, mailNickname: companyEmail.split("@")[0], userPrincipalName: companyEmail, usageLocation: "US", passwordProfile: { forceChangePasswordNextSignIn: false, password: randomPassword() } }),
      });
    }
    microsoftUserId = microsoftUser.id;
    const licenseSku = await resolveLicenseSku(profile.microsoft_license_sku_id);
    if (licenseSku) {
      await graph(`/users/${encodeURIComponent(microsoftUser.id)}/assignLicense`, { method: "POST", body: JSON.stringify({ addLicenses: [{ skuId: licenseSku }], removeLicenses: [] }) });
    }
    microsoftTap = await createTemporaryAccessPass(microsoftUser.id);
    ...
    await logEvent(db, {
      ...
      details: { microsoft_user_id: microsoftUserId, status: microsoftStatus, provider: "aws_integration_api", temporary_access_pass_issued: true, temporary_access_pass_method_id: microsoftTap.methodId, temporary_access_pass_lifetime_minutes: microsoftTap.lifetimeInMinutes, temporary_access_pass_one_time: true },
    });
  } catch (error) {
    await logEvent(db, { conversion_id: conversion.id, application_id: application.id, event_type: "onboarding", step: "microsoft_365", status: "failed", actor_user_id: actorUserId, error: error instanceof Error ? error.message : String(error), details: { provider: "aws_integration_api" } });
  ...
  if (profile.team_type) {
    await db.from("team_member_profiles").upsert({ user_id: userId, team_type: profile.team_type, status: "active", pay_type: profile.pay_type || "unpaid", hourly_rate: profile.hourly_rate, include_in_payroll: Boolean(profile.include_in_payroll), can_clock_in: Boolean(profile.can_clock_in), can_track_work: Boolean(profile.can_track_work), can_do_site_visits: Boolean(profile.can_do_site_visits), can_do_social_outreach: Boolean(profile.can_do_social_outreach), can_work_support_tickets: Boolean(profile.can_work_support_tickets), can_send_claim_codes: Boolean(profile.can_send_claim_codes), can_send_owner_password_reset: Boolean(profile.can_send_owner_password_reset), can_use_demo_mode: profile.can_use_demo_mode !== false, allowed_work_types: profile.allowed_work_types || [], manager_id: body.managerId || conversion.manager_id || null, updated_at: new Date().toISOString() }, { onConflict: "user_id" });
  }
  ...
  const microsoftSetupHtml = microsoftTap
    ...
    : `<p><strong>Microsoft 365:</strong> Your Microsoft setup is still being finalized. TheOutHaven support will resolve the Microsoft provisioning step.</p>`;
  const email = renderEnterpriseEmail({ subject: "Welcome to TheOutHaven", preview: `Your TheOutHaven employee account is ready: ${companyEmail}`, heading: "Welcome to TheOutHaven", intro: `Hi ${escapeHtml(application.first_name)}, your employee account is ready.`, html: `<p><strong>Company email:</strong> ${escapeHtml(companyEmail)}</p><p><strong>Role:</strong> ${escapeHtml(profile.admin_role)}</p><p><strong>Department:</strong> ${escapeHtml(profile.department || application.career_jobs?.department || "TheOutHaven")}</p><p>Use the secure setup button below to create your TheOutHaven CRM password.</p>${microsoftSetupHtml}<p>If you need help, contact support@theouthaven.com.</p>`, ctaUrl: activationUrl, ctaLabel: "Set up your employee account" });
  ...
  const lastError = microsoftStatus === "failed" ? "Microsoft 365 provisioning or Temporary Access Pass issuance failed; CRM access was created." : !welcomeSent ? "Employee welcome email could not be delivered. Re-run onboarding to replace the one-time Temporary Access Pass and resend setup instructions." : null;
  await db.from("career_team_conversions").update({ user_id: userId, company_email: companyEmail, microsoft_user_id: microsoftUserId, provisioning_status: provisioningStatus, provisioned_at: now, welcome_sent_at: welcomeSent ? now : null, last_error: lastError }).eq("id", conversion.id);
  await logEvent(db, { conversion_id: conversion.id, application_id: application.id, event_type: "onboarding", step: "crm_and_welcome", status: welcomeSent ? "completed" : "failed", actor_user_id: actorUserId, error: welcomeSent ? null : clean((welcomeResult as any).error) || "Welcome email was not sent.", details: { user_id: userId, company_email: companyEmail, welcome_sent: welcomeSent, temporary_access_pass_issued: Boolean(microsoftTap), temporary_access_pass_value_persisted: false } });
  return { success: true, conversionId: conversion.id, companyEmail, userId, microsoftUserId, microsoftStatus, temporaryAccessPassIssued: Boolean(microsoftTap), temporaryAccessPassLifetimeMinutes: microsoftTap?.lifetimeInMinutes || null, welcomeEmailSent: welcomeSent, provisioningStatus };
}
...
async function offboard(db: any, actorUserId: string, body: any) {
  ...
  let microsoftStatus = "skipped";
  try {
    if (conversion.microsoft_user_id) {
      await graph(`/users/${encodeURIComponent(conversion.microsoft_user_id)}`, { method: "PATCH", body: JSON.stringify({ accountEnabled: false }) });
      microsoftStatus = "disabled";
      await logEvent(db, { conversion_id: conversionId, application_id: conversion.application_id, event_type: "offboarding", step: "microsoft_365", status: "completed", actor_user_id: actorUserId, details: { status: microsoftStatus, provider: "aws_integration_api" } });
    } else {
      await logEvent(db, { conversion_id: conversionId, application_id: conversion.application_id, event_type: "offboarding", step: "microsoft_365", status: "skipped", actor_user_id: actorUserId, details: { reason: "no_microsoft_user" } });
    }
  } catch (error) {
    microsoftStatus = "failed";
    await logEvent(db, { conversion_id: conversionId, application_id: conversion.application_id, event_type: "offboarding", step: "microsoft_365", status: "failed", actor_user_id: actorUserId, error: error instanceof Error ? error.message : String(error), details: { provider: "aws_integration_api" } });
  }
  const now = new Date().toISOString();
  await db.from("career_team_conversions").update({ offboarding_status: microsoftStatus === "failed" ? "partial_failure" : "completed", offboarded_at: now, offboarded_by: actorUserId, last_error: microsoftStatus === "failed" ? "Microsoft 365 account could not be disabled; TheOutHaven access was revoked." : null }).eq("id", conversionId);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const result = action === "onboard" ? await onboard(db, admin.userId, body) : action === "offboard" ? await offboard(db, admin.userId, body) : (() => { throw new Error("Unsupported career workflow action."); })();
    return json(result);
  } catch (error) {
