...
async function cropAndSavePostageAsset(batchId: string, itemId: string, labelPngBase64: string) {
  ...
  const { error: uploadError } = await getAdminDatabaseClient().storage
    .from(TEMPLATE_BUCKET)
    .upload(path, trimmed, { contentType: "image/png", cacheControl: "60", upsert: true });
  if (uploadError) throw uploadError;
  return `${getAdminDatabaseClient().storage.from(TEMPLATE_BUCKET).getPublicUrl(path).data.publicUrl}?v=${Date.now()}`;
}
...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: reserved, error: reserveError } = await getAdminDatabaseClient()
    .from("mailing_batch_items")
    .update({
  ...
  try {
    ...
    const { data: purchaseUpdated, error: purchaseUpdateError } = await getAdminDatabaseClient()
      .from("mailing_batch_items")
      .update({
