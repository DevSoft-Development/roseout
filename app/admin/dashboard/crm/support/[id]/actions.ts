...
async function replyToCustomer(ticketId: string, body: string, actor: any) {
  ...
  const sms = await sendSupportTicketSmsReply({
    ticketId,
    body: message,
    authorName,
...
export async function supportCaseAction(formData: FormData) {
  ...
  if (operation === "assign_self") {
    ...
    await replyToCustomer(ticketId, String(formData.get("body") || ""), actor);
    ...
    const macroKey = String(formData.get("macro_key") || "").trim();
    if (!macroKey) throw new Error("Choose a macro before applying it.");
    const macro = await getSupportMacro(macroKey);
    if (macro.body) await replyToCustomer(ticketId, macro.body, actor);
    const status = validateMacroStatus(macro.set_status);
    const priority = validateMacroPriority(macro.set_priority);
    if (status) await updateCanonicalSupportStatus(ticketId, status, actor.user_id);
    if (priority) await updateCanonicalSupportPriority(ticketId, priority, actor.user_id);
    ...
    await updateCanonicalSupportStatus(ticketId, "closed", actor.user_id);
  } else if (operation === "reopen") {
    await updateCanonicalSupportStatus(ticketId, "reopened", actor.user_id);
  } else if (operation === "create_task") {
    const title = String(formData.get("title") || `Follow up on support ticket ${ticketId}`).trim();
  ...
  refresh(ticketId);
  redirect(`/admin/dashboard/crm/support/${ticketId}`);
}
