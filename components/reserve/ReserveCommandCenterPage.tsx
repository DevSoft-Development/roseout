...
import { dedupeFloorResources, getFloorSnapshotState, resourceCapacity, resourceId, resourceName } from "@/lib/reservations/floorSnapshot";
...
function ReserveCommandCenterContent() {
  ...
  const [assigningResourceId, setAssigningResourceId] = useState("");
  ...
  async function updateStatus(reservation:Reservation, status:string){
    if(status === "seated" && !["checked_in","arrived"].includes(String(reservation.status))){ setSelectedId(reservation.id); setMessage({ tone:"warning", text:"Check in this guest before seating." }); return; }
  ...
  async function assignResource(reservation:Reservation, resource:any){
    if(!reservation?.id){ setMessage({ tone:"warning", text:"Select a reservation first." }); return; }
    const state = getFloorSnapshotState(resource, dayReservations);
    const id = resourceId(resource);
    if(!id){ setMessage({ tone:"error", text:"Table assignment needs backend support before it can be used." }); return; }
    if(!state.available){ setMessage({ tone:"error", text:"That table is unavailable." }); return; }
    setUpdatingId(reservation.id); setAssigningResourceId(id); setMessage(null);
    try {
      const response = await fetch("/api/reserve/portal/assign-resource", { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ reservation_id: reservation.id, location_id: reservation.location_id, location_type: reservation.location_type, resource_id: id, adminLocationId: adminLocationId || undefined }) });
      const data = await response.json().catch(()=>({}));
      if(!response.ok) throw new Error(data.error || "We could not assign this table. Please try another table.");
      setReservations((prev)=>prev.map((r)=>r.id===reservation.id?data.reservation:r));
      setSelectedId(reservation.id); setAssigningReservationId(""); setMessage({ tone:"success", text:`Table assigned to ${reservation.customer_name || "guest"}.` }); await loadAll();
    } catch{ setMessage({ tone:"error", text:"We could not assign this table. Please try another table." }); }
    finally { setUpdatingId(""); setAssigningResourceId(""); }
  }
  ...
  const topActions = <><Link className="reserve-soft rounded-full px-4 py-2 text-sm font-black" aria-disabled={!locationId} title={locationId ? "Open public booking page" : "Select a location before opening the booking page."} href={getReserveBookingUrl(locationId, locationType) || "#"}>Booking page <ExternalLink className="inline" size={14}/></Link><Link className="reserve-soft rounded-full px-4 py-2 text-sm font-black" title={locationId ? "Open embed page" : "Select a location before opening the embed."} href={getReserveEmbedUrl(locationId) || "#"}>Embed</Link><Link className="reserve-soft rounded-full px-4 py-2 text-sm font-black" title={locationId ? "Open QR tools" : "QR tools are not configured for this location yet."} href={getReserveQrUrl(locationId) || "#"}><QrCode className="inline" size={14}/> QR Code</Link><ReserveQuickActionButton disabled={!locationId} title={!locationId ? "Select a location before creating a reservation." : undefined} onClick={()=>setModal("reservation")}><Plus size={14} className="inline"/> New Reservation</ReserveQuickActionButton><ReserveQuickActionButton disabled={!locationId} title={!locationId ? "Select a location before adding a walk-in." : undefined} onClick={()=>setModal("walkin")}><UserPlus size={14} className="inline"/> Walk-in</ReserveQuickActionButton></>;

  return <ReserveCommandCenterShell locationName={locationName} locationId={locationId} locationType={locationType} activeTab={activeTab} activeSection={activeSection} onTabChange={switchTab} actions={topActions} setupEnabled={setupEnabled} userLabel={adminLocationId ? "Admin location mode" : "Owner workspace"}>

    {assigningReservation && <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4"><div className="reserve-card max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-[2rem] p-4"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase reserve-muted">Assign table</p><h2 className="mt-1 text-2xl font-black">{assigningReservation.customer_name||"Guest"} · Party {assigningReservation.party_size}</h2><p className="mt-1 text-sm reserve-muted">Available tables are highlighted. Unavailable tables are disabled.</p></div><button type="button" onClick={()=>setAssigningReservationId("")} className="reserve-soft rounded-full px-3 py-1 text-sm font-black">Close</button></div><div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{floorResources.map((resource)=>{const state=getFloorSnapshotState(resource, dayReservations);const id=resourceId(resource);const assigning=assigningResourceId===id;return <button key={id||resourceName(resource)} type="button" disabled={!state.available || Boolean(assigningResourceId)} onClick={()=>assignResource(assigningReservation, resource)} className={`reserve-soft rounded-xl p-3 text-left transition disabled:cursor-not-allowed disabled:opacity-50 ${state.available?"cursor-pointer hover:scale-[1.01] ring-2 ring-emerald-500/50":""}`}><p className="text-sm font-black">{resourceName(resource)}</p><p className="text-xs reserve-muted">Capacity {resourceCapacity(resource)||"—"}</p><p className="mt-2 whitespace-nowrap text-[11px] font-black">{assigning?"Assigning table…":state.status} · {state.available ? "Click to assign" : "Unavailable"}</p>{state.reservation&&<p className="mt-1 truncate text-xs reserve-muted">{state.reservation.customer_name||"Guest"} · Party {state.reservation.party_size||"—"}</p>}</button>})}</div></div></div>}
