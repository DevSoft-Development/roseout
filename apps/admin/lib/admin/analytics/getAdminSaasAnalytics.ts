...
function isColumnMissing(error: any) {
  const code = String(error?.code || "");
  const message = String(error?.message || "").toLowerCase();
...
async function count(table: string, filter?: (query: QueryBuilder) => QueryBuilder) {
  let query = getAdminDatabaseClient().from(table).select("*", { count: "exact", head: true });
...
async function safeSelect(label: string, unavailable: string[], table: string, columns: string, configure?: (query: QueryBuilder) => QueryBuilder) {
  let query = getAdminDatabaseClient().from(table).select(columns);
  if (configure) query = configure(query);
  const { data, error } = await query;
  if (error) {
    unavailable.push(label);
    if (!isMissingOptionalTable(error)) {
      console.error(`ADMIN_ANALYTICS_${label.toUpperCase().replace(/[^A-Z0-9]+/g, "_")}_FAILED`, error.message || error);
    }
    return [];
  }
  return data || [];
...
async function safeSelectFirstAvailable(
  label: string,
  unavailable: string[],
  table: string,
  ...
  for (const columns of columnOptions) {
    ...
    if (isMissingOptionalTable(error)) break;
    if (!isColumnMissing(error)) break;
  }

  unavailable.push(label);
  if (lastError && !isMissingOptionalTable(lastError)) {
    console.error(`ADMIN_ANALYTICS_${label.toUpperCase().replace(/[^A-Z0-9]+/g, "_")}_FAILED`, lastError.message || lastError);
  }
  return [];
}
...
function resultCount(event: AnalyticsEvent | SearchEvent) {
...
function searchLabel(event: AnalyticsEvent | SearchEvent) {
  const metadata = event.metadata || {};
  return (
    text((event as SearchEvent).raw_query) ||
    text(event.normalized_query) ||
    text((event as SearchEvent).search_query) ||
    text((event as AnalyticsEvent).query) ||
    text(metadata.query) ||
    text(metadata.search) ||
    text(metadata.search_query) ||
    text(metadata.raw_query) ||
    text(metadata.normalized_query)
  );
}
...
function isSearchNoResult(event: SearchEvent) {
  const explicitCount = event.result_count !== undefined && event.result_count !== null && event.result_count !== "";
  if (explicitCount && n(event.result_count) === 0) return true;

  const hasBreakoutCounts =
    event.restaurant_count !== undefined ||
    event.activity_count !== undefined ||
    event.pair_count !== undefined;

  if (hasBreakoutCounts && n(event.restaurant_count) + n(event.activity_count) + n(event.pair_count) === 0) return true;

  const issueText = `${event.issue_type || ""} ${event.no_results_reason || ""} ${event.no_pairs_reason || ""}`.toLowerCase();
  return issueText.includes("no_result") || issueText.includes("no result") || issueText.includes("zero_result") || issueText.includes("no_match");
}
...
export async function getAdminSaasAnalytics(): Promise<AdminSaasAnalytics> {
  ...
  const [profilesCount, userProfilesCount, newProfiles, newUserProfiles, totalLocations, searchableLocationsExact, pendingClaims, openSupportTickets] = await Promise.all([
    ...
    safeCount("searchable locations", unavailable, "locations", (q) => q.eq("is_searchable", true)),
  ...
  const [events, searchEvents, dailyRows, locations, reservationsRows, outingRows, logs] = await Promise.all([
    safeSelect("analytics events", unavailable, "analytics_events", "id,event_name,event_type,query,normalized_query,result_count,response_time_ms,conversion_step,location_id,source_location_id,user_id,anonymous_id,session_id,metadata,created_at", (q) => q.gte("created_at", since).order("created_at", { ascending: false }).limit(10000)),
    safeSelectFirstAvailable(
      "search events",
      unavailable,
      "search_events",
      [
        "id,created_at,raw_query,normalized_query,search_query,search_type,primary_domain,result_count,restaurant_count,activity_count,pair_count,timing_ms,success,had_issue,issue_type,no_results_reason,no_pairs_reason,metadata",
        "id,created_at,raw_query,normalized_query,search_type,result_count,restaurant_count,activity_count,pair_count,timing_ms,success,had_issue,issue_type,no_results_reason,no_pairs_reason,metadata",
        "id,search_query,created_at",
      ],
      (q) => q.gte("created_at", since).order("created_at", { ascending: false }).limit(10000),
    ),
  ...
  const searchRows = searchEvents as SearchEvent[];
  ...
  const searchEventCount = searchRows.length;
  const totalSearches = Math.max(analyticsSearchCount, searchEventCount);

  const analyticsNoResults = analyticsEvents.filter((event) => isSearchEvent(event) && resultCount(event) === 0).length;
  const searchNoResults = searchRows.filter(isSearchNoResult).length;
  const noResultSearches = Math.max(analyticsNoResults, searchNoResults);

  const timingRows = searchRows.map((row) => n(row.timing_ms)).filter((value) => value > 0);
  const averageSearchMs = timingRows.length ? Math.round(timingRows.reduce((sum, value) => sum + value, 0) / timingRows.length) : 0;
  const slowSearches30d = timingRows.filter((value) => value >= 3000).length;
  const failedSearches30d = searchRows.filter((row) => row.success === false || row.had_issue === true || Boolean(text(row.issue_type))).length;
  ...
  const searchableLocations = searchableLocationsExact || locationRows.filter((row) => row.is_searchable === true).length;
