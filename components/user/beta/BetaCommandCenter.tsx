...
export default function BetaCommandCenter({ assignments, weekStart, giveawayStatus, feedbackCount, testMode = false }: { assignments: Assignment[]; weekStart: string; giveawayStatus?: string | null; feedbackCount: number; profileComplete: boolean; testMode?: boolean }) {
  ...
  async function choose(item: any, type: "single_location" | "paired_outing" | "none", flags: Record<string, boolean>) { setBusy(true); setError(null); try { setSelected(type === "none" ? { none: true } : item); await persist("selection", { beta_search_run_id: runId, chosen_result_type: type, selected_none: type === "none", result: item, result_type: type, ...flags }); setStep(Math.max(step, weekNumber === 3 ? 3 : weekNumber === 4 ? 3 : 4)); setNotice(type === "none" ? "Thanks — we recorded that none matched." : "Your choice was saved for beta review."); } catch (e: any) { setError(e.message || "We could not save that choice."); } finally { setBusy(false); } }
  async function saveItem(item: any, type: "single_location" | "paired_outing") { setSaved((s) => [...s, item]); await choose(item, type, { was_saved: true }); }
  ...
  return <div className="grid gap-6">
    ...
    <section className={card}><p className="text-xs font-black uppercase tracking-[.28em] text-rose-200">Step 1</p><h2 className="mt-2 text-2xl font-black">Write out the outing you’re looking for in a real sentence.</h2><p className="mt-2 text-sm text-white/60">Tell us what kind of outing you want, who it’s for, where you want to go, the vibe, budget, or occasion. You can also ask for more than one part of an outing, like dinner and an activity.</p><form onSubmit={(e) => { e.preventDefault(); runSearch(false); }} className="mt-5 grid gap-3 md:grid-cols-[1fr_auto]"><input className={input} value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Example: I’m looking for dinner and something fun to do after in Brooklyn with a grown, romantic vibe."/><button disabled={busy || !query.trim()} className={primary}>{busy ? "Searching…" : "Find My Outing"}</button></form></section>
    {step >= 2 && <section className={card}><p className="text-xs font-black uppercase tracking-[.28em] text-rose-200">Step 2</p><h2 className="mt-2 text-2xl font-black">Review the results we found</h2><div className="mt-5 grid gap-6">{pairs.length > 0 && <BetaResultSection title="TheOutHaven Recommends" subtitle="Restaurant and activity combinations matched to your search.">{pairs.map((p, i) => <PairCard key={keyFor(p, i)} pair={p} onSelect={() => choose(p, "paired_outing", { was_selected: true })} onSave={() => saveItem(p, "paired_outing")}/>)}</BetaResultSection>}{restaurants.length > 0 && <BetaResultSection title="Restaurant Picks" subtitle="Food spots matched to cuisine, vibe, and location.">{restaurants.map((r, i) => <ResultCard key={keyFor(r, i)} result={r} type="restaurant" onSelect={() => choose(r, "single_location", { was_selected: true })} onSave={() => saveItem(r, "single_location")}/>)}</BetaResultSection>}{activities.length > 0 && <BetaResultSection title="Experience Picks" subtitle="Activities matched to your outing plan.">{activities.map((r, i) => <ResultCard key={keyFor(r, i)} result={r} type="activity" onSelect={() => choose(r, "single_location", { was_selected: true })} onSave={() => saveItem(r, "single_location")}/>)}</BetaResultSection>}{pairs.length === 0 && restaurants.length === 0 && activities.length === 0 && fallbackResults.length > 0 && <BetaResultSection title="More Matches" subtitle="Additional places matched to your search.">{fallbackResults.map((r, i) => <ResultCard key={keyFor(r, i)} result={r} type="unknown" onSelect={() => choose(r, "single_location", { was_selected: true })} onSave={() => saveItem(r, "single_location")}/>)}</BetaResultSection>}{pairs.length === 0 && restaurants.length === 0 && activities.length === 0 && fallbackResults.length === 0 && <div className="rounded-3xl border border-white/10 bg-black/25 p-5 text-sm font-bold text-white/65">No strong matches came back for this search. Try adding a borough, city, or vibe.</div>}</div><button disabled={busy} onClick={() => choose({ none: true }, "none", {})} className={`mt-4 ${secondary}`}>None of these match what I was looking for.</button></section>}
    {weekNumber === 2 && step >= 2 && <section className={card}><p className="text-xs font-black uppercase tracking-[.28em] text-rose-200">Step 3</p><h2 className="mt-2 text-2xl font-black">Refine your search</h2><div className="mt-4 flex flex-wrap gap-2">{["Change the location","Make it closer","Make it more affordable","Make it more upscale","Make it more romantic","Make it more fun","Show better paired outings","Show places closer together","Other"].map((x) => <button key={x} onClick={() => setRefine((r) => r.includes(x) ? r.filter((v) => v !== x) : [...r, x])} className={`rounded-full border px-3 py-2 text-xs font-black ${refine.includes(x) ? "border-rose-200 bg-rose-500/25" : "border-white/10 bg-black/25"}`}>{x}</button>)}</div><input className={`mt-3 w-full ${input}`} value={refineText} onChange={(e) => setRefineText(e.target.value)} placeholder="Example: Make it more upscale and keep both places within 15 minutes of each other."/><button disabled={busy} onClick={() => runSearch(true)} className={`mt-3 ${primary}`}>Update My Results</button></section>}
    {step >= 4 && <section className={card}><p className="text-xs font-black uppercase tracking-[.28em] text-rose-200">Feedback</p><h2 className="mt-2 text-2xl font-black">Tell us how well we did</h2><FeedbackFields weekNumber={weekNumber} mode={mode} feedback={feedback} setFeedback={setFeedback}/><button disabled={busy} onClick={submitFeedback} className={`mt-4 ${primary}`}>{weekNumber === 4 ? "Submit final feedback" : "Finish weekly check-in"}</button></section>}
