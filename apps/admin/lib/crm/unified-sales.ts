...
export function deriveRecommendations(location:any,state:any,opps:any[],activeMarketing:boolean,activeOffering:boolean):ProductRecommendation[]{
  ...
  const hasRes=reservation(location), demand=num(state?.demand_score), reserveScore=num(location.reservation_opportunity_score);
  ...
  const rows:ProductRecommendation[]=[
    ...
    rec({key:"reserve",status:!restaurant(location)?"covered":hasRes?"review":isClaimed?"sell_now":"blocked",priority:!restaurant(location)?1:hasRes?40:isClaimed?Math.max(75,reserveScore):45,reason:!restaurant(location)?"Reserve is not a primary fit for this location type.":hasRes?"An existing reservation path is detected. Review as a competitive or integration opportunity.":isClaimed?"Restaurant has no recognized online reservation capability.":"Reservation gap is visible, but claiming the business should come first.",evidence:reserveEvidence.length?reserveEvidence:[hasRes?"Existing reservation path detected":"No recognized online reservation path"],confidence:reserveScore>=50||hasRes?"high":"medium",pipelineKey:"reserve_pro",productKey:"reserve",activeOpportunityId:reserveOpp?.id||null,stage:reserveOpp?.stage||null}),
...
"reservation_url","reservation_link","booking_url","external_reservation_url","reservation_provider_url","internal_reservations_enabled","uses_internal_reservations",
