...
function sanitizeGeneratedSite(input: unknown): GeneratedSite | null {
  ...
  return {
    hero: { heading: String(hero.heading).slice(0, 120), subheading: String(hero.subheading).slice(0, 260), ctaLabel: String(hero.ctaLabel || "Plan your visit").slice(0, 60) },
...
async function ownedLocation(user: { id: string; email?: string | null }, locationId: string) {
  const { data } = await supabaseAdmin.from("locations").select("id,name,title,location_type,category,primary_category,cuisine,neighborhood,city").eq("id", locationId).or(`owner_user_id.eq.${user.id},owner_email.eq.${user.email || ""},claimed_by_email.eq.${user.email || ""}`).maybeSingle();
  return data || null;
}
...
export async function POST(request: Request) {
  ...
  const direction = getWebsiteDesignDirection(String((website.theme as any)?.design_direction_id || ""));
  ...
  const { data: usage } = await supabaseAdmin.from("location_website_ai_usage").select("generation_type,status,estimated_cost_micros,started_at").eq("location_id", locationId).gte("started_at", monthStart.toISOString());
  const rows = usage || [];
  if (rows.some((row) => row.status === "running")) return NextResponse.json({ error: "A website generation is already running." }, { status: 409 });
  ...
  const model = process.env.WEBSITE_AI_MODEL || "gpt-4o-mini";
  const { data: usageRow, error: usageError } = await supabaseAdmin.from("location_website_ai_usage").insert({ website_id: website.id, location_id: locationId, generation_type: generationType, status: "running", model, request_key: requestKey }).select("id").single();
  ...
  try {
    ...
    const completion = await openai.chat.completions.create({ model, temperature: 0.4, max_tokens: 900, response_format: { type: "json_object" }, messages: [
    ...
    await supabaseAdmin.from("location_website_ai_usage").update({ status: "succeeded", input_tokens: completion.usage?.prompt_tokens || 0, output_tokens: completion.usage?.completion_tokens || 0, estimated_cost_micros: 0, completed_at: new Date().toISOString() }).eq("id", usageRow.id);
