...
function ReserveCommandCenterContent() {
  ...
  async function assignResource(reservation:Reservation, resource:any){
    ...
    try {
      ...
      const data = await response.json(); if(!response.ok || !data.success) { const details = data || {}; const message = details.error || "We could not assign this table. Please try another table."; const error = new Error(details.debugId && adminLocationId ? `${message} Debug ID: ${details.debugId}` : message) as Error & { details?: any }; error.details = details; throw error; }
      setReservations((prev)=>prev.map((r)=>r.id===reservation.id?data.reservation:r));
      setSelectedId(reservation.id); setAssigningReservationId(""); setAssignmentDiagnostics(null); setMessage({ tone:"success", text:"Table assigned. You can now seat the guest." });
      try { await loadAll({ silent:true }); } catch {
        // Keep assignment success visible; do not reopen assign mode when refresh fails.
      }
    } catch(error){ const raw = error instanceof Error ? error.message : ""; const details = (error as any)?.details || {}; const text = raw || friendlyError(error, "We could not assign this table. Please try another table."); if(details.debugId && adminLocationId){ setAssignmentDiagnostics({ debugId: details.debugId, reservationId: reservation.id, selectedTableLabel: resourceName(resource), selectedTableId: resourceId(resource), selectedTableSource: resourceAssignmentPayload(resource).resource_source, locationId: reservation.location_id || locationId, message: details.error || text, errorCode: details.debug?.code, errorMessage: details.debug?.message, payloadPreview: details.debug?.payloadPreview }); } setMessage({ tone:"error", text }); }
    finally { setUpdatingId(""); }
  }
  ...
  async function sendTableReady(reservation:Reservation){
    ...
    try {
      ...
      setSelectedId(reservation.id); setMessage({ tone:"success", text:`Table ready text sent to ${getReservationGuestName(reservation)}.` }); await loadAll({ silent:true });
  ...
  return <ReserveCommandCenterShell locationName={locationName} locationId={locationId} locationType={locationType} activeTab={activeTab} activeSection={activeSection} onTabChange={switchTab} actions={topActions} setupEnabled={setupEnabled} userLabel={adminLocationId ? "Admin location mode" : "Owner workspace"} actingContext={actionContext()}>
    ...
    {assigningReservation && <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4"><div className="reserve-card max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-[2rem] p-5">{assignmentDiagnostics?.debugId && adminLocationId && <details className="mb-4 rounded-2xl border border-amber-300/30 bg-amber-300/10 p-3 text-sm"><summary className="cursor-pointer font-black text-amber-100">Assignment diagnostics</summary><div className="mt-2 space-y-1 text-amber-50/90"><p>Debug ID: {assignmentDiagnostics.debugId}</p><p>Reservation: {assignmentDiagnostics.reservationId}</p><p>Table: {assignmentDiagnostics.selectedTableLabel || "Selected table"} ({assignmentDiagnostics.selectedTableId || "label-only"})</p><p>Source: {assignmentDiagnostics.selectedTableSource || "manual_label"}</p><p>Location: {assignmentDiagnostics.locationId}</p><p>Message: {assignmentDiagnostics.message}</p>{assignmentDiagnostics.errorCode&&<p>Error code: {assignmentDiagnostics.errorCode}</p>}{assignmentDiagnostics.errorMessage&&<p>Error message: {assignmentDiagnostics.errorMessage}</p>}{assignmentDiagnostics.payloadPreview&&<div><p className="font-black">Payload preview:</p><ul className="ml-4 list-disc"><li>bookable_item_id: {assignmentDiagnostics.payloadPreview.bookable_item_id || "null"}</li><li>bookable_item_name: {assignmentDiagnostics.payloadPreview.bookable_item_name || "Selected table"}</li><li>bookable_item_type: {assignmentDiagnostics.payloadPreview.bookable_item_type || "table"}</li></ul></div>}</div></details>}<div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase reserve-muted">Assign table</p><h2 className="mt-1 text-2xl font-black">{getReservationGuestName(assigningReservation)} · Party {assigningReservation.party_size}</h2><p className="mt-1 text-sm reserve-muted">Unavailable tables are disabled for this reservation time.</p></div><button type="button" onClick={()=>setAssigningReservationId("")} className="reserve-soft rounded-full px-3 py-1 text-sm font-black">Close</button></div><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{resources.map((resource)=>{const state=getFloorSnapshotState(resource, dayReservations);return <button key={resourceId(resource) || resourceName(resource)} type="button" disabled={!state.available || updatingId===assigningReservation.id} onClick={()=>assignResource(assigningReservation, resource)} className="reserve-soft rounded-2xl p-4 text-left disabled:cursor-not-allowed disabled:opacity-50"><p className="text-lg font-black">{resourceName(resource)}</p><p className="text-sm reserve-muted">Capacity {resourceCapacity(resource)||"—"}</p><p className="mt-3 text-xs font-black">{state.status} · {state.available ? "Available" : "Unavailable"}</p>{state.reservation&&<p className="mt-1 truncate text-xs reserve-muted">{getReservationGuestName(state.reservation)} · Party {state.reservation.party_size||"—"}</p>}</button>})}</div></div></div>}
