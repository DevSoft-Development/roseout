...
function isNightlifeOnlyForSportsWatch(r: EnterpriseLocation): boolean {
  ...
  const nightlifeOnly =
    /\bclub\b|\bdance club\b|\bnightclub\b|\blive dj\b|\bdj\b|\bspeakeasy\b|\brooftop lounge\b|\brooftop\b|\broof top\b|\bskyline\b/.test(text);
...
function isActivityLike(r: EnterpriseLocation) {
  ...
  return Boolean(
    r.activity_name ||
      ...
      /\bbowling\b|\bkaraoke\b|\bmuseum\b|\bhookah\b|\blounge\b|\brooftop\b|\broof top\b|\brooftop bar\b|\brooftop lounge\b|\bsports bar\b|\bsports lounge\b|\bpub\b|\btavern\b|\bbar and grill\b|\bcocktail bar\b|\bwine bar\b|\bbar\b|\bnightlife\b|\bspeakeasy\b|\barcade\b|\bmusic\b|\btheater\b|\btheatre\b|\bgallery\b|\bpark\b|\bescape room\b|\bclub\b|\bspa\b/.test(categoryText) ||
      /\bbowling\b|\bkaraoke\b|\bmuseum\b|\bhookah\b|\blounge\b|\brooftop\b|\broof top\b|\brooftop bar\b|\brooftop lounge\b|\bsports bar\b|\bsports lounge\b|\bpub\b|\btavern\b|\bbar and grill\b|\blive sports\b|\bwatch party\b|\bgame day\b|\btvs?\b|\bscreens?\b|\bcocktail bar\b|\bwine bar\b|\bbar\b|\bnightlife\b|\bspeakeasy\b|\bskyline\b|\barcade\b|\blive music\b|\btheater\b|\btheatre\b|\bescape room\b/.test(fullText),
  );
}
const CURATED_TERMS = [
  "romantic",
...
export function rankRestaurantResults(
  ...
  const ranked = filterRestaurantResults(results, intent).map((result) => ({
    result,
    score: relevance(result, intent, "restaurant"),
  }));

  return ranked.sort((a, b) => b.score - a.score).map(({ result }) => result);
}
export function rankActivityResults(
  results: EnterpriseLocation[],
  intent: SearchIntent,
) {
  const ranked = filterActivityResults(results, intent).map((result) => ({
    result,
    score: relevance(result, intent, "activity"),
  }));
