...
export default function AdminTestTunePage() {
  ...
  <section className="rounded-3xl border border-white/10 bg-white/[0.03] p-5"><h2 className="text-xl font-black">Warnings Snapshot</h2><p className="mt-2 text-sm text-white/65">Total actionable warnings from current Search QA run.</p><p className="mt-3 text-3xl font-black text-rose-200">{warningRows.length}</p><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{warningBreakdown.map((card)=><div key={card.label} className="rounded-2xl border border-white/10 bg-[#14110f] p-4"><p className="text-xs uppercase tracking-[0.18em] text-white/55">{card.label}</p><p className="mt-2 text-2xl font-black text-white">{card.count}</p></div>)}</div></section>
