...
exports.GEO_TAXONOMY = void 0;
exports.normalizeGeoTerm = normalizeGeoTerm;
exports.detectGeoIntent = detectGeoIntent;
exports.getGeoAliases = getGeoAliases;
exports.getGeoCenter = getGeoCenter;
exports.isSameGeoFamily = isSameGeoFamily;
exports.scoreGeoMatch = scoreGeoMatch;
exports.shouldExcludeByGeo = shouldExcludeByGeo;
...
const add = (name, type, parent, lat, lon, aliases = []) => base.push({ name, aliases: [name.toLowerCase(), ...aliases], type, state: parent.state ?? "NY", city: parent.city, borough: parent.borough, county: parent.county, region: parent.region, latitude: lat, longitude: lon, defaultRadiusMiles: type === "neighborhood" ? 3 : type === "city" ? 6 : 12 });
...
exports.GEO_TAXONOMY = base;
const norm = (s) => s.toLowerCase().replace(/[’']/g, "'").replace(/\s+/g, " ").trim();
function normalizeGeoTerm(input) { if (!input)
    return null; const n = norm(input); return exports.GEO_TAXONOMY.find((g) => [g.name, ...g.aliases].some((a) => norm(a) === n)) ?? null; }
function detectGeoIntent(query) {
...
function getGeoAliases(geo) { const rec = "geoStrictness" in geo ? normalizeGeoTerm(geo.raw ?? geo.neighborhood ?? geo.city ?? geo.borough ?? geo.county ?? geo.region ?? geo.state ?? "") : geo; return rec ? Array.from(new Set([rec.name, ...rec.aliases, rec.type === "neighborhood" ? rec.name : undefined, rec.borough, rec.city, rec.county, rec.region, rec.state].filter(Boolean))) : []; }
function getGeoCenter(geo) { return geo.latitude != null && geo.longitude != null ? { latitude: geo.latitude, longitude: geo.longitude } : null; }
function eq(a, b) { return Boolean(a && b && norm(String(a)) === norm(String(b))); }
function isSameGeoFamily(record, geo) { if (!geo.raw)
    return true; if (geo.state && record.state && !eq(record.state, geo.state))
    return false; if (geo.neighborhood && eq(record.neighborhood, geo.neighborhood))
    return true; if (geo.borough && eq(record.borough, geo.borough))
    return true; if (geo.city && eq(record.city, geo.city))
    return true; if (geo.region === "Long Island")
    return record.state === "NY" && ["nassau", "suffolk"].some(t => norm([record.city, record.borough, record.neighborhood, record.address, record.search_document].join(" ")).includes(t)); return true; }
function scoreGeoMatch(record, geo) {
    if (!geo.raw)
        return 0;
    if (geo.state && record.state && !eq(record.state, geo.state))
        return -200;
    if (geo.neighborhood && eq(record.neighborhood, geo.neighborhood))
        return 130;
    if (geo.borough) {
        if (eq(record.borough, geo.borough))
            return 125;
        if (record.borough && geo.city && eq(record.city, geo.city)) {
            return geo.geoStrictness === "strict" ? -90 : -70;
        }
    }
    if (geo.city && eq(record.city, geo.city))
        return 80;
    const text = norm([record.neighborhood, record.borough, record.city, record.state, record.address, record.search_document].join(" "));
    if (getGeoAliases(geo).some(a => text.includes(norm(a))))
        return geo.borough ? 95 : 60;
    if (geo.county && text.includes(norm(geo.county)))
        return 55;
    if (geo.region === "Long Island" && record.state === "NY" && /(nassau|suffolk|hempstead|huntington|garden city|mineola|long beach)/.test(text))
        return 70;
    return geo.geoStrictness === "strict" ? -60 : -20;
}
function shouldExcludeByGeo(record, geo) { if (!geo.raw)
    return false; const score = scoreGeoMatch(record, geo); if (geo.state && record.state && !eq(record.state, geo.state))
    return true; if (geo.region === "Long Island" && /long island city|\blic\b|queens/.test(norm([record.neighborhood, record.borough, record.city].join(" "))))
    return true; return geo.geoStrictness === "strict" && score < -100; }
