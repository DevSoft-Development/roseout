...
const reasonOptions = [
  ...
  ["missing_features", "I need features you don’t have"],
  ["business_closed", "The business is closing / closed"],
  ["switching_service", "I’m switching to another service"],
  ["temporary_pause", "I only need to reduce costs temporarily"],
  ["other", "Other"],
] as const;
...
export default async function CancelLocationSubscriptionPage() {
  ...
  return <div className="space-y-6 text-white">
    <section className="rounded-3xl border border-rose-400/20 bg-rose-500/[0.07] p-6 sm:p-8">
      <p className="text-xs font-black uppercase tracking-[0.22em] text-rose-200">Before you cancel</p>
      <h1 className="mt-3 text-3xl font-black sm:text-4xl">Review what {getLocationName(location, "this location")} will lose</h1>
      <p className="mt-3 max-w-3xl text-sm font-bold leading-6 text-white/60">Nothing ends today. If you confirm cancellation, Partner Pro remains available until {formatDate(periodEnd)} and then the paid features below are removed.</p>
    </section>
    ...
    <section className="rounded-3xl border border-white/10 bg-white/[0.04] p-6">
      <h2 className="text-xl font-black">Features and data access affected</h2>
      <div className="mt-4 grid gap-3 lg:grid-cols-2">{lossItems.map((item) => <div key={item.title} className="rounded-2xl border border-white/10 bg-black/20 p-4"><p className="font-black">{item.title}</p><p className="mt-1 text-sm font-bold leading-6 text-white/50">{item.detail}</p></div>)}</div>
    </section>

    {!offerAlreadyUsed ? <section className="rounded-3xl border border-[#f5b700]/30 bg-[#f5b700]/10 p-6">
      <p className="text-xs font-black uppercase tracking-[0.2em] text-[#ffd85d]">A save offer based on your time with us</p>
      <h2 className="mt-2 text-2xl font-black">{annual ? `${offer.discountPercent}% off your next annual renewal` : offer.label}</h2>
      <p className="mt-2 text-sm font-bold leading-6 text-amber-50/70">You’ve been subscribed for about {tenureMonths} month{tenureMonths === 1 ? "" : "s"}. This offer is automatically calculated from tenure and can be used once for this subscription.</p>
    </section> : <section className="rounded-3xl border border-white/10 bg-white/[0.04] p-5 text-sm font-bold text-white/55">A retention discount has already been used on this subscription. You can still keep the plan or continue with cancellation.</section>}
    ...
    <form action="/api/business/billing/cancel-retention" method="POST" className="rounded-3xl border border-white/10 bg-white/[0.04] p-6">
      ...
      <h2 className="text-xl font-black">Why are you thinking about leaving?</h2>
      <p className="mt-2 text-sm font-bold text-white/50">Your answer is saved with the cancellation attempt so we can improve the product.</p>
      ...
      <div className="mt-6 grid gap-3 md:grid-cols-3">
        <Link href="/locations/dashboard/billing" className="flex items-center justify-center rounded-2xl border border-white/10 px-5 py-4 text-sm font-black hover:bg-white/10">Keep my plan</Link>
        ...
        <button type="submit" name="decision" value="confirm_cancel" className="rounded-2xl border border-rose-400/40 px-5 py-4 text-sm font-black text-rose-100 hover:bg-rose-500/10">Confirm cancellation</button>
      </div>
      <p className="mt-4 text-xs font-bold leading-5 text-white/35">Confirming cancellation schedules it for the end of the current paid period. It does not delete the location or erase existing records.</p>
    </form>
  </div>;
}

function Metric({ label, value }: { label: string; value: number }) {
  return <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-5"><p className="text-xs font-black uppercase tracking-[0.16em] text-white/40">{label}</p><p className="mt-2 text-3xl font-black">{Number(value || 0).toLocaleString()}</p></div>;
}
