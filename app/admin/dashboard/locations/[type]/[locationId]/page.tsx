...
async function getLocation(type: LocationType, locationId: string): Promise<LocationRecord | null> {
  const table = type === "restaurants" ? "restaurants" : "activities";
  const { data } = await supabase.from(table).select("*").eq("id", locationId).maybeSingle();
  return data ? ({ ...data, id: locationId, locationType: type } as LocationRecord) : null;
}
...
export default async function AdminLocationCrmDetail({ params }: { params: Promise<{ type: string; locationId: string }> }) {
  ...
  const location = await getLocation(type, locationId);
  ...
  const claimLink = `${process.env.NEXT_PUBLIC_SITE_URL || "https://theouthaven.com"}/claim/${locationId}`;
  ...
  const basePath = `/admin/dashboard/locations/${type}/${locationId}`;
  ...
  return <main className="min-h-screen bg-[#090706] px-4 pb-10 pt-5 text-white sm:px-6 lg:px-8"><div className="mx-auto max-w-[1500px] space-y-6">
    ...
    <section className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
      ...
      <AdminCard title="Claim Access"><div className="space-y-2"><Link href={`/admin/dashboard/claim-qrs?type=${type}&id=${locationId}`} className="block rounded-xl border border-white/10 bg-[#1b1210] px-3 py-2">Print QR Code Label</Link><Link href={mailto} className="block rounded-xl border border-white/10 bg-[#1b1210] px-3 py-2">Resend QR Code Email</Link><p className="text-xs text-white/50">Claim link: {claimLink}</p></div></AdminCard>
