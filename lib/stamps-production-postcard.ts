...
import {
  getStampsConfiguration,
  STAMPS_PRODUCTION_ENDPOINT,
  STAMPS_PRODUCTION_WSDL,
  STAMPS_V160_NAMESPACE,
  type PostcardAddress,
} from "@/lib/stamps-postcard";

export type ProductionPostcardProofResult = {
  ok: true;
  businessName: string;
...
let cachedWsdl: string | null = null;
...
function readXmlTag(xml: string, tag: string) {
  ...
  return matches.length ? decodeXml(matches[0][1].trim()) : null;
}
...
function assertLiveConfiguration() {
  ...
  if (config.mode !== "live") throw new Error("Controlled production postage requires STAMPS_MODE=live.");
  if (!config.configured) throw new Error("Stamps.com production account credentials are not configured.");
  if (!config.postcardEnabled) throw new Error("Stamps.com production postcard access is disabled.");
  if (!config.livePurchasesEnabled) throw new Error("Live postage purchasing is disabled by STAMPS_LIVE_PURCHASES_ENABLED.");
  if (config.endpointUrl !== STAMPS_PRODUCTION_ENDPOINT || config.wsdlUrl !== STAMPS_PRODUCTION_WSDL) {
    throw new Error("Stamps.com production connection must use the approved SWS/IM v160 endpoint and WSDL.");
  }
  return config;
}

async function loadWsdl() {
  if (cachedWsdl) return cachedWsdl;
  const config = assertLiveConfiguration();
  const response = await fetch(config.wsdlUrl, { cache: "no-store" });
  if (!response.ok) throw new Error(`Could not load Stamps.com production v160 WSDL (${response.status}).`);
  const wsdl = await response.text();
  const namespace = wsdl.match(/targetNamespace=["']([^"']+)["']/i)?.[1];
  if (namespace !== STAMPS_V160_NAMESPACE) {
    throw new Error("Stamps.com production WSDL namespace does not match the approved SWS/IM v160 namespace.");
  }
  cachedWsdl = wsdl;
  return wsdl;
}

async function getMailingIndiciumItemElement() {
  if (cachedMailingIndiciumItemElement) return cachedMailingIndiciumItemElement;
  const wsdl = await loadWsdl();
  ...
  if (!candidates[0]?.name) throw new Error("Stamps.com production v160 WSDL did not expose a mailing indicium item type.");
...
async function soapCall(operation: string, body: string) {
  const config = assertLiveConfiguration();
  await loadWsdl();
  const requestXml = `<?xml version="1.0" encoding="utf-8"?>\n<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:sws="${escapeXml(STAMPS_V160_NAMESPACE)}"><soapenv:Header/><soapenv:Body><sws:${operation}>${body}</sws:${operation}></soapenv:Body></soapenv:Envelope>`;
  const startedAt = Date.now();
  const response = await fetch(config.endpointUrl, {
    method: "POST",
    headers: {
      "Content-Type": "text/xml; charset=utf-8",
      SOAPAction: `"${STAMPS_V160_NAMESPACE}/${operation}"`,
    },
    body: requestXml,
    cache: "no-store",
  });
  ...
  // Production SOAP bodies are deliberately never logged. They may contain
  // account credentials, authenticators, addresses, or live indicium data.
...
function credentialsXml() {
  const { credentials } = assertLiveConfiguration();
...
function findPostcardRate(responseXml: string) {
  ...
  const postcard = blocks.find((block) => readXmlTag(block, "PackageType") === "Postcard" && readXmlTag(block, "ServiceType") === "US-FC")
    || blocks.find((block) => readXmlTag(block, "PackageType") === "Postcard");
  if (!postcard) throw new Error("Stamps.com did not return an approved USPS postcard rate.");
  const amount = Number(readXmlTag(postcard, "Amount"));
  if (!Number.isFinite(amount) || amount <= 0) throw new Error("Stamps.com returned an invalid postcard postage amount.");
  return {
    amount,
...
export async function runSinglePostcardProductionProof(
  address: PostcardAddress,
  integratorTxId: string,
): Promise<ProductionPostcardProofResult> {
  assertLiveConfiguration();
  if (!integratorTxId.startsWith("toh-postcard-live-") || integratorTxId.length > 100) {
    throw new Error("A valid reserved production IntegratorTxID is required before purchasing postage.");
  }
  ...
  if (!cityStateZipOk || !addressMatch || !cleanseHash) {
    const detail = returnCode ? ` Stamps return code: ${returnCode}.` : "";
    throw new Error(`Stamps.com did not confirm this destination as a deliverable USPS address.${detail}`);
  }
  ...
  const indiciumItemElement = await getMailingIndiciumItemElement();
  // Transaction-sensitive call: intentionally one attempt only. Callers must
  // never automatically retry this operation after an error or timeout.
