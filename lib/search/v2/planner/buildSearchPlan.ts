...
function resolveTravelPolicy(query: string, parsedWalkMinutes: number | null | undefined) {
  const q = query.toLowerCase(); const walking = /\b(walk|walking|walkable|walking distance|on foot)\b/.test(q); const driving = /\b(drive|driving|by car|car ride)\b/.test(q);
  ...
  const explicitWalkMinutes = parsedWalkMinutes ?? numericToken(walkAfter?.[1]); const explicitDriveMinutes = numericToken(driveAfter?.[1] ?? driveBefore?.[1]);
  const hasWalkMinutes = explicitWalkMinutes != null && explicitWalkMinutes > 0; const hasDriveMinutes = explicitDriveMinutes != null && explicitDriveMinutes > 0;
  const mode: TravelMode = walking || hasWalkMinutes ? "walking" : driving || hasDriveMinutes ? "driving" : "unspecified";
  const hard = Boolean(hasWalkMinutes || hasDriveMinutes || explicitMiles); const constraint: DistanceConstraintType = hard ? "hard" : walking || driving || /\b(near|nearby|close to|around|short drive)\b/.test(q) ? "soft" : "none";
  const maxWalkingMinutes = hasWalkMinutes ? Number(explicitWalkMinutes) : walking && !/\bshort walk\b/.test(q) ? 30 : null; const maxDrivingMinutes = hasDriveMinutes ? Number(explicitDriveMinutes) : null;
...
function parsePartySize(query: string) { const q = query.toLowerCase(); const numeric = q.match(/\b(?:for|group of|party of)\s+(\d{1,2})\b/); if (numeric) return Number(numeric[1]); const words: Record<string, number> = { two:2,three:3,four:4,five:5,six:6,seven:7,eight:8,nine:9,ten:10 }; const word = q.match(/\b(?:for|group of|party of)\s+(two|three|four|five|six|seven|eight|nine|ten)\b/); return word ? words[word[1]] : null; }

export async function buildSearchPlan({ input }: { input: SearchPlannerInput }): Promise<SearchPlan> {
  const p = deterministicParse(input); const travel = resolveTravelPolicy(input.query, p.walkMinutes);
  ...
  const place = p.place; const current = input.userLocation; const geoRecord = place ? normalizeGeoTerm(place[0] ?? place[1]) ?? normalizeGeoTerm(place[1]) : null;
  const useDefaultMarketCoordinates = !anchored && !place && !current; const latitude = current?.latitude ?? geoRecord?.latitude ?? (useDefaultMarketCoordinates ? DEFAULT_MARKET_CENTER.latitude : null); const longitude = current?.longitude ?? geoRecord?.longitude ?? (useDefaultMarketCoordinates ? DEFAULT_MARKET_CENTER.longitude : null);
  const radiusMiles = current?.radiusMiles ?? geoRecord?.defaultRadiusMiles ?? (place ? 8 : DEFAULT_MARKET_CENTER.radiusMiles); const coordinateFirstLocality = Boolean(place?.[3] === "LONG_ISLAND" && latitude != null && longitude != null);
  const resolvedCity = p.genericAnchor ? null : coordinateFirstLocality ? null : geoRecord?.city ?? (geoRecord?.type === "city" ? geoRecord.name : place?.[1] ?? null); const resolvedBorough = p.genericAnchor ? null : geoRecord?.borough ?? (geoRecord?.type === "borough" ? geoRecord.name : place?.[2] ?? null); const resolvedCounty = geoRecord?.county ?? (geoRecord?.type === "county" ? geoRecord.name : place?.[4] ?? null);
  ...
  const plan: SearchPlan = {
    ...
    anchor:{ requested:anchored, rawName:anchored?p.anchorName:null, locationId:null, name:anchored?p.anchorName:null, latitude:null, longitude:null, entityType:anchored?p.anchorEntityType:"none", generic:anchored&&p.genericAnchor, exactNameRequired:anchored&&p.exactNameRequired },
  ...
  validateSearchPlan(plan); return deepFreeze(plan);
}
