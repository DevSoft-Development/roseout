...
export const metadata: Metadata = buildMetadata({
  ...
  description:
    "Compare TheOutHaven Essentials and Partner Pro features, pricing, reservations, marketing tools, and business analytics.",
  path: "/business/plans",
});
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white">
      ...
      <section className="relative overflow-hidden px-4 pb-20 pt-32 sm:px-6 lg:px-8">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(225,6,42,0.24),transparent_36%),linear-gradient(180deg,#0a0708,#050505_60%)]" />
        <div className="relative mx-auto max-w-7xl">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <p className="mt-5 text-base font-semibold leading-7 text-white/60 sm:text-lg">
              Start with Essentials for free. Choose Partner Pro for the
              complete reservation, guest-management, marketing, and analytics toolkit.
            </p>
          </div>

          <div className="mx-auto mt-12 grid max-w-6xl gap-5 lg:grid-cols-[0.8fr_1.2fr]">
            <EssentialsCard />
            <PartnerProPricingCard claimHref={claimHref} />
          </div>
...
function EssentialsCard() {
  return (
    <article className="flex h-full flex-col rounded-[2rem] border border-white/10 bg-black p-6 shadow-2xl shadow-black/40 sm:p-8">
      <span className="mb-5 w-fit rounded-full border border-white/10 bg-white/[0.05] px-4 py-2 text-xs font-black uppercase tracking-[0.18em] text-white/60">
        Build your presence
      </span>
      <h2 className="text-3xl font-black tracking-tight">Essentials</h2>
      <p className="mt-3 text-sm leading-6 text-white/55">
        Build a trusted presence where guests plan outings, then send them
        directly to your existing contact and booking channels.
      </p>
      ...
      <ul className="mt-7 flex-1 space-y-3">
        {essentialsFeatures.map((feature) => (
          ...
          >
            <span className="mr-2 text-emerald-300" aria-hidden="true">✓</span>
            {feature}
          </li>
        ))}
      </ul>
      <Link
        href={claimHref}
        className="mt-7 inline-flex w-full items-center justify-center rounded-2xl border border-white/15 bg-white/[0.05] px-7 py-4 text-sm font-black text-white transition hover:bg-white hover:text-black"
      >
        Claim Your Location
      </Link>
    </article>
  ...
  return (
    ...
    >
      <div className="border-b border-white/10 bg-[linear-gradient(135deg,rgba(225,6,42,0.16),transparent_55%)] p-6 sm:p-8">
        ...
        <h2 id="full-plan-comparison" className="mt-3 text-3xl font-black tracking-tight sm:text-4xl">
          See exactly what each plan includes
        </h2>
        <p className="mt-3 max-w-3xl text-sm font-semibold leading-6 text-white/55">
          Every advertised feature is shown below with the availability or
          limit for Essentials and Partner Pro.
        </p>
      </div>
      ...
      <div className="divide-y divide-white/10">
        {planFeatureGroups.map((group) => (
          <div key={group.title} className="p-4 sm:p-6">
            <h3 className="px-2 text-sm font-black uppercase tracking-[0.18em] text-white/75">
              {group.title}
            </h3>
            <div className="mt-4 overflow-x-auto rounded-2xl border border-white/10">
              <table className="w-full min-w-[680px] border-collapse text-left">
                <thead className="bg-white/[0.055]">
                  <tr>
                    <th scope="col" className="w-1/2 px-4 py-4 text-xs font-black uppercase tracking-[0.14em] text-white/45">Feature</th>
                    <th scope="col" className="w-1/4 px-4 py-4 text-xs font-black uppercase tracking-[0.14em] text-white/65">Essentials</th>
                    <th scope="col" className="w-1/4 bg-[#e1062a]/10 px-4 py-4 text-xs font-black uppercase tracking-[0.14em] text-red-100">Partner Pro</th>
                  </tr>
                </thead>
                <tbody>
      ...
      <div className="flex flex-col gap-4 border-t border-white/10 bg-white/[0.035] p-6 sm:flex-row sm:items-center sm:justify-between sm:p-8">
        ...
        <Link
          href={`${claimHref}?plan=monthly`}
          className="inline-flex items-center justify-center rounded-2xl bg-[#e1062a] px-7 py-4 text-sm font-black text-white shadow-2xl shadow-red-500/25 transition hover:bg-red-500"
        >
          Choose Partner Pro
        </Link>
      </div>
