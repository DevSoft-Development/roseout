...
export async function POST(req: Request) {
  try {
    ...
    const result = await submitLocationClaim({
      ...
      source: body.source || "claim-activity",
      userId: user?.id || null,
    });
    if (!result.ok) return Response.json({ error: result.error }, { status: result.status || 400 });
    return Response.json({ success: true, message: "Activity claim submitted.", claimId: result.claimId, status: result.status, target: result.target });
  } catch (error: any) {
