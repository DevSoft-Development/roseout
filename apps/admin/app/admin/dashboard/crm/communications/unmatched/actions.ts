...
export async function markUnmatchedCrmSmsRead(conversationId: string) {
  ...
  const { error: conversationError } = await getAdminDatabaseClient()
  ...
  const { error: notificationError } = await getAdminDatabaseClient()
...
export async function matchUnmatchedCrmSmsConversation(conversationId: string, contactId: string) {
  ...
  const { data: accountLinks, error: accountError } = await getAdminDatabaseClient()
  ...
  if (accountId) {
    const { data: locationLink, error: locationError } = await getAdminDatabaseClient()
  ...
  if (existing?.id && existing.id !== conversationId) {
    const { error: messageMoveError } = await getAdminDatabaseClient()
      .from("crm_messages")
      .update({ conversation_id: existing.id })
      .eq("conversation_id", conversationId);
    if (messageMoveError) throw messageMoveError;

    const { error: notificationMoveError } = await getAdminDatabaseClient()
      .from("crm_message_notifications")
      .update({
    ...
    const { error: targetUpdateError } = await getAdminDatabaseClient()
      .from("crm_conversations")
      .update({
    ...
    const { error: sourceArchiveError } = await getAdminDatabaseClient()
      .from("crm_conversations")
