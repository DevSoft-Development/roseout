...
create unique index if not exists ux_location_website_ai_usage_request_key
  on public.location_website_ai_usage (request_key)
  where request_key is not null;

create unique index if not exists ux_location_website_ai_one_running
  on public.location_website_ai_usage (website_id)
  where status = 'running';

create index if not exists idx_location_website_ai_usage_location_started
  on public.location_website_ai_usage (location_id, started_at desc);

create index if not exists idx_location_website_ai_usage_website_started
  on public.location_website_ai_usage (website_id, started_at desc);
...
values (
  'ai_website_builder_policy',
  jsonb_build_object(
    'initialBuildsIncluded', 1,
    'fullRedesignsPerMonth', 2,
    'maxConcurrentGenerations', 1,
    'aiImageGenerationEnabled', false,
    'maxEstimatedCostMicrosPerLocationMonth', 5000000
  ),
  now()
)
