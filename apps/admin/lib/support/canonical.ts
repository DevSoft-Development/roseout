...
async function appendSystemEvent(ticketId: string, body: string, metadata: Record<string, unknown> = {}) {
  ...
  const { error } = await getAdminDatabaseClient().from("support_ticket_messages").insert({
    ticket_id: ticketId,
    actor_type: "system",
    sender_role: "system",
...
export async function addCanonicalSupportMessage(input: {
  ...
  await getAdminDatabaseClient().from("support_tickets").update(ticketPatch).eq("id", input.ticketId);
  return data;
}
