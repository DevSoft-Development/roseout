...
import {
  ...
  type GooglePlaceLegacyCompat as Place,
...
const ACTIVITY_HINTS =
  /lounge|bar|club|karaoke|bowling|arcade|escape|museum|theater|movie|comedy|golf|paint|axe|activity|activities|speakeasy|hookah|cigar|spa|music/i;
...
function parseArgs(): Args {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (equalsIndex >= 0) {
      out.set(arg.slice(2, equalsIndex), arg.slice(equalsIndex + 1));
    } else if (argv[index + 1] && !argv[index + 1].startsWith("--")) {
      out.set(arg.slice(2), argv[++index]);
    } else {
...
Imports real Google Places API (New) results into TheOutHaven outer markets. No fake venues are generated.
...
function stateFor(market: SupportedMarket) {
  return market === "NORTHERN_NJ" ? "NJ" : "NY";
}
...
function parseAddress(
  ...
  return {
    address: cleaned,
    city: parts.length > 2 ? parts.at(-2)! : fallbackCity,
    state: match?.[1] || fallbackState,
    zip_code: match?.[2] || "",
  };
}

function score(place: Place) {
  return Math.max(
    50,
    Math.min(
      98,
      Math.round(
        Number(place.rating || 0) * 14 +
          Math.min(25, Math.log10(Number(place.user_ratings_total || 0) + 1) * 10) +
          (place.photos?.length ? 6 : 0) +
          (place.website ? 5 : 0),
      ),
    ),
  );
...
function photoUrl(placeId?: string) {
  const relative = placeId ? publicGooglePlacePhotoUrl(placeId) : null;
  return relative ? `https://www.theouthaven.com${relative}` : null;
}
...
function inferTable(category: string, type: string): Table {
  if (type === "restaurants" || type === "activities") return type;
  return ACTIVITY_HINTS.test(category) ? "activities" : "restaurants";
}

function slugTerm(category: string) {
  return normalize(category).replace(/ /g, "_") || "restaurant";
}
...
async function googleTextSearch(query: string) {
  return searchPlacesTextLegacyCompat(query);
}
...
async function existingByFallback(
  ...
  for (const table of ["locations", "restaurants", "activities"] as const) {
    ...
    const found = data?.find(
      (row: any) =>
        ...
        normalize(row.address) === normalize(address),
    );
...
async function insertWithSupportedColumns(
  supabase: SupabaseClient,
  table: Table,
...
async function savePlace(
  ...
  const resolvedMarket = inferMarketFromCityStateCounty({
    city: addr.city,
    state: addr.state,
    county,
  ...
  const imageUrl = photoUrl(place.place_id);
  ...
  const common: Record<string, unknown> = {
    ...
    import_source: "outer_market_google_places_new",
    status: "approved",
    quality_score: qualityScore,
    theouthaven_score: qualityScore,
    popularity_score: Math.min(
      100,
      Math.round(Math.log10(Number(place.user_ratings_total || 0) + 1) * 35),
    ),
    review_score: Number(place.rating || 0) * 20,
    search_keywords: [place.name, category, city, market, ...(place.types || [])]
      .filter(Boolean)
      .map(String),
  ...
  const row =
    table === "restaurants"
      ? {
          ...
          cuisine_type: term,
          primary_tag: term,
          reservation_url: extractReservationUrl(place),
        }
      : {
          ...common,
          ...
          date_style_tags: ["date night", "group-friendly", term],
        };

  if (dryRun) return "created";

  const qr = await createClaimQr(
  ...
  if (table === "restaurants") {
    await syncRestaurantToLocation(data as any);
  } else {
    await syncActivityToLocation(data as any);
  }
  return "created";
}
...
async function main() {
  ...
  if (!SUPPORTED_MARKETS.includes(market)) {
    throw new Error(`--market must be one of ${SUPPORTED_MARKETS.join(", ")}`);
  }

  const cities = csv(args.get("cities")).length
    ? csv(args.get("cities"))
    : DEFAULT_CITIES[market];
  const categories = csv(args.get("categories")).length
    ? csv(args.get("categories"))
    : ["brunch", "seafood", "steakhouse", "lounge"];
  ...
  const effectiveLimit =
    force || !dailyLimit ? requestedLimit : Math.min(requestedLimit, dailyLimit);
      ...
      try {
        ...
        for (const place of places) {
          ...
          try {
            ...
            const byFallback = await existingByFallback(
              supabase,
              merged.name || "",
              addr.address,
              addr.city,
            ...
            counters.errors.push(
              `${query} / ${place.place_id}: ${serializeError(error)}`,
            );
          }
        }
      } catch (error) {
  ...
  if (!dryRun) {
    await supabase.from("import_logs").insert({
      job_name: "outer_market_google_places_import_new",
      imported_count: counters.created,
      error: counters.errors.slice(0, 5).join("; ") || null,
      meta: summary,
    });
  }
