...
async function learnFromResolvedAiReplies(limit: number) {
  ...
  const ticketIds = [...new Set(aiMessages.map((m) => String(m.ticket_id)).filter(Boolean))];
  const [{ data: tickets, error: ticketError }, { data: messages, error: messageError }] = await Promise.all([
    supabase.from("support_tickets").select("id,status,category,priority").in("id", ticketIds),
    supabase.from("support_ticket_messages").select("id,ticket_id,actor_type,direction,body,created_at").in("ticket_id", ticketIds).order("created_at", { ascending: true }),
  ]);
  if (ticketError) throw ticketError;
  if (messageError) throw messageError;

  const ticketById = new Map((tickets || []).map((t) => [String(t.id), t as Row]));
  const messagesByTicket = new Map<string, Row[]>();
  for (const m of messages || []) {
    const id = String(m.ticket_id);
    const bucket = messagesByTicket.get(id) || [];
    bucket.push(m as Row);
    messagesByTicket.set(id, bucket);
  }
  ...
  for (const ai of aiMessages as Row[]) {
    ...
    const sourceArticleIds = Array.isArray(ai.metadata?.ai_source_article_ids) ? ai.metadata.ai_source_article_ids.map(String).filter(Boolean).sort() : [];
    if (!sourceArticleIds.length) { skipped++; continue; }

    const timeline = messagesByTicket.get(String(ai.ticket_id)) || [];
    const aiTime = new Date(String(ai.created_at)).getTime();
    const corrected = timeline.some((m) => m.actor_type === "admin" && new Date(String(m.created_at)).getTime() > aiTime);
    if (corrected) { skipped++; continue; }
    const inbound = [...timeline].reverse().find((m) => m.direction === "inbound" && new Date(String(m.created_at)).getTime() < aiTime && String(m.body || "").trim());
    if (!inbound) { skipped++; continue; }

    const normalizedQuestion = normalize(String(inbound.body));
    const responseText = String(ai.body || "").trim().slice(0, 900);
    if (normalizedQuestion.length < 4 || !responseText) { skipped++; continue; }
    const signature = sourceArticleIds.join(",");

    const { data: existing, error: existingError } = await supabase
      .from("support_learned_responses")
      .select("*")
    ...
    const shouldPromote = successCount >= PROMOTE_SUCCESSES && confidence >= PROMOTE_CONFIDENCE;
    const status = shouldPromote ? "active" : String(existing?.status || "candidate") === "disabled" ? "candidate" : String(existing?.status || "candidate");
    const now = new Date().toISOString();
    const payload = {
      normalized_question: normalizedQuestion,
      source_signature: signature,
      response_text: existing?.response_text || responseText,
      ...
      promoted_at: shouldPromote ? existing?.promoted_at || now : existing?.promoted_at || null,
      disabled_at: shouldPromote ? null : existing?.disabled_at || null,
      updated_at: now,
    };
    const { error: upsertError } = await supabase.from("support_learned_responses").upsert(payload, { onConflict: "normalized_question,source_signature" });
    if (upsertError) throw upsertError;
    accepted++;
    if (shouldPromote && existing?.status !== "active") promoted++;
  }
  return { scanned: aiMessages.length, accepted, promoted, skipped };
}

async function evaluateLearnedFailures(limit: number) {
  const { data: learnedMessages, error } = await supabase
    .from("support_ticket_messages")
    .select("ticket_id,created_at,metadata")
    .eq("direction", "outbound")
    .contains("metadata", { learned_response: true })
    .order("created_at", { ascending: false })
    .limit(limit);
  if (error) throw error;
  if (!learnedMessages?.length) return { scanned: 0, failures: 0, disabled: 0 };

  const ticketIds = [...new Set(learnedMessages.map((m) => String(m.ticket_id)).filter(Boolean))];
  const { data: tickets, error: ticketError } = await supabase.from("support_tickets").select("id,status").in("id", ticketIds);
  if (ticketError) throw ticketError;
  const statusByTicket = new Map((tickets || []).map((t) => [String(t.id), String(t.status)]));
  ...
  for (const message of learnedMessages as Row[]) {
    ...
    const responseId = String(message.metadata?.learned_response_id || "");
...
async function logRun(startedAt: string, learned: Row, failures: Row) {
  await supabase.from("cron_job_runs").insert({
    ...
    success_count: Number(learned.accepted || 0),
    skipped_count: Number(learned.skipped || 0),
    failed_count: 0,
