  ...
  try {
    ...
    if (location.included_domain_claimed_at && location.included_domain_name) {
      ...
      if (claimedDomain !== domain) {
        return publicResult(domain, false, "included_domain_already_claimed", "This location has already used its included domain benefit.", {
          claimedDomain,
        });
      }
    }
    ...
    const premium = registration.isRegistryPremium || renewal.isRegistryPremium;
    ...
    return publicResult(domain, true, "included_with_partner_pro", "Included with Partner Pro.", {
      available: true,
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unable to check domain eligibility right now." },
      { status: 502 },
    );
  }
}
