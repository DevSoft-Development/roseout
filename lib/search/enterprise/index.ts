...
export async function runEnterpriseSearch(query: string, options?: EnterpriseSearchOptions): Promise<EnterpriseSearchResult> {
  ...
  try {
    ...
    const requestNearMeDebug = {
      ...
      userLatitudePresent: Number.isFinite(Number(options?.body?.userLatitude ?? options?.body?.user_latitude)),
      userLongitudePresent: Number.isFinite(Number(options?.body?.userLongitude ?? options?.body?.user_longitude)),
