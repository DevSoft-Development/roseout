...
export async function GET() {
  ...
  const { data, error } = await getAdminDatabaseClient().from("ai_trust_incidents").select("*").order("created_at",{ascending:false}).limit(100);
...
export async function POST(request: Request) {
  ...
  const { data, error } = await getAdminDatabaseClient().from("ai_trust_incidents").insert(payload).select("*").single();
  if (error) return NextResponse.json({error:error.message},{status:500});
...
export async function PATCH(request: Request) {
  ...
  const { data, error } = await getAdminDatabaseClient().from("ai_trust_incidents").update(update).eq("id",id).select("*").single();
  if (error) return NextResponse.json({error:error.message},{status:500});
