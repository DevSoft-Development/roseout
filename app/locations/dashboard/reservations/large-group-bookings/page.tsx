...
export default async function LargeGroupBookingsPage({ searchParams }: { searchParams?: Promise<DemoSearchParams> }) {
  ...
  const { data: bookings, error } = locationId
    ? await supabase
        ...
        .select("id,customer_name,customer_email,customer_phone,reservation_date,reservation_time,party_size,status,occasion,prix_fixe_interest,group_booking_notes,large_group_payment_mode,deposit_status,guarantee_status,created_at")
    ...
    <main className="min-h-screen bg-[#050607] px-4 py-6 text-white sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        {error ? (
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-left text-sm">
                <thead className="border-b border-white/10 bg-white/[0.04] text-xs uppercase tracking-wide text-white/50">
                  <tr><th className="px-4 py-3">Guest</th><th className="px-4 py-3">Date / time</th><th className="px-4 py-3">Party</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Occasion</th><th className="px-4 py-3">Payment</th><th className="px-4 py-3">Contact</th></tr>
                </thead>
                <tbody className="divide-y divide-white/10">
                  {bookings.map((booking: any) => (
                    <tr key={booking.id} className="align-top">
                      ...
                      <td className="px-4 py-4"><span className="rounded-full bg-white/10 px-2.5 py-1 text-xs font-black capitalize">{String(booking.status || "pending").replaceAll("_", " ")}</span></td>
          ...
          <div className="rounded-[1.5rem] border border-white/10 bg-white/[0.03] p-8 text-center"><h2 className="text-xl font-black">No large group bookings yet</h2><p className="mt-2 text-sm text-white/60">New large-party requests will appear here automatically.</p></div>
        )}
      </div>
    </main>
  );
