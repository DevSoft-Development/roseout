...
export function applyLocationAdminFilters<T>(initial: T, filters: ProfileAdminFilters): T { let q = initial as T & FilterQuery; const search = buildLocationSearchOr(filters.search); if (search)
    q = q.or(search) as T & FilterQuery; for (const key of ['market', 'state', 'county', 'borough', 'city', 'neighborhood'] as const)
    if (filters[key])
        q = q.eq(key, filters[key]) as T & FilterQuery; if (filters.active)
    q = q.eq('active', filters.active === 'true') as T & FilterQuery; if (filters.searchable)
    q = q.eq('is_searchable', filters.searchable === 'true') as T & FilterQuery; if (filters.hidden)
    q = q.eq('is_hidden', filters.hidden === 'true') as T & FilterQuery; if (filters.lowLevel)
    q = q.eq('is_low_level', filters.lowLevel === 'true') as T & FilterQuery; return q as T; }
