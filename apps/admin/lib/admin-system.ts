...
import { ADMIN_ROLES, type AdminRole } from "@theouthaven/auth/admin-roles";
...
async function countSuperadmins() {
  const { count, error } = await getAdminDatabaseClient()
    .from("admin_users")
...
export async function listAdminRoleSummaries(): Promise<AdminRoleSummary[]> {
  const { data: staff, error } = await getAdminDatabaseClient().from("admin_users").select("role");
  if (error) throw error;
...
export async function listAdminStaffSecurity(): Promise<AdminStaffSecurityRow[]> {
  const { data: staff, error } = await getAdminDatabaseClient()
    .from("admin_users")
  ...
  for (let page = 1; page <= 10; page += 1) {
    const { data, error: authError } = await getAdminDatabaseClient().auth.admin.listUsers({ page, perPage: 200 });
...
export async function getAdminSecurityOverview() {
  ...
  const { data: audit, error } = await getAdminDatabaseClient()
...
export async function addAdminRoleMember(input: {
  ...
  const { data: existing, error: lookupError } = await getAdminDatabaseClient()
    .from("admin_users")
    .select("id,user_id,email,full_name,role,created_at")
    .eq("email", email)
    .maybeSingle();
  ...
  if (existing) {
    ...
    const { data: updated, error: updateError } = await getAdminDatabaseClient()
      .from("admin_users")
  ...
  const { data: created, error: insertError } = await getAdminDatabaseClient()
    .from("admin_users")
...
export async function changeAdminRoleByAdminId(input: {
  ...
  if (!isAdminRole(input.role)) throw new Error("Invalid admin role.");
  const { data: current, error } = await getAdminDatabaseClient()
    .from("admin_users")
    .select("id,user_id,email,full_name,role,created_at")
    .eq("id", input.adminId)
    .maybeSingle();
...
export async function removeAdminRoleMember(input: {
  ...
  const { error: deleteError } = await getAdminDatabaseClient().from("admin_users").delete().eq("id", input.adminId);
...
export async function changeAdminRole(input: {
  ...
  const { error: updateError } = await getAdminDatabaseClient()
    .from("admin_users")
    .update({ role: input.role })
    .eq("user_id", input.targetUserId);
  if (updateError) throw updateError;
...
export async function setAdminAccessState(input: {
  ...
  const { data: authUser, error: authError } = await getAdminDatabaseClient().auth.admin.updateUserById(
    input.targetUserId,
