...
export async function getLocationDemandBenchmark(locationId:string){
  const {data:location,error}=await getAdminDatabaseClient().from('locations').select('id,neighborhood,borough,county,category,primary_category,primary_tag,cuisine,cuisine_type,location_type').eq('id',locationId).maybeSingle();
  ...
  let query=getAdminDatabaseClient().from('gtm_location_state').select('location_id,demand_score,locations!inner(id,neighborhood,borough,county,category,primary_category,primary_tag,cuisine,cuisine_type,location_type)').neq('location_id',locationId).limit(500);
  ...
  const {data:state}=await getAdminDatabaseClient().from('gtm_location_state').select('demand_score').eq('location_id',locationId).maybeSingle();
...
export async function getGtmHeatSignals(limit=20){
  const {data,error}=await getAdminDatabaseClient().from('gtm_location_state').select('location_id,demand_score,association_level,gtm_status,locations!inner(neighborhood,borough,county,category,primary_category,primary_tag,cuisine,cuisine_type,location_type)').eq('suppressed',false).order('demand_score',{ascending:false}).limit(1000); if(error)throw error;
