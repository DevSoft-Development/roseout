...
export async function createProfileRun(input: CreateProfileRunInput) {
  const created = await getAdminDatabaseClient()
  ...
  if (ids.length === 0) {
    const targets = await getAdminDatabaseClient()
      .from("locations")
      .select("id")
      .eq("active", true)
  ...
  try {
    for (let start = 0; start < ids.length; start += INSERT_BATCH_SIZE) {
      const inserted = await getAdminDatabaseClient()
    ...
    const updated = await getAdminDatabaseClient()
      .from("location_search_profile_runs")
      .update({
...
export async function resumeProfileRun(runId: string, retryFailed = false) {
  const now = new Date().toISOString();
  let items = getAdminDatabaseClient()
    .from("location_search_profile_run_items")
    .update({
      status: "pending",
      available_at: now,
  ...
  const run = await getAdminDatabaseClient()
    .from("location_search_profile_runs")
    .update({
      status: "running",
