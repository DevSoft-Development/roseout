...
async function upsertConnection(input: {
  ...
  if (connectionId) {
    const { error } = await getAdminDatabaseClient().from("marketing_social_connections").update(row).eq("id", connectionId);
    if (error) throw error;
  } else {
    const { data, error } = await getAdminDatabaseClient().from("marketing_social_connections").insert(row).select("id").single();
