...
export async function POST(
  ...
  const auth = await requireAdminApiRole(["superadmin", "admin", "manager", "ambassador", "partner_ambassador"]);
  ...
  const { data: location, error: locationError } = await getAdminDatabaseClient()
    .from("locations")
  ...
  const { data: existingLink } = await getAdminDatabaseClient()
  ...
  if (!accountId) {
    const { data: existingAccount } = await getAdminDatabaseClient()
      .from("crm_accounts")
      .select("id")
  ...
  if (!existingLink) {
    const { error: linkError } = await getAdminDatabaseClient().from("crm_account_locations").insert({
  ...
  const { data: existingOpportunity } = await getAdminDatabaseClient()
