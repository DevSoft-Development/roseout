...
import { CalendarDays, MapPin, Search, Sparkles, TicketCheck, Users, QrCode, Megaphone, Route, ExternalLink, RefreshCw } from "lucide-react";
...
export default function DestinationPicker({ onCreate }: { onCreate: (destination: Destination) => Promise<void> }) {
  ...
  return (
    <section className="rounded-[1.35rem] border border-rose-300/15 bg-[radial-gradient(circle_at_top_right,rgba(236,11,91,0.12),transparent_36%),rgba(255,255,255,0.025)] p-4 sm:p-5">
      <div>
        <p className="text-xs font-black uppercase tracking-[0.2em] text-rose-200">Fast create</p>
        <h3 className="mt-1 text-lg font-black text-white">Choose where the link should go</h3>
        <p className="mt-1 text-sm leading-6 text-white/45">Pick a real TheOutHaven record. The destination, title, link type, and tracking metadata are filled automatically.</p>
      </div>
        ...
        {TYPES.map((item) => {
          ...
          return (
            <button
              key={item.value}
              type="button"
              onClick={() => { setType(item.value); setSearch(""); }}
      ...
      <label className="admin-field mt-4 flex min-h-11 min-w-0 items-center gap-2 rounded-xl border border-white/10 bg-[#0b0b0d] px-3 text-white focus-within:border-rose-300/50 focus-within:ring-4 focus-within:ring-rose-300/10">
        ...
        <input
          ...
          placeholder={`Search ${TYPES.find((item) => item.value === type)?.label.toLowerCase() || "destinations"}...`}
          className="min-w-0 flex-1 bg-transparent py-2 text-sm font-semibold outline-none placeholder:text-white/30"
        />
      ...
      <div className="mt-3 max-h-[360px] space-y-2 overflow-y-auto pr-1">
        {!loading && !results.length ? (
          <div className="rounded-2xl border border-dashed border-white/10 p-5 text-center text-sm font-semibold text-white/40">No matching destinations found.</div>
        ) : null}
        {results.map((destination) => (
          <article key={`${destination.type}-${destination.id}`} className="flex min-w-0 flex-col gap-3 rounded-2xl border border-white/10 bg-black/15 p-3 sm:flex-row sm:items-center">
            ...
            <button
              type="button"
              disabled={creatingId === destination.id}
              onClick={() => void create(destination)}
              className="inline-flex min-h-10 shrink-0 items-center justify-center gap-2 rounded-xl bg-[#ec0b5b] px-3 text-xs font-black text-white shadow-md shadow-rose-950/20 transition hover:bg-rose-500 disabled:cursor-not-allowed disabled:opacity-50"
            >
