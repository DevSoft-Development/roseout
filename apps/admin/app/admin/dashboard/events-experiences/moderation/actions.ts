...
async function creatorEmails(locationId: string | null, organizationId: string | null) {
  ...
  if (organizationId) {
    const { data: members, error } = await getAdminDatabaseClient()
    ...
    for (const member of members || []) {
      if (member.email) emails.add(String(member.email).trim().toLowerCase());
        ...
        const { data } = await getAdminDatabaseClient().auth.admin.getUserById(String(member.user_id));
  ...
  if (locationId) {
    ...
    for (const owner of owners || []) {
      ...
      const { data } = await getAdminDatabaseClient().auth.admin.getUserById(String(owner.user_id));
      if (data.user?.email) emails.add(data.user.email.trim().toLowerCase());
    }
  }
...
async function loadModerationItem(caseId: string, subjectType: ModerationSubject, subjectId: string) {
  const { data: fraudCase, error: caseError } = await getAdminDatabaseClient()
    .from("fraud_cases")
...
async function assertRelatedAccountsCanPublish(item: Awaited<ReturnType<typeof loadModerationItem>>) {
  ...
  if (item.location_id) {
    const { data: location, error } = await getAdminDatabaseClient()
      .from("locations")
  ...
  if (item.organization_id) {
    const { data: organization, error } = await getAdminDatabaseClient()
      .from("organizations")
      .select("stripe_connect_account_id,stripe_connect_charges_enabled,stripe_connect_payouts_enabled")
...
export async function moderateEventExperienceAction(formData: FormData) {
  ...
  if (decision === "request_details") {
    ...
    const [{ error: caseError }, { error: noteError }] = await Promise.all([
      getAdminDatabaseClient().from("fraud_cases").update({
      ...
      getAdminDatabaseClient().from("fraud_case_notes").insert({
        case_id: caseId,
    ...
    const itemUpdate = subjectType === "event"
      ? getAdminDatabaseClient().from("events").update({ status: "cancelled", searchable: false, updated_at: now }).eq("id", subjectId)
      : getAdminDatabaseClient().from("experiences").update({ status: "archived", searchable: false, updated_at: now }).eq("id", subjectId);
    ...
    const itemUpdate = subjectType === "event"
      ? getAdminDatabaseClient().from("events").update({ status: "scheduled", searchable: true, updated_at: now }).eq("id", subjectId)
      : getAdminDatabaseClient().from("experiences").update({ status: "published", searchable: true, updated_at: now }).eq("id", subjectId);
    const [{ error: itemError }, { error: caseError }] = await Promise.all([
      itemUpdate,
      getAdminDatabaseClient().from("fraud_cases").update({
        status: "closed",
