...
import { getSupportMacro, setSupportGroup, setSupportTags, validateMacroPriority, validateMacroStatus } from "@/lib/support/operations";
...
export async function supportCaseAction(formData: FormData) {
 ...
 else if(operation==="reply") await createSupportReply({ticketId,actorType:"admin",authorName:actor.full_name||actor.email||"TheOutHaven Support",authorEmail:actor.email||undefined,message:String(formData.get("body")||"")});
 ...
 else if(operation==="macro") { const macro=await getSupportMacro(String(formData.get("macro_key")||"")); if(macro.body) await createSupportReply({ticketId,actorType:"admin",authorName:actor.full_name||actor.email||"TheOutHaven Support",authorEmail:actor.email||undefined,message:macro.body}); const status=validateMacroStatus(macro.set_status); const priority=validateMacroPriority(macro.set_priority); if(status) await updateCanonicalSupportStatus(ticketId,status,actor.user_id); if(priority) await updateCanonicalSupportPriority(ticketId,priority,actor.user_id); if(macro.assigned_group) await setSupportGroup(ticketId,macro.assigned_group); if(macro.tags?.length) await setSupportTags(ticketId,macro.tags); }
