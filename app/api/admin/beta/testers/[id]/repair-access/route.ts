...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const repair = await repairBetaAccessForEmail({ email: tester.email, fullName: tester.name, phone: tester.phone, testerType: tester.tester_type, applicationId: tester.application_id, actor: auth.adminUser, sendInviteIfNeeded: false });
