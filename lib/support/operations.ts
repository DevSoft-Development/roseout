...
export async function getSupportOperationsSettings() {
  const [groups, slas, macros, triggers, automations] = await Promise.all([
  ...
  for (const result of [groups, slas, macros, triggers, automations]) if (result.error) throw result.error;
  return {
    groups: groups.data ?? [],
    slas: slas.data ?? [],
    macros: macros.data ?? [],
...
export async function getSupportMacro(key: string) {
  ...
  return data as {
    ...
    name: string;
    body: string | null;
    set_status: SupportStatus | null;
    set_priority: SupportPriority | null;
    assigned_group: string | null;
    tags: string[] | null;
  };
}

export function validateMacroStatus(value: unknown): SupportStatus | null {
  return value && isSupportStatus(value) ? value : null;
}

export function validateMacroPriority(value: unknown): SupportPriority | null {
  return value && isSupportPriority(value) ? value : null;
}
