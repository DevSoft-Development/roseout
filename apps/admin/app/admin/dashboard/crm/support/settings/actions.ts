...
async function save(table: string, id: string, payload: Record<string, unknown>) { const db=getAdminDatabaseClient(); const query = id ? db.from(table).update(payload).eq("id", id) : db.from(table).insert(payload); const { error } = await query; if (error) throw error; }
...
export async function supportSettingsAction(formData: FormData) {
  ...
  if (operation === "delete") { if (!id) throw new Error("Record id is required."); const { error } = await getAdminDatabaseClient().from(tableFor(entity)).delete().eq("id", id); if (error) throw error; refresh(); return; }
  if (entity === "group") {
