...
export default async function WebsiteHostingOperationsPage() {
  ...
  const [nodesResult, websitesResult] = await Promise.all([
    supabaseAdmin
      ...
      .select("id,name,provider,instance_name,region,public_ip,status,accepting_new_sites,max_sites,cpu_percent,memory_percent,disk_percent,last_health_check_at,updated_at")
  ...
  return (
    <AdminPageShell>
      <AdminPageHeader
        ...
        subtitle="Live control-plane view of TheOutHaven-generated websites, Lightsail nodes, capacity, deployment state, DNS, SSL, and server health."
        actions={
          <>
      ...
      <AdminKpiGrid>
        ...
        <AdminKpiCard label="Deploying" value={deployingSites} helper="Sites currently publishing" />
      ...
      <AdminSectionCard className="p-5">
        <div className="mb-5">
          ...
          <h2 className="mt-1 text-2xl font-black text-white">Server health and site capacity</h2>
        </div>
            ...
            return (
              <article key={node.id} className="rounded-3xl border border-white/10 bg-black/20 p-5">
                ...
                <div className="mt-4 flex flex-wrap gap-x-6 gap-y-2 text-xs font-semibold text-white/45">
                  ...
                  <span>Last check: {formatDate(node.last_health_check_at)}</span>
                </div>
              </article>
            );
          })}
      ...
      <AdminSectionCard className="p-5">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-white/50">Telemetry comes from the existing hosting-node heartbeat and generated-site records. This dashboard does not call AWS directly or invent a second monitoring source.</p>
          </div>
