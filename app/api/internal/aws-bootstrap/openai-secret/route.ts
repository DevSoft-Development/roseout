...
const ROUTE_PATH = "/api/internal/aws-bootstrap/openai-secret";
...
function configuredOpenAiKey() {
  ...
  if (!value || value === "aws-assistant") return "";
  return value;
}

function encryptValue(publicKey: string, value: string) {
  const bytes = Buffer.from(value, "utf8");
  if (bytes.length < 20 || bytes.length > MAX_KEY_BYTES) throw new Error("invalid_openai_key_length");
...
export async function POST(request: Request) {
  ...
  try {
    return NextResponse.json(
      { ok: true, ciphertext: encryptValue(publicKey, apiKey) },
