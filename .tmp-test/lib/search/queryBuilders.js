...
const restaurantRelevantVibes = (vibes = []) => vibes.filter((vibe) => !["nightlife", "club"].includes(vibe));
const activityRelevantVibes = (intent) => (intent.normalizedIntent?.vibeTerms ?? intent.vibes ?? [])
    .filter((vibe) => vibe !== "rooftop" || intent.activityIntents.includes("rooftop"));
const geoTerms = (intent) => uniq([
...
function buildRestaurantSearchInput(intent) {
    ...
    if ((intent.boroughs.length > 0 || Boolean(intent.geoIntent)) &&
