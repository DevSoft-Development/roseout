...
export const TERMINAL_WAITLIST_STATUSES = [
  "booked",
  "expired",
  "cancelled",
  "seated",
...
export async function handleReserveWaitlistGET(request: NextRequest) {
  try {
    const { searchParams } = new URL(request.url);

    const locationId =
      cleanString(searchParams.get("adminLocationId")) ||
      cleanString(searchParams.get("locationId"));

    const reservationDate =
      cleanString(searchParams.get("date")) ||
    ...
    const { data, error } = await supabaseAdmin
      ...
      // notified is still active because the guest has been offered a slot
      // but has not converted, expired, cancelled, or been seated.
    ...
    return NextResponse.json({
      success: true,
      waitlist: (data || []).map(normalizeWaitlistRow),
    });
  } catch (error) {
    return NextResponse.json(
      {
...
export async function handleReserveWaitlistPOST(request: NextRequest) {
  try {
    ...
    if (!locationId || !reservationDate || !reservationTime) {
      return NextResponse.json(
        { error: "Missing required waitlist details." },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json({
      success: true,
      waitlist: normalizeWaitlistRow(insertResult.data),
      waitlist_position: Number(count || 0) + 1,
    });
  } catch (error) {
    return NextResponse.json(
      {
