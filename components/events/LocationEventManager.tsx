...
function Metric({label,value}:{label:string;value:string|number}){return <div className="rounded-2xl border border-white/10 bg-black/20 p-4"><p className="text-xs font-black uppercase tracking-[0.12em] text-white/35">{label}</p><p className="mt-2 text-2xl font-black">{value}</p></div>}
...
export default function LocationEventManager({locationId,location,events,metrics}:{locationId:string;location:{name:string;address:string|null;city:string|null;state:string|null;zip_code:string|null};events:EventRow[];metrics:Metrics}){
  ...
  return <div className="space-y-5">
    ...
    <form action={createLocationEventAction} className="space-y-5">
      ...
      <Step number={3} title="Date, time, and place" description="Your location is prefilled so owners do not have to re-enter the venue every time.">
        <div className="grid gap-4 md:grid-cols-2"><Field label="Starts"><input name="starts_at" type="datetime-local" required className={inputClass}/></Field><Field label="Ends" optional><input name="ends_at" type="datetime-local" className={inputClass}/></Field><Field label="Venue"><input name="venue_name" required defaultValue={location.name} className={inputClass}/></Field><Field label="Address"><input name="address" required defaultValue={location.address||""} className={inputClass}/></Field><Field label="City"><input name="city" required defaultValue={location.city||""} className={inputClass}/></Field><Field label="State"><input name="state" required defaultValue={location.state||"NY"} className={inputClass}/></Field><Field label="ZIP code"><input name="zip_code" required defaultValue={location.zip_code||""} className={inputClass}/></Field><Field label="Timezone"><input name="timezone" required defaultValue="America/New_York" className={inputClass}/></Field></div>
      </Step>
