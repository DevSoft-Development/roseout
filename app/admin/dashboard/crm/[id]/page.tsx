...
import { ACTIVITY_CHILD_TABS, calculateLocationHealthScore, compareMetric, dedupeRecommendations, freshnessState, maskSensitiveValue, normalizeActivityChildTab, type ActivityChildTab } from "@/lib/admin/location-intelligence";
import { getLocationQrOperationalAlerts, getLocationQrStatus } from "@/lib/admin/location-qr-status";
...
export default async function CRMDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ tab?: string; commTab?: string; activityTab?: string; range?: string; compare?: string }> }) {
  const admin = await requireAdminRole(ADMIN_PAGE_ACCESS.crm);
  const { id } = await params;
  const query = await searchParams;
  const activeTab = normalizeCrmDetailTab(query.tab);
  const activityChildTab = normalizeActivityChildTab(query.tab, query.activityTab);
  ...
  return <AdminPageShell>
      ...
      {activeTab === "analytics" ? <ActivityIntelligenceWorkspace business={business} related={related} publishability={publishability} childTab={activityChildTab} range={query.range} compare={query.compare} adminRole={admin.role} canEdit={canEdit} /> : null}
...
function ActivityIntelligenceWorkspace({ business, related, publishability, childTab, range, compare, adminRole, canEdit }: { business: BusinessCRMRow; related: any; publishability: any; childTab: ActivityChildTab; range?: string; compare?: string; adminRole: string; canEdit: boolean }) {
  const base = `/admin/dashboard/crm/${business.id}?tab=analytics&activityTab=`;
  const health = calculateLocationHealthScore({ ...business, publishReady: publishability.qualityStatus === "publish_ready", readyToApprove: publishability.isReadyToApprove, galleryCount: normalizeImageList((business as any).gallery_images || (business as any).images || (business as any).photos).length, qrCodeCount: related.qrCodes?.length || 0, qrCodes: related.qrCodes || [], activeOfferCount: related.offers?.length || 0, reviewCount: related.reviews?.length || 0, average_rating: (business as any).average_rating, hasFreshData: Boolean((business as any).last_analytics_event_at || business.updated_at) });
  const recommendations = dedupeRecommendations([
    { key: "main-photo", title: "Add a main photo", category: "Photo quality", complete: Boolean((business as any).main_image || (business as any).image_url), href: `${base}health-score#photos`, impact: 8 },
    { key: "publishability", title: "Repair publishability", category: "Publishability", complete: publishability.qualityStatus === "publish_ready", href: `${base}search-performance#diagnostics`, impact: 12 },
    { key: "search-tags", title: "Add search tags", category: "Search visibility", complete: Boolean((business as any).search_tags || (business as any).tags), href: `/admin/dashboard/crm/${business.id}?tab=listing`, impact: 7 },
    { key: "hours", title: "Add hours", category: "Listing accuracy", complete: Boolean((business as any).operating_hours), href: `/admin/dashboard/crm/${business.id}?tab=profile`, impact: 8 },
    { key: "claim", title: "Complete owner setup", category: "Claim and ownership", complete: Boolean((business as any).is_claimed || (business as any).owner_id), href: `/admin/dashboard/crm/${business.id}?tab=claims`, impact: 6 },
  ]);
  const metrics = [
    ["Location health score", `${health.score}/100`, "health-score", "verified"], ["Search visibility status", publishability.isSearchable ? "Searchable" : "Not searchable", "search-performance", "verified"], ["Publishability status", publishability.qualityStatus, "search-performance", "verified"], ["Listing views", business.profile_views_30d || 0, "analytics", "partial"], ["Search impressions", business.search_appearances_30d || 0, "search-performance", "partial"], ["Profile opens", business.profile_views_30d || 0, "analytics", "partial"], ["Website clicks", (business as any).website_clicks_30d || 0, "analytics", "partial"], ["Phone clicks", (business as any).call_clicks_30d || 0, "analytics", "partial"], ["Direction clicks", (business as any).direction_clicks_30d || 0, "analytics", "unavailable"], ["Reservation starts", (business as any).reservation_starts_30d || 0, "reservations", "partial"], ["Reservation completions", business.reservation_completions_30d || 0, "reservations", "verified"], ["Offer views", (business as any).offer_views_30d || 0, "growth-performance", "attributed"], ["Offer claims", related.offerClaims?.length || 0, "growth-performance", "verified"], ["VIP signups", related.vipMembers?.length || 0, "growth-performance", "verified"], ["Event leads", related.eventLeads?.length || 0, "growth-performance", "verified"], ["QR scans", (business as any).qr_scans_30d || 0, "growth-performance", "partial"], ["Reviews received", related.reviews?.length || 0, "reviews", "verified"], ["Average rating", (business as any).average_rating || "—", "reviews", "verified"], ["Support tickets", related.supportTickets?.length || 0, "system-logs", "verified"], ["Failed communications", (related.communications || []).filter((c:any)=>String(c.status||"").toLowerCase()==="failed").length, "communications-performance", "verified"], ["Recent admin changes", related.logs?.length || 0, "audit-history", "verified"], ["Data-quality alerts", publishability.reasons?.length || 0, "search-performance", "verified"], ["Operational alerts", related.supportTickets?.filter((t:any)=>!["closed","resolved"].includes(String(t.status||"").toLowerCase())).length || 0, "system-logs", "verified"], ["Growth alerts", recommendations.filter((r:any)=>r.category.includes("Growth")).length, "growth-performance", "verified"],
  ] as const;
  const tabs = ACTIVITY_CHILD_TABS.map((id) => ({ id, label: id.split("-").map(w=>w[0].toUpperCase()+w.slice(1)).join(" ") }));
  const period = range || "last-30-days";
  return <section className="space-y-5" aria-labelledby="activity-workspace-title"><AdminSectionCard className="p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.25em] text-rose-200">Enterprise Location Intelligence Workspace</p><h2 id="activity-workspace-title" className="mt-2 text-2xl font-black">Activity {tabs.find(t=>t.id===childTab)?.label}</h2><p className="mt-2 text-sm text-white/60">Canonical CRM activity workspace using existing location analytics, reviews, reservations, Growth Pro, communications, audit logs, support, QR, claims, and publishability data. Range: {period}; comparison: {compare || "previous-period"}.</p></div><div className="flex flex-wrap gap-2">{["today","last-7-days","last-30-days","last-90-days","custom"].map(r=><Link key={r} href={`${base}${childTab}&range=${r}`} className={`rounded-full border px-3 py-2 text-xs font-black ${period===r?"border-rose-200 bg-rose-600 text-white":"border-white/10 bg-white/[0.04] text-white/65"}`}>{r.replace(/-/g," ")}</Link>)}</div></div><nav aria-label="Activity workspace" className="mt-4 flex gap-2 overflow-x-auto pb-1">{tabs.map(t=><Link key={t.id} href={`${base}${t.id}`} className={`shrink-0 rounded-full px-3 py-2 text-xs font-black focus:outline-none focus:ring-2 focus:ring-rose-300/50 ${childTab===t.id?"bg-white text-black":"border border-white/10 bg-black/20 text-white/70"}`}>{t.label}</Link>)}</nav></AdminSectionCard>{childTab === "overview" ? <><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">{metrics.map(([label,value,target,trust])=>{ const c=compareMetric(Number(value)||0, null); return <Link key={label} href={`${base}${target}`} className="rounded-2xl border border-white/10 bg-white/[0.04] p-4"><p className="text-xs font-black uppercase tracking-[0.14em] text-white/40">{label}</p><p className="mt-2 text-2xl font-black text-white">{String(value)}</p><p className="mt-1 text-xs text-white/45">Trust: {trust} · {c.label}</p></Link>})}</div><ActivityAlerts recommendations={recommendations} publishability={publishability}/></> : null}{childTab !== "overview" ? <ActivityDetailPanel childTab={childTab} business={business} related={related} health={health} recommendations={recommendations} adminRole={adminRole} canEdit={canEdit} publishability={publishability}/> : null}</section>;
}
function ActivityAlerts({ recommendations, publishability }: { recommendations:any[]; publishability:any }) { const alerts = [...(publishability.reasons || []).map((r:string)=>({severity:"warning", title:r, action:"Open Search Visibility Diagnostics"})), ...recommendations.map((r:any)=>({severity:"recommended", title:r.title, action:r.category}))]; return <AdminSectionCard className="p-5"><h3 className="text-xl font-black">Alerts and anomalies</h3>{alerts.length ? <div className="mt-3 grid gap-3 md:grid-cols-2">{alerts.slice(0,10).map((a,i)=><div key={`${a.title}-${i}`} className="rounded-2xl border border-amber-300/20 bg-amber-500/10 p-4"><b className="text-amber-100">{a.severity}</b><p className="mt-1 text-white">{a.title}</p><p className="text-sm text-white/60">Evidence-based action: {a.action}</p></div>)}</div> : <EmptyPanel title="No alerts" text="No deterministic alerts were found for this range."/>}</AdminSectionCard> }
function ActivityDetailPanel({ childTab, business, related, health, recommendations, adminRole, canEdit, publishability }: any) { const rows = childTab === "audit-history" ? (related.logs || []) : childTab === "system-logs" ? (related.logs || []) : childTab === "reviews" ? (related.reviews || []) : childTab === "reservations" ? (related.reservations || []) : childTab === "communications-performance" ? (related.communications || []) : recommendations; const title = childTab.split("-").map((w:string)=>w[0].toUpperCase()+w.slice(1)).join(" "); if (childTab === "health-score") return <AdminSectionCard className="p-5"><h3 className="text-xl font-black">Deterministic health score: {health.score}/100</h3><div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{health.categories.map((c:any)=><div key={c.key} className="rounded-2xl border border-white/10 bg-black/20 p-4"><b>{c.key.replace(/[A-Z]/g," $&")}</b><p className="mt-1 text-2xl font-black">{c.score}%</p><p className="text-xs text-white/45">Weight {c.weight}; completed {c.completed}; failed {c.failed}; warnings {c.warnings}; unavailable {c.unavailable}</p></div>)}</div><ActivityAlerts recommendations={recommendations} publishability={publishability}/></AdminSectionCard>; if (childTab === "reports") return <AdminSectionCard className="p-5"><h3 className="text-xl font-black">Executive Reports</h3><p className="mt-2 text-sm text-white/60">Generate editable internal or owner-facing drafts from verified metrics. Owner-facing reports hide internal notes, private IDs, and customer PII.</p><div className="mt-3 flex flex-wrap gap-2">{["Executive summary","Weekly performance","Monthly performance","Search performance","Reservation performance","Growth performance","Communication performance","Reviews and reputation","Health and readiness","Audit and compliance","Incident summary"].map(t=><button key={t} className="rounded-full border border-white/10 px-3 py-2 text-xs font-black text-white/70">{t}</button>)}</div></AdminSectionCard>; return <AdminSectionCard className="p-5"><h3 className="text-xl font-black">{title}</h3><p className="mt-2 text-sm text-white/60">Trust indicators, freshness ({freshnessState((business as any).last_analytics_event_at || business.updated_at)}), permission mode ({canEdit ? adminRole : "read-only"}), pagination, and masking are applied before display.</p><div className="mt-4 overflow-x-auto"><table className="min-w-full text-left text-sm"><thead><tr>{["Timestamp","Type","Status","Safe summary","Trust"].map(h=><th key={h} className="border-b border-white/10 p-3 text-xs uppercase text-white/40">{h}</th>)}</tr></thead><tbody>{rows.length ? rows.slice(0,25).map((r:any,i:number)=><tr key={r.id||i}><td className="border-b border-white/10 p-3">{formatDate(r.created_at || r.updated_at || r.submitted_at)}</td><td className="border-b border-white/10 p-3">{maskSensitiveValue(r.category || r.action || r.status || r.rating || title, adminRole)}</td><td className="border-b border-white/10 p-3">{maskSensitiveValue(r.status || "observed", adminRole)}</td><td className="border-b border-white/10 p-3">{maskSensitiveValue(r.message || r.body || r.subject || r.title || r.note || "No sensitive payload shown", adminRole)}</td><td className="border-b border-white/10 p-3">verified / partial</td></tr>) : <tr><td colSpan={5} className="p-6 text-center text-white/45">No {title.toLowerCase()} data for this range. Tracking may be unavailable or zero activity was confirmed.</td></tr>}</tbody></table></div></AdminSectionCard>; }
...
function OperationsWorkspace({ business, related, activeTab, adminRole, canEdit }: { business: BusinessCRMRow; related: any; activeTab: string; adminRole: string; canEdit: boolean }) {
  ...
  const alerts = dedupeOperationalAlerts([
    ...
    !qrStatus.hasClaimCode && ["high", "Claim code missing", "This location does not have a reusable owner claim code.", "Generate a claim code or QR claim link.", `${base}claims`],
