...
export type HfSemanticRetrievalItem = {
  location: any;
  request: RetrievalRequest;
  similarity: number;
  semanticSimilarity: number;
  foodSimilarity: number | null;
};

export type HfSemanticRetrievalResult = {
  mode: HfSearchMode;
  items: HfSemanticRetrievalItem[];
...
function domainOfRequest(request: RetrievalRequest) {
  return request.desiredRole === "restaurant" || request.desiredRole.endsWith("_restaurant") ? "restaurant" : "activity";
}
...
export function buildHfSearchQueryDocument(plan: SearchPlan) {
  const lines = [
    ...
    plan.geo.neighborhood || plan.geo.borough || plan.geo.city
      ? `Area: ${[plan.geo.neighborhood, plan.geo.borough, plan.geo.city].filter(Boolean).join(", ")}`
      : "",
  ].filter(Boolean);
  return lines.join("\n");
}

export async function retrieveHfSemanticRows({
  plan,
  supabase,
  requests,
  trace,
  ...
  try {
    ...
    const embedding = await fetchHuggingFaceEmbedding(buildHfSearchQueryDocument(plan), {
      timeoutMs: Number(process.env.SEARCH_HF_QUERY_EMBEDDING_TIMEOUT_MS || 900),
    });
    ...
    const ids = [...new Set(perDomain.flatMap((lane) => lane.rows.map((row) => String(row.location_id)).filter(Boolean)))];
    ...
    trace.decisions.push({
      stage: "hf_semantic_retrieval",
      decision: "hf_retrieval_fallback",
      reason: message,
    });
