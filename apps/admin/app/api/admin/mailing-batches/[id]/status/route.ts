...
export async function POST(req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (action === "printed") {
      ...
      const { data: claimItems, error: claimItemError } = await getAdminDatabaseClient()
        .from("mailing_batch_items")
      ...
      if (eligible.length) {
        ...
        const { error: claimCodeError } = await getAdminDatabaseClient()
        ...
        const { error: locationError } = await getAdminDatabaseClient()
          .from("locations")
