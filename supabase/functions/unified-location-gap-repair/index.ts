...
const PROVIDERS = [
  ...
  ["tablescheck.com", "TableCheck"],
  ["eatapp.co", "Eat App"],
  ["simpleerb.com", "SimpleERB"],
] as const;
...
function normalizeGoogleHours(value: any) {
  ...
  const weekdayDescriptions = Array.isArray(value.weekdayDescriptions)
    ...
    : Array.isArray(value.weekday_descriptions)
...
function normalizeUrl(value: unknown) {
  ...
  try {
    ...
    try { return new URL(`https://${value.trim()}`).toString(); } catch { return null; }
  }
}
...
function extractLinks(html: string, base: URL) {
  ...
  for (const match of decoded.matchAll(regex)) {
    try { results.push(new URL(match[1], base).toString()); } catch { /* ignore */ }
  }
  ...
  for (const path of DISCOVERY_PATHS) {
    ...
    try {
      ...
      if (response.status === 403 || response.status === 429) {
        return { status: "blocked", match: null, note: `Website returned ${response.status}` };
      }
      if (!response.ok) continue;
      const contentType = response.headers.get("content-type") || "";
      if (!contentType.includes("text/html")) continue;
      const html = await response.text();
      const matches = extractLinks(html, url).map(reservationMatch).filter(Boolean) as Array<{ url: string; provider: string }>;
      if (matches.length) return { status: "found", match: matches[0], note: `Found on ${url.pathname}` };
    } catch (error) {
      return { status: "failed", match: null, note: error instanceof Error ? error.message : "Website discovery failed" };
    } finally {
...
async function googleDetails(placeId: string, key: string) {
  ...
  if (!response.ok) {
    const error = new Error(String(body?.error?.message || `Google Place Details failed: ${response.status}`));
...
serve(async (req) => {
  ...
  const candidates = (rows || []).filter((row: any) => {
    if (row.is_demo === true || row.is_training === true) return false;
  ...
  const counters = {
    ...
    reservationBlocked: 0,
    reservationFailed: 0,
    managedCoreSkipped: 0,
    failed: 0,
  };

  for (const row of candidates as any[]) {
    const update: Record<string, unknown> = {
      gap_repair_last_checked_at: new Date().toISOString(),
      gap_repair_status: "checked",
      gap_repair_error: null,
    };
    ...
    try {
      ...
      if (needsCore && row.google_place_id && !managed(row)) {
        ...
        if (blank(row.operating_hours) && normalizedHours) {
          update.operating_hours = normalizedHours;
          update.google_regular_opening_hours = place.regularOpeningHours || place.regular_opening_hours || null;
          update.google_current_opening_hours = place.currentOpeningHours || place.current_opening_hours || null;
          update.hours_source = "google_places_details_unified_repair";
        ...
        if (blank(row.website) && place.websiteUri) {
          update.website = place.websiteUri;
          counters.websitesFilled += 1;
        }
        const phone = place.nationalPhoneNumber || place.internationalPhoneNumber;
        if (blank(row.phone) && phone) {
          update.phone = phone;
          counters.phonesFilled += 1;
        }
      } else if (needsCore && managed(row)) {
        counters.managedCoreSkipped += 1;
      }
      ...
      if (needsReservationDiscovery) {
        if (!website) {
          update.reservation_discovery_status = "no_website";
          update.reservation_discovery_source = "unified_gap_repair";
          update.reservation_discovery_notes = "No website available for free discovery";
          update.reservation_discovery_checked_at = new Date().toISOString();
        } else {
          const discovery = await discoverReservation(website);
          update.reservation_discovery_status = discovery.status;
          update.reservation_discovery_source = "website_crawl";
          update.reservation_discovery_notes = discovery.note;
          update.reservation_discovery_checked_at = new Date().toISOString();
          update.reservation_last_checked_at = new Date().toISOString();
          if (discovery.match) {
            ...
            update.reservation_source = "website_crawl";
            update.reservation_source_url = website;
            counters.reservationFound += 1;
          } else if (discovery.status === "not_found") {
            counters.reservationNotFound += 1;
          } else if (discovery.status === "blocked") {
            counters.reservationBlocked += 1;
            retryHours = 24 * 7;
          } else {
            counters.reservationFailed += 1;
            retryHours = 24;
          }
        }
      }
      ...
      const retry = status === 429 ? 6 : status >= 500 ? 12 : 24;
      await supabase.from("locations").update({
        gap_repair_status: "failed",
        gap_repair_error: error instanceof Error ? error.message.slice(0, 1000) : String(error).slice(0, 1000),
        gap_repair_last_checked_at: new Date().toISOString(),
        gap_repair_next_attempt_at: new Date(Date.now() + retry * 60 * 60 * 1000).toISOString(),
      }).eq("id", row.id);
    }
  }
