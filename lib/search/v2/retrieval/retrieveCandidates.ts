...
export function profileRetrievalLimit(plan: SearchPlan, request: RetrievalRequest) {
...
function enforceHardDistanceRows(rows: any[], plan: SearchPlan, trace: SearchTrace, stage: string) {
  ...
  if (rejected) trace.decisions.push({ stage, decision: "hard_candidate_origin_distance_rows_removed", reason: JSON.stringify({ rejected, maxDistanceMiles: max, maxDrivingMinutes: plan.pairing.maxDrivingMinutes, travelMode: plan.travel.mode, geoSource: plan.geo.source }) });
  return filtered;
}
async function retrieveLegacyAtSharedLevel(supabase: SupabaseClient, requests: ReturnType<typeof buildRetrievalRequests>, plan: SearchPlan, trace: SearchTrace) {
  ...
  for (const level of levels) {
    ...
    const viable = (!required.restaurant || hasRestaurant) && (!required.activity || hasActivity); const coveredDomains = Number(hasRestaurant) + Number(hasActivity); const rowCount = rows.reduce((sum, lane) => sum + lane.rows.length, 0);
...
function asGeoScopeLevel(level: GeoLevel | null | undefined): GeoScopeLevel | null { switch (level) { case "exact_neighborhood": return "neighborhood"; case "city": return "city"; case "borough_or_county": return "county"; case "market": return "market"; case "state": case null: case undefined: return null; default: { const exhaustive: never = level; return exhaustive; } } }
