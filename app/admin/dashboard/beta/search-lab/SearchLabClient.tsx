...
function getPerformance(result: SearchLabBatchResult) {
  return (
    (result.data?.performance as SearchLabResult | undefined) ||
    ((result.data?.debug as SearchLabResult | undefined)?.performance as
      | SearchLabResult
      | undefined) ||
    null
  );
...
function getTotalMs(result: SearchLabBatchResult) {
  return Number(
    result.data?.total_ms ||
      (result.data?.performance as any)?.total_ms ||
      (result.data?.debug as any)?.performance?.total_ms ||
      0,
  );
}
...
function isCriticalSpeed(result: SearchLabBatchResult) {
  const speedStatus =
    result.data?.speed_status ||
    result.data?.speedStatus ||
    (result.data?.performance as any)?.speed_status ||
    (result.data?.debug as any)?.performance?.speed_status;
  return speedStatus === "critical";
}

function getOutingTimeConfidence(result: SearchLabBatchResult) {
  return ((result.data?.parsedIntent as any)?.outingTimeConfidence ||
    (result.data?.debug as any)?.normalizedIntent?.outingTimeConfidence ||
    result.data?.outingTimeConfidence ||
    "none") as string;
}

function getBatchPerformanceSummary(results: SearchLabBatchResult[]) {
  const successfulResults = results.filter((result) => result.success);
  const totalMsValues = successfulResults
    .map(getTotalMs)
    .filter((value) => value > 0);
  const averageTotalMs =
    totalMsValues.length > 0
      ? Math.round(
          totalMsValues.reduce((sum, value) => sum + value, 0) /
            totalMsValues.length,
        )
      : 0;
  const slowestResult = successfulResults.reduce<SearchLabBatchResult | null>(
    (slowest, result) =>
      !slowest || getTotalMs(result) > getTotalMs(slowest) ? result : slowest,
    null,
  );
  return {
    ...
    overFiveSecondsCount: successfulResults.filter(
      (result) => getTotalMs(result) > 5000,
    ).length,
    overEightSecondsCount: successfulResults.filter(
      (result) => getTotalMs(result) > 8000,
    ).length,
    fallbackUsedCount: successfulResults.filter((result) =>
      Boolean(result.data?.fallbackUsed),
    ).length,
    outOfBoroughCount: successfulResults.filter((result) =>
      Boolean(
        (result.data?.debug as any)?.boroughStrictnessApplied &&
        (result.data?.debug as any)?.outOfBoroughResultCount > 0,
      ),
    ).length,
    relaxedFeatureCount: successfulResults.filter((result) =>
      Boolean(
        (result.data?.debug as any)?.restaurantRecoveryRelaxedFeature ||
        (result.data?.debug as any)?.featureMissingPenaltyApplied,
      ),
    ).length,
    explicitTimeCount: successfulResults.filter(
      (result) => getOutingTimeConfidence(result) === "explicit",
    ).length,
    vagueTimeCount: successfulResults.filter(
      (result) => getOutingTimeConfidence(result) === "vague",
    ).length,
    noTimeCount: successfulResults.filter(
      (result) => getOutingTimeConfidence(result) === "none",
    ).length,
  };
}
  ...
  return lines
    .filter(
      (line) =>
        typeof line === "string" &&
        line.trim() &&
        !/undefined|null$/i.test(line.trim()),
    )
    .join("\n");
}
...
function getWebsite(item: any) {
  return pickFirst(item?.website, item?.website_url, item?.url) as
    | string
    | undefined;
}
function getReservation(item: any) {
  return pickFirst(
...
function getTags(item: any) {
  const tags = pickFirst(item?.tags, item?.vibe_tags, item?.best_for_tags);
  return Array.isArray(tags) ? tags.join(", ") : (tags as string | undefined);
}
function getWhy(item: any) {
  return pickFirst(
    item?.why_it_matched,
    item?.whyMatched,
    item?.match_reason,
...
function getSearchTypeLabel(item: any) {
  return String(
    pickFirst(
      item?.location_type,
      item?.type,
...
function formatSearchResultForCopy(item: any, context: { query: string }) {
  const type = getSearchTypeLabel(item).toLowerCase().includes("activity")
    ? "Activity"
    : "Restaurant";
  return compactLines([
    ...
    type === "Restaurant"
      ? `Cuisine: ${pickFirst(item?.cuisine, item?.cuisine_type, item?.food_type) || ""}`
      : `Activity Type: ${pickFirst(item?.activity_type, item?.activityType) || ""}`,
...
function formatPairForCopy(pair: any, context: { query: string }) {
  const restaurant =
    pair?.restaurant || pair?.restaurantCard || pair?.venueA || {};
...
function getVisibleResults(response: SearchLabResult | null) {
  ...
  const pairs = Array.isArray(response?.pairCards)
    ? (response.pairCards as any[])
    : [];
  return { cards, pairs };
}
...
function formatAllResultsForCopy(response: SearchLabResult, query: string) {
  ...
  const market =
    (response.marketFiltering as any)?.resolvedMarket ||
    (response.debugParity as any)?.resolvedMarket ||
    "";
  const lines = cards
    .map(
      (item, i) =>
        `${i + 1}. [${getSearchTypeLabel(item)}] ${getName(item) || "Result"}\n${formatSearchResultForCopy(item, { query })}`,
    )
    .concat(
      pairs.map(
        (pair, i) =>
          `${cards.length + i + 1}. [Pair] ${getName(pair?.restaurant || {}) || "Restaurant"} + ${getName(pair?.activity || {}) || "Activity"}\n${formatPairForCopy(pair, { query })}`,
      ),
    );
  return compactLines([
...
function formatDebugJsonForCopy(response: SearchLabResult) {
  const safePayload = {
    ...
    debugParity: response.debugParity,
    parsedIntent: response.parsedIntent,
    performance: response.performance,
    marketFiltering: response.marketFiltering,
    rawCounts: response.rawCounts,
    publicCounts: response.publicCounts,
  };
...
function formatPromptSummaryForCopy(response: SearchLabResult, query: string) {
  ...
  return compactLines([
    "TheOutHaven Beta Search Lab Summary",
    "Prompt:",
    query,
    "",
    "Resolved Intent:",
    ...
    `Activities: ${response.activities || 0}`,
    `Pairs: ${response.pairs || 0}`,
    `Final Displayed: ${((response.finalResultNames as any[]) || []).length || 0}`,
    "",
    "Top Results:",
    ...((response.finalResultNames as any[]) || [])
      .slice(0, 5)
      .map((name, i) => `${i + 1}. ${name}`),
    "",
    "Key Debug:",
    `Fallback Used: ${response.fallbackUsed || false}`,
    `Fallback Reason: ${debug.fallbackReason || debug.noPairsReason || ""}`,
    `Market Guardrail Rejected: ${debug.marketGuardrailRejected || 0}`,
    `Parser Source: ${response.intentParserSource || response.parser_source || ""}`,
    `Fast Path Matched: ${response.fastPathMatched || false}`,
    `Performance: ${safeStringify(response.performance || {})}`,
  ]);
}
...
function getIntent(result: SearchLabResult | undefined) {
  ...
  return (
    (result?.parsedIntent as SearchLabResult | undefined) ||
    (debug?.normalizedIntent as SearchLabResult | undefined) ||
    (debug?.intent as SearchLabResult | undefined) ||
    {}
  );
}
...
function getGeo(
  intent: SearchLabResult,
  result: SearchLabResult | undefined,
): SearchLabResult {
  return asSearchLabResult(
    intent.geo ||
      intent.location ||
      result?.effectiveGeo ||
      result?.originalGeo,
  ...
  const fields = [
    ...
    [
      ...
      pickFirst(
        ...
        intent.needs_activity,
        intent.activityRequired,
      ),
    ],
    [
      "Wants pairing",
      pickFirst(
        intent.wantsPairing,
        intent.wants_pairing,
        intent.pairingRequested,
      ),
    ],
    [
      "Geo raw",
      pickFirst(geo.raw, geo.rawQuery, intent.geoRaw, result.originalGeo),
    ],
    ...
    [
      "Outing date label",
      pickFirst(intent.outingDateLabel, result.outingDateLabel),
    ],
    [
      "Outing time label",
      pickFirst(intent.outingTimeLabel, result.outingTimeLabel),
    ],
    [
      "Outing date/time",
      pickFirst(intent.outingDateTimeText, result.outingDateTimeText),
    ],
    [
      "Time confidence",
      pickFirst(intent.outingTimeConfidence, result.outingTimeConfidence),
    ],
    [
      "Parsed date text",
      pickFirst(intent.parsedDateText, result.parsedDateText),
    ],
    [
      "Parsed time text",
      pickFirst(intent.parsedTimeText, result.parsedTimeText),
    ],
    [
      "Parsed ISO",
      pickFirst(intent.parsedDateTimeISO, result.parsedDateTimeISO),
    ],
    [
      "Occasion",
      pickFirst(intent.occasion, intent.occasionType, intent.occasion_type),
    ],
    [
      "Restaurant intent terms",
      pickFirst(
        intent.restaurantIntentTerms,
        intent.restaurant_intent_terms,
        result.restaurantRpcTerms,
        result.restaurantRpcTermsPruned,
      ),
    ],
    [
      "Activity intent terms",
      pickFirst(
        intent.activityIntentTerms,
        intent.activity_intent_terms,
        result.activityRpcTerms,
        result.activityRpcTermsPruned,
      ),
    ],
  ];
    ...
    <div className="mt-4 grid gap-3 md:grid-cols-2">
      {fields.map(([label, value]) => (
        <div key={label as string} className="rounded-2xl bg-white/[.04] p-3">
          ...
          <p className="whitespace-pre-wrap break-words font-semibold text-white/85">
            {formatValue(value)}
          </p>
        </div>
      ))}
...
function PerformanceSummary({ result }: { result: SearchLabResult }) {
  ...
  const totalMs = Number(result.total_ms || performance.total_ms || 0);
  const speedStatus =
    result.speed_status || result.speedStatus || performance.speed_status;
  const debug = (result.debug as any) || {};
  const fields = [
  ...
  return (
    <div className="mt-4 rounded-3xl border border-white/10 bg-black/20 p-4">
      <div className="flex flex-wrap gap-2">
        ...
        {totalMs > 8000 ? (
          <span className="rounded-full bg-red-300 px-3 py-1 text-xs font-black text-black">
            Over 8s
          </span>
        ) : null}
      </div>
      <div className="mt-3 grid gap-2 md:grid-cols-4">
        {fields.map(([label, value]) => (
          <div key={String(label)} className="rounded-2xl bg-white/[.04] p-2">
            ...
            <p className="break-words text-xs font-black text-white/85">
              {formatValue(value)}
            </p>
          </div>
        ))}
...
function ResultDetails({ result }: { result: SearchLabResult }) {
  return (
    <>
      ...
      <MlSearchDebugPanel
        mlDebug={
          (result.mlDebug as any) ||
          ((result.debug as any)?.mlSearchDebug as any)
        }
      />
...
export default function SearchLabClient({
  ...
  const [copiedBatchResultIndex, setCopiedBatchResultIndex] = useState<
    number | null
  >(null);
    ...
    try {
      if (searchLines.length > 1) {
        const settledResults = await Promise.allSettled(
          searchLines.map((line) => runSingleSearch(line)),
        );
        setBatchResults(
    ...
    <div className="space-y-4">
      <div className="rounded-3xl border border-white/10 bg-white/[.04] p-5">
        ...
        <button
          onClick={() => run()}
          disabled={loading}
          className="mt-3 rounded-full bg-rose-600 px-5 py-3 text-sm font-black disabled:opacity-60"
        >
      ...
      {error ? (
        <div className="rounded-3xl border border-red-400/30 bg-red-500/10 p-5 font-semibold text-red-100">
          {error}
        </div>
      ) : null}
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-black">
                Testing {batchResults.length} searches one at a time
              </h2>
            </div>
            <button
              type="button"
            ...
            >
              {copiedAllBatchResults
                ? "Copied all results"
                : "Copy all results"}
            </button>
          </div>
          <div className="mt-5 grid gap-3 md:grid-cols-4">
            {Object.entries(getBatchPerformanceSummary(batchResults)).map(
              ([label, value]) => (
            ...
            {batchResults.map((batchResult, index) => (
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  >
                    {copiedBatchResultIndex === index
                      ? "Copied"
                      : "Copy this result"}
                  </button>
                </div>
                {batchResult.success && batchResult.data ? (
                  <ResultDetails result={batchResult.data} />
                ) : (
                  <p className="mt-3 font-semibold text-red-100">
                    Error: {batchResult.error || "Search test failed."}
                  </p>
                )}
              </article>
            ))}
      ...
      {result ? (
        ...
        >
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <div className="flex w-full flex-wrap gap-2 sm:w-auto">
              <button
                type="button"
                disabled={
                  !getVisibleResults(result).cards.length &&
                  !getVisibleResults(result).pairs.length
                }
                onClick={() =>
                  copyWithFeedback(
                    "all-results",
                    formatAllResultsForCopy(result, query),
                  )
                }
                className="w-full rounded-full border border-rose-300/40 bg-rose-600/20 px-3 py-2 text-xs font-black text-rose-50 disabled:opacity-50 sm:w-auto"
              >
                {copiedAction === "all-results" ? "Copied" : "Copy All Results"}
              </button>
              <button
                type="button"
                disabled={!result.debug && !result.debugParity}
                onClick={() =>
                  copyWithFeedback("debug-json", formatDebugJsonForCopy(result))
                }
                className="w-full rounded-full border border-white/15 bg-white/[.06] px-3 py-2 text-xs font-black text-white/80 disabled:opacity-50 sm:w-auto"
              >
                {copiedAction === "debug-json" ? "Copied" : "Copy Debug JSON"}
              </button>
              <button
                type="button"
                onClick={() =>
                  copyWithFeedback(
                    "prompt-summary",
                    formatPromptSummaryForCopy(result, query),
                  )
                }
                className="w-full rounded-full border border-white/15 bg-white/[.06] px-3 py-2 text-xs font-black text-white/80 sm:w-auto"
              >
                {copiedAction === "prompt-summary"
                  ? "Copied"
                  : "Copy Prompt + Summary"}
              </button>
            </div>
          ...
          <div className="mt-5 space-y-3">
            {[
              ...getVisibleResults(result).cards.map((card: any) => ({
                type: "card",
                item: card,
              })),
              ...getVisibleResults(result).pairs.map((pair: any) => ({
                type: "pair",
                item: pair,
              })),
            ].map((entry, index) => (
                ...
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    <p className="text-xs uppercase tracking-[0.18em] text-white/40">
                      {entry.type === "pair"
                        ? "Pair"
                        : getSearchTypeLabel(entry.item)}
                    </p>
                    <h3 className="font-black">
                      {entry.type === "pair"
                        ? `${getName(entry.item?.restaurant || {}) || "Restaurant"} + ${getName(entry.item?.activity || {}) || "Activity"}`
                        : getName(entry.item) || "Result"}
                    </h3>
                  </div>
                  <button
                    type="button"
