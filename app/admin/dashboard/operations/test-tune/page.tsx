...
export default function AdminTestTunePage() {
  ...
  <section className="grid gap-3 rounded-3xl border border-white/10 bg-white/[0.03] p-5 sm:grid-cols-2 lg:grid-cols-5"><button onClick={()=>run("Run Search QA","/api/admin/search-qa")} disabled={Boolean(loading)} className="rounded-xl bg-rose-500 px-4 py-3 text-sm font-black">{loading==="Run Search QA"?"Running...":"Re-run Search QA"}</button><button onClick={()=>run("Backfill Semantic Data","/api/admin/semantic-nightly?all=true&limit=100")} disabled={Boolean(loading)} className="rounded-xl border border-white/15 px-4 py-3 text-sm font-black">{loading==="Backfill Semantic Data"?"Running...":"Backfill Semantic Data"}</button><button onClick={()=>run("Run Semantic Cleanup","/api/admin/semantic-nightly")} disabled={Boolean(loading)} className="rounded-xl border border-white/15 px-4 py-3 text-sm font-black">{loading==="Run Semantic Cleanup"?"Running...":"Run Semantic Cleanup"}</button><button onClick={()=>run("Fix Missing Coordinates","/api/admin/locations/cleanup-missing-address")} disabled={Boolean(loading)} className="rounded-xl border border-white/15 px-4 py-3 text-sm font-black">{loading==="Fix Missing Coordinates"?"Running...":"Fix Missing Coordinates"}</button><button onClick={()=>run("Rebuild Intent Tags","/api/admin/semantic-nightly?intent_only=true")} disabled={Boolean(loading)} className="rounded-xl border border-white/15 px-4 py-3 text-sm font-black">{loading==="Rebuild Intent Tags"?"Running...":"Rebuild Intent Tags"}</button></section>
