  ...
  return (
    <section className="mb-5 rounded-3xl border border-white/10 bg-white/[0.03] p-5 sm:p-6">
      ...
      {renderer === "v3" ? <div className="mt-6">
        <div className="flex flex-wrap items-end justify-between gap-2"><div><p className="text-sm font-black">Choose your V3 concept</p><p className="mt-1 text-xs text-white/45">Saved per location. These concepts are selectable now, but their clean-room preview renderers are still being developed.</p></div><span className="text-[11px] font-black uppercase tracking-[0.12em] text-amber-200">Preview-only rollout</span></div>
        <div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          {WEBSITE_V3_CONCEPTS.map((item) => {
            ...
            return <button key={item.id} type="button" disabled={saving} onClick={() => void save("v3", item.id)} className={`rounded-2xl border p-4 text-left transition ${active ? "border-[#ff2142]/50 bg-[#ff2142]/10" : "border-white/10 bg-black/20 hover:bg-white/[0.05]"}`}>
              <div className="flex items-center justify-between gap-2"><span className="font-black">{item.name}</span><span className="text-[9px] font-black uppercase tracking-[0.1em] text-white/35">In development</span></div>
