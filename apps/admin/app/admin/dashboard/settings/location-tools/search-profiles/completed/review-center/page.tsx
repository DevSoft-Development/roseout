...
async function loadReviewedProfiles(
  adminDb: ReturnType<typeof getAdminDatabaseClient>,
  status: string,
  from: number,
  to: number,
) {
...
export default async function CompletedReviewCenterUpdatesPage({
  ...
  const profilesResult = await loadReviewedProfiles(adminDb, status, from, to);
  ...
  for (const ids of chunk(locationIds, 100)) {
    const locationResult = await adminDb
      .from("locations")
