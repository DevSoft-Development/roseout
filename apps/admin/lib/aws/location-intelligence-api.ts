...
function config() {
  const baseUrl = String(
    process.env.AWS_LOCATION_INTELLIGENCE_API_URL || "",
  )
    .trim()
    .replace(/\/$/, "");
  const secret = String(
  ...
  if (!baseUrl || !secret) {
    throw new Error("aws_location_intelligence_api_not_configured");
  }
  if (!/^https:\/\//i.test(baseUrl)) {
    throw new Error("aws_location_intelligence_api_requires_https");
  }
  ...
  if (!response.ok) {
    throw new Error(
      (parsed as { error?: string } | null)?.error ||
        `aws_location_intelligence_api_http_${response.status}`,
    );
  }
...
export function readGoogleBudgetSummaryViaLocationIntelligenceApi() {
  return signedJson<GoogleBudgetSummary>(
    "/v1/google-budget/summary",
    "GET",
  );
}
