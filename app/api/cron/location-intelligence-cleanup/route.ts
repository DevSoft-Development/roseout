...
export async function GET(request: Request) {
  ...
  if (!isPrivateAwsBackgroundRequest(request)) {
    return Response.json(
      {
        ok: false,
        error: "Location Intelligence cleanup executes only in the private AWS background runtime.",
      },
      { status: 409 },
    );
  }
  ...
  try {
    const result = await processPublishReadyCleanupCanary(requestedLimit(request));
    return Response.json(result, { status: result.failed > 0 ? 207 : 200 });
  } catch (error) {
    return Response.json(
      {
        ok: false,
        error: error instanceof Error ? error.message : "Location Intelligence cleanup failed",
      },
      { status: 500 },
    );
  }
