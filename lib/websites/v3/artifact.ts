...
function shell(profile:V3PageProfile,name:string,title:string,kicker:string,body:string){
...
export function buildV3PreviewArtifact(args:{homeHtml:string;website:BusinessWebsite;location:GeneratedWebsiteLocationSnapshot;profile:V3PageProfile;conceptName:string}):WebsiteV3PreviewArtifact{
 const {homeHtml,website,location,profile,conceptName}=args;const name=nameOf(website,location);const photos=photosOf(location);
 ...
 const visit=`<section class="section"><div class="visit"><article><span>Address</span><h3>${esc(location.address||"Location details")}</h3></article><article><span>Hours</span><p>${esc(location.hours||"Current hours update automatically.")}</p></article><article><span>Contact</span><h3>${esc(location.phone||"Reserve online")}</h3></article></div></section>`;
 return {defaultPage:"home",pages:[
  {id:"home",label:"Home",path:"index.html",html:homeHtml},
  {id:"menu",label:"Menu",path:"menu.html",html:shell(profile,name,location.menu?.title||"The Menu",conceptName,`<section class="section"><div class="grid">${menuItems}</div></section>`)},
  {id:"gallery",label:"Gallery",path:"gallery.html",html:shell(profile,name,"Gallery",conceptName,`<section class="section"><div class="gallery">${gallery}</div></section>`)},
  {id:"events",label:"Events",path:"events.html",html:shell(profile,name,"Events & Experiences",conceptName,`<section class="section">${events}</section>`)},
  {id:"visit",label:"Visit",path:"visit.html",html:shell(profile,name,"Plan Your Visit",conceptName,visit)},
 ]};
}
