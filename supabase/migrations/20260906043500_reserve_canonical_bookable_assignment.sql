-- Canonicalize host floor resources before writing location_reservations.bookable_item_id.
-- Host views may send layout_items.id values, while the reservation FK references
-- location_bookable_items(id). Preserve the visual resource label/type while only
-- storing a valid canonical bookable item ID (or NULL for layout-only resources).
...
begin
  ...
  -- p_resource_id may be a layout_items.id from the host floor. Only persist an ID
  -- that actually belongs to location_bookable_items so the reservation FK remains valid.
  if p_resource_id is not null then
    select b.id into v_bookable_item_id
  ...
  if v_bookable_item_id is null and nullif(trim(coalesce(p_resource_label,'')),'') is not null then
    select b.id into v_bookable_item_id
     ...
     where b.location_id = p_location_id
       ...
       and lower(trim(coalesce(b.item_type,''))) = lower(trim(coalesce(p_resource_type,'')))
  ...
  update public.location_reservations
     set bookable_item_id = v_bookable_item_id,
