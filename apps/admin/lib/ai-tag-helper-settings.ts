...
export async function getAiTagHelperSettings(): Promise<AiTagHelperSettings> {
  try { const {data}=await getAdminDatabaseClient().from("app_settings").select("value").eq("key",AI_TAG_HELPER_SETTINGS_KEY).maybeSingle(); return normalizeAiTagHelperSettings(data?.value); }
