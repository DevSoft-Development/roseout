...
export async function GET(request: Request) {
  ...
  if (!adminUser || adminUser.role !== "superadmin") {
    return credentialsRedirect({ instagram_error: "Superadmin access is required." });
  }
  if (adminUser.user_id !== state.userId) {
    return credentialsRedirect({ instagram_error: "Instagram connection session does not match the signed-in admin." });
  }
