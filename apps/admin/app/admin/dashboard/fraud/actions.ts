...
export async function updateFraudCase(formData: FormData) {
  ...
  const { error } = await getAdminDatabaseClient().from("fraud_cases").update(patch).eq("id", caseId);
  if (error) throw error;

  await getAdminDatabaseClient().from("fraud_audit_log").insert({
    case_id: caseId,
...
export async function addFraudCaseNote(formData: FormData) {
  ...
  const { error } = await getAdminDatabaseClient().from("fraud_case_notes").insert({ case_id: caseId, note, actor_user_id: admin.user_id });
  if (error) throw error;

  await getAdminDatabaseClient().from("fraud_cases").update({ last_activity_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq("id", caseId);
  revalidatePath("/admin/dashboard/fraud");
  redirect(`/admin/dashboard/fraud?case=${encodeURIComponent(caseId)}`);
}
...
export async function applyFraudAction(formData: FormData) {
  ...
  const { error } = await getAdminDatabaseClient().from("fraud_actions").insert({
    case_id: caseId,
...
export async function triageFraudReport(formData: FormData) {
  ...
  const { data: report, error: reportError } = await getAdminDatabaseClient().from("fraud_reports").select("*").eq("id", reportId).single();
  if (reportError) throw reportError;

  if (action === "dismiss") {
    const { error } = await getAdminDatabaseClient().from("fraud_reports").update({ status: "dismissed", updated_at: new Date().toISOString() }).eq("id", reportId);
    if (error) throw error;
  } else {
    const { data: caseId, error: caseError } = await getAdminDatabaseClient().rpc("fraud_ensure_case", {
      p_subject_type: report.subject_type,
      p_subject_id: report.subject_id,
      p_reason: `Human report: ${report.reason}`,
    });
    if (caseError) throw caseError;
    const { error } = await getAdminDatabaseClient().from("fraud_reports").update({ status: "linked", case_id: caseId, updated_at: new Date().toISOString() }).eq("id", reportId);
    if (error) throw error;
    await getAdminDatabaseClient().from("fraud_audit_log").insert({ case_id: caseId, event_type: "report_linked", actor_user_id: admin.user_id, payload: { report_id: reportId } });
  }
