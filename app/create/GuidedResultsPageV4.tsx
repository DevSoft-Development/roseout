...
function PairCardView({ item, rank, premium, walkingRequested, onUse }: { item: CompletePair; rank: number; premium: boolean; walkingRequested: boolean; onUse: () => void }) {
  ...
  return (
    <article className={`flex h-full flex-col rounded-[1.5rem] border bg-[#0b0b0b] p-4 shadow-xl shadow-black/30 ${premium ? "border-[#e1062a]/35" : "border-white/10"}`}>
      ...
      <div className="mt-auto flex items-center gap-2 pt-5"><button type="button" onClick={onUse} className="flex-1 rounded-full bg-[#e1062a] px-5 py-3.5 text-xs font-black uppercase tracking-[0.1em] transition hover:bg-[#ff1744]">Use This Plan →</button>{route ? <a href={route} target="_blank" rel="noopener noreferrer" onClick={() => track("planner_pick_route_clicked", { step: 3, rank, placement_group: premium ? "premium" : "organic", sponsored })} className="rounded-full border border-white/10 px-4 py-3 text-xs font-black text-white/65 hover:border-white/20 hover:text-white">Route</a> : null}</div>
    </article>
  );
}
