...
export default async function MailingBatchLiveProofPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: activeItems, error } = await getAdminDatabaseClient()
  ...
  const { data: objects, error: storageError } = await getAdminDatabaseClient().storage.from(BUCKET).list(folder, { limit: 1000 });
